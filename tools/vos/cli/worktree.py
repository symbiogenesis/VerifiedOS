...
def verify(root: Path, path: Path, base: str, *, branch: str | None = None,
    ...
    if found is None or found.bare or found.prunable:
...
def create(root: Path, lane: str, base: str, *, branch: str | None = None
    ...
    ignored = _git(owner, "check-ignore", "--", f"{DIRECTORY}/", allowed=(0, 1))
    if not ignored:
    ...
    if any(Path(record.path) == target for record in registered(owner)):
    ...
    _git(owner, "worktree", "add", "-b", name, "--", str(target), expected)
