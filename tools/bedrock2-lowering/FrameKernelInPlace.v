...
(* =========================================================================
   FrameKernelInPlace.v

   The frame service's pure value kernel, authored inside Rupicola's subset in
   its one-live-array shape and lowered to bedrock2 by relational compilation,
   then to C by bedrock2's printer. This is the in-place half of the census
   pair, and it mirrors the `in-place-phased` row of
   `tools/vos/static_memory_transform.py`: the exclusively owned input is
   overwritten with its mapped values while the checksum accumulates, and the
   XOR pass runs over that same buffer.

   What the function computes. For a byte buffer `bs` and a length `len` no
   ...
   and `y` replaces `bs` in memory. `frame_value` states the same value in plain
   ...
   descriptor, no staging, no reservation, no retirement and no zeroization, and
   the buffer's storage is the caller's; the second loop leaves no mapped value
   ...
   computation per access. FrameKernelRetained.v is the same program with the
   first loop's store sent to a second buffer, and the census reads the
...
Module FrameInPlace.
  ...
  Definition frame_kernel_inplace (bs : ListArray.t byte) (len : word)
    : ListArray.t byte :=
    let/n c := Byte.x00 in
    let/n (c, bs) :=
      ranged_for_u (word.of_Z 0) len
        (fun '\< c, bs \> tok idx _ =>
           ...
           let/n bs := ListArray.put bs idx v in
           (tok, \< c, bs \>))
        \< c, bs \> in
    let/n bs :=
      ranged_for_u (word.of_Z 0) len
  ...
  (* Bounded agreement with the list reference, at the whole buffer. Each is a
     closed computation and none of them is a statement about every input. *)
  Example agrees_empty :
    frame_kernel_inplace [] (word.of_Z 0) = frame_value [] .
  Proof. vm_compute; reflexivity. Qed.

  Example agrees_edges :
    frame_kernel_inplace [Byte.x00; Byte.x01; Byte.x7f; Byte.xff] (word.of_Z 4)
    = frame_value [Byte.x00; Byte.x01; Byte.x7f; Byte.xff].
  Proof. vm_compute; reflexivity. Qed.

  Example agrees_wrapping :
    frame_kernel_inplace [Byte.x55; Byte.xaa; Byte.x80; Byte.x03; Byte.xfe]
                         (word.of_Z 5)
  ...
  #[global]
  Instance spec_of_frame_kernel_inplace : spec_of "frame_kernel_inplace" :=
    fnspec! "frame_kernel_inplace" bs_ptr len /
      (n : nat) (bs : ListArray.t byte)
      (pr : word.unsigned len <= Z.of_nat n) R,
    { requires tr mem :=
        (sizedlistarray_value AccessByte n bs_ptr bs * R)%sep mem;
      ensures tr' mem' :=
        tr' = tr /\
        (sizedlistarray_value AccessByte n bs_ptr
           (frame_kernel_inplace bs len) * R)%sep mem' }.
  ...
  Derive frame_kernel_inplace_br2fn SuchThat
         (defn! "frame_kernel_inplace"("bs", "len")
              { frame_kernel_inplace_br2fn },
          implements frame_kernel_inplace)
         As frame_kernel_inplace_br2fn_ok.
  Proof.
    Time compile.
  Time Qed.

  Definition frame_kernel_inplace_c : string :=
    Eval vm_compute in
      c_module [("frame_kernel_inplace", frame_kernel_inplace_br2fn)].
End FrameInPlace.

Require Import Coq.Strings.String.
Goal True. idtac "=== ASSUMPTIONS ===". exact I. Qed.
Print Assumptions FrameInPlace.frame_kernel_inplace_br2fn_ok.
Redirect "frame_kernel_inplace" Compute FrameInPlace.frame_kernel_inplace_c.
