...
The precedent is [the Wasm oracle's](../wasm-oracle/README.md): a loop a later reader re-runs, one source against released packages, with a hand-run recipe and no `run.py` command, the label naming a validator the entry point does not carry. The emitted C stays outside the tree for two reasons stated at [THIRD-PARTY.md](../../THIRD-PARTY.md): bedrock2's printer prepends its own load-and-store preamble, which is bedrock2's text under bedrock2's terms, and the license-mark rule has no ruling for a `.c` kind. Nothing under `proofs/` moves: the proof gate compiles that directory in a switch carrying `rocq-core` and nothing else, so a file requiring Rupicola there would turn `run.py proofs` red.
...
From the host the driver re-launches itself in WSL with every Windows path translated; inside WSL it runs as `python3` with the same arguments. `--source` and `--owner` take other files, which is how a probe is run against a modified copy without touching the tree. The verified exit is one more compile in the stage, after the driver has left `DescriptorCheck.vo` there:
...
Q2's fifth bullet, host half. Every figure below is taken with the switch and `ccomp` above. **The timings decide nothing and are labelled so.** The same file's `Time compile.` measured 34.5 s and 64.5 s in two runs an hour apart, on a host on AC with one sibling proof build running at `-j4` throughout, and 224.8 s in a run on battery under a ten-lane fan-out; a figure with a spread of six is a property of the machine, not of the derivation. What is load-bearing is the counts, the sizes and the digests, and those reproduce exactly. The C figures are the bytes between the quotes of the `Redirect` output, as [DIGESTS.md](DIGESTS.md) states.
