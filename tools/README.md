...
so the reverse is open and is taken: that gate seeds one defect per rule and reports
the result through [vos/seeded.py](vos/seeded.py) like every other loop that seeds
one, which is what keeps a mutation whose seed no longer applies and a mutant the rule
said nothing about from arriving as one undifferentiated failure. [Its README](quarantine/README.md) states what each
