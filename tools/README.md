...
`testrig adapt`, `testrig bmc`, `placement export`, `placement check`,
...
`memory-certificates encode`, `boot roster`, `boot-handoff layout` and `kernel reader` use
this checkout or a file already on disk and answer on either lane. `proofs status` takes
...
| `boot` | wsl; `roster` on either lane | M7.1's harness under the [roster and boot-recipe contract](../docs/implementation/contracts/boot-roster.md). `boot roster` reads the contract's roster on either lane and names the members without an executable product; `compose` and `run` run in the guest, where the boot record and the emulator meet. `compose RECIPE` places each member's assembly, refuses missing, statement-only, overlapping and out-of-region members, and writes the image and its SHA-256 boot record; `run RECIPE` re-reads every bound input, refuses a stale component, boots the image on the golden emulator and holds the console, event and commit-trace digests against the recipe, refusing an emulator that does not exit 0. The tracked recipe composes fixtures, which are not the real producers; roster acceptance stays open. |
...
| `supervisor` | wsl | [The bounded supervisor C](../supervisor/README.md). `check` compares generated C decisions with the Gallina reference in the locked native prover and exercises bounded effect sequencing and refusal prefixes. Target backend, kernel effect bindings and observed boot/fault order remain open. |
| `boot-crypto` | wsl | [The boot signature verifiers](../firmware/crypto/README.md). `run` compares bounded C with pinned standard vectors and an independent executable oracle; `--gallina` also compares the ML-DSA reference. Target lowering and firmware composition remain open. |
...
| `kernel` | wsl; `reader` on either lane | M4.4's host-side differential. `vectors` compiles [quickchick/KernelVectors.v](quickchick/KernelVectors.v) against its `Require` closure in the CertiRocq oracle's switch; `check` compiles the [kernel C](../kernel/README.md) in its host model with the lane's C compiler and holds it, and the trace reader [vos/kernelrun.py](vos/kernelrun.py), against every generated line, then holds the C to release instants and consumer refusals the harness states itself, counted apart; `mutants` runs authored defects in both as the positive control and credits each kill to a Gallina vector, a fixed consumer control or the harness's release expectation. Those commands supply host-model agreement. `restore --simulator PATH --build-receipt FILE` separately emits and executes the scalar final-restore primitive and generated defects, checking ordered register writes and dispatch against a source-bound model receipt. Complete kernel C bring-up remains open. |
...
Three more are the RVFI rig's and sit beside them for the same reason: [rvfi.py](vos/rvfi.py) is the wire format TestRIG defines and the projection from one of its packets onto the commit trace's records, [vengine.py](vos/vengine.py) is the stream generator, the socket, the seeded defects and the shrinker, and [rtltrace.py](vos/rtltrace.py) is the RTL harness's retirement frame, its protocol refusals and its projection through `rvfi.py`'s. None decides anything about behaviour: what a divergence *is* stays [trace.py](vos/trace.py)'s, so the rig, the RTL adapter and the corpus adjudicate through one function rather than three.
...
$ python tools/run.py evidence --no-proofs       # and without the proof gate
...
here. K-88 is the rule with that shape and its cases exercise the registered artifact readers: the
...
The [document-count maintenance contract](../docs/implementation/contracts/document-counts.md) keeps narrative counts optional while retaining checks on their owners.
