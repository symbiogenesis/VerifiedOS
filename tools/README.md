...
| [freeze-report.py](freeze-report.py) | host | Runs the profile-freeze measurement instrument against the contract that defines it: everything the encoding format already fixes is computed, every column whose operand does not exist names the symbol it waits on, and the twelve CI predicates run over the record. It cannot be run end to end until M1.4, and its verdict line says so. |
...
| [rtl.py](rtl.py) | host and WSL | The RTL lane: `provenance` parses the synthesis record on the host; `lint` and `elaborate` run Verilator in the guest, the second elaborating the imported core at the curated configuration and at a baseline and naming every structure the disabling parameters remove. |
...
$ python tools/freeze-report.py               # the freeze's ordered act, and what it waits on
$ python tools/freeze-report.py --json        # the same, as the record the CI gate reads
$ python tools/freeze-report.py --markdown    # and as the rendering a curator reads
