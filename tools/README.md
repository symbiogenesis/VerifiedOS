...
| `static-memory` | host | Replays synthetic capacity contracts and disjoint byte ledgers, checks fixed placements, and compares deterministic heuristics with a bounded exact oracle. [The experiment contract](../docs/implementation/static-memory-experiments.md) separates feasibility, optimality replay and incomplete search from target admission. `--json` binds working-tree sources and supplied inputs. |
