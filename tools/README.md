...
| `static-memory` | host | Replays capacity ledgers, checked placements and bounded exact oracles; `structure`, `transform`, `reclaim`, `scale` and `modes` exercise laminar construction, executable service variants, fixed reclamation schedules and larger planning families. [The experiment contract](../docs/implementation/static-memory-experiments.md) separates finite evidence and incomplete search from target admission. `--json` binds working-tree sources and inputs. |
