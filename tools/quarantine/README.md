...
| [freeze-report.py](freeze-report.py) | The instrument, run against [the contract that defines it](../../docs/implementation/contracts/freeze-measurement.md), in three renderings |
...
| --- | --- | --- |
| The freeze analyzer, with K-77 | **M1.8b**, behind the M8a gate and M1.4′ | `freeze-report.py` reports fewer than three of §4's three inputs absent, **run after `run.py model freeze-emit`**. That clause is what makes the condition mechanical rather than a property of somebody's working directory: the three streams are looked for under a build tree [.gitignore](../../.gitignore) excludes, so a stream whose producer has landed still reads absent on a checkout where nothing has produced it, and the count alone would answer differently on two trees at the same revision. Since M1.4′ the link map and the encoded image answer to that run and the sidecar stream does not, so the condition is half met and the missing half is M1.2's backend. The first stream to land is the first run in which a column is a measurement rather than a symbol; at that point the instrument is on the critical path of a decision being taken, and a rule that holds it against its contract belongs where every landing pays for it. |
...
The reverse act is the forward one read backwards, and it is four moves and no design: the instrument and its check go back to `tools/` and `tools/vos/`, the rule's row goes back into [tools/check-rules.md](../check-rules.md), its mutants go back into [tools/run.py selftest](../vos/cli/selftest.py), and the floors its enumerations owe go back into the floors group. K-83 is what makes the list exhaustive: while it is green, nothing outside this directory has come to depend on what is inside it, so there is no fifth move to discover.
