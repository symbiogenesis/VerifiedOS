...
| K-63 | counts | every requirement id the model files this tool reads cite names a live requirement | R-05-152, and the reach `corpus.MODEL_FACTS` declares; `model/` is outside the checker's corpus, so a Sail comment arguing from a requirement that does not exist rendered as a citation and pointed at nothing |
