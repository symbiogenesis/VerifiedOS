...
Each cites the governing requirement for every row and is defective, never authoritative, where it disagrees with the register. `tools/check-derived-views.ps1` checks all three in both directions — that every cited ID resolves, and that every requirement in a view's bearing subsections is carried.

Register traces cite the prose by `<a id="r-ss-nnn">` bookmark rather than by line number, so editing prose moves the target with the text. `tools/check-traces.ps1` holds the four properties a symbolic reference can still violate silently: every cited bookmark resolves exactly once, every requirement carries a trace, every `r-*` bookmark in the prose names a live requirement, and the section a trace displays is the section its bookmark sits in.

No count in any of these documents is maintained by hand either. `tools/check-counts.ps1` recomputes every figure they assert — the coverage totals and per-section table, the crown-jewel status ratio, the absence and open-CSR row counts — from the artifact that owns it, and rewrites them under `-Fix`. A derived fact restated where nothing checks it is the defect all three tools exist to prevent: a line number, a membership list, and a count are the same mistake at three granularities.
