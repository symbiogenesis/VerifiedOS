...
| Rowhammer, RowPress, and DRAM read-disturbance bit flips | An SRAM bistable latch has no leaking capacitor or refresh cycle, so repeated activation and row-open-time disturbance have no analog; RFM and PRAC are deleted rather than retuned. SRAM's far weaker disturb modes are carried as detected faults in the detection table below | **🕳️ Absent** |
| Memory-bus probing and DIMM or module interposition | On-die SRAM has no external memory bus, socketed or soldered module, or die-to-die link to probe or interpose, so the memory encryption and integrity or anti-replay tree a DRAM design needs are deleted. An attacker already inside the package is an invasive physical attacker, explicitly out of scope | **🕳️ Absent** |
...
| Variable latency on a secret operand | Secret-reachable operations are fixed-latency or flow-rejected: integer divide, the vector FPU including subnormals, and atomics are fixed-latency, and misaligned accesses trap rather than split. The one named exception carries a discharged flow obligation: no admitted crypto kernel uses `vfdiv`/`vfsqrt`. This claim covers only architectural timing: power and near-field electromagnetic leakage are outside the model, the crypto core is unmasked, and §17 records that residual | **🛡️ Enforced**<br>**✋ Rejected** |
| Secret-dependent address timing across SRAM banks | Indexed or runtime-strided vector access varies with the element-address distribution across SRAM banks, the address-timing channel remaining after cache deletion. The flow discipline rejects secret-labeled element addresses, and the non-work-conserving schedule burns unused time, confining variable latency to the issuing partition's slot. The architectural-timing scope and §17 residual above bound this row equally | **✋ Rejected** |
...
| Path traversal and `../` escape | “Resolve a string to an object” is unexpressible without an ambient namespace. A path is only an app-local alias for a manifest capability; no runtime `mount`/`bind`, global directory, or path-based capability lookup can reach outside the manifest | **🕳️ Absent** |
| Symlink and hardlink races and `/tmp`-style filename TOCTOU | The capability *is* the object, and no symlink indirection exists to redirect an alias, leaving no re-resolution window between check and use | **🕳️ Absent** |
...
| Silent storage corruption and bit rot | Patrol reads scrub decay and read-disturb, rewriting pages approaching the correction limit, so bit rot is corrected on schedule; corruption that outruns the scrub still fails authentication rather than returning silently | **🔔 Detected** |
