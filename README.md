...
| Rowhammer, RowPress, and DRAM read-disturbance bit flips | An SRAM bistable latch has no leaking capacitor or refresh cycle for repeated activation to disturb, so RFM and PRAC are deleted rather than retuned. SRAM's far weaker disturb modes are detected faults in the table below | **🕳️ Absent** |
| Memory-bus probing and DIMM or module interposition | On-die SRAM has no external memory bus, module, or die-to-die link to probe, deleting the memory encryption and anti-replay tree a DRAM design needs. An attacker inside the package is an invasive physical attacker, out of scope | **🕳️ Absent** |
...
| Interconnect and quality-of-service contention channels | The admission proof emits a static time-division fabric schedule; best-effort arbitration and programmable bandwidth regulation do not exist. The NoC model is unauthored, so this absence is structural, not yet proved | **🕳️ Absent**<br>**🚩 Residual** |
| Contention between high-assurance memory islands | A high-assurance island takes a whole SRAM macro or tier, leaving no shared path or reachable arbiter between it and any peer. The island-isolation statement shares the unauthored-model residual above | **🕳️ Absent**<br>**🚩 Residual** |
| Contention between low-sensitivity islands sharing an SRAM macro | Static per-island arbitration schedules away the contention; only periphery, power delivery, and thermal mass stay shared, all outside every crown-jewel domain. The same unauthored-model residual applies | **🕳️ Absent**<br>**🚩 Residual** |
| Variable latency on a secret operand | Secret-reachable operations are fixed-latency or flow-rejected: integer divide, the vector FPU including subnormals, and atomics are fixed-latency, and misaligned accesses trap rather than split. The one exception is a discharged flow obligation: no admitted crypto kernel uses `vfdiv`/`vfsqrt` | **🛡️ Enforced**<br>**✋ Rejected** |
| Secret-dependent address timing across SRAM banks | Indexed and runtime-strided vector timing varies with the element-address distribution across banks, the one address-timing channel that survives cache deletion. The flow discipline rejects secret-labeled element addresses, and the non-work-conserving schedule confines what remains to the issuing partition's slot | **✋ Rejected** |

The two timing rows claim architectural timing only: power and near-field electromagnetic leakage are outside the model, and the crypto core is unmasked, a recorded residual.
...
| Shell injection and `system()`-style string-to-process execution | Nothing turns composed text into an action: there is no `fork()`/`exec()`, `PATH` or executable lookup, or shell. *Run this command* asks the service manager to start a capability-delegated compartment | **🕳️ Absent** |
| Argument and environment injection | There is no environment block, and a command interpreter pipes typed values between typed-signature commands, never byte streams for each stage to reparse | **🕳️ Absent** |
...
| Termination and progress channels: one partition observing whether another finished, how far it got, or that it faulted | The frame is non-work-conserving across confidentiality boundaries: a partition that idles, diverges, or faults burns its slot without moving any boundary, so peers observe the schedule, never its progress. The channel is absent rather than bandwidth-limited, making non-interference termination- and progress-sensitive. A compartment can read its own slot width, a recorded residual | **🕳️ Absent** |
...
| Implicit integer wrap | Arithmetic is total by typing: with no exceptions and no unwinding, neither wrapping nor trapping is the meaning of an unannotated operation, and modular and saturating arithmetic survive only as explicitly named operations. The closure is *no implicit wrap anywhere* | **✋ Rejected** |
| Overflow or underflow over static bounds | Overflow-freedom rides as range side conditions on the arithmetic rules: the on-device checker decides them where operand bounds are closed numerals, and a release-time proof term over the base image covers bounds that depend on runtime values. Tier 2 scopes the representation-and-provenance obligations out, and the closure bounds the operation, never the value's meaning: in-range but wrong remains functional correctness at Tier-0/1 | **✋ Rejected**<br>**✅ Proved**<br>**🚩 Residual** |
| Silently dropped security-bearing verdicts: integrity, freshness, admission, transaction | Relevance typing denies weakening for a security-bearing result, so a binary cannot eliminate a verdict without examining it. The grade bounds the *drop*, never the *response*: proceeding after an examined failure is well-typed, so wrong response remains functional correctness at Tier-0/1. It binds only the admitted set; the pre-admission boot substrate carries the same obligation as ordinary Tier-0 proof, and Tier 2 scopes examined verdicts out | **✋ Rejected**<br>**✅ Proved**<br>**🚩 Residual** |
...
| Secret-dependent traps, fault classes, and the restart they cause | A secret-dependent fault is an information flow, so confidentiality governs the fault path: constant-time typing makes trap choice a function of public inputs, restart consumes only the offender's slots in the non-work-conserving frame, and the sentinel receives a labeled fault from a closed enumeration, never verbose logs. A fault on a compartment's *own* data carries no platform-secret obligation; crash-record shape bounds the disclosed class, with no theorem closing it, a recorded residual | **✋ Rejected**<br>**🕳️ Absent** |
...
| A value carrying more than one admissible encoding, parting byte identity from value identity | Each descriptor used for signing, hashing, addressing, or equality carries a machine-checked canonicity theorem: decode is injective over admissible inputs, and re-encoding a decoded input reproduces its bytes, so no alternate encoding can substitute for one value. Slack is removed from the descriptor rather than normalized at decode, and a malleable published grammar is pinned to a canonical subset. Without the theorem, a format cannot serve an identity role | **✅ Proved**<br>**✋ Rejected** |
| Query injection, the `SQL`-shaped class | There is no query language or second database: metadata and search are typed views of one keyspace, and a query supplies a namespace capability and a typed bounded predicate, not command text. Even a maximally permissive predicate returns only capabilities derivable from the one supplied; authority, not escaping, is the bound | **🕳️ Absent**<br>**🛡️ Enforced** |
...
| Test-mode re-entry, TAP unlock, and factory-mode escape | A fixed acyclic lifecycle over one-way OTP fuses omits these transitions: leaving test disables every debug and manufacturing interface at once, nothing reopens them, and a once-debuggable part never enters production. Production's one outgoing edge is an authenticated, terminal RMA transition, preceded by crypto-erase, to a debuggable device with no production keys and no return path | **🛡️ Enforced** |
...
| Link and radio state-machine flaws | A Lustre control plane refines a formal model of the standard's state machine, making unmodeled states, transitions, and timers unreachable rather than merely untested. The claim is model conformance, not protocol security: the model's faithfulness is a review-gated crown-jewel specification, and the standard's composed session security a recorded residual | **✅ Proved** |
...
| --- | --- | --- |
| Single-event and multi-cell upsets, and latent errors accumulating past correction | Every array, from SRAM and the register and vector files to the matrix scratchpads, interconnect, and tag plane, is ECC-protected and corrected; validity tags use stronger DECTED because a flip can forge or destroy authority. Interleaving presents adjacent-cell strikes as separable single-bit errors, background scrubbing repairs latent errors before another arrives, and whole-codeword writes leave no sub-granule path. Uncorrectable errors become fail-stop sentinel events, never returned values; only deployment-grade radiation hardening lowers the upset *rate* at its source | **🔔 Detected** |
...
| A dead, stuck, or narrowly trimmed noise source silently weakening every draw from the one entropy root | Independent sources spanning at least two physical mechanisms feed one conditioner, each attributed and health-tested at startup and continuously after. Failure is fail-stop, never a degraded draw, and a weak boot root halts the device. A failure the tests miss remains a recorded residual, hence detection rather than absence | **🔔 Detected** |
| Voltage or clock glitching, laser, and electromagnetic fault injection carrying the machine outside its own semantics | Coverage is stated per injection: ECC corrects stored-state faults or fails stop, capability corruption traps on tag or bounds checks, the multikernel confines live-kernel faults for crash-only restart, and layered watchdogs reach wedged cores. Coverage remains evidence rather than theorem, and consumer-grade transient datapath strikes are unclaimed; both are recorded residuals | **🔔 Detected** |
...
| A partition alive but wedged, or a runaway holding its core | Watchdogs occupy failure domains disjoint from the cores, the main clock tree, and the scheduler: the sentinel monitor first restarts, revokes, or rolls back, and the root of trust's always-on timer on an independent slow clock is the last resort. Bark is a pending bit read at the next slot boundary, never an asynchronous interrupt; bite resets safely because transactional state loses only uncommitted work. The surgical tier remains slot-granular, only bite responding within a slot, a recorded limit | **🔔 Detected** |
...
| --- | --- | --- |
| Injection into an interpreter an application brings with it, web content in the browser being the standing case | **The compartment author.** Only platform name resolution, command execution, search, and configuration are structured rather than textual; an app's own string parser remains injectable within its compartment. The platform bounds the class rather than fixing it, manifest capabilities limiting the blast radius to app authority, and functional correctness is mandatory only for the TCB | **🤝 Transferred** |
| Upset rates and component reliability beyond what the die's own correction covers: automotive, avionic, and orbital duty | **The deployment.** Radiation hardening is the source-rate lever an enclosure cannot supply, and it changes no computation. The platform mandates correction at every point, and the deployment selects the grade on that one stated axis, from a commodity consumer panel to hardened automotive and orbital parts | **🤝 Transferred** |
| The user granting the authority they meant to grant | **The human.** The sole runtime declassifier is an attested, unspoofable powerbox whose grants are CHERI-bounded to the named object, making the mechanism trustworthy and auditable. No proof can establish that the user named the right object: the comprehension half of that ceiling is irreducible, and the abuse-resistance half (a well-shaped or oft-repeated ask a competent user grants and would refuse on reflection) is booked open | **🤝 Transferred** |
