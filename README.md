...
| Escape from an embedded script engine into its host | The platform ships one pure-interpreter Wasm engine carrying machine-checked soundness and robust guest confinement against the pinned guest semantics: an adversarial module influences its host and peers only through the imports and exports its embedding declared; an app that rolls its own engine instead keeps the transferred row below | **✅ Proved**<br>**🤝 Transferred** |
...
| Differential and correlation power analysis and their electromagnetic equivalents against the crypto core | The secret-handling datapath is masked, and its *d*-probing and composition theorems are verified on the artifact against a stated probing model; the model is an axiom about the silicon, its faithfulness the recorded residual | **✅ Proved**<br>**🚩 Residual** |
