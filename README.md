...
[Open standards are preferred wherever these guarantees permit them](docs/spec.md#r-04-001a). Reuse their interfaces and semantics; document a concrete conflict or an owned implementation gap before replacing them. The [OpenCL/SPIR-V and HIP source-compatibility path](docs/implementation/compute-compatibility.md) targets certified native vector/matrix code built before generation admission, with bounded runtime resources, and the [graphics surface that path can carry is Vulkan SC's](docs/implementation/compute-compatibility.md#graphics-api-surface), every pipeline compiled before admission and OpenGL entering only as build-time source. Both are planned work, and they change none of the guarantees below.

The first release is a [general-purpose laptop](docs/spec.md#r-18-004a) in the shape of a Chromebook or a Fedora Atomic machine, stated as a floor of eight things one composed configuration must carry together: hold a two-way call over Wi-Fi, stream its webcam into that call, move Wi-Fi data, open secure sessions, play one video stream, run a desktop of applications opened and closed at will while a build runs among them, hold a language model resident and generating, and open its content formats through verified parsers. It carries [no cellular radio, eUICC or Bluetooth](docs/spec.md#r-02-003b), so it contains no foreign computer at all; the phone is a later instantiation. The machine is split in two. A [fixed tier](docs/spec.md#r-07-037f) keeps the design's full determinism: the kernel, root of trust, crypto core, network stack, trusted path, storage and every service with a deadline each run in a share of time and memory fixed when the image was composed, with [every structure whose size depends on input](docs/spec.md#r-08-046) sized ahead of time and given a declared action for filling up. The user's applications run above it in an [elastic domain](docs/spec.md#r-07-037e): a fixed envelope of cores and memory inside which apps launch and close without a reboot, share time by a [verified proportional-share dispatch](docs/spec.md#r-07-037g) and grow their heaps from a [revocation-gated pool](docs/spec.md#r-08-047b), with the native guarantees below still holding of every native member and interpreter host. [Precomposed Wasm application hosts](docs/spec.md#r-14-013e) let the device build, validate, launch and replace app bundles in the running session. Guests have [proved arena confinement](docs/spec.md#r-14-013a); object-level safety inside guest memory requires additional evidence. Native packages and system changes still enter through [signed successor generations](docs/spec.md#r-13-001a) at the next boot, which [discards what was not declared durable](docs/spec.md#r-17-035a). The [resident toolchain](docs/spec.md#r-13-027) produces both kinds of artifact while the user keeps working; its app bundles remain interpreted data, and its native output never runs in the generation that built it. The web browser is on neither side of that: its [no-JIT, one-compartment-per-origin design](docs/spec.md#r-14-008) is fixed, and its port is [the one the first release defers](docs/spec.md#r-18-004), which on a laptop of this class is [its largest forfeit](docs/spec.md#r-17-053b). No form factor is fixed by the design: [phone and workstation instantiations](docs/spec.md#r-02-003) share its principles and differ in physical particulars, and [a server is an ensemble](docs/spec.md#r-02-003a) of whole machines, each its own chip with its own memory, root of trust, and attested software, joined by point-to-point [ensemble links](docs/spec.md#r-15-228b) whose every transfer instant is fixed by the same proof that schedules each chip and that share no memory, so a model too large for one machine is [split across machines when the image is composed](docs/spec.md#r-15-171a) and the design [scales by adding machines](#one-die-is-one-machine-and-a-server-is-several), never by enlarging one.
...
The kernel, root of trust, crypto core, network and link stacks, trusted path, storage, and every service with a deadline form a [fixed tier](docs/spec.md#r-07-037f) that keeps the design's whole static discipline: time-triggered slots, a memory plan fixed before boot, and admission against proved worst-case times. The user's applications run in an [elastic domain](docs/spec.md#r-07-037e): one confidentiality label inside a composition-fixed envelope of cores, slots and memory. Inside it, installed native members launch and close, and newly built Wasm app bundles run in precomposed interpreter hosts without reboot; they share the envelope's time by a [verified earliest-eligible-virtual-deadline dispatch](docs/spec.md#r-07-037g) that serves the focused app first, and grow their heaps from a [revocation-gated pool](docs/spec.md#r-08-047b). Nothing is preempted and no timer is added: the toolchain places a poll site on every loop back-edge, so each native member [yields within a checked bound](docs/spec.md#r-07-037h), replacing the need to prove a bound on every guest loop. [Every interpreter phase obeys that native bound](docs/spec.md#r-14-013g). Every runtime choice stays inside one label and moves no instant or byte outside the envelope, so non-interference across labels, CHERI confinement, admission, W^X and the timing absences all hold unchanged. What apps in the domain can see of one another, the same contention any desktop exposes, is [booked as a residual](docs/spec.md#r-17-007b) rather than claimed away.

### Fixed-latency on-die memory with end-to-end ECC

All memory is on the same die as the cores, error-corrected from the register file outward, and comes in two fixed speeds: fast SRAM for the working set and for code with deadlines, and a denser, slower class of gain cells for bulk data like framebuffers, media, and AI model weights. Which class an address sits in is settled when the image is built, and nothing between a core and a cell keeps track of what was accessed recently, so every access to a class costs that class's constant, whatever ran before it: no history for timing to leak, nothing for a worst-case analysis to guess at. That one property deletes the L1/L2/L3 hierarchy, the coherence protocol, the DRAM channels, and the per-row counters a DRAM design needs against Rowhammer. Being on-die, there is no bus to probe and no module to lift, so memory encryption and an anti-replay tree are deleted rather than proved. Both classes carry the same tags, ECC, and revocation check, but the SRAM is more private. Both also share one 36-bit physical address space, so 64 GB bounds one machine, model weights included, an ensemble adding machines and never address bits: the specification states that ceiling as a bet on where useful local inference plateaus, resting on the denser class, which is the least-proven component in the design and whose capacity counts only once a measured part supplies it.

### One die is one machine, and a server is several

A machine is one die: its cores, both memory classes, its root of trust, and its 36-bit address space are on that die, and [no chiplet, bonded stack, or die-to-die link](docs/spec.md#r-15-162) joins a second die to it, because a second die is a second mask set, fab lot, and supply-chain object to trust. The design scales the other way. [A server is an ensemble](docs/spec.md#r-02-003a) of whole machines, each with its own memory, root of trust, kernel instances, and attested software generation, joined by [ensemble links](docs/spec.md#r-15-228b): point-to-point wires that carry fixed-size frames in time slots [the same admission proof that schedules the fabric](docs/spec.md#r-11-017a) fixes, with no arbitration, no flow control, and no retry, so a peer can fill its own slots and never reassign this machine's slots. Each payload frame is error-corrected at a fixed latency and carried inside [a session between two mutually attested machines](docs/spec.md#r-12-015d) whose identities and software the composition names, with the keys never leaving the crypto core; a payload frame carries indices into a table handed over when the ensemble was composed, never a load, a store, or a capability, so a compromised peer reaches exactly that table. A model too large for one machine is [split across machines at composition](docs/spec.md#r-15-171a), every part resident and the traffic per step fixed, so what an ensemble adds is capacity and aggregate throughput in proportion to machine count; a single conversation's token rate stays the constant one machine's memory grant and the link's slots fix. This is the shape [statically scheduled inference racks already take](docs/background/inspirations.md#groqs-tensor-streaming-processor-static-scheduling-carried-across-chips-and-the-shared-address-space-an-ensemble-does-not-take), with the differences stated where they are taken: those racks pool every chip's memory into one address space and describe no cryptography on the wire, and this design does neither. What it leaves open, the composed isolation proof across the wire, the two-machine attestation relation, and the wire's physical exposure, is [booked as residual](docs/spec.md#r-17-058h); the first release is a laptop, and the ensemble is a later instantiation whose gating work [the plan prices](docs/implementation/implementation-checklist.md#12-build-order-milestones-and-execution-state).
...
CHERI's bounds are spatial; a mandatory typed assembly language (TAL) checks two more guarantees at install time. Use-after-free: freed memory is made unreachable by bounded capability revocation and reclamation and by linear and affine capabilities, which cannot be duplicated. Uninitialized reads: the TAL rejects any binary that could read a location before writing it, and a slot is zeroed as it enters each live range the offline memory plan gives it, so no earlier tenant's data survives. Neither adds a hardware engine.
...
The [certifying toolchain is resident](docs/spec.md#r-13-027): the compilers, the prover, the composer and the package-construction passes ship as ordinary contained compartments, so the machine can read, edit, build, prove and install its own system without a second machine, and every generation it runs it can also produce from the source closure its own store holds. Self-hosting adds capacity and no authority, because the local toolchain is [an untrusted producer like any other](docs/spec.md#r-13-001c): its output is checked rather than believed, no admission rule reads that the producer was local, and [native output waits for a successor generation while Wasm app bundles can run immediately](docs/spec.md#r-02-008). App hosts can [reuse validated interpreter data and call bounded native services](docs/spec.md#r-14-013h), while [replacement retires the old binding and its grants](docs/spec.md#r-14-013f). A compiler writes store objects it holds no execute authority over; the measured-boot loader remains the only writer of executable memory, and the app host interprets guest bytes as data. On the laptop a build [runs as an ordinary member of the desktop's elastic domain](docs/spec.md#r-13-027a), sharing that envelope with the user's other applications and reaching nothing outside it, while a separate composition mode with its own admission-proved schedule remains available; what makes a locally composed generation installable is the holder's own consent act over [a signing key that never leaves the crypto core](docs/spec.md#r-13-027b). Two limits are stated rather than glossed: the two firmware images the boot ROM's own immutable key verifies can be rebuilt locally to check them but not signed locally, and a locally authored source closure is attested by this device's store and by nothing else.

### Memory planned before boot, with one elastic pool for the desktop

In the fixed tier every buffer, table, and stack has backing storage assigned before boot, with bounds checked before a system image is admitted. Runtime requests bind preassigned pool slots, and a full pool returns a declared exhaustion result. This removes online placement and global memory-pressure recovery, while preserving fixed memory budgets and predictable timing. Idle reservations, alignment, layout gaps, and memory awaiting safe reuse still cost capacity; free space in one pool cannot necessarily serve another. The desktop is the deliberate exception: each elastic domain owns [one pool](docs/spec.md#r-08-047a) whose size is fixed at composition and whose contents are allocated at runtime by a [verified pool service](docs/spec.md#r-08-047b) that hands out exactly bounded, zeroed chunks and reuses one only after revocation has swept every stale capability to it. Robson's fragmentation bound and quarantine overhead come back inside that pool and are [booked](docs/spec.md#r-08-047e); nothing inside it can grow the pool, borrow fixed-tier memory, or [hold a capability outside the domain](docs/spec.md#r-08-047c), and running short [hibernates a background app rather than killing anything silently](docs/spec.md#r-08-047d). The [workflow profile design](docs/implementation/workflow-profiles.md) adds preconfigured workflow recipes and a checked hibernation path for reusing inactive apps' private working sets; its runtime and proofs remain planned. The non-normative [static-memory research agenda](docs/background/static-memory-research.md) examines these limits and ways to improve useful capacity while preserving the design's isolation and lack of address translation.
...
The [Sail model](model/) is a modified copy of an existing one and keeps that project's [BSD two-clause license](model/LICENCE), which carves out the libraries it builds against, each on its own terms. [Copyright and licensing](COPYRIGHT.md) says which license covers which path and why, and [third-party components](THIRD-PARTY.md) lists everything here that somebody else wrote.
