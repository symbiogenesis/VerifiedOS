...
Per §5, the artifact the independent-specification-review release gate audits is the [atomic-requirements register](requirements-register.md) — each normative obligation as a numbered requirement with an acceptance criterion, traced to the crown-jewel spec it constrains and to the prose as rationale. It covers all eighteen normative sections as 909 numbered requirements, and records fourteen extraction defects — normative claims that resisted atomic restatement, which §5 treats as spec defects rather than register omissions.

Two **derived views** collect what the register states across many entries but no document held, one per day-one deliverable in §18's schedule:
...
Each cites the governing requirement for every row and is defective, never authoritative, where it disagrees with the register. `tools/check-derived-views.ps1` checks both in both directions — that every cited ID resolves, and that every requirement in a view's bearing subsections is carried.
