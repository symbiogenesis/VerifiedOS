...
| Link and radio state-machine flaws | A Lustre control plane refines a formal model of the standard's state machine, making unmodeled states, transitions, and timers unreachable rather than merely untested. This claims model conformance, not protocol security: faithfulness to the published standard is a review-gated crown-jewel specification, while the standard's composed session security remains a recorded residual | **✅ Proved** |
