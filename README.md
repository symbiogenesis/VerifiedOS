...
| A declassification grant wider than the object it named | The non-interference theorem models a powerbox grant as a delimited release, CHERI-bounded to the object consent named, never a general H→L conduit. Whether the user named the right object is transferred below | **✅ Proved**<br>**🛡️ Enforced** |
...
| Implicit integer wrap | With no exceptions and no unwinding, neither wrapping nor trapping can be the meaning of an unannotated operation; modular and saturating arithmetic survive only as explicitly named operations | **✋ Rejected** |
| Overflow or underflow over static bounds | Range side conditions ride on the arithmetic rules: the on-device checker decides them where operand bounds are closed numerals, and a release-time proof term covers bounds that depend on runtime values. Tier 2 scopes the obligation out, the residual; in-range but wrong remains functional correctness | **✋ Rejected**<br>**✅ Proved**<br>**🚩 Residual** |
| Silently dropped security-bearing verdicts: integrity, freshness, admission, transaction | Relevance typing denies weakening a security-bearing result, so a binary cannot eliminate a verdict without examining it. The grade bounds the *drop*, never the *response*: proceeding after an examined failure remains functional correctness, the residual | **✋ Rejected**<br>**✅ Proved**<br>**🚩 Residual** |
...
| Secret-dependent traps, fault classes, and the restart they cause | Constant-time typing makes trap choice a function of public inputs, restart consumes only the offender's slots, and the sentinel receives a labeled fault from a closed enumeration, never verbose logs. Crash-record shape bounds the disclosed class with no theorem closing it, a recorded residual | **✋ Rejected**<br>**🕳️ Absent** |
...
| Link and radio state-machine flaws | A Lustre control plane refines a formal model of the standard's state machine, making unmodeled states, transitions, and timers unreachable. The claim is model conformance, not protocol security: model faithfulness is review-gated, and composed session security a recorded residual | **✅ Proved** |
...
| --- | --- | --- |
| Single-event and multi-cell upsets, and latent errors accumulating past correction | Every array from the register file to main memory and the tag plane is ECC-corrected, with stronger DECTED on validity tags because a flip can forge or destroy authority. Interleaving separates adjacent-cell strikes, background scrubbing repairs latent errors, and an uncorrectable error fails stop rather than returning a value | **🔔 Detected** |
...
| Voltage or clock glitching, laser, and electromagnetic fault injection | ECC corrects stored-state faults, capability corruption traps on tag and bounds checks, the multikernel confines live-kernel faults for crash-only restart, and layered watchdogs reach wedged cores. Coverage remains evidence rather than theorem, and consumer-grade transient datapath strikes are unclaimed; both are recorded residuals | **🔔 Detected** |
...
| A theorem that is true and empty: a premise nothing satisfies, a specification anything refines, or a quantifier ranging over nothing | Each theorem carries a machine-checked satisfiability witness and, for refinement or policy claims, an instance the specification rejects; because general vacuity is undecidable, this is a per-theorem obligation | **✅ Proved** |
