...
| History-dependent prefetch channels | Prefetchers are absent; no access history steers a hardware fetch | **🕳️ Absent** |
...
| Kernel lock contention | There is no shared mutable kernel data and there are no kernel locks or kernel threads; the kernel is entered only by a synchronous trap or the slot-boundary timer, and runs on the caller's budget | **🕳️ Absent** |
...
| Implicit integer wrap | Arithmetic is total by typing: wrapping is defined-but-wrong and trapping has nowhere to go in a machine with no exceptions and no unwinding, so neither is the meaning of an unannotated operation, and modular or saturating arithmetic survives only as an explicitly named operation. The closure is *no implicit wrap anywhere* | **✋ Rejected** |
