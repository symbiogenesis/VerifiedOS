...
Per §5, the artifact the independent-specification-review release gate audits is the [atomic-requirements register](requirements-register.md) — each normative obligation as a numbered requirement with an acceptance criterion, traced to the crown-jewel spec it constrains and to the prose as rationale. It covers all eighteen normative sections as 901 numbered requirements, and records eleven extraction defects — normative claims that resisted atomic restatement, which §5 treats as spec defects rather than register omissions.
