# Modeled block-device contract

This is M5.3's reviewed composition interface for an executable storage image and
...
The modeled slave presents logical blocks at the availability-service boundary of
...
use this interface for a simulation wrapper within the reviewed scope; a
...
| PIO, one outstanding command, widths, encodings, geometry, service steps, completion and reset rules below | Reviewed composition choices owned by this contract; they are not deductions from the address map. Changes require renewed device review and affected evidence. |
| Raw NAND, fixed-function code and host FTL on the production path | R-12-025 through R-12-030 and the [storage lower half](../docs/spec.md#r-12-025). Mapping this logical abstraction to that path remains owed; accepting this contract does not authorize a firmware-bearing controller. |
...
| --- | --- | --- | --- |
| `0x00` | `VERSION` | RO | `1`, identifying this ABI. |
...
| `FLUSH` | Complete a durability barrier over preceding successful writes. This contract is write-through, so there is no acknowledged volatile write backlog. | Report the backend's barrier failure. Do not convert the failure into success or undo a preceding successful write. |
...
write-through success makes the durable boundary explicit before
...
The device review accepts the PIO abstraction, write-through durable success,
complete-access refusal and reset race ordering together, with simulation-only
use at the RTL boundary. The existing
