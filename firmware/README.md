...
| [rot/boot_verify.c](rot/boot_verify.c) | The RoT's measured release of the boot core into the M-mode stage, in the contract's order | A signature verifier: it calls the one bound by its policy; the existing boot-handoff harness still uses its explicit fixture |
...
| [harness/rot_stage_main.c](harness/rot_stage_main.c) | The host driver that runs the release with main SRAM as buffers, and the fixture signature verifier with a racing arm that rewrites the input during verification | Firmware; the fixture verifier is not a signature scheme |
