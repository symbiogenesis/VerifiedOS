...
| `CSetBounds` with a large immediate | ⚠️ | R-15-067d | decide | A further code-size candidate of the same class as §3's two, and it should be measured the same way R-15-067d measures the bitfield pair: against actual generated output at the freeze, dropped if the delta is immaterial. R-15-036n makes that measurement one act rather than several, so this row is decided beside the bitfield pair and the single-check multi-save rather than on its own. |
