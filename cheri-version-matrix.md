...
| Separate capability register file (MIPS coprocessor model) | 1.0 | ➖ | R-15-007i | no | Superseded by the merged file at v9, which the profile now states — see §7. |
...
| NULL capability = all-zeroes, untagged (so BSS works) | 1.10 | ✅ | R-15-007j, R-15-182, R-15-060 | no | Load-bearing rather than conventional here — `cbo.zero` writes zeroed data *and* cleared tags (R-15-182), and eager zeroize is the platform's Write-before-Read mechanism — and now carried: the residual-semantics clause makes ISAv9 govern where the profile is silent, and §4.1 names this instance because it is the one the zeroize discipline reads back. It inherits over the narrowed fields, so R-15-007a's round-trip proof discharges it at these widths. |
...
| Sentries auto-unseal when installed in the exception PC, not only when jumped to | 8.0 | ✅ | R-15-007j, R-15-073, R-07-022 | no | The trap path installs `MTCC` and saves `MEPCC`, and whether a sentry installed in `MEPCC` unseals on `mret` was a semantics question the profile did not answer — on the domain-entry path, so silence was expensive. The residual-semantics clause answers it: ISAv8 made it unseal, ISAv9 carries it, the profile records no deviation, so it unseals. |
...
| **Capability exceptions reported via `xtval`, not a capability cause register** | 8.0 | ✅ | R-15-073a | no | Adopted: the cheaper answer as well as the standards one, reusing `mtval` rather than adding a bank. It cost one row in §5.1 and removed the named extraction defect. |
...
| Unaligned-base CHERI exception code | 8.0 | ✅ | R-15-007j, R-15-084, R-15-073a | no | The *behavior* was always settled — misaligned data accesses trap (R-15-084) and fetch is bundle-aligned — and only the cause encoding was open, R-15-073a fixing where a capability exception reports but not whether an unaligned base reports as a CHERI cause at all or as the standard misaligned one. The residual-semantics clause makes that ISAv9's answer rather than one the freeze has to make, the profile recording no deviation on it. |
...
| **Merged register file always** | 9.0 | ✅ | R-15-007i, R-07-015, R-15-214 | no | Now stated, and it was never cosmetic: R-07-015 and R-15-214 make the restore total over "every general-purpose register, **capability register**, and CSR a partition can name", which reads as an enumeration over two files. On a merged file it is one file of 32 × (64+1) bits, and *that* is the set the total-restore obligation quantifies over. One row in §1 of the profile. |
| **Tag clearing rather than exceptions for non-monotonic modification** (from Morello) | 9.0 | ✅ | R-15-007h, R-15-007e | no | Now stated, and the two sides were never equivalent for this platform: trapping would make every failed derivation a control-flow event with a WCET term and a cause encoding, while tag clearing makes it a silent data result that faults later at dereference. R-15-007e already reasoned about `cincoffset` producing an out-of-representable-region result, which only coheres on the tag-clearing side, so the profile was committed before it said so. |
...
| **`Svyrg`** (was `Svucrg`) — PTE-based capability load faults and capability dirty tracking | ➖ | R-08-005, R-08-005a, R-08-005b, R-15-038 | no | MMU-borne and unusable here. Its two transferable lessons are now explicit rather than inherited: R-08-005b makes the tag clear part of the load's own definition, while R-08-005a refuses to compress RVY's **4-bit PTE state machine** into the CHERIoT-lineage bit. The bit is only the load-time live/revoked predicate; epoch, sweep, and quarantine remain separate protocol state. |
...
| **Vector: stores always clear capability tags in memory** | ✅ | R-15-115a | no | Reached independently after the gap was named. R-15-115 says vector data carries no tags, which is what a vector store *writes*; R-15-115a says what happens to the tag of the granule it overwrites, and RVY gives the same answer. Without the clause a vector store reads as leaving a stale tag over overwritten data — a forgery primitive. One clause in §8 of the profile. |
...
| Instruction renaming to the `Y*` mnemonic space; opcode relocation; format rename | ✅ | R-17-048a | no | R-17-048a's original estimate — differential-testing oracles "degrading in proportion to the parameterization rather than all at once" — was made against a CHERI-RISC-V sharing mnemonics, encodings, and CSR names with `sail-cheri-riscv`. RVY has since renamed every instruction, relocated the entire encoding space into Custom-3, moved sealing to a CT field, restructured the CSRs, and renamed the format, leaving no shared surface to fail proportionally. The requirement and §11 now carry the estimate in two terms: *algebraic* oracles (bounds encode/decode, the representable-region predicates, monotonicity) degrade in proportion, and *instruction-level* oracles (Spike, QEMU, the CHERI test suites) do so only while they sit on the ISAv8/v9 lineage the format re-parameterizes, closer to all at once as they track RVY. The conclusion is unchanged: the proportional term is what makes the deviation list affordable, and the wholesale term is why its length is a quantity to keep small. |
...
| Shadow memory + load filter + revoker | ✅ / ⛔ | R-08-005, R-08-005a, R-08-009 | no | The deterministic load filter and its one-bit-per-8-byte, ECC-protected bank-side shadow are carried and sized; the software sweep is retained. The autonomous memory-walking revoker remains excluded by R-08-009. |
...
**No row is a defect: the register requires nothing the frozen profile does not carry, and the one hole the profile named is closed.**

**Three rows closed at once, and it is worth recording what closed them.** The profile now carries a residual-semantics clause (R-15-007j): where it is silent on the *behaviour* of a construct it carries, ISAv9 governs and the recorded deviations are exhaustive. That converts the all-zeroes NULL decode, the unsealing of a sentry installed in `MEPCC` on `mret`, and the unaligned-base cause encoding from questions the freeze had to answer into lookups against the pin. The clause deliberately does **not** run over membership — what exists is still decided by the closed CSR table, the exclusions named one by one, and the trap on every unallocated encoding — so it closes behavioural silence without turning the curated profile into "everything ISAv9 has, minus the exclusions." That is why the six ISAv8/v9 instructions below (`CRAM`/`CRRL`, `CSetBoundsExact`, `CLoadTags`, `CClearTags`, `CMOVN`/`CMOVZ`, `CTestSubset`) are untouched by it and stay individual decisions, and why the two refusals — the `menvcfg` CHERI enable and `misa.Y` — are not weakened by it either.

**Two rows are corrections to claims the profile makes.**

7. **"Permanently bespoke" was a true dichotomy when written and is not one now.** RVY separates the base ISA from a *parameterized capability encoding format* (`RV64Y` + `RV64LYA`, with an `_L<params>` naming scheme), and states that every RVY base requires an encoding format to complete it. The profile's 64+1-bit format is a different parameterization of that same slot. This does not reopen R-15-007 — 36-bit addresses and 8/6-bit mantissas fall outside anything RVY will ratify — but the argument for retirement should engage with the mechanism rather than with a fork/adopt binary that the standard no longer presents.
8. **The custom opcode space the profile's three bespoke instructions occupy has been claimed upstream.** At v0.9.8 RVY relocated its own instructions into what is Custom-3 for RVI and reserved Custom1–3 wholesale when the base ISA is RVY. No defect today, since the profile is not RVY-based — but it forecloses the encoding-level rapprochement that finding 7 would otherwise leave open, and the freeze should choose the encoding knowing that.

**One row is a cheap statement the profile should make because the standard has now made the opposite one explicit.**

9. **No runtime CHERI disable.** RVY's `misa.Y` now resets to 1 and clearing it disables CHERI entirely, gated only by a root-capability check on the implicit-check CSRs. R-15-052's read-only `misa` already refuses this, but on a machine whose whole protection story is CHERI, "no runtime ISA morphing" and "no runtime CHERI disable" are the same sentence and only one is written.
...
11. **`AUICGP`, for global-address materialization.** This row was closed on the pre-stratification density model and R-15-036k reopens it. A dictionary entry carries its immediate, so a `cgp`-relative offset, identical at every site that names one global, is a single entry shared across the image, while the `auipcc` displacement it replaces is an entry per site; the question is therefore not relocatability but hit rate, and R-15-036l sends it to the same freeze measurement.
