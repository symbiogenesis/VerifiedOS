...
**Behavioural silence is governed by the pin.** The residual-semantics clause (R-15-007j) says that where the profile is silent on the *behaviour* of a construct it carries, ISAv9 governs and the recorded deviations are exhaustive. The all-zeroes NULL decode, the unsealing of a sentry installed in `MEPCC` on `mret`, and the unaligned-base cause encoding are therefore lookups against the pin rather than questions for the freeze. The clause deliberately does **not** run over membership — what exists is decided by the closed CSR table, the exclusions named one by one, and the trap on every unallocated encoding — so it closes behavioural silence without turning the curated profile into "everything ISAv9 has, minus the exclusions." That is why the six ISAv8/v9 instructions below (`CRAM`/`CRRL`, `CSetBoundsExact`, `CLoadTags`, `CClearTags`, `CMOVN`/`CMOVZ`, `CTestSubset`) are untouched by it and stay individual decisions, and why the two refusals — the `menvcfg` CHERI enable and `misa.Y` — are not weakened by it either.

**Four rows are live design questions the freeze must answer.**
