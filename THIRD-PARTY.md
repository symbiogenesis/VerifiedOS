...
This project takes no election here, the component being read rather than incorporated; the election belongs to the milestone that would admit it. Two facts about the pin bear on that milestone beside its terms: the upstream's default feature set enables `memory64`, which R-14-013b requires as part of the complete curated and proved Core 3.0 engine, and the same release names support for [WebAssembly's deterministic profile](https://github.com/WebAssembly/profiles/blob/main/proposals/profiles/Overview.md) as a build-configuration option. Neither default or flag supplies the platform engine's proof. Neither is a licensing question, and both are recorded where the interim is defined.
