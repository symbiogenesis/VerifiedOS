...
| astral-sh/setup-uv | `MIT` | Host and guest CI download and run the action to install uv and cache its downloads. The reviewed revision `bec219d24cd3e171d82865faccec33120bb574f4` has an [MIT LICENSE](https://github.com/astral-sh/setup-uv/blob/bec219d24cd3e171d82865faccec33120bb574f4/LICENSE). No action source or bundled dependency is redistributed here. |
| actions/checkout and actions/setup-python | `MIT` | Host and guest CI fetch the checkout and Python interpreter. The reviewed [checkout LICENSE](https://github.com/actions/checkout/blob/3d3c42e5aac5ba805825da76410c181273ba90b1/LICENSE) and [setup-python LICENSE](https://github.com/actions/setup-python/blob/5fda3b95a4ea91299a34e894583c3862153e4b97/LICENSE) credit GitHub and contributors. No action source or bundled dependency is redistributed here. |
| actions/upload-artifact | `MIT` | Guest CI retains logs and receipts. The reviewed revision `043fb46d1a93c77aae656e7c1c64a875d1fc6a0a` has an [MIT LICENSE](https://github.com/actions/upload-artifact/blob/043fb46d1a93c77aae656e7c1c64a875d1fc6a0a/LICENSE), crediting GitHub and contributors. Neither action code nor compiled third-party tools are uploaded as project evidence. |
| actions/cache | `MIT` | Guest CI caches source downloads and, for manual runs, installed toolchains through the same revision's `restore` and `save` actions. The pinned v6.0.0 revision `2c8a9bd7457de244a408f35966fab2fb45fda9c8` has an [MIT LICENSE](https://github.com/actions/cache/blob/2c8a9bd7457de244a408f35966fab2fb45fda9c8/LICENSE), crediting GitHub and contributors. No action source or bundled dependency is redistributed here. |
| GitHub CLI | `MIT` | The runner-supplied `gh` reads guest workflow history to skip unchanged successful revisions. The reviewed [v2.78.0 LICENSE](https://github.com/cli/cli/blob/v2.78.0/LICENSE) credits GitHub; the installed version follows the runner image. No CLI source or executable is redistributed here. |
| opam | `LGPL-2.1-only` with the upstream linking exception | [Guest bootstrap](tools/ci/README.md) downloads a checksum-verified client and imports the project switch snapshots. The reviewed 2.5.2 revision `ad0564f20c6306bc83085fe663cab903fe887208` states its terms in [LICENSE](https://github.com/ocaml/opam/blob/ad0564f20c6306bc83085fe663cab903fe887208/LICENSE). The client and installed switches stay in the build environment; this repository distributes neither. |
| Z3 | `MIT` | Sail's solver, installed at the pin in [env.py](tools/vos/env.py). The reviewed [5.1.0 LICENSE.txt](https://github.com/Z3Prover/z3/blob/z3-5.1.0/LICENSE.txt) credits Microsoft. Guest bootstrap installs the released `z3-solver` package into its private prefix; no solver source or binary is distributed here. |
...
This project takes no election here, the component being read rather than incorporated; the election belongs to the milestone that would admit it. Two facts about the pin bear on that milestone beside its terms: the upstream's default feature set enables `memory64`, which R-14-013b makes eligible only under the generation's curated and proved allowlist, and the same release names support for [WebAssembly's deterministic profile](https://github.com/WebAssembly/profiles/blob/main/proposals/profiles/Overview.md) as a build-configuration option. Neither default or flag supplies the platform engine's proof. Neither is a licensing question, and both are recorded where the interim is defined.
...
| Candidate | Consumer it decides | Terms, and the artifact that read them | What the consumer would open |
| --- | --- | --- | --- |
| SMTCoq | Q3b, whose SMT facts take R-05-017's in-kernel reconstruction route | `CeCILL-C` version 1.0, at [the discovery collections](docs/assurance/proof-reuse/foundations.md#discovery-collections-and-search-limits) | The plugin's own `LICENSE` at the selected version |
| DeepSEA | Q19c's builder comparison, with Q2b's representation and lowering comparison | No license of its own, the tree's sole license text being CompCert's, at [the closest matches](docs/languages/verification-strategy.md#closest-matches-to-the-complete-workflow) | That tree's single license text and its README, at the revision incorporated and for the files actually taken |
| CertiSuSLik's HTT, VST, and Iris certification backends | Q19c's builder comparison | No license file, with `GPL-3.0` declared in package metadata alone, at [the certified-synthesis reading](docs/languages/verification-strategy.md#embedded-verification-and-certified-synthesis) | Each backend's own package metadata and source headers, in its own `TyGuS/ssl-htt`, `TyGuS/ssl-vst`, or `TyGuS/ssl-iris` tree rather than in the synthesizer's |
| FznDrcpCheck | Q5b, through the arm Q27a records | `MIT`, at [F06](docs/assurance/proof-reuse/foundations.md#f06-fzndrcpcheck-and-the-drcp-proof-system-reference) | Its `LICENSE` and `dune-project` at the revision selected |
| The CADE-26 line's Rocq LRAT checker | Q5b, through the arm Q27a records | Unread. Its distribution states no terms at all, at [F08](docs/assurance/proof-reuse/foundations.md#f08-lrat-and-its-two-certified-checkers-reference) | A license file inside the authors' `rat-checker.tar.gz`, or the authors themselves |

The three shapes below decide rather than merely exist, and each is recorded with its consumer.

**A copyleft term on a proof-surface dependency decides that dependency's consumer.** R-05-017 admits an SMT result only through in-kernel reconstruction and names that route after the plugin, so the terms reach Q3b exactly to the extent Q3b takes the plugin and not merely the style. What has already landed is out of their reach: `run.py model smt` puts the transcribed property suite to a solver directly and imports no witness, so no local result depends on the plugin today. The reading therefore decides Q3b against the line [the discovery reading](docs/assurance/proof-reuse/foundations.md#discovery-collections-and-search-limits) already draws through that grant, and the answer is not supplied by a forge classifier, by the terms of other Rocq plugins, or by the permissive licenses of the solvers whose witnesses the plugin accepts.

**A non-commercial term forecloses vendoring outright rather than pricing it.** DeepSEA is read and cannot become anything else, so the containment that keeps the verified compiler and the control-plane compiler usable as external producers does not apply: the act contemplated there is running a producer, and the act contemplated here is taking source. The grant is read per file and at the revision incorporated rather than off the parent project's name. The plan's Q27c cell states the reason and it is carried here as that cell's rather than as a reading taken in this record: CompCert's own current license dual-licenses a named subset under `LGPL-2.1-or-later` beside the non-commercial agreement, while the older copy DeepSEA vendors names GPL v2 for that subset, so two trees under one project's license do not carry one grant. [The CompCert and SECOMP decomposition](#compcert-and-secomp) names the dual-licensed directories and leaves the file enumeration to that license; which files the DeepSEA tree actually carries, and under which text, is what its own reading decides.

**An absent term is not a permissive one.** [F08](docs/assurance/proof-reuse/foundations.md#f08-lrat-and-its-two-certified-checkers-reference) reads the CADE-26 line's two checkers and finds one grant and one silence; what follows for the consumer is that the silent one is unlicensed until its authors say otherwise rather than free to take, one paper's two artifacts being two grants and the granted term reaching its sibling not at all. Nor does the `lrat_isa` checker that [the static-memory certificate integration](#optional-static-memory-research-tools) already fetches under a read MIT license supply terms for this row, that being a separate Isabelle/HOL development: this row's silence says nothing about that fetch, and that fetch says nothing about this row. The same reading holds for the CertiSuSLik backends, whose metadata declaration is not made permissive by the synthesizer's `BSD-2-Clause` license file sitting beside them in a different tree.

The surveyed members left out of the table decide no consumer here and are recorded as such rather than omitted, each with the ground that disposes of it before terms are reached: [the Lean certificate checkers](docs/assurance/proof-reuse/foundations.md#f09-the-lean-certificate-checkers-unqualified-lead) on their prover, [SCIP's exact mode and its VIPR certificates](docs/assurance/proof-reuse/foundations.md#f10-scips-exact-mode-and-vipr-certificates-unqualified-lead) on having no machine-checked component at all, and [the pseudo-Boolean proof-logging results](docs/assurance/proof-reuse/foundations.md#f11-pseudo-boolean-proof-logging-in-solvers-reference) on the unverified solvers their certificates attest for. [VeriPB and CakePB](docs/assurance/proof-reuse/foundations.md#f07-veripb-and-cakepb-reference) join them on the same kind of ground rather than on terms: CakePB's prover is HOL4, which [the plan's Q27a arms](docs/implementation/implementation-checklist.md#q-assessment-actions) group with the Lean developments as failing R-05-020's first condition before any yield is weighed, and VeriPB's own elaborating checker is unverified. No row is written for a candidate whose terms decide nothing.
