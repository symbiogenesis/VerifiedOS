...
| CakePB, and VeriPB beside it | Q5b, through the arm Q27a records | The CakeML `BSD-3-Clause` notice, and `MIT OR Apache-2.0` at the user's option, at [F07](docs/assurance/proof-reuse/foundations.md#f07-veripb-and-cakepb-reference) | Each project's own notice at the revision selected, these being two artifacts under two grants |
...
**A copyleft term on a proof-surface dependency decides that dependency's consumer.** R-05-017 admits an SMT result only through in-kernel reconstruction and names that route after the plugin, so the terms reach Q3b exactly to the extent Q3b takes the plugin and not merely the style. What has already landed is out of their reach: `run.py model smt` puts the transcribed property suite to a solver directly and imports no witness, so no local result depends on the plugin today. The reading therefore answers which of two acts Q3b performs, modifying a module or only calling one, and the answer is not supplied by a forge classifier, by the terms of other Rocq plugins, or by the permissive licenses of the solvers whose witnesses the plugin accepts.

**A non-commercial term forecloses vendoring outright rather than pricing it.** DeepSEA is read and cannot become anything else, so the containment that keeps the verified compiler and the control-plane compiler usable as external producers does not apply: the act contemplated there is running a producer, and the act contemplated here is taking source. The grant is read per file and at the revision incorporated rather than off the parent project's name. The plan's Q27c cell states the reason and it is carried here as that cell's rather than as a reading taken in this record: CompCert's own current license dual-licenses a named subset under `LGPL-2.1-or-later` beside the non-commercial agreement, while the older copy DeepSEA vendors names GPL v2 for that subset, so two trees under one project's license do not carry one grant. [The CompCert and SECOMP decomposition](#compcert-and-secomp) enumerates the current subset; which files the DeepSEA tree actually carries, and under which text, is what its own reading decides.

**An absent term is not a permissive one.** The Rocq checker of the CADE-26 certified-checking line ships as a bare archive from its authors' page stating no terms, so it is unlicensed until its authors say otherwise. The `BSD-3-Clause` headers the ACL2 checker beside it carries are a contrast and not a grant reaching the Rocq one, one paper's two artifacts being two grants. Neither is the `lrat_isa` checker that [the static-memory certificate integration](#optional-static-memory-research-tools) already fetches under a read MIT license, which is a separate Isabelle/HOL development: that fetch supplies no terms for this row, and this row's silence says nothing about that fetch. The same reading holds for the CertiSuSLik backends, whose metadata declaration is not made permissive by the synthesizer's `BSD-2-Clause` license file sitting beside them in a different tree.

Three surveyed members decide no consumer here and are recorded as such rather than omitted: [the Lean certificate checkers](docs/assurance/proof-reuse/foundations.md#f09-the-lean-certificate-checkers-unqualified-lead), [SCIP's exact mode and its VIPR certificates](docs/assurance/proof-reuse/foundations.md#f10-scips-exact-mode-and-vipr-certificates-unqualified-lead), and [the pseudo-Boolean proof-logging results](docs/assurance/proof-reuse/foundations.md#f11-pseudo-boolean-proof-logging-in-solvers-reference). Their prover, their unverified solver lineage, or their lack of any machine-checked component disposes of them before terms are reached, so no row is written for a candidate whose terms decide nothing.
