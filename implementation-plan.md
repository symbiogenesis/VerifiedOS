...
The capability-and-provenance C memory model it widens toward is itself already mechanized in Coq (Zaliva et al., a comprehensive CHERI-C semantics adapting Cerberus, ASPLOS 2024), so the backend implements a known semantics rather than devising one; the net-new engineering is the CompCert memory-model widening and code generation, not the semantics.
...
- **Plan:**
  ...
  4. Defer only the *proofs*: the functional-refinement proof (C ⊑ Gallina spec) and the non-interference theorem (§8), layered on later without changing the Gallina spec or the C; when they begin, **CDT revocation leads as the early kill-switch** (the hardest l4v subproof, never done in Coq for an seL4-class capability model, so it validates or falsifies the route first, the seL4-vs-CertiKOS disposition in [architectural-alternatives.md](architectural-alternatives.md)).
