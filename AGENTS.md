...
- **An outside critique is folded, never kept beside the documents.** A critique or assessment arriving at the repository root is a working note no rule reads, so its judgment goes into [docs/critique.md](docs/critique.md), each paragraph naming the plan item that would answer it, its work into the plan's Q-series, and the file is deleted. Every disposition it carries is checked against the register before it becomes a claim or an item, since a row can be false on the day it lands.
