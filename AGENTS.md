...
**Retire only an owned lane whose work is integrated.** Inspect its changes and confirm its commits are contained in the integration revision, remove it with `git worktree remove`, then delete its branch with `git branch -d`. Never force removal, reset an existing branch to reuse a name, or move a live host-managed checkout. The [tool guide](tools/README.md#worktree-isolation-during-fan-out) owns creation, verification and cleanup.
