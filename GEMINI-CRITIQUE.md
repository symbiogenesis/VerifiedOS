...
| a bespoke capability format, presented as one | The field widths are right and the framing is not: R-15-007 re-parameterizes CHERI Concentrate, keeping the bounds algorithm, the capability algebra, and the sentry and instruction semantics and changing the widths and one exponent-packing decision, which is what leaves the Cambridge results reusable and owes a representation-correctness proof rather than a re-proof. |
| a worst-case peripheral latency of one major frame, 10 to 50 ms | No major-frame length is stated anywhere in the corpus, and the mechanism the figure assumes is not the design's: aperiodic events get dedicated polling or sporadic slots sized into the frame (R-07-034), and a device server whose cadence would make the switch constant a dominant term leaves the slot wheel for a core of its own (R-11-011), so the worst case is the polling slot's period and never the frame. |
| a baseline clock of 400 to 600 MHz forced by the DVFS ban | No clock figure is stated in the corpus. The posture the figure guesses at is the design's own and is booked as a cost: R-15-193 provisions to TDP so that throttling never engages and makes critical temperature a fail-stop, R-17-030b carries that as a denial the platform performs on itself, and the energy paragraph of [docs/critique.md](docs/critique.md) prices the sustained-clock consequence. |
...
- **The zeroize tax on fine-grained slots** (3.6) is answered at the context-switch row of [docs/performance-estimates.md](docs/performance-estimates.md): the band is read against a switch rate R-11-011 bounds, a server whose cadence would drive the switch-duty ratio past the composition threshold being pinned to a core where it performs no partition switch at all.
