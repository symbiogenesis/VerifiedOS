...
**The out-of-order rejection is hostage to the memory bet.** *The latency wall it exists to hide is deleted* presumes flat, low-latency, multi-gigabyte on-die SRAM, and R-15-163 makes the vertical tier count conditional on an unsolved low-temperature p-type device result rather than graded by effort. If the memory bet degrades toward a single planar tier, the argument weakens in proportion.
...
**R-05-094 against R-13-012** is the strongest finding. R-05-094 gives the Tier-2 certificate four conjuncts; R-13-012 gives it a six-item subset of R-05-029's eleven, adding definite initialization and data-race freedom. R-13-012's accept text claims the content is read off R-05-029 rather than enumerated independently, while enumerating a set that differs from R-05-094's. That is the self-sourcing rule violated inside the entry that states it.

**R-16-015 against R-08-031**, with the defect on R-16-015's side rather than R-08-031's. R-08-031 says no compartment reads an architectural cycle, time, retirement, or performance counter, and its accept cites R-15-077, which deletes the counters outright. R-16-015 nonetheless lists *raw counter reads by holders of the fine-grained-time permission* among four replay-nondeterminism sources. Since the counters do not exist, the loose phrase is R-16-015's; what it means is time-service reads at granted precision.
