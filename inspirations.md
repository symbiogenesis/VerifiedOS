...
The remaining GrapheneOS answers stay **logged but not yet imported** — the "identified but not imported" posture of the COSMIC lockstep note (above): a **duress PIN/password that wipes on entry** (a crypto-erase of the sealed key, cheap because *lose the key = erase memory*, §15) and **per-connection MAC randomization**.
