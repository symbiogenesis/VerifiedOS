...
A second answer is now imported alongside it: **USB data gated on the lock state** — a charging-only Before-First-Unlock device leaves its USB data lanes' IOMMU window (§15) unopened and defers new-peripheral authorization to post-unlock powerbox consent (§8), so juice-jacking and lock-screen wired extraction have no data path (§12); and because charging must outlive the lock, **USB-PD contract negotiation is specified as a fixed-function sequencer** (no firmware, the radio link-layer timing-block pattern, §12), closing the platform's former USB-PD gap in passing.
A third answer follows: a **duress credential that crypto-erases on entry** — presented instead of the ordinary one, it commands the RoT to destroy the sealing root so every user-data domain becomes unrecoverable in the time it takes to zeroize a key (*lose the key = erase memory*, §15), the coerced-unlock countermeasure now booked as a defended case (§3, §9, §12).
The one remaining operational-security answer stays **logged but not yet imported** — the "identified but not imported" posture of the COSMIC lockstep note (above): **per-connection MAC randomization**.
