...
The remaining GrapheneOS answers stay **logged but not yet imported** — the "identified but not imported" posture of the COSMIC lockstep note (above): a **duress PIN/password that wipes on entry** (a crypto-erase of the sealed key, cheap because *lose the key = erase memory*, §15), **USB data disabled while locked** (the platform's open [USB-PD gap](critique.md) marks the same territory), and **per-connection MAC randomization**.
