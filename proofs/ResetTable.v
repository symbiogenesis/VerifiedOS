...
(* =========================================================================
   ...
   first step that draws entropy. R-15-198 makes mode transitions, standby
   exit and deep-sleep wake re-entries into suffixes of the same table, and
   R-15-190b puts every re-execution of the chain the RoT sequences at such
   a suffix, so the release obligation is stated at every entry point the
   table declares and not on the cold path alone.
   ...
   Q33 owns and the composition checks over it, with an admitted table
   computed and every malformed table the entry names refused. The island
   a domain is bound to is read from MemoryPlan.v's `PowerVector`, the same
   binding its label checks read, so the table carries no second copy of
   R-15-228's map. Every quantity the register leaves to composition is a
   field of the ResetTable record; nothing is admitted and nothing is
   axiomatized, and the Print Assumptions block at the end reports every
   ...
   i.   The rest of the table. Its schema in the composition and its
        ...
        watchdog-bounded timeout, the dependency order among steps other
        than the release points, the absence of a cycle, and entry points
        that are suffixes of the table are the table's own authoring,
        crown-jewel row 12 (R-15-198), which the unassigned proof map
        prices as a separate slice. The step vocabulary below carries what
        R-15-198a's clauses read and no more.
   ...
   iv.  R-15-190b's chain-execution clause and the lowering of the staged
        ...
        through it; and R-15-189g's containment at every release point,
        which reads the mode an early-released island runs.
   ...
   5. The islands ordered ahead and the inference island are fields.
      ...
      and the inference island one index. The order is read on the whole
      table: each island ordered ahead is released before the inference
      island is released and before any of its fills completes, a fill
      being an image-derived domain's completion read, the label read from
      the vector. Its own fills precede its own release by the release
   ...
   6. The entropy clauses are read on the cold path. R-15-198a begins the
      ...
      them; and the verdict's extension stands before the first
      `DrawsEntropy`. That none of the fills and re-verifications draws is
      structural: a draw is its own step and a completion read is not one.
      A fill standing after the verdict is admitted, since with a staged
      release the inference island's re-verification follows the shell
      island's release, and where that release is preceded by a draw it
      follows the verdict too.
   7. Nothing here is gated on a load. R-15-198a's release point is gated
      ...
      a load, and the schema carries no quantity a load could be.
   8. Boolean rather than propositional wherever the witnesses must
      compute, with the release check proved sound and complete against its
      obligation, so a check answering false refutes the table rather than
      reporting that a decision procedure moved.
   ...
   Non-vacuity (R-05-165, R-05-166). The release obligation is stated of an
   arbitrary table and vector and proved of the check in both directions.
   Inhabitation is concrete: a staged table the checks admit, with a
   re-entry it declares, and a second admitted table confirming a
   session-derived domain ahead of the staging; beside them ten refused
   tables, each refused by one check with the other two holding: the
   release check refusing three, the staging check one and the entropy
   checks six, four of which fail one entropy clause each so that every
   clause is separated from the others by a table of its own; the
   one-release-point table, admitted by the release check and refused by
   the staging order, with the one-release-point reading refusing the
   admitted table; and two generated families over the admitted table's
   cold path, every deletion and every adjacent transposition, whose
   verdicts are computed: exactly the eight deletions that remove a
   released island's read are refused, and exactly one transposition, the
   one moving the inference release ahead of its last read.
   (*| BEGIN derived: cited entries |*)
   Owner: docs/requirements-register.md
   Requirements: R-02-003 R-05-163 R-05-164 R-05-165 R-05-166 R-09-006a R-09-006b R-15-247d
      R-15-247f R-15-189g R-15-189j R-15-190a R-15-190b R-15-195 R-15-198 R-15-198a R-15-228
   SHA256: 10f93815f3ed7c35f1edb252ab916a6787254678438ce44e11c4b91b8cfeb3b3
   (*| END derived |*)
   ========================================================================= *)
...
(* -------------------------------------------------------------------------
   R-15-198a's release points (readings 1 to 4).
   ------------------------------------------------------------------------- *)
...
(* -------------------------------------------------------------------------
   R-15-198a's staging order (reading 5).
   ------------------------------------------------------------------------- *)
...
Proof.
  ...
  assert (Hahead := all_of_members _ t.(ahead_islands) a Hst Ha). cbv beta in Hahead.
...
Definition entropy_ok (t : ResetTable) : bool :=
  andb (tests_begin_after_rot_rail t.(table_steps))
       (andb (tests_run_under t.(table_steps)) (verdict_before_draws t.(table_steps))).

(* T4 (R-15-198a, R-09-006a): an admitted table draws nothing before the
   verdict is extended. *)
...
Proof.
  ...
  destruct (andb_split _ _ Hrest) as [ _ Hv ].
...
Definition reset_table_ok (t : ResetTable) (v : PowerVector) : bool :=
  andb (release_points_ok t v) (andb (staging_ok t v) (entropy_ok t)).

(* -------------------------------------------------------------------------
   The demo table, over MemoryPlan.v's `held_vector`: four domains, the
   first bound to island 0 and the other three to island 1, whose labels
   the demo plan's holdings derive (the arenas' domain 1 session-derived,
   the weights' domain 2 and the payload's domain 3 image-derived). Island
   0 is the one island ordered ahead and island 1 the inference island.
   The table brings the RoT up, begins the tests, locks the spine, brings
   the memory controller up, reads island 0's domain, extends the verdict,
   draws, releases island 0, and then reads island 1's domains, the
   weights last, before releasing island 1; it declares one re-entry at
   the first domain step, the spine and the controller staying up across a
   running mode. Every figure is a witness value.
   ------------------------------------------------------------------------- *)
...
Definition late_entry_table : ResetTable := with_entries demo_table (cons 4 (cons 13 nil)).
...
(* The inference island read and released ahead of island 0. *)
...
Definition inference_first_table : ResetTable := with_steps demo_table inference_first_steps.
...
Example the_staged_table_is_admitted :
  ...
  /\ suffix_from 4 demo_steps
     = cons (DomainReady 0) (cons (DomainComplete 0) (cons EntropyVerdictExtended
       (cons DrawsEntropy (cons (IslandRelease 0) (cons (DomainReady 1)
       (cons (DomainComplete 1) (cons (DomainReady 3) (cons (DomainComplete 3)
       (cons (DomainReady 2) (cons (DomainComplete 2) (cons (IslandRelease 1)
       nil))))))))))) :=
...
Proof. exact (release_points_ok_sound demo_table held_vector eq_refl). Qed.
...
(* R3 (R-15-198a, R-15-190b): a re-entry entered past a domain's reads is
   refused, the cold path of the same table being admitted. *)
(*| discharges: R-15-198a, R-15-190b |*)
...
(* The four entropy clauses of one walk, in the order reading 6 states
   them: the tests begun at the first step after the RoT's rail, the tests
   under the spine lock, the tests under the controller bring-up, and the
   verdict before every draw. *)
Definition entropy_clauses (s : list TableStep) : list bool :=
  ...
  (cons (andb (step_precedes EntropyTestsBegin MemoryControllerUp s)
              (step_precedes MemoryControllerUp EntropyVerdictExtended s))
  (cons (verdict_before_draws s) nil))).

(* A table the entropy checks refuse and the release and staging checks
   admit. *)
Definition refused_by_entropy_alone (t : ResetTable) : bool :=
  andb (negb (entropy_ok t))
       (andb (release_points_ok t held_vector) (staging_ok t held_vector)).
...
(* R5 (R-15-198a, R-09-006a): the entropy clauses. Six tables are refused
   by the entropy checks alone. Four of them fail one clause each, a
   ...
   controller, and the controller brought up first. The tests begun after
   the spine lock and the tests run ahead of the table each fail two. *)
(*| discharges: R-15-198a, R-09-006a |*)
Example the_entropy_clauses_each_refuse_their_table :
  ...
  = cons (cons true (cons true (cons true (cons false nil))))
    ...
    (cons (cons true (cons false (cons false (cons true nil)))) nil)))))
  /\ map_over refused_by_entropy_alone
              (cons early_draw_table (cons late_begin_table
              (cons verdict_between_table (cons controller_first_table
              (cons late_tests_table (cons serialized_table nil))))))
  = cons true (cons true (cons true (cons true (cons true (cons true nil)))))
...
Example the_one_point_reading_refuses_the_staged_table :
  ...
  /\ releases_follow_own_reads held_vector one_point_steps = true
  /\ step_precedes (IslandRelease 0) (IslandRelease 1) one_point_steps = true
  /\ staging_ok (with_steps demo_table one_point_steps) held_vector = false :=
  conj eq_refl (conj eq_refl (conj eq_refl (conj eq_refl (conj eq_refl eq_refl)))).
...
Example a_confirmation_ahead_of_the_staging_is_no_fill :
  reset_table_ok (with_steps demo_table session_first_steps) held_vector = true
  /\ staging_ok (with_steps demo_table session_first_steps) kind_read_vector = true
  /\ staging_ok (with_steps demo_table session_first_steps) mislabelled_vector = false :=
  conj eq_refl (conj eq_refl eq_refl).

(* R7 (R-15-198a): the generated families over the staged table's cold
   path. Of its sixteen deletions exactly the eight removing a released
   island's read are refused, and of its fifteen adjacent transpositions
   exactly the one moving the inference release ahead of its last read. *)
(*| discharges: R-15-198a |*)
