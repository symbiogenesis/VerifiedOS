...
| `B-01` | `P-2` | Every SRAM array is ECC-protected and corrected, the validity tag bits take the stronger code, and no sub-granule write reaches the array; interleaving and scrubbing keep a latent error from accumulating and an uncorrectable one is a fail-stop rather than a returned value. The *deliberately induced* fault is the same pair carried further, since it lands outside the model rather than inside it: the mechanism catching each injection is enumerated, a compiler-emitted control-flow signature and a fail-safe decision encoding cover the three sequences where one skipped instruction is the whole decision, and the sentinel is lockstepped because a detector cannot report its own corruption. That half is coverage rather than a theorem and is booked as such | detected / residual | R-15-175, R-15-177, R-15-178, R-15-181, R-16-008b, R-16-008c, R-16-008d, R-17-058b |
...
| `B-01` | `P-6` | `Zkt` and `Zvkt` are the architectural data-independent-timing contract, and bare self-exclusion from that list is not a pass; the two named exclusions are variable-latency divide and the vector accesses whose element addresses distribute over banks at runtime, each admitted only on the flow discipline proving no secret reaches the operand or the address. That contract is the *observation* the cell carries: instruction latency and emitted addresses. The analog observation is outside it and outside every theorem stated over it, the crypto core carrying no masking and no probing-model claim, so the pair is carried architecturally and booked for its analog half rather than covered by one mode. The instruction encoding adds a case to the contract without adding a mechanism to it: fetch latency is fixed per bundle and carries no data-dependent term, and the balancing obligation on secret-dependent arms is restated over encoded bundle count rather than instruction count, discharged statically at composition | hardware-enforced / admission-rejected / residual | R-15-011, R-15-036e, R-15-036f, R-15-053, R-15-085, R-15-085a, R-17-058a |
