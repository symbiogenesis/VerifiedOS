...
| `B-01` | `P-3` | CHERI is the sole spatial mechanism, privilege is a permission on the program-counter capability, and domain entry is a sealed capability jump | hardware-enforced | R-07-018, R-15-068, R-15-073 |
...
| `B-02` | `P-7` | Content addressing names a binary by its closure, so identity is computed rather than asserted and transfer is a set difference | proved | R-13-001, R-13-007, R-13-008 |
...
| `B-05` | `P-7` | The content address is a per-domain keyed digest, deterministic within a domain and incomparable across domains | proved | R-10-015, R-10-017 |
...
| `B-06` | `P-4` | Mutual authentication is required with no silent downgrade, and the legacy generations a downgrade would reach are absent from the silicon | absent | R-12-041, R-12-042 |
...
| `B-06` | `P-7` | No persistent link-layer identifier exists in hardware, every address being a draw from the one entropy root, which is necessary and not sufficient for unlinkability | hardware-enforced | R-15-132, R-15-133, R-17-055 |
