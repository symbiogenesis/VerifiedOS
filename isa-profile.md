...
| Vector | **V** (RVV), VLEN per core class (§7) | R-15-001, R-15-113 |
...
| Keccak-f[1600] | single vector instruction, fixed-latency permutation; **vector-bearing cores only**, not the S-class RoT | RISC-V PQC TG, RVG-84 | R-15-056, R-15-057, R-15-059 |
...
| Initialization-tag plane (Mon CHÉRI-derived) | carried instead by the §5 definite-initialization attribute; one tag plane per SRAM word, not two | R-15-035 |
...
## 6. Microarchitectural mandates carried by the profile
...
| Ztso realization | in-order issue plus a FIFO store buffer; the only reordering is store→later-load bypass. That the buffer exposes no ordering weaker than TSO is a named bring-up gate alongside `Zkt`/`Zvkt` | R-15-015, R-15-016 |
...
## 8. Implementation timing contracts
...
## 10. Freeze and re-pin obligations
