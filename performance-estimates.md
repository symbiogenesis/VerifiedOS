...
| Core µarch | Static-only branch prediction, no BHT/BTB/RAS (§15) | Removed | **−10% to −30%** | branchy control code | The loss is vs a TAGE-class *dynamic* predictor and is **pipeline-depth-driven, not memory-driven**: a branch's cost is the fetch-to-resolve distance, unaffected by the SRAM main memory, so the "no long DRAM round-trips" argument does **not** apply here. The retained *static* predictor (backward-taken / forward-not-taken) is not idle: it correctly resolves loop back-edges (~90%+ on loop-dominated code) at **zero silicon and zero predictor state**, converting the hottest branches from a taken-branch bubble into a correct early redirect. The residual loss concentrates on **data-dependent** branches, where nothing static helps, but the short non-speculative pipe keeps each mispredict cheap (a few front-end bubbles, not an OoO flush) and `Zicond` (gain row) removes many such branches outright. Deterministic by construction (no predictor-state variance), so it adds **no WCET variance** (§11). Compounds with in-order. |
...
| ISA removed | No prefetch / non-temporal hints, Zicbop/Zihintntl (§15) | Removed | **≈0%** | memory-bound streaming | **No downside in this design.** A prefetcher exists to hide DRAM round-trips by warming a cache ahead of demand; this machine has **neither a cache nor DRAM**, so a prefetcher has literally nowhere to place data, and the flat-SRAM load-use latency that remains is hidden by **static instruction scheduling** on the exposed-ILP in-order pipe (the compiler's job, not a hardware prefetcher's). Whatever latency-bound cost exists is already booked under the no-cache / SRAM rows below; scoring it here too would **double-count**. Streaming is bandwidth-bound, and a prefetcher adds no bandwidth. Explicit loads only. |
...
| Mem / security | CHERI purecap, 128-bit capabilities (§7, §8, §15) | Added | **−2% to −12%** | pointer-heavy | Footprint/bandwidth from doubled pointer width; the tag-storage component is now ≈0% (native SRAM tag bits, row below) and SRAM main memory's higher bandwidth and lower latency relieve the pointer-width bandwidth pressure, so the upper bound is tighter than on a DRAM design; many workloads <5%. |
...
| Mem / security | No hardware caches, flat SRAM hierarchy (§15) | Substituted | **−10% to −50%** | latency-bound / pointer-chasing scalar | The L1/L2/LLC hierarchy is deleted: over flat SRAM main memory there is no slow tier to cache, so latency-bound scalar code that would have hit a cache now pays main-SRAM latency on an in-order core (pure pointer-chase can exceed the range), bounded by SRAM's low latency (a small multiple, not the order-of-magnitude a cacheless *DRAM* design would pay) and partly recovered off-device by static layout; ≈0 for streaming and the vector/matrix paths (explicit scratchpads). A deliberate perf-for-proof trade: deletes the dominant WCET term, the cache-timing side-channel class, the coherence protocol and directory, and the way-partition apparatus. |
...
- **General-purpose scalar / interactive code: ≈ −55%, range −45% to −75%** vs a conventional out-of-order RISC-V application core.
  In plain terms, ordinary software *feels like* it runs at roughly **one-quarter to one-half** the speed of what you would otherwise buy, the deliberate, spec-wide price of deleting speculation, dynamic prediction, reactive clocking, and the cache hierarchy, and of paying the CHERI/encryption/integrity/isolation taxes.
