...
| Mem / security | ECC across all SRAM: main memory and on-die arrays (§15, §16) | Added | **−1% to −4%** | memory-bound | User-flagged "a little." Every SRAM array (main memory, register files, scratchpads, native tag bits, integrity-tree-node cache; there are no data caches) carries SECDED, DECTED on the capability-critical bits, with interleaving and scrubbing; correction is off the critical path, so the added cost is mostly SRAM check-bit area plus a small scrub-bandwidth share. |
