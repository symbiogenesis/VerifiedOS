...
| Mem / security | CHERI tag storage: tag table + tag cache on tag-less DRAM (§15) | Added | **−1% to −8%** | pointer-dense / memory-bound | LPDDR6 has no tag bit, so the 1-bit-per-granule CHERI validity tag rides a reserved-DRAM tag table behind a tag cache; a hierarchical tag cache (the Cambridge CHERI *Efficient Tagged Memory* design) holds it to low single digits by exploiting that most lines carry no capability, rising for pointer-dense footprints; address- not data-dependent, so no timing channel; the tag cache is partitioned per island. |
...
| Mem / security | End-to-end ECC: the DRAM path (3 layers) and every on-die SRAM array (§15, §16) | Added | **−1% to −4%** | memory-/cache-bound | User-flagged "a little." On-die caches, register files, scratchpads, and the tag/integrity caches carry SECDED (DECTED on the capability-critical arrays) with interleaving and scrubbing; correction is off the critical path, so the added cost is mostly SRAM check-bit area plus a small scrub-bandwidth share. |
