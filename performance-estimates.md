...
| Mem / security | ECC across all SRAM: main memory and on-die arrays (§15, §16) | Added | **−1% to −4%** | memory-/cache-bound | User-flagged "a little." Every SRAM array (main memory, caches, register files, scratchpads, native tag bits, integrity-tree-node cache) carries SECDED, DECTED on the capability-critical bits, with interleaving and scrubbing; correction is off the critical path, so the added cost is mostly SRAM check-bit area plus a small scrub-bandwidth share. |
...
| Memory-bound streaming | **−30% to −55%** | Integrity tree + no-prefetch + bank partitioning, softened by no-MMU and SRAM's lower base latency. |
