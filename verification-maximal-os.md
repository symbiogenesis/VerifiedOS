...
**Defended:** remote network attackers **including hostile radio infrastructure (rogue base stations and access points attacking the cellular/Wi-Fi stacks)**; hostile web content; malicious or compromised apps, servers, and drivers; malicious DMA peripherals; evil-maid physical access (measured boot + attestation + FDE + authenticated memory encryption with DRAM-wide integrity and anti-replay, §15); **forensic extraction of a seized device that has been unlocked at least once** — the *After First Unlock* state, whose per-profile volume keys are evicted on lock or idle timeout to return the device to the keys-not-resident *Before First Unlock* state through an attested transition (§9, §10, §15); software supply-chain attack in both forms — **tampering** (corrupted artifacts or a trusting-trust compiler: reproducible builds + DDC + proof objects, §13) and **subversion** (a malicious-but-memory-safe upstream dependency, xz/liblzma-shaped: confined at runtime by intra-app library compartmentalization, §13/§14).
...
- **Credential & unlock service.**
  The lock-screen authentication compartment that gates the Before-First-Unlock → After-First-Unlock transition (§9): it matches the primary credential (PIN/passphrase) and runs **biometric matching** — the compartment assignment the biometric credential-vault language (above) otherwise lacked — with the biometric sensor a register-slave streaming raw samples over an IOMMU-confined interface block (§15) and the matcher ordinary contained safe Rust confined to its own sub-manifest (§14).
...
- **Transparent total memory encryption and integrity, mandatory.**
  ...
  **Integrity and anti-replay — a Merkle counter-tree whose root lives in an on-die controller register — are mandatory DRAM-wide, not merely for the sealed root and key material.**
  ...
  Scope: the **physical** adversary, for confidentiality *and* integrity/freshness alike — still nothing against on-die compromise (CHERI + capabilities + IOMMU).
