...
- **The single-mechanism concentration — CHERI alone, a net proof shrink.**
  ...
  (3) **Privilege is now a capability, not a ring — untested at application-class multicore scale:** Machine-mode-only (CHERIoT-lineage) puts CSR / interrupt / context-switch authority behind a CHERI permission on the PCC instead of the S/U ring, deleting the mode-transition machinery, the S-mode CSR bank, and trap delegation from both the microarchitecture and the kernel proof.
...
This section is **non-normative** and now lives in its own document — see **[Evaluated Architectural Alternatives](architectural-alternatives.md)** — while remaining part of the living document.
