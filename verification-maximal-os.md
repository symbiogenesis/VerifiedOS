...
- **Crypto: verified to the reduction, not only the implementation; post-quantum by default** (ML-KEM/ML-DSA).
  ...
  - **(1) Functional correctness** — the implementation refines the abstract scheme.
    **Fiat-Crypto** (Coq-native) for classical field arithmetic; **libcrux/HACL\*** for PQ primitives — the explicit, *interim* F\*/Z3 widening, now with a named Coq-native destination (below), not an open-ended "until ports land."
...
The verified HAL (§5) — the minimal formally verified `unsafe` primitive layer — is a **contained, non-TCB** artifact: it is proven, but verification ≠ TCB membership, and its failure is bounded by CHERI + IOMMU + capability confinement like any other compartment.
...
- **Single address space — no MMU, no VSpace objects.**
  ...
  There is one physical address space and **CHERI bounds are the sole in-core spatial isolation** — with no PMP either (§15), CHERI is the one spatial mechanism and the IOMMU (§15) confines DMA — so the map/unmap invocations, the page-table walk, `satp` switching, and TLB-shootdown paths, and their proofs, are gone rather than verified.
...
- **Non-interference as a formal theorem over the component graph.**
  ...
  The kernel's Coq proof is extended to a non-interference theorem (seL4-NI lineage) over that fixed graph.
  This is a *theorem about the existing structure*, not a new mechanism.
  It covers explicit information flow; the *timing* side is closed separately by the formal isolation semantics of the §15 partitioning hardware — cache, DRAM channel/sub-channel and bank binding, NoC, and coherence-island mechanisms — the two designed to compose into one partition-level guarantee (§17).
  The security policy model joins the crown-jewel list (§17).
- **Declassification = explicit capability use.**
  Information crossing a confidentiality boundary requires a capability authorizing that channel; capability use *is* declassification — already gated, auditable, manifest-declared.
...
- **Verified storage stack — four layers, one prover (Coq).**
  ...
  **One verified codebase, two instances:** system-integrity (TCB) and user-data (outside it — verification ≠ TCB membership).
- **Mutable FS = the L1–L2 versioned CoW B-tree — bcachefs's featureset (aside from compression) made verifiable.**
  ...
  The user-data root is **sealed to the RoT with a monotonic counter** — authenticated storage, so offline tampering and disk rollback are detected.
...
- **Stateless:** running system = immutable image + compiled declarative config generation + enumerated mutable volumes; everything else is tmpfs, gone at reboot.
...
- **Temporal admission:** the task set admits only with a machine-checked schedulability proof — for the static cyclic executive (§7) an **interval-arithmetic check** (Coq, same checker): the slot WCETs fit the major frame and each task's period is harmonic with it, not Prosa-style response-time analysis.
  Monitor availability and watchdog-before-deadline ship as theorems; radio deadlines (HARQ feedback, ACK windows, idle-mode DRX paging reception) are §11-admitted hard tasks; the same proof yields the hardware watchdog's window parameters.
...
- **Ring data plane (the secure io_uring):** all bulk I/O rides bounded SPSC shared-memory rings with notification wakeups (the sDDF/virtio idiom).
  ...
  Zero-copy = a delegated memory capability with a matching IOMMU window, torn down with the session.
...
- **Filesystem and block servers.**
  The user-data filesystem is the non-TCB instance of the §10 four-layer verified stack (L0 journal ⋈ L1 CoW B-tree ⋈ L2 semantics ⋈ L3 confidentiality) — verified, but contained like any server.
...
- **Network:** IPv6-only single stack; verified parsers at every boundary; TLS 1.3 with hybrid PQ key exchange; WireGuard-style tunnels; DNS-over-TLS in its own compartment; Roughtime-authenticated time.
  NIC reachable only by capability; DMA IOMMU-confined.
- **Radio stack (the dissolved modem):** software-defined radio as ordinary contained compartments, no baseband processor anywhere.
  ...
  - **Regulatory guarantee — three immutability layers, defense-in-depth (revised).**
...
- **Drivers:** one compartment per device; register and DMA access go through the verified HAL primitives (§5), so driver *logic* is fully safe Rust — device registers are reached through a **typed register interface** (named, bounds-checked fields in the `tock-registers` idiom) that is the safe-Rust face of the same register-description-language layout the HAL is generated and verified against (§5), so driver code never open-codes a shift or mask; DMA only through explicit IOMMU grants; restartable without reboot.
  USB fully in userland with per-device authorization.
- **Embedded-controller functions, dissolved:** power sequencing and reset control are RoT duties (§15); battery gauging is on-die coulomb-counting analog read by a contained server; thermal sensing feeds the sentinel; keyboard/touch are register-slave scan interfaces behind ordinary drivers.
...
- **Display & render:** per-surface and per-input capabilities; no ambient observation of input or output — keylogging and screen-scraping are unexpressible.
  Secure-path UI for powerbox/consent; capture requires per-window capabilities.
  Rendering is software on V-class cores (§15).
  Surfaces are plain memory under CHERI (single-address-space, no MMU — §15); the only display *device* is the scanout controller — a firmware-free open-RTL DMA block behind a static IOMMU window over the framebuffer (§15).
...
- **`Zicond`** (czero.eqz/nez) — the branchless constant-time select.
  **Doubly load-bearing now:** with static-only prediction and no RAS, branchless selection avoids the forward-branch penalty on hot paths as well as closing the data-dependent-branch leak.
...
- **M-class ×2 — matrix/AI.**
  ...
  **Coprocessor line, drawn exactly:** every byte the matrix unit moves is core-issued with explicit capability operands — no independent DMA mastership, no IOMMU context, no firmware.
...
- **FEC units — attached to the radio V-cores under the same coprocessor line.**
  LDPC/polar/turbo decoders as fixed-geometry arithmetic: deterministic iteration bounds, core-issued capability-operand movement, no firmware.
...
- **On-die transceiver as register-slave datapath:** direct-RF or zero-IF ADC/DAC chains with digital front end (filtering, decimation), configured via capability-gated MMIO, streaming I/Q to/from the radio V-cores through IOMMU-confined windows.
...
A page that escalates to a connection wakes the application islands through that boot-chain-variant path (and the eUICC, §12, a register-slave until then).
...
- **Transparent total memory encryption and integrity, mandatory.**
  ...
  **Integrity and anti-replay — a Merkle counter-tree whose root lives in an on-die controller register — are mandatory DRAM-wide, not merely for the sealed root and key material.**
  ...
  Scope: the **physical** adversary, now for confidentiality *and* integrity/freshness alike — still nothing against on-die compromise (CHERI + capabilities + IOMMU).
  The cost is paid in the spec's standing currency: bulk vector/matrix traffic now carries the Merkle-tree bandwidth-and-latency tax on top of encryption latency, reducing effective throughput below the raw ~100–200 GB/s aggregate (memory subsystem above); the tree walk is address- not data-dependent, so it raises the §11 worst-case memory-access latency deterministically rather than opening a data-timing channel.
...
- **IOMMU mandatory, default-deny;** all DMA through explicit windows.
  Governs true devices only (NIC, flash interface, USB, scanout, audio, radio transceiver stream); V/M cores are cores, governed by CHERI (single-address-space, no MMU).
...
- **Coherence islands = confidentiality domains.**
  ...
  **Across islands: no coherence protocol exists** — cross-island communication only through designated ring windows (§12) under explicit cache management (Zicbom-class, **proven under Ztso + cache-op semantics**).
  ...
  The radio-pinned V-cores form their own island.
...
- **Scanout controller:** the one graphics device — a small open-RTL firmware-free DMA block behind a static IOMMU window; audio I/O follows the same pattern.
...
- **The admission tradeoff — refuse-uncertified-code, and a new tooling obligation.**
  ...
  The exchange is deliberate and one-directional: the hardware universal contract (CHERI + caps + IOMMU) is retained beneath the certificate, so **containment is unchanged — only the admitted set narrows** to code that can carry the proof.
...
- **The single-mechanism concentration — CHERI alone, a net proof shrink.**
  ...
  (3) **Privilege is now a capability, not a ring — untested at application-class multicore scale:** Machine-mode-only (CHERIoT-lineage) puts CSR / PMP / interrupt / context-switch authority behind a CHERI permission on the PCC instead of the S/U ring, deleting the mode-transition machinery, the S-mode CSR bank, and trap delegation from both the microarchitecture and the kernel proof; the wager is that this holds on an application-class *multikernel*, where CHERIoT is only a single-core microcontroller existence proof, mitigated by the locked PMP backstop (§15) surviving as the CHERI-disjoint failure domain via its lock bit (binding even Machine mode) and by the kernel being verified.
  Net: a deletion of the MMU, the walker, the VM subsystem, and the S/U rings with their CSR and delegation surface, against a narrowing of in-band spatial and privilege redundancy to a single proven mechanism — CHERI — with no disjoint backstop, the hedge being CHERI's own formal verification.
...
- **The WCET seam — derived, at the price of a timing-annotation crown jewel.**
  Proving *schedulability given WCET numbers* (§11 — now an interval-arithmetic check over the static cyclic schedule, §7) while naming no vehicle to derive those numbers would assert a verified path where none exists — the same gap the Sail ⋈ RTL seam above addresses for the timing hyperproperties.
...
- **The verified-storage seam — a fresh proof and a re-homing prerequisite.**
  The four-layer storage stack (§10) takes each layer's best-in-class artifact but **re-proves the ported ones in Coq** — VeriBetrFS's B^ε-tree design sheds its Dafny/Z3 proof, Perennial's implementation sheds its Go runtime — so five residuals are named rather than hidden.
...
The open-RTL substrate exists per class — Ara (ETH/PULP) for V-class, Gemmini for M-class, open LDPC/polar cores for FEC, CVA6/Ibex hosts, integrated-OpenTitan upstream, TDM NoCs in research RTL — but **CHERI-purecap extension of each is new RTL and Sail work**, joining the base CHERI-RV64 gap in order: C-class + RVV under CHERI → V-class → M-class (frozen matrix ISA + capability-checked movers) → FEC units → coherence islands + TDM NoC.
...
**Staging:** the system-integrity instance (TCB) leads; the user-data bcachefs-class instance follows on the same codebase; below-the-line block services (Rust) run availability-only from the start.

**Radio staging: the trust structure does not wait for integration.**
An off-die register-slave SDR transceiver (AD936x-class) behind a fixed/switched certified analog front end gives the identical architecture today — dissolved stack, pinned V-cores, verified parsers, three-layer emission immutability — with on-die RF later buying only the physical/interface simplification.
Software existence proofs (srsRAN/OpenAirInterface, openwifi) anchor feasibility; rewriting them as safe-Rust Tier-1 servers with generated verified parsers is a major workstream, and module-style certification of frozen attested radio generations is the parallel path.
...
This section is **non-normative** and now lives in its own document — see **[Evaluated Architectural Alternatives](architectural-alternatives.md)**.
It records evaluated architectural alternatives and their disposition.
Kept out of the normative spec body while remaining part of the living document.
