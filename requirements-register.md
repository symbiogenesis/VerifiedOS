...
**R-15-007b** MUST: Permissions are a **non-orthogonal enumerated encoding**: the admitted permission sets are enumerated at freeze time as a lattice with its join and meet, and monotonicity is restated over that lattice rather than over independent bits. The enumeration is **total over the 5-bit field** — all 32 codepoints name an admitted set, the residue taking the lattice's bottom element — it carries **no software-defined permission bits**, and it **separates `Permit_Seal` from `Permit_Unseal`**, admitting no set that holds both.
