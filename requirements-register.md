...
· Accept: **the profile has declined both other reliefs the sweep could have had, and this is the one that costs nothing.** R-08-004b declines the revocation colour, so the sweep is the only reclamation mechanism there is, and R-08-009 refuses the CHERIoT-Ibex TBRE sweep engine under R-15-010 test 5, so the walk is software whose pass time is the R-08-007a quarantine. This instrument is core-issued, touching memory only where an instruction says so, which is the property that test protects, and it collapses the inspection loop from a capability load and a tag test per granule to one instruction per group.
· Accept: **the ground is issue and not memory traffic, R-15-203 being why.** Native tag bits sit in the array row beside the data and are read in parallel with it, so a tag-only read activates what a data read would and buys no bank cycle; what it buys is the sweep's quantum, a group's tags arriving as one operand rather than as N load-use latencies with no cache and no second issue slot to hide them. The saving is bounded and stated as bounded: the group size where a group's tags are all clear, one instruction where they are all set. What it reduces is the derived constant of R-08-007 and the R-08-007a quarantine and §11 background slot budget downstream of it, never a security bound, containment being the R-08-006 epoch flip.
