...
· Accept: the admissibility argument is R-15-007f's and not a new one, *less* capability semantics on the path rather than more, one check where the sequence performs *k*, against the one distinguished statically-known capability on the machine (R-15-074) and from the one backend that emits it (R-18-014a). It stays one canonical 32-bit instruction (R-15-036a) whose register list and stack adjustment are site-invariant operands, so each realized pair is one dictionary entry serving every site (R-15-036k). It is deterministic in architectural state at fixed operand-value-independent latency (R-15-010 test 1), its latency a function of the static register-list operand, which puts it on the R-15-053 list at one entry per list length (R-15-095); it holds no state across a partition switch (test 3, so nothing joins the R-15-214 flush set), adds no admission-test case (R-15-012), mints no authority (test 4) and runs no walker (test 5), its fixed-count sequencer holding nothing at any instruction boundary because the instruction is architecturally atomic and no interrupt boundary exists inside it (R-15-070). It is a candidate and not a commitment, competing for the same opcode space and freeze measurement as R-15-007e and R-15-067a, and the expected outcome is that it is dropped.
