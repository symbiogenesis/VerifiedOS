...
· Accept: this is the one place the narrowing changes the algebra rather than the representation, so it is proved here rather than inherited: every derivation, `candperm` included, lands in the enumerated set, and no reachable instruction sequence produces one outside it. Enumeration is available because the complete permission lattice and the full set of sealed-capability classes are known at composition time (R-15-005, R-13-001), and bounded: no permission the profile carries leaves the lattice, R-15-074's local/global and `store-local` and R-15-003's access-system-registers included. That a heavily compressed non-orthogonal permission field is buildable and compilable-against rests on **CHERIoT 1.0's 6-bit encoding of twelve permissions, the sole shipped precedent**, so the requirement stands on the bit budget R-15-007 freezes and on one existence proof rather than on a standards-track trend; the upstream history, RVY's own attempt at a compressed encoding included, is carried by [cheri-version-matrix.md](cheri-version-matrix.md). Totality stands in place of an integrity check, and is why R-15-007a owes no legal-permissions invariant for tagged capabilities: a sparse encoding must reject the codepoints it does not use, a total one has none to reject, so the invariant is a property of the encoding rather than a check over it, the residue assigned to the bottom element so the arbitrary part of the assignment fails closed.
· Trace: CJ-CERISE, CJ-TAL-SOUND
...
· Accept: **software-defined permission bits are declined, and the object type is the ground.** An SDP bit is uninterpreted by hardware, and its upstream consumer is a runtime distinguishing software classes of capability it did not create; classification here is composition-time and machine-enforced, the otype set fixed at composition with no runtime type allocation (R-07-002b), and a sealed capability is unusable rather than merely marked: sixteen enforced classes where four advisory bits would sit. An SDP field would also have to come out of this one, the format having no spare bit (R-15-007), and would be permission-shaped state outside the lattice's join and meet, which the monotonicity restatement would have to carry without interpreting. The absence is a **divergence and not an omission**, the upstream mandate carried by [cheri-version-matrix.md](cheri-version-matrix.md) and the disposition by R-17-048a, this dialect being permanently bespoke with the re-pin retired.
· Trace: CJ-CERISE, CJ-TAL-SOUND
...
· Accept: **seal and unseal are separated because both consumers exist and are distinct paths.** The kernel mints a grant slot and hands out a handle sealed with a composition-fixed otype (R-08-004a), and unseals the slot for the duration of a cross-domain call (R-08-004c). A merged element would give the redeem path mint authority, and the redeem path runs on every cross-domain call while minting happens at delegation; mint authority over a grant otype is the authority to forge a delegation, the one thing the address-keyed revocation model assumes only the kernel creates (R-08-004b). The asymmetry upstream split for is accordingly sharper here, and separating makes it structural in R-14-002's sense (absent from the derivation forest by construction rather than true of one audited distribution) at a cost of one codepoint of thirty-two, which declining SDP leaves the field free to spend. R-15-007l applies the same instrument to Store∧Execute, making W^X a property of the encoding and displacing CHERIoT's multi-rooted hierarchy. The split does not reach sentries: capability jump-and-link unseals a forward-edge sentry and seals the return as instruction semantics (R-15-068), holding neither permission.
· Trace: CJ-CERISE, CJ-TAL-SOUND
...
· Accept: this makes R-15-007's "re-parameterized rather than bespoke" usable rather than descriptive (an unstated behaviour is a lookup against the pin, never implementer latitude), and the residue it closes is behavioural in every case: an all-zeroes granule decodes as untagged NULL, which eager zeroize reads back (R-15-182, R-15-060); a sentry installed in `MEPCC` unseals on `mret` as it does when jumped to (R-15-073, R-07-022); and which cause code an unaligned base raises is ISAv9's answer rather than this profile's choice (R-15-073a, R-15-084). Each inherits over the *narrowed* fields, so R-15-007a's representation-correctness proof discharges the format-side cases at the frozen widths and no per-property clause is owed. The inheritance is version-pinned rather than tracking: ISAv10 is not a source and neither is the retired RVY line (R-17-048a), a published Cambridge release being an amendment that reruns the review gate (R-18-034). A behaviour later found to deviate without appearing in the list is a defect in the list, not latitude an implementation may keep.
· Trace: CJ-SAIL, CJ-CERISE
...
· Accept: `CRAM`/`CRRL`'s consumer upstream is a **runtime allocator** asking how far to round a request and how to align it, and R-08-010 deletes the online allocator: allocation here reads a pre-assigned slot. R-15-007c's residue does not restore that consumer, which decides the cluster: a dynamically narrowed subobject's address is a composition-time slot base plus offset plus a dynamic index times a composition-time stride, so its residue class modulo any composition-time alignment is known **where the layout is decided** even where the address is not known to the compartment holding it, and its length is its type. Exactness is therefore statically decidable from the slot plan, and `CRAM` computes at runtime what the plan has already fixed: a question only the layout's owner can answer, answered here for every slot by a composition-time tool.
· Accept: **`CSetBoundsExact` is declined in the stronger direction, inexactness being a leak of authority rather than a loss of precision.** An outward-rounded narrow grants the derived capability up to a 2^-8 fraction of the length beyond the subobject on each side, and under the linear/affine ownership the plan is built on (R-08-011) a narrow that *splits* ownership hands that excess to the recipient while the retainer keeps it. What is owed is that the leak be **impossible**, not that it be reported: a runtime assertion fires in production after the layout is already wrong, which is not where this platform discharges checks of this class. The exactness side condition sits in the CHERI-TAL narrowing rule and is discharged against the plan.
· Accept: **the exception code is refused a second time on its own ground.** R-15-007h puts every failed derivation on the tag-clear side, carrying no CHERI cause code, no `mtval` case (R-15-073a), and no control-flow term in a §15 WCET entry, the cause codes being ISAv9's frozen with the profile (R-15-014). An inexact narrow is **monotone**, the fault `CSetBoundsExact` raises being on a derivation that succeeded, so admitting it would give the profile its only trap on a successful monotone derivation, and a cause code for the one event the rule above deletes everywhere else.
· Accept: the third answer, a **software computation of the exponent**, is relocated rather than declined, and priced where it lands: it is R-08-011's own rounding, run once per slot by the tool that already computes padding and alignment above the threshold, costing no runtime instruction, opcode, Sail clause, or admission-test case (R-15-012). It costs padding, already booked by R-15-007c at one part in 256 of the object and zero below 256 bytes where bounds are byte-exact, plus quantization of a dynamic slice boundary to the enclosing array's representable granule (the same one part in 256, and nothing for arrays at or under 256 bytes). The absence is a **recorded divergence rather than an omission**, its upstream history carried by [cheri-version-matrix.md](cheri-version-matrix.md) and its disposition by R-17-048a.
· Trace: CJ-MEMPLAN, CJ-TAL-SOUND
...
· Accept: **both upstream consumers are absent, and the second decides it.** The first is a garbage collector's pointer-inside-object test, and R-08-010 compiles the heap into a whole-program slot plan with neither a collector nor an online allocator to run it. The second is argument validation at a domain boundary, which this platform does have: R-04-004 gives the switcher save and restore and the local/global discipline, and that discipline bounds a delegated buffer's **lifetime**, enforced on the store path by R-15-074's permission rather than computed at the boundary. The buffer's **extent and rights** are fixed a phase earlier, by the §12 IDL message type and the manifest's import/export tables (R-05-124, R-05-117) over the composition-fixed initial distribution (R-07-019), with R-05-096b's join rejecting overlapping cross-compartment grants; a caller cannot hand across more than the edge admits, and an edge that would is a composition-time join failure, not a runtime branch.
· Accept: **no consumer of the two-operand form survives.** What remains at runtime is whether a delegated buffer is long enough and carries the rights an operation needs, a question about one capability that `cgetlen` and `cgetperm` answer against a composition-time constant, carried with the dialect R-15-007 pins. The containment form buys something only where the checking party already holds the enclosing capability, and the live case of that shape, a peer writing into a window its server owns, is closed by R-05-149a's indices against one exact-bounded base capability, making the containment the hardware's bounds check on the access the server derives. The failure disposition is fixed besides: the fault costs nothing on the path that succeeds, where a tested form spends a forward conditional under static-only prediction at a full mispredict penalty (R-15-019, R-15-054), in code the §11 budget charges to every cross-compartment call, to reach the fail-stop and supervised restart (R-12-001) the failing arm would take anyway.
· Accept: **the instruction is not the inheritance its presence in ISAv8/v9 suggests, and R-15-007j is why.** That clause inherits ISAv9 behaviour over *narrowed* fields; the permission field is not narrowed but re-encoded as R-15-007b's enumerated lattice, and `CTestSubset`'s permission half is a bitwise containment test over independent bits, of which the lattice has no reading. Restating it as the lattice order costs nothing in semantics, the enumerated meet already giving that order, but makes the instruction **bespoke**: its own Sail clause, its own comparator over the bounds encoding decoded at 8 and 6 mantissa bits, and a stated agreement between the order tested and the monotonicity R-15-007b proves over the lattice, rather than an ISAv9 case admitted at narrowed widths. The absence is a **recorded divergence rather than an omission**, the promotion out of the experimental appendix and what it does and does not settle being carried by [cheri-version-matrix.md](cheri-version-matrix.md), and the disposition by R-17-048a.
...
· Accept: **the profile has declined both other reliefs the sweep could have had, and this is the one that costs nothing.** R-08-004b declines the revocation colour, so the sweep is the only reclamation mechanism there is, and R-08-009 refuses the CHERIoT-Ibex TBRE sweep engine under R-15-010 test 5, so the walk is software whose pass time is the R-08-007a quarantine. This instrument is core-issued, touching memory only where an instruction says so, which is the property that test protects, and collapses the inspection loop from a capability load and a tag test per granule to one instruction per group.
· Accept: **the ground is issue and not memory traffic, R-15-203 being why.** Native tag bits sit in the array row beside the data and are read in parallel with it, so a tag-only read activates what a data read would and buys no bank cycle; what it buys is the sweep's quantum, a group's tags arriving as one operand rather than N load-use latencies with no cache and no second issue slot to hide them. The saving is bounded and stated as bounded: the group size where a group's tags are all clear, one instruction where they are all set. It reduces the derived constant of R-08-007 and the R-08-007a quarantine and §11 background slot budget downstream of it, never a security bound, containment being the R-08-006 epoch flip.
· Accept: **neither fixed parameter is residue R-15-007j could close.** An implementation-defined group size is a variable loop bound in a §11 term, which is why the group is fixed with the profile (R-15-014); and the load filter is indexed by a loaded capability's base (R-08-005a), which this instruction has none of, so no filtered reading exists for it to take, and the stored reading is what the sweep needs, a tag standing over a stale capability being what it walks to find.
· Accept: it clears the five admission tests (R-15-010): one Sail clause over architectural state (1); a latency independent of the tags returned, so no implementation may end early on an empty group, which puts it on the R-15-053 list with one entry in the timing-annotated model (2, R-15-011); no state surviving a partition switch, so nothing joins the R-15-214 flush set (3); no authority minted, the result being an integer, and the same lattice element a tag-preserving capability load requires authorizing it, so it reports nothing a sequence of loads would not (4); and nothing autonomous (5). It takes a dialect encoding and contends for none of the custom opcode space R-15-007e, R-15-067a, R-15-067e, and R-15-069a compete for, which is why it is not conditioned on the R-15-014a measured freeze.
...
· Accept: **it is not the sweep's write side, though it reads as it.** The R-08-007 sweep clears the tag of a capability whose base falls under a set R-08-005a revocation bit and leaves its neighbours in the same group standing, a per-granule conditional clear that a group-wide unconditional one cannot express. What the sweep uses it already has: tag clearing here is a property of the **write path**, a non-capability store clearing the granule's tag exactly as a vector store clears every granule it overwrites (R-15-115a) and a non-capability DMA write clears the granules it covers (R-15-183, R-15-204).
· Accept: **the residue is *tags cleared where the data must survive*, and the zeroize discipline empties it rather than narrowing it.** Where a region is reclaimed the data is dead and goes with the tags, which is `cbo.zero` at one fixed per-line latency over both planes' codewords (R-15-182, R-15-060), and a collapsed power domain is readmitted through the array-wide form of the same pass (R-15-189j). Leaving data in place while dropping its tags is the outcome the §7 eager zeroize and the disclosure half of Write-before-Read exist to prevent (R-05-126), so the case the instruction serves is one this platform declines to be in. This is R-15-061's decline in the other half of the same family, with the difference that here the write path has already done the work rather than there being no work to do.
...
· Accept: **`Zicond` cannot be widened to cover the case, the gap being the idiom and not the width.** R-15-054 selects by zeroing one arm and OR-ing the two together, and the recombining `or` is a bitwise operation over a capability value, which R-05-136 refuses at the root; on the merged file (R-15-007i) an integer instruction's result is an integer, so `czero.eqz`/`czero.nez` reaches the destination with the tag gone and the authority with it. A capability needs the **merge** form, which is what a conditional move is and a conditional zero is not, and that carries both polarities rather than one: a two-way select is the two instructions back to back, each writing the destination whole, with no recombining operation left to perform.
· Accept: **both fallbacks are costs the profile has declined elsewhere.** A branch makes the timing of a pointer select a function of the condition, the leak R-15-054 was adopted to close, at a full mispredict-equivalent penalty on a forward conditional (R-15-019, R-15-023). The alternative needing no instruction, storing both capabilities and reloading the chosen one through the capability indexed load (R-15-007e) at a branchlessly computed index, is constant-time and worse on both axes it touches: three memory operations and a load-use latency where a register mux has one entry, and an authority written to a stack slot in order to select it, handing the compartment-switch scrub (R-15-069a) and the R-08-007 sweep one more capability-bearing location. Selecting an address with `czero` and rederiving through `csetaddr` covers only the in-object case, choosing an offset within one authority rather than between two, and says nothing where the candidates differ in bounds or permission.
· Accept: **what it buys past cycles is which half of the constant-time story a secret-dependent pointer choice lands in.** R-05-067 hardens structured secret-dependent code branchless and types it against the CT taint discipline, sending only the unstructured residual to the relational Sail logic and its per-program certificate (R-05-072); a select the ISA cannot express is a branch, and a branch on a secret leaves the typed half for that residual.
· Accept: **selection is not construction**, so the instruction stands clear of the refusal it appears to approach: the result is one of the two operand registers unchanged, no field is recombined and no capability formed, and monotonicity and provenance hold trivially rather than by argument (R-05-136, R-15-007a).
