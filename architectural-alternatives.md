...
That is one to two orders of magnitude better than a cell swap on every axis this design counts, and it is still refused here, for the reason R-17-058a already gives about masking: **its deliverable is a measured reduction factor, not a statement over the Sail model.** Every figure in this entry is of that shape, a 1000× standard-deviation ratio, a 10× dependence reduction, a 97.5% overlap, a mutual-information decay curve. R-17-058a declines masking because a leakage-model theorem is an axiom about the silicon in the §6 sense and because *attenuation no theorem stands on is evidence about difficulty, and difficulty is what this document declines to count everywhere else*; that reasoning transfers to the bitcell and to the periphery unit without modification, and neither gets in through a door masking was refused at.
...
Re-open on R-17-058a's own four conditions, which govern here unchanged: a probing- or leakage-model statement authored as a crown-jewel specification beside the `Zkt`/`Zvkt` one, the array's data-independence theorem carried as a §5 obligation and verified on the artifact, the composition property shown to survive the macro rather than the cell, and any randomness booked against the entropy root and the §11 slot.
...
The 2025 selector study projects SRAM-class *area* at short LLC retention, not storage-class retention or NAND-class density. The reviewed literature demonstrates [200 mm manufacturing integration](https://doi.org/10.1088/1674-4926/43/10/102501), [300 mm array integration](https://doi.org/10.1109/VLSITechnologyandCir46783.2024.10631340), and megabit-class arrays, with foundry engagement still at the research-portfolio stage ([TSMC's stated SOT-MRAM exploration](https://research.tsmc.com/page/mram/5.html) is framed around sub-2 ns, denser-than-6T-SRAM cache memory, not an offered embedded option), not the dozens or hundreds of independently addressed magnetic tiers that would make a 2T-per-bit medium compete with 3D NAND.
...
## Oxide-semiconductor gain-cell (2T0C) working memory: the quadrant the non-volatile entries leave empty is volatile, n-type-only, and its refresh is the sweep the platform already owns; first-ranked dense-class candidate
...
Neither family occupies the remaining quadrant: a **volatile, charge-based, back-end-stackable cell whose worst-corner retention is long enough that refresh degenerates into a fixed, composition-scheduled sweep**.
...
The published trajectory: imec's [first functional 2T0C cell at >400 s retention](https://www.imec-int.com/en/press/imec-demonstrates-capacitor-less-igzo-based-dram-cell-400s-retention-time) (IEDM 2020), then [>10^3 s retention, >10^11 read/write cycles, and <10 ns writes](https://www.imec-int.com/en/articles/capacitor-less-igzo-based-dram-cell-excellent-retention-endurance-and-gate-length-scaling) (IEDM 2021), then [retention beyond 4.5 hours](https://www.imec-int.com/en/articles/disrupting-dram-roadmap-capacitor-less-igzo-dram-technology); a [dual-gate cell at ION = 1500 µA/µm](https://ieeexplore.ieee.org/document/10019488/) (IEDM 2022); a [stacked two-layer vertical channel-all-around bit-cell at 4F² effective area](https://www.researchgate.net/publication/378064875_First_Demonstration_of_Stacked_2T0C-DRAM_Bit-Cell_Constructed_by_Two-Layers_of_Vertical_Channel-All-Around_IGZO_FETs_Realizing_4F_2_Area_Cost) (IEDM 2023); an [8×8 monolithic multi-deck array](https://doi.org/10.1126/sciadv.adu4323) (*Science Advances* 2025); and, decisive for who funds the maturation, [Kioxia and Nanya's OCTRAM](https://www.kioxia.com/en-jp/rd/technology/topics/topics-77.html) (IEDM 2024), a 4F² DRAM on an IGZO vertical transistor at >15 µA/cell on current and <1 aA/cell off current: the commodity DRAM roadmap itself moving onto the oxide channel, where every candidate in the entry above matures on a boutique one.

**Run against the strikes this document actually uses, it clears the ones the non-volatile class cannot.**
- **Volatility is preserved, so objection 4 never convenes.** The state is charge; rail collapse remains the strongest zeroize the platform has (R-15-189n); no line cipher and no epoch-key or tag-reset apparatus is owed; and the capability tag stays a bit beside the data in the same granule exactly as in SRAM (R-15-203). The honest edge of this card is the residual below: hours of retention is also hours of power-off remanence.
- **Its refresh is not the machinery the DRAM deletion targeted.** Conventional DRAM's tens-of-milliseconds retention forces an autonomous distributed refresh engine, with the activation-counting and back-off apparatus deleted alongside it (§15). At minutes-to-hours worst-corner retention the rewrite obligation has the same shape as the background scrub the platform already mandates (R-15-177): a fixed, composition-sized, data-independent sweep (R-15-189k), plausibly the same sweep, verify-and-rewrite rather than verify-and-correct. Nothing is demand-triggered, counted, or sensed (R-15-189h); the cadence is a worst-thermal-corner build constant under the fixed-operating-point discipline (R-15-188).
...
Against the deferred SOT-MRAM shape it dominates on the axes this design counts: volatile rather than cryptographically volatile (no remanence, no cipher stage, no epoch-key protocol), no new §17 physical fault surface, capacitive gate-node writes in place of SOT current pulses (no IR-drop and thermal write envelope, no write-heating endurance driver), and commodity-roadmap maturation.
...
Re-open behind measured triggers of the same kind the entry above sets: (1) a megabit-class monolithic multi-deck array carrying the native tag bit, ECC, spares, and yield repair; (2) fixed read and write latency at the macro, sense path included, across the retention corner; (3) disturb characterization on the actual stacked array, any stacking-borne analog of RowHammer included; (4) Vth drift bounded under the composed bias profile over deployment life; (5) the worst-corner retention-to-sweep cadence and its bandwidth booked against the §11 slot classes and the RETAINED accounting of R-15-189k; and (6) discharge-to-clear characterization across the hours-scale storage node, so that R-15-189n's one-bit confirmation covers the gain cell's longer time constant.
Non-normative; no spec-body change.

**Honest residual (§17):** the hours-scale retention that makes the sweep cheap is an hours-scale power-off remanence window on the same numbers.
Until trigger (6) is measured, rail collapse must be shown to *discharge the storage nodes* rather than merely stop refreshing them, and "volatile" is a time constant rather than a property.
The window is bounded and ends in discharge, which no non-volatile candidate can say; but between power loss and discharge confirmation the cell holds plaintext charge exactly as SRAM holds it for milliseconds, three to four orders longer.
...
Backside power is independently disqualifying because opaque metal blocks IRIS's optical path.
...
That is a real result and it is a resonator, a capacitor array, and a shift register, not a core; the widely quoted 4000-fold figure is a ten-to-fifteen-year projection contingent on integrated MEMS resonators reaching 99.97% recovery.
...
Every constant-time obligation here is stated against the ratified `Zkt`/`Zvkt` architectural leakage model, and instantaneous power draw and near-field emission are *outside* that model (R-17-058a, §17), so there is no theorem for a logic style to improve: the DPA class is not narrowed, not detected, and not transferred, and a quieter rail moves it from unaddressed to unaddressed-and-attenuated.
Worse, a DPA-resistance claim carried by a logic family is an assumption about the silicon rather than a statement over the Sail model, which is the precise ground on which masking was declined at R-17-058a: adopting adiabatic logic *for security* would import that axiom while adding a bespoke cell library, and a leakage claim verified against a physical model the die does not satisfy is the crown-jewel failure mode one layer down.
...
Their side-channel benefit is real and is declined as a *reason*, because the class it would improve is outside the leakage model no theorem here contains (R-17-058a), and because a logic-family leakage claim is the same silicon axiom masking was declined over.
...
It is declined for R-17-058a's reason without modification, the same ground the adiabatic families and the security bitcells are declined on: instantaneous power draw and near-field emission are *outside* the ratified `Zkt`/`Zvkt` architectural leakage model (R-17-058a, §17), so there is no theorem for a logic style to improve, and a DPA-resistance claim carried by a cell family is an axiom about the silicon rather than a statement over the Sail model.
...
The third value's use as a NULL spacer for power-analysis resistance is declined at R-17-058a exactly as masking, the adiabatic families, and the security bitcells are, and is noted as non-additive with the density case in any event.
