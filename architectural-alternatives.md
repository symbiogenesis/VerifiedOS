...
**Where it ranks.** Off the abandon-substrate scale entirely: SEAM-V *is* RISC-V RVV, so unlike the belt/EPIC/EDGE targets it imports without an ecosystem fork, and the objection is purely microarchitectural. Among throughput levers it sits with the FGMT/PRET entry's rejected half and below the admitted macro-op-fusion win: fusion is architecturally transparent, adds no hidden state, opens no timing channel, and *tightens* WCET, whereas SEAM-V's decoupling does the opposite on all four counts.
...
- **The granule write path.** §15 specifies that no sub-granule write exists on the fabric: the atomic unit is the ECC codeword with its validity and initialization tag bits, written whole at fixed latency, with a sub-granule store merged in a constant read-modify-write stage at the controller. A variable-rate block changes size when written, forcing relocation and a variable-extent read-modify-write; the fixed-latency granule contract, and with it a fixed entry in the timing-annotated Sail model, is gone.
- **The integrity and anti-replay tree.** Its freedom from a data-timing channel rests on an address-indexed, data-independent node mapping. Compression makes the address-to-node mapping a function of content, so tree traversal becomes data-dependent: the property the retained tree is credited with is falsified, and the encryption ordering question (compress-then-encrypt) reintroduces the ratio oracle at the controller rather than at the filesystem.
...
**Disposition (declined for the transparent class; the fixed-rate class already normative and here restated as a rule):** no transparent or variable-rate compression exists anywhere in the memory path, no compressed cache, no compressed main-memory pool, no compressed swap (there is no swap), and no runtime memory deduplication; the granule write path, the address-indexed integrity tree, and the native tag bits stand unmodified.
...
- **CryptOpt translation validation: rejected by §5's own standing rule (an earlier form of this spec adopted it).**
  ...
  **What decides against it is the previous bullet.** Once the field arithmetic is verified C through CHERI-CompCert it is *already functionally correct* and, being straight-line, *already constant-time* on that compiler's stock output as verified on the artifact.
  So the equivalence checker's entire remaining yield is **throughput on a path that is already correct and already leak-free**, and §5's standing rule (established on IPET: *any verified tool that exists only to tighten an already-sound bound is inadmissible, take the trivial sound bound*) reads on artifacts as well as bounds: a net-new verified artifact bought for speed alone spends the scarce currency to buy the free one, the platform's axioms exactly inverted.
