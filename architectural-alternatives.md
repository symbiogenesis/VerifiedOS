...
The defect is the trap-and-emulate model and PMP-as-primary, *not* Machine-mode kernel residence itself — which the platform now embraces under capability-gated privilege (the single-privilege-mode entry below): once CHERI governs privilege as a PCC permission, a Machine-mode kernel is confined by unforgeable capabilities, whereas MultiZone's M-mode code is confined by nothing finer than coarse PMP.
...
**PMP is dominated in both roles — and now dropped entirely.**
...
(1) The coarse **PMP backstop** that once supplied a CHERI-disjoint failure domain is itself now dropped (the drop-PMP entry below): rather than hedge a CHERI *logic* fault with a second in-band mechanism, the platform hedges it with the **formal verification of CHERI** (RTL ⊑ Sail §18; the Oxford/Google CHERIoT-Ibex conformance proof; Codasip's app-class deployment) — *verify rather than hedge*.
(2) In the single-address-space model the design already converges on (the Mill entry above), the MMU **never supplied a per-compartment second layer to begin with**: intra-core compartments are separated by CHERI bounds over one shared map, so the page tables were an identity-shaped translation, not the disjoint per-compartment isolation the fault argument imagines — the "second layer" was largely notional. (3) Keeping it would mean **carrying an admission-test-5 violation** (the autonomous walker) as standing defense-in-depth — trading the profile's own determinism rule for a layer whose fine-grained disjointness the SAS design does not actually instantiate.
...
It is bounded, not blind: CHERI is the **primary, byte-granular, formally-modeled** mechanism the whole design already leans on (§8, §13); the IOMMU and coherence-island separation cover the DMA and cross-domain-timing residuals a page table never addressed anyway (physical-reach now resting on CHERI partition bounds, PMP dropped — the drop-PMP entry below); and because the kernel is **bespoke** (seL4's capability / IPC *design* re-proved in Coq, not stock seL4's C), it is built single-address-space-native — dropping the VSpace object classes — rather than retrofitting SAS onto a page-table kernel.
...
The remaining kernel-shrink simplification the terminus names — the table-driven cyclic executive replacing MCS — is **now also adopted** (§7, §11).
...
**CHERIoT is the existence proof, and it now has silicon.**
...
It completes the arc the MMU deletion and the mode collapse began: PMP is a *third* in-band spatial mechanism (after Sv39 and the S/U ring, both already gone), and RISC-V PMP provides a **strict subset** of CHERI's protection at coarse (≈16-region) granularity, so once CHERI is present and formally modeled, PMP is redundant Sail surface.
...
**The bet, stated.**
Resting all in-core spatial protection on CHERI is the same wager as single-address-space and single-privilege-mode, now with no coarse fallback at all.
...
The adopted mechanism re-homes onto that verified lineage — the **address-indexed sibling** of its theorem, proved in the one prover — the same move this document makes elsewhere (adopt the *design*, re-home the *proof* onto Coq: seL4 below, VeriBetrFS in §10), now applied to a capability *extension*.
...
- **CHERI is a wash, now with an implementation to verify against.**
...
What transfers from seL4's maturity is the **design, ABI, C implementation, abstract specification, and the now-in-scope multikernel/CHERI engineering** — *not the proof.*
...
The claim is thus conditional and honest — **superior iff (a) engineering is free and (b) seL4's 2024 completion is in-scope**, both now stipulated.
...
It is best read as the **terminus of the kernel-shrinking directions** — the **now-adopted table-driven cyclic executive that has replaced MCS** (§7, §11) atop the **now-adopted single-address-space CHERI isolation that has deleted the VM subsystem** (the MMU-deletion entry above), the two base simplifications having collapsed the kernel to exactly that frozen state machine.
...
It is the terminus of the kernel-shrink path (the now-adopted cyclic executive ⋈ the now-adopted single-address-space MMU-deletion, above, which subsumes the frozen-page-table step), coherent only **after** those simplifications land — as they now have — **and** the seL4-in-Coq specification and proof are complete and frozen — a gen-2 direction contingent on spec-freeze, in the same "iff the binding constraint appears" slot as the belt, not a base move.
...
It is logged as a **spec-freeze-contingent gen-2 direction** (terminus of the now-adopted cyclic-executive and MMU-deletion simplifications, above).
