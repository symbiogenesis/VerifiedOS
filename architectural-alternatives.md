...
The profile forks standard RVV twice (scalar-FP-free at R-15-040, `vstart`-free at R-15-040a), deletes the C extension and encodes code in a bespoke fixed-rate dictionary format instead (R-15-036a), mandates Ztso in place of RVWMO, freezes a bespoke matrix extension, a bespoke Keccak instruction, a bespoke capability indexed load/store (R-15-007e), and a bespoke bitfield extract/insert (R-15-067a), runs a re-parameterized 64+1-bit capability format whose RVY re-pin is retired (R-15-007, R-17-048a), and runs M-mode-only purecap with CHERI as the sole protection mechanism.
...
1. **It wins on a booked scarce quantity, not on cycles.** Fusion takes the cycles for free, so a cycle argument is either already collected or unfalsifiable. The two quantities that count are **proof surface** and **code size**, the latter an admission quantity rather than a preference: no I-cache, the code-size penalty the `C` exclusion books at R-15-036, and a composition-time SRAM capacity budget.
...
**One item was kept separate and has since resolved, in the direction the separation predicted.** The adjacent question is code size, and the tempting answer was **reviving the C extension**, or a restricted `VerifiedOS-C` profile of compressed aliases of already-admitted instructions. That answer is now foreclosed rather than merely deferred, and the ground is the sentence this paragraph originally used to set it aside: an amendment on this axis must buy code size *without* reopening variable-length fetch, mid-instruction reinterpretation, or the fetch-alignment machinery. The **fixed-rate dictionary encoding** now normative at R-15-036a does exactly that, and dominates a restricted `C` on both of gate 1's quantities at once. On **proof surface**, its decoder is a constant `Fin N → Instr` into the already-admitted instruction type discharged by reflection, where RVC's expansion function carries field extraction, sign extension, immediate scaling, register remapping, and a set of reserved, hint, and illegal-immediate cases the profile would inherit rather than choose; and its decode is a function of a bundle and a slot index, so the entry-point argument RVC forces on the binary-level proofs has no case to close. On **code size** it is simply denser (R-15-036h). Two further facts finish it: RVC has no compressed form for this profile's own hot instruction, the indexed capability load and store (R-15-007e), so "adopting the standard" was already partly illusory; and the conformance value that made a standard encoding worth its inherited baggage is spent, R-15-007d having retired the RVY re-pin outright.

The methodological point generalizes past this instance and is why the gate is retained rather than retired: **the gate did its job by admitting a container rather than an instruction.** Every candidate the class enumerated was an *instruction*, and all three failed. The item that passed is a **fetch format below the instruction level**, which is why it satisfies gate 4 vacuously (no opcode space is consumed, so no collision with a future ratified extension is possible and no re-pin obligation arises) and gate 3 structurally (a ROM constant and a stateless decoder join no flush set and add no admission-test case). A gate written for instructions did not have to be widened to admit it; the item simply cleared every clause as written (R-15-036d). The lesson recorded for the next proposal: the amendment class is not coextensive with the *instruction* class, and an item that wins on the same booked quantities without touching opcode space is measured by the same five gates and can pass them all.
...
**Disposition:** all three declined, and the *instruction* class is closed rather than open: test-bit-and-branch buys 4 bytes on a pattern fusion and the static-prediction rule already serve, a bespoke base ISA trades the oracles that report spec-versus-intent divergence for a delta the amendment shape captures anyway, and bespoke capability semantics forfeit the Cambridge results on the arrow with the least slack.
The gate itself is retained as the standing test, and has since been exercised once with the opposite outcome: the fixed-rate dictionary encoding cleared all five clauses as written and is normative at R-15-036a, retiring the code-size penalty R-15-036 formerly accepted and foreclosing `VerifiedOS-C` with it. A later proposal is therefore measured rather than argued, in either direction.
Non-normative as to the three instruction candidates; the encoding entry above records a spec-body change made elsewhere.
...
The format that beats both on this axis, Franz and Kistler's **Juice** slim binaries (an adaptively compressed abstract syntax tree, the artifact a type checker most wants to see), fails on the other half of the problem instead and is weighed in the Oberon-system entry below: shipping a tree requires a **code generator on the device**, which §14 admits none of, and would move the compiler into the on-device trust base at the moment §5 and §13 are moving it out.
...
CHERIoT already replaces container-style loading with compartment export and import tables and sealed entry points, sealed by a loader; the platform adopts that structure (sentries and the switcher, §7, §8, §15) and re-grounds it on this platform's own RV64 capability format (64+1 bits over a 36-bit address space, R-15-007) and a verified reader, rejecting CHERIoT's encoding and unverified loader, the same adopt-the-structure, reject-the-encoding move the ISA profile makes for the rest of CHERIoT: what is rejected is a second encoding forking the model (R-15-005), not the narrowing itself.
...
The lineage is older than any of them: **Oberon** (Wirth and Gutknecht, 1987) is the pole's origin and its purest statement, a single-address-space system with no processes, no rings, and no capabilities, whose entire isolation story is that every instruction came from the trusted compiler; the entry below weighs its system mechanisms one at a time, and Singularity is best read as Oberon's modules with a verified channel contract added, Verve as the first attempt to give Oberon's premise something to stand on.
...
## The Oberon system model: executable text, the collector, load-time module linking, condition monitors, syntax-tree mobile code, and RISC5 as a substrate; six mechanisms weighed one at a time, none imported

The entry above rejects language-based isolation as a *sole* mechanism; **Oberon is that pole's origin**, thirty years ahead of Singularity and with the trusted runtime replaced by a trusted compiler and nothing else, so the family deserves the same mechanism-by-mechanism reading the belt, EPIC, and Wasm entries get rather than a verdict inherited from its descendants.
It also deserves it for the opposite reason: what the family got *right* is imported wholesale in [inspirations.md](inspirations.md) (whole-stack parsimony as a method, the quiescent point for deferred bulk work, the module key as a load-time refusal, Oberon-07's deletion gate, and the Active Cells and Oberon-V convergences), which makes the question here narrow and answerable: of the **system** mechanisms Oberon and its successors built, does any one of them import?
Six are separable, and the answer is no six times, each for a reason the platform has already recorded elsewhere.

**1. Executable text: the signature idea, and the one class the design most deliberately deletes.**
In Oberon there is no shell and no terminal, because any text displayed anywhere is a command surface: middle-clicking the token `Module.Command` invokes an exported parameterless procedure, which then scans the surrounding text for its own parameters.
The steelman is real and is not a user-interface preference: it deletes an entire subsystem, since a system whose documents are its command line needs no command interpreter, no quoting layer, and no second language to learn, which is the *deletion* instinct §4 runs on.
It fails on the mechanism rather than the intent, and fails twice over: the resolution step is **a name looked up in a global module namespace**, and the arguments are **text the invoked procedure parses**, which are precisely the two bug classes §12 and §13 render unexpressible (path-and-name resolution, and command/argument injection), the second of which is the reason no component here turns composed text into an action.
A third count is authority: an Oberon command runs with the authority of the entire system, the ambient-authority pattern §8 exists to make unstatable.
The distilled atom is the *interaction* property, that invocation lives inside the document rather than in a separate shell, and it is already banked twice: as Plan 9's plumber and BeOS's translators re-grounded on capabilities and typed messages ([inspirations.md](inspirations.md)), and as §12's typed command interpreter, which pipes typed values between typed-signature commands instead of a byte stream each stage re-parses.
What is refused is name resolution, not clickable invocation.

**2. The collector: the mechanism is rejected, its scheduling rule is the one thing the family exports.**
Oberon deletes explicit deallocation by adding an automatic mark-and-sweep collector, which is the trade §10 refuses outright (no managed runtime, no garbage collector) and which the HLLCA entry above already books against the Lisp- and Java-machine branch on WCET grounds: unbounded pauses falsify the §11 tables.
The platform makes the *opposite* deletion on the same axis, removing the **allocator** rather than the free operation and replacing it with the whole-program static memory plan (§8), so Oberon lands on the far side of a decision already taken with five converging lines behind it ([inspirations.md](inspirations.md)).
The family's own real-time descendant confirms the direction rather than the mechanism: **Oberon RTS** has to promote the collector to an explicit scheduled process with dedicated hardware timers before the system will hold a deadline, which is the admission that an implicit collector and a timing contract do not compose.
The separable atom is not the collector but the **rule by which it is scheduled**, that deferred bulk work runs only where the root set is statically known, and that atom imports to §8's revocation sweep rather than to any collector; it is developed in [inspirations.md](inspirations.md) and is the one proposal in this family with a spec-body consequence.

**3. Load-time module linking: the check imports, the linker does not.**
Oberon loads modules on demand, links them against the module table at load time, and can unload them, with the interface key checked at exactly that moment.
The *check* is imported and re-grounded (content-addressed closures and the CHERI-TAL admission pass, §10, §13); the *linker* is deleted by the entry above, which declines an on-device loader as an attacker-facing grammar in the trust base, and by §7's static composition, under which the callee set is enumerated before anything runs and there is no moment at which a new name could be resolved.
Load-time linking is the mechanism that makes Oberon's key check possible and is not the reason it is valuable: what makes it valuable is refusing a mismatch with no compatibility shim, which survives the deletion of the linker intact.

**4. Active Oberon's active objects and `AWAIT`: the family's concurrency answer, and a runtime scheduler in notation.**
Active Oberon (Wirth and Gutknecht, 1996 to 1998), carried into the **A2** operating system by way of AOS and Bluebottle, gives an object an activity of its own, marks type-bound procedures `EXCLUSIVE` to make them per-instance monitors, and blocks an activity on `AWAIT(condition)`, re-evaluating the waiting conditions whenever an exclusive region on that object is left.
The steelman is the strongest in the family: synchronizing on **conditions rather than signals** deletes the lost-wakeup and hand-rolled-condition-variable classes by construction, which is this document's own favorite move, and A2 demonstrated that the model carries a real multiprocessor operating system rather than a research kernel.
It fails at §7 and fails structurally: re-evaluating a set of conditions at every monitor exit **is a runtime scheduler**, its cost is a function of how many activities are waiting on that object and what they are waiting for, so it is data-dependent dispatch on a critical path, and the model presumes the preemptive priority scheduling A2 supplies and §7 deletes entirely (no priorities, no run queue, no runtime scheduling decision, no budget donation).
The property `AWAIT` buys is bought here by three constructions that carry no scheduler: bounded SPSC rings with linear ownership transfer and explicit atomics (§12), latched pending state read by ordinary loads in the owner's own slot (§7, the asynchronous-interrupt-deletion entry above), and time-triggered slots that need no wakeup at all because arrival is polled at a time already proved to fit.
The *other* half of the active-object idea, the object as a statically composed unit of concurrency with explicit channels, is convergent rather than rejected, and is banked as **Active Cells** ([inspirations.md](inspirations.md)) alongside Barrelfish and Cerebras.

**5. Juice and slim binaries: mobile code as a compressed syntax tree, rejected at the on-device code generator.**
Franz and Kistler's **Juice** (ETH, then UC Irvine, 1996 onward) distributes Oberon programs as an adaptively compressed **abstract syntax tree** rather than as bytecode or machine code, denser than Java class files and semantically higher-level, with the target generating native code on arrival and able to re-optimize it later.
The steelman is that it is the most verification-sympathetic mobile-code format ever shipped, since a syntax tree is the artifact a type checker most wants to see, which is a real point against both ELF's offset-linked tables and Wasm's stack machine (the packaging entry above).
It fails on the half that is not the format: shipping a tree **requires a code generator on the device**, which is runtime code generation (§14 admits none), demands a writable-then-executable path the W^X invariant has no room for, and would move the whole compiler into the on-device trust base at exactly the moment §5 and §13 are moving it out, since the checked artifact would then be one the device itself produced.
The distilled atom, that what is admitted should be a structured, typed, cross-reference-free object rather than a container to be parsed, is already the content-addressed capability image with its CHERI-TAL derivation carried as a first-class component (§13), which is Juice's insight with the code generation performed off-device and the tree replaced by a typing derivation over the final machine code.

**6. RISC5 as a substrate: the sharper statement of the criterion the minimal-ISA entry applies.**
The minimal-ISA entry above rejects OISC and transport-triggered architectures partly for being too extreme to survive as a substrate; **RISC5 is the same axis's moderate point, and unlike them it shipped**: fourteen instructions, sixteen registers, single-cycle, a few hundred lines of Verilog, carrying a compiler, an operating system, and a graphical environment on a low-cost FPGA.
So it is the strongest existing counter-evidence to that entry's disposition, and it loses anyway, on the substrate-cost disqualifier (32-bit, no vector unit, no capability extension, no atomics story, no verified compiler backend, and every artifact stated against its model would have to be re-minted) and on a criterion this entry states more sharply than the minimal-ISA one does: **the instruction set of record is chosen by whose semantics is already mechanized, not by whose instruction count is smaller.**
A thousand lines of Verilog nobody has formalized is *more* proof work than a large model somebody already mechanized in the prover the budget holds; RISC5 has a book and a Verilog file, RV64 has a Sail model, CHERI, RVV, and CompCert.
The same verdict falls one layer down on **Lola-2**, Wirth's own logic-description language, in which RISC5 was restated after the Verilog: it is smaller and more regular than SystemVerilog and has no mechanized circuit semantics in Coq, which is exactly the anchor test the HDL entry below applies to Hardcaml, Chisel, SpinalHDL, and Clash, and Lola-2 fails it with less industrial weight behind it than any of them.

**Where it ranks.**
Off the abandon-substrate scale for five of the six, which are operating-system and language mechanisms that change no instruction set; RISC5 alone lands on it, beside OISC and TTA but a rung less radical and with a working system behind it, which is why it draws the sharper statement of the criterion rather than the same verdict.
Against the language-based-isolation entry above, this is the ancestor rather than a sibling: Singularity's software-isolated processes are Oberon's modules with a verified channel contract added, and Verve's typed-assembly nucleus is the first serious attempt to give Oberon's premise, that every instruction came from the trusted compiler, something to stand on.

**Disposition:** nothing imports as a system mechanism.
Executable text is the name-resolution and text-to-action class §12 and §13 delete, with the interaction property already banked as the plumber, the translators, and the typed command interpreter; the collector is refused by §10 with the static memory plan taking the opposite deletion on the same axis; load-time linking is refused by §7 and the packaging entry above while its key check is imported as content-addressed admission; `AWAIT` is a runtime scheduler §7 has no place for, its lost-wakeup guarantee re-bought by rings, latched state, and time-triggered slots; Juice needs an on-device code generator §14 admits none of; and RISC5 and Lola-2 lose on mechanized semantics rather than on size.
What the family does contribute is a method and one scheduling rule, both recorded in [inspirations.md](inspirations.md).
Non-normative here; the single proposed spec-body consequence, binding §8's revocation-sweep quanta to the swept domain's slot boundaries so the sweep never overlaps its own mutator, is stated in that document and is not yet taken.
...
Lockstep, meanwhile, was **logged for G5** (COSMIC dual-core lockstep, a fault-*detection* complement to §7's per-core kernel duplication; [inspirations.md](inspirations.md)) and has since been **imported for exactly one core**: the S-class sentinel runs as a detection-only lockstepped pair (§16), on the reasoning below that the software alternatives dominate everywhere the doubted core can host the checking software, and that the resident detector is the one place it cannot.
...
By those axes the software options *dominate* hardware DCLS here and are the preferred G5 answer to the datapath residual, hardware DCLS reserved for the two cases they cannot serve.
The first is an ultra-tight fault-tolerant time interval of a few cycles, for hard actuation.
The second is **the detector itself**, and it is the one the platform actually carries: every software option below is a program running on the core being doubted, which is sound where something else observes that core and circular where the core in question *is* the observer, so the S-class sentinel (§15) is realized as a detection-only lockstepped pair reporting divergence to the RoT (§16) while every other core stays unreplicated. The tier above catches a sentinel that stops; nothing but a twin catches one that continues having suppressed the event it existed to raise. Its area cost is the smallest instance of the figure above, the minimal scalar core doubled, and its comparator holds no architectural state, so the Sail model is untouched on the ground §15 states for radiation hardening.
The software options, then, for everything that is not the detector:
...
All of these stay **deployment-graded to G5**: nothing is carried by default, the accepted residual (a rare wrong datapath result) standing for the consumer build exactly as today. The sentinel pair is the one exception and it is not a grade: it is carried in every instantiation, because what it protects is the detection posture itself rather than a deployment's reliability target.
...
Lockstep/TMR random-fault masking stays a **G5 gen-2 option**, subtractively admissible at its honest 2–3× *core-logic* area (the arrays stay ECC-protected, §15) if a safety case ever demands masking over containment; lockstep *detection* is no longer deferred where the doubted core is the detector, the sentinel pair being that case and the only one.

**Where it ranks.**
Off the abandon-substrate scale (a reliability/verification microarchitecture, not an ISA): DIVA ranks as the *microarchitectural sibling* of the zkVM entry and below the adopted approach on the same "static all-inputs proof beats per-run check" grounds; lockstep/TMR ranks as a deferred G5 reliability option for the datapath, and as an imported one for the sentinel alone.
...
That dependency is **discrete rather than graded**: tier count is gated on complementary devices at a back-end thermal budget, and low-temperature p-type is unsolved, so the honest two-case reading is a working vertical lever with tier count as an ordinary cost question, or no vertical lever and a single planar tier near 2 GB at a full reticle (the chiplet entry below carries the argument, and §18's staging is written against the one-tier case).
...
That ceiling is sharper than a quality cap, and the sharp part is what the residual below books rather than this paragraph: a 6T cell is *complementary*, so every tier needs pFETs as well as nFETs built at that temperature, and low-temperature p-type is the unsolved half.
The oxide semiconductors that survive a back-end thermal budget (indium-gallium-zinc-oxide and its relatives) are **n-type only**; crystalline-silicon pFETs need a front-end activation anneal the finished tier beneath cannot take; and the p-type candidates that would close the gap (tellurium, tin monoxide) are research-stage in the specific way that matters, their hole mobilities already usable while their switching is not, tin monoxide reaching tens of cm²/V·s at a sub-threshold swing and on/off ratio no memory cell can be built from.
The route that sidesteps the anneal is **layer transfer**, bonding a separately annealed blanket layer and patterning it in place, and it is worth distinguishing from the bonded die-stacking declined above rather than letting the resemblance stand: what is transferred is unpatterned, carries no mask set and no separate identity, and is patterned in the same lot with the tiers below it, so the trust structure stays singular; what it does add is a **second wafer supplier**, which is a §17 supply-chain fact and not the second-die boundary §15 refuses.
...
**And the shape of that risk is discrete, not gradual, which is what the fallback it feeds has to be read against.**
"Bend capacity, never the mechanism" reads as though a shortfall arrives as fewer tiers, so that eight becomes five and the roster shrinks accordingly; but the gating item is the low-temperature p-type device above, and that item does not under-deliver by degrees.
Either complementary devices are buildable at the back-end thermal budget, in which case tier count is an ordinary yield-and-cost question and the graded fallback behaves exactly as written, or they are not, in which case the answer is **one planar tier** and the vertical lever is absent rather than reduced.
The interim capacity in that second case is the single-tier figure, near 2 GB at a full reticle and near 1 GB at a High-NA half field, and the phone envelope is not reachable at all rather than reachable late.
Naming it this way changes no disposition (the bonded stack and the chiplet stay declined, and a shortfall still bends the roster), but it changes what the staging plan is entitled to assume: the 8-to-16-tier step is a **discontinuity gated on a materials result**, not a schedule slip, and the roster that has to work is the one-tier roster.
A second maturity fact belongs beside it. **CFET is confined to the upper memory tiers, and CFET is a front-end technology**: the demonstrated device stacks nFET over pFET in the same front-end flow, with commercial introduction projected around the A7 node in the early 2030s, so placing it on tiers that are by construction back-end and low-temperature presumes the sequential, layer-transferred flavour rather than the demonstrated one.
That is admissible and is what the entry means, but it stacks the design's two least-mature levers on the same tiers, and the CFET density gain (on the order of a 35 to 45 percent 6T cell-area reduction against a fin-based cell) should be read as conditional on the same materials result rather than as an independent lever.
...
**The one thing that does touch the ceiling, and why it is not the class this entry declines.** The **fixed-rate dictionary instruction encoding** (R-15-036a) is not a stored artifact that expands at load; it is the **resident form of code**, fetched and decoded in that form on every cycle, so it reduces live SRAM bytes rather than stored ones and is a capacity lever in the sense the paragraph above denies to artifact compression. It is nonetheless on the admitted side of the sorting rule for every reason the rule exists: it is **fixed-rate** (a fixed-width bundle, fixed-width slots, a constant dictionary), so no coded unit is content-dependent and the fixed-latency memory controller's constants are untouched; it carries **no runtime compressor and no decoder state**, so there is no occupancy or timing oracle to read and nothing surviving a partition switch; it needs **no compactor or recompressor**, the encoding being frozen with the image; and it never touches a **shared** resource, code being immutable, freeze-committed, and publicly hashed, so the *Safecracker*-class mechanism cited above has no attacker-chosen plaintext to introduce and no victim secret to co-locate with. The native tag bits are likewise unaffected: the encoding applies to instruction fetch, not to the granule write path, and capability-bearing data is untouched. It is, in short, the strongest instance of the rule rather than an exception to it, and the reason this entry's disposition can admit it in the same breath that it declines the transparent class.
...
**Honest residual (§17):** the capacity ceiling the SRAM entry books is *narrowed* by the fixed-rate encodings, never removed, and narrowed by a constant factor known at composition rather than an opportunistic one, so the standing fallback for data remains to bend capacity: fewer origins, smaller models, and a leaner roster, never a reactive memory mechanism; resident code has the distinct last-resort alternative below. Declining variable-rate compression deliberately forgoes the average-case footprint a transparent compressor would reclaim on compressible data, the accepted price of a capacity figure that is arithmetic rather than statistical; and the quantization lever is lossy, so accuracy, not safety, is the axis it spends, a model-quality tradeoff owned by the compartment and visible at admission rather than a platform property.
