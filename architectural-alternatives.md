...
The 2T SOT topology also improves on the older 1T STT comparison this entry previously leaned on: the read current crosses the MTJ while the write current runs through the SOT channel, separating read and write optimization, sharply reducing read disturb, and keeping the high write current out of the tunnel barrier.
...
**The first correction: 2T SOT is not the one-transistor, DRAM-class cell this entry previously described.**
