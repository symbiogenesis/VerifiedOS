...
**Resolved (this revision): the consistent fix is taken — deterministic refresh replaces the reactive loop.**
...
✅ BLE link-layer timing (150 µs T_IFS) in software under TDM scheduling is arguably harder than LTE HARQ and is nowhere addressed despite BT L2CAP/GATT appearing in §12. **Resolved (this revision): the sub-slot turnaround is booked below the software line as a fixed-function timing sequencer, not a foreign computer.** The one radio deadline too tight for software under interrupt jitter — BLE T_IFS (150 µs), 802.11 SIFS (10/16 µs), 802.15.4 turnaround — is met by a **fixed-function timing sequencer in the register-slave transceiver datapath** (§15): a hardware timer plus a small finite state machine that, on a hardware packet-end event, flips the RX/TX path and gates a software-prepared buffer out at the exact deadline, fetching no instructions and running no firmware (the FEC/digital-front-end category, not a controller), and passing the five-part admission test the way the digital front end and I/Q DMA already do (deterministic; fixed 150 µs so no data-timing channel; bounded state reset per event; register-slave, no authority; scheduled-DMA autonomy, not a memory walker). Everything with protocol semantics stays in software — connection-event/slot scheduling (a §11 hard task), channel selection, framing/whitening/CRC, link-layer encryption via the crypto core, and the link-layer state machine as a Lustre control plane (§12). This is the **split-MAC / SoftMAC** discipline (time-critical turnaround in fixed hardware, protocol in host software), the same *hardness-at-the-boundary, patchable-software-above* rule the regulatory layering applies to emission one level down; a Bluetooth/Wi-Fi *controller* (FullMAC firmware) is rejected as a §4 foreign computer, and pure-software turnaround (Sora-style dedicated cores) as spending the tightest budget on the most jitter-sensitive path. Prior art confirms the partition is standard: Linux `mac80211` SoftMAC, Nordic/Zephyr's hardware BLE tIFS timer, and — the firmware-free open-RTL start-from — openwifi's FPGA low-MAC (§18). See the link-layer-timing entry in architectural-alternatives.md and §12/§15/§18 in verification-maximal-os.md.
...
✅ CHERI's headline intra-app use — compartmentalizing a linked third-party library so a malicious or buggy dependency is confined to the capabilities it actually needs — is unbooked: the finest compartment the spec draws is one-per-app (§14), so a linked library sits *inside* that single compartment with the app's full authority.
The Tier-2 whole-app memory-safety certificate (§13) plus `#![forbid(unsafe_code)]` and the ban on `-sys` C libraries delete the *memory-safety* dimension of a bad dependency but not the *authority* one — a memory-safe-but-malicious library (the xz/liblzma logic-backdoor archetype) reaches everything the app can.
The fix needs no new mechanism: the CHERIoT switcher, sealed capabilities, and per-compartment manifests are all present, just never applied below the app line.
Relatedly, "software supply-chain tampering" is listed as Defended (§3) but the named mechanism is build-pipeline integrity (reproducible builds + DDC + proof objects + SBOM) — artifact tampering-in-transit, not runtime confinement of a subverted-but-memory-safe upstream.
**Resolved (this revision): the mechanism is booked as a first-class, statically-composed capability.**
§14 gains a normative *intra-app library compartmentalization* bullet — an app is one least-authority *domain* that may be statically sub-divided into library compartments, each a node in the machine-checked component graph (§7) with its own least-authority sub-manifest (§8), cross-compartment calls mediated by the same seal/switch primitives (§7, the CHERIoT switcher) that separate whole apps, the intra-app graph *fixed at build time* so it neither mints dynamic privilege nor escapes the §8 non-interference theorem.
It is **required** for dependencies that parse attacker-controlled input (codecs, decompressors, wire parsers) and for third-party libraries handed capabilities beyond compute, available to every app otherwise — so a subverted-but-memory-safe dependency (the xz/liblzma archetype) is confined to its sub-manifest rather than the app's full authority, the per-origin browser discipline generalized to the dependency graph.
§13 now books supply-chain defense as *two* mechanisms — build-pipeline integrity for tampering, compose-time authority confinement for subversion — and §3 splits the threat-model entry accordingly.
No architectural-alternatives.md entry: like macro-op fusion, this applies an already-present mechanism (the CHERIoT switcher, sealed capabilities, per-compartment manifests) at a finer grain, not an evaluated alternative.
...
✅ Fourth: you carried two verified constant-time compilation paths (Jasmin-CHERI *and* CompCert-CT); with performance subordinated, one suffices.
**Resolved (this revision): the Jasmin backend is dropped.**
The project uses no speculative execution, so Jasmin's speculative-constant-time edge — its one advantage CompCert-CT lacks — buys nothing, leaving only its hand-tuned field-arithmetic speed to replace.
Constant-time now rides a single verified-compiler path, **CompCert-CT preservation**, for the whole verified-C crypto core; the field-arithmetic hot path (ECC/bignum) recovers hand-assembly-grade performance via **CryptOpt**-style verified translation validation — an untrusted randomized-search superoptimizer whose output a small **Coq-verified equivalence checker** admits against its **Fiat-Crypto** spec, so speed is bought by a *checker* (the artifact-not-pedigree / CHERI-TAL discipline of rearchitecture C), not a second compiler.
**The simplification is the Jasmin drop itself** — the only *hard* crypto-compilation dependency is now CompCert-CT, on which the field arithmetic is already correct and constant-time (just slower); CryptOpt does **not** target CHERI today (the retarget is a §18 workstream) and is a **net-new but deferrable performance-recovery lane**, cheaper than the Jasmin backend it replaces only on the trust axis — a small verified equivalence checker plus an untrusted search, versus a whole verified optimizing compiler.
CryptOpt scope is *straight-line field arithmetic* only; control-flow-heavy primitives (Keccak, AES, ChaCha, ML-KEM/ML-DSA NTT and samplers) stay on the CompCert-CT path.
See the CompCert-CT + CryptOpt entry in architectural-alternatives.md and §5/§6/§18 in verification-maximal-os.md.
...
Three levers the goal function — *engineering is free; trust is the scarce resource* — rewards most but the spec under-spends: deleting more *in-band* mechanism to shrink the proofs, buying back *in-model* performance that costs no trust, and pulling *proof-production* tooling that costs no trust.
...
- ✅ **Scalar `F`/`D` is a second floating-point datapath behind the RVV every core already mandates.**
  The profile is RV64G**V**, and even the C-class control cores carry VLEN=256 vector (§15) — so scalar `F`/`D` is a redundant FP unit whose distinctive cost is a crown jewel: the fixed-latency-FPU-*including-subnormals* timing contract plus the `FDIV`/`FSQRT` constant-time carve-out (§15; priced at −1–6% in performance-estimates.md).
  Name the scalar-FP consumer or fold scalar float onto single-element vector ops on the unit already present — deleting the scalar FP datapath, its `fcsr`/dynamic-rounding-mode state (which otherwise context-switches and joins the fence.t set), and one of the two FP timing crown jewels, for an ABI change and per-op vector-setup cost the subordinated-performance goal absorbs.
  This is #17's *name-the-consumer-or-cut-it* parsimony applied to the largest remaining orphaned datapath, and it is invisible to the off-device perf levers (performance-recovery-todo.md) precisely because it is a *hardware* deletion, not a compiler pass.
  **Resolved (this revision): the scalar `F`/`D` datapath is cut** — floating point is vector-only, scalar floats computed as single-element (VL=1) RVV ops on the one FPU (§15).
  The `f`-register file and the dynamic rounding-mode state (`frm`) are deleted (rounding is static), and the *scalar* fixed-latency-including-subnormals + `FDIV`/`FSQRT`-CT contract — one of the two FP timing crown jewels — is deleted rather than re-stated, leaving a single FP timing contract on the vector FPU.
  Honestly, the FP *arithmetic* does not vanish (it lives on the retained vector FPU), and standard RVV nominally requires scalar `F`/`D`, so this is a deliberate Sail-modeled *vector-FP-without-scalar-FP* fork, its ABI change and per-op vector-setup cost absorbed by the subordinated-performance goal. See the scalar-floating-point-deletion entry in architectural-alternatives.md.

- ✅ **The retained S/U split is the largest unstated entry in #5's missing redundancy ledger.**
  §15 kept "S/U modes retained" and never said why, though the candidate-A logic applies almost verbatim: CHERI is "the sole in-core spatial mechanism," yet the supervisor/user ring boundary persisted as a second in-band isolation axis — and the CheriOS/CHERIoT existence proofs the MMU deletion leans on are themselves *machine-mode-only*, carrying no S/U at all.
  **Resolved (this revision): the spec collapses to Machine-mode-only** (CHERIoT-lineage, privilege as a CHERI permission on the PCC rather than a ring; §7, §15).
  The one apparent obstacle — that the PMP backstop's independence rested on M-below-S privilege disjointness — does *not* hold, and is now moot regardless: PMP has since been dropped entirely (redundant against formally-verified CHERI; the drop-PMP disposition), so CHERI is the sole memory-protection mechanism and both the S/U ring and the coarse backstop are gone.
  The redundancy #5 flagged is thus not merely *stated* but *deleted*.
  See the single-privilege-mode entry in architectural-alternatives.md.

✅ **Small dead vocabulary the same parsimony indicts.**
Under W^X with no on-device codegen (§14), nothing synchronizes the instruction stream after the image is wired, so `Zifencei`/`fence.i` has no runtime consumer; and if scalar FP folds into the vector unit, the dynamic-rounding-mode state goes with it.
...
**Resolved (this revision): all three items are dispositioned.**
`Zifencei`/`fence.i` is **cut** — dropped from the base (now `IM_Zicsr`) with a §15 exclusion bullet: it exists only to make self-modifying or freshly-generated code visible, and under W^X the instruction stream is immutable once execute-only capabilities are wired over the read-only content-addressed image, so nothing writes-then-executes; the one measured-boot image-load write is ordered by the boot/reset sequence and the already-adopted `Zicbom`-class cache management, not an ISA-visible `fence.i` (RISC-V carves `Zifencei` out of the base precisely so a system may omit it, and it is insufficient for the cross-hart case the share-nothing kernel never creates).
The **dynamic-rounding-mode state is already gone** — the scalar-FP-onto-vector-FPU fold this bullet's conditional anticipated has since landed, deleting `frm` under the static-rounding mandate (§15 "Scalar floating-point entirely").
The **matrix extension is kept and its threshold now stated**: §15 admits it only where it clears an **order-of-magnitude sustained dense-GEMM margin (about 8–10× throughput, and wider per-watt) over the M-class's own VLEN=1024 RVV unit** — structural in the systolic array's K²-MACs-per-O(K)-operand-bandwidth reuse versus the vector unit's bandwidth-bound one-dimensional rate (raw peak 4–8×, carried to a decade sustained on the large dense GEMM inference presents) — below which GEMM folds onto that RVV unit, the scalar-FP parsimony one level up.
See §15 (ISA line, the `Zifencei` exclusion, the M-class topology bullet) and performance-estimates.md.

### Performance that costs no trust — the in-model lever the perf doc omits

- ✅ **Macro-op fusion is the one IPC recovery that passes the five-part admission test and touches no proof — and it is named nowhere.**
  §2 admits ILP only from "wide in-order issue, RVV," and performance-recovery-todo.md is deliberately all *off-device software* levers, so decoder-stage fusion of common adjacent pairs (address formation, load-effective-address, compare-and-branch, the dependent-ALU chains the deleted C extension and the absent dynamic predictor make relatively costlier) is unbooked.
  It is a pure function of the static instruction stream: deterministic (test 1), operand-value-independent (test 2), combinational decode with no state surviving a partition switch (test 3), no new authority (test 4), no autonomous behavior (test 5).
  It is architecturally transparent — fused and unfused executions reach identical architectural state — so it rides the existing RTL ⊑ Sail *functional* refinement and disturbs no binary-level certificate, no WCET table (a fused pair is one more fixed-latency entry, if anything tightening WCET), and no constant-time proof; the sole obligation is to list the fused set in the timing-annotated model.
  It recovers issue efficiency (not code size — that stays lost with C) against the no-C / no-dynamic-prediction penalties the spec accepts, at zero trust cost.
  This is the missing *in-model* sibling to the perf doc's off-device wins, and it is exactly what a RISC-V ISA (designed for fusion) is meant to give a core that forgoes both compression and dynamic prediction.
  **Resolved (this revision): decoder-stage macro-op fusion is booked as an admitted in-model performance mechanism.**
  §2's ILP clause now names it beside wide in-order issue and RVV; §15 gains a normative *Macro-op fusion* paragraph — the fused set (address formation/LEA, compare-and-branch, dependent-ALU chains) frozen with the proof and listed in the timing-annotated Sail model, architecturally transparent (a fused and unfused execution reach identical architectural state) so it rides the existing RTL ⊑ Sail *functional* refinement and disturbs no binary-level certificate, no constant-time proof, and no WCET table (a fused pair is one more fixed-latency entry, if anything tightening WCET); the *Functional vs. hyperproperty* timing-table description names the fused-pair latency entries. performance-estimates.md books the recovery row; performance-recovery-todo.md notes fusion as the one *in-model* pure win, its off-device list unchanged.
  No architectural-alternatives.md entry — like Ztso and static-only prediction, this is a §15 profile property, not an evaluated alternative.
