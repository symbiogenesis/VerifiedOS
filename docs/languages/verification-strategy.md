...
Keep **`VerifiedComponents` as a bounded, target-parametric Rocq library pilot**, with one CHERI instance. Pursue **Vela** as a clean-slate language with graded dependent types, general effect handlers, structured async/cancellation and controlled concurrent interior mutability, alongside flow-sensitive loans and explicit representations, authored under [three assurance presets over one surface](#assurance-presets-over-one-surface). These are [full-language design commitments](#full-language-design-commitments), with Q25 owning their combined design and qualification plan. Successful pilot combinators establish reusable proof engineering; they do not establish Vela's calculus or compiler.

The [implementation checklist](../implementation/implementation-checklist.md#q-assessment-actions) schedules Q19's tooling and conditional library qualification and Q20's integrated authoring and optimized-artifact trials. **Q2c and Q21a/Q21b close their present commissioning decisions by reasoned rejection**: the concrete foundation and its machine/contextual connections cannot be responsibly priced for these consumers. Their [foundation map](../hardware/cheri-foundation-map.md), [core design](core-design.md) and [compiler comparison](compiler-route-contract.md) retain the contracts and reopening evidence. The Clight/CHERI-CompCert route remains required; the proposed direct explicit-memory capability IR is unselected, and no frontend, foundation or paired compiler trial opens.

Start with diagnostics and selective reuse on existing proofs. Q19c's component pilot remains blocked until its concrete operation and representation laws are checked. Most specifications in [the crown-jewel inventory](../assurance/crown-jewels.md) remain unauthored; those specifications and the missing machine connection retain their existing priority and owners. Core design can proceed without claiming to discharge them.
...
The pilot qualifies scoped callback rules; the full core targets flow-sensitive loans with inferred restoration at every supported exit. Its design includes effect polymorphism, scoped discharge, handlers and suspension together with their resource rules. Implementation can qualify fragments in order without treating the pilot's exclusions as the language's feature boundary. A declared refinement fragment precedes domain automation and explicit proof. Dependent results retain runtime-discovered indices as real data with erased evidence.
...
## Full Language Design Commitments
...
[handlers and async lifecycles](handlers-async.md),
[interior mutability](interior-mutability.md), and the
[assurance presets and evidence policy](assurance-profiles.md) those rules are
selected under. The
...
Each domain supplies its carrier, operations, ordering/equality, algebraic laws, admissible structural rules and semantic interpretation. Its constraint procedure must return reconstructable evidence; unresolved constraints remain explicit obligations. Domain selection is explicit at public interfaces and specializes off device. A custom domain may express an analysis, but claims of erasure, exact use, exclusive permission or noninterference each need their own supporting theorem. In particular, an idempotent domain cannot manufacture single-use authority. There is no coercion from a cost or security grade to memory ownership, and a product of domains needs composition laws for their interaction.

Prove substitution and context scaling, graded elimination, phase separation, usage-aware soundness and erasure preservation for the combined rules. Mutation changes resource predicates over stable identities and snapshots; no grade allows types to depend on unsynchronized changing storage. Interpret the calculus in universe-stratified CIC and the selected program logic. The source grade structure projects into the [existing TAL attributes](../spec.md#r-05-132) and separately checked CIC obligations; it adds neither a general on-device grade interpreter nor new kernel conversion rules. Routine programs use inferred common grades and library contracts; algebra authors carry the stronger proof burden.

### General Handlers With Resource-Safe Resumptions

Include user-defined algebraic effects, polymorphic effect interfaces and scoped handlers. Specify handler lookup, forwarding, deep/shallow resumption behavior and result/resource transformations before accepting elaboration. [Soundly Handling Linearity](https://arxiv.org/abs/2307.09383) and [Affect](https://iris-project.org/pdfs/2025-popl-affect.pdf) motivate checking continuation usage and retained state together.

A captured continuation records its whole transitive resource context, loans, effects and exit obligations. One-shot resumption transfers that context once. Discarding it requires disposal of every outstanding resource, erasure of must-erase data and explicit elimination of mandatory verdicts naming their outcomes; affine use alone cannot justify abandoning them. Multi-shot resumption is a supported design case only for a context whose duplication is proved valid, with per-resumption effects and resource costs accounted. An exclusive loan or single-use protocol token cannot be duplicated by storing the continuation in a cell. Shared mutable captures still require their invariant and interference rules on every resumption.

The VerifiedOS realization lowers control transfers and cleanup to explicit bounded control flow and represented state, respecting the [absence of exceptions and stack unwinding](../spec.md#r-05-100). An effect handler neither changes the composition-fixed service graph nor grants ambient authority. Continuation storage, nesting and resumptions need admitted bounds; general handlers do not imply an unbounded runtime stack or heap. Qualification includes nested and forwarded handlers, zero/one/multiple resumptions, resource-returning errors, cancellation inside handlers and optimization preservation under resumed effects.
...
| --- | --- |
| Eagerness | Calling an async function constructs an owned dormant computation. Explicit scoped spawning registers it; execution starts at an admitted activation. |
| Suspension | Every `await` is a semantic checkpoint, even for a ready result. Resumption follows the declared activation order; readiness cannot bypass cancellation or invariant checks. |
| Extent | Children belong to a structured scope. That scope cannot finish while a child retains its resources; detached tasks are outside this profile. |
| Reference strength | n/a: this profile has structured extent. The scope owns each registered task until its terminal state; losing a user handle cannot silently destroy it. |
| Destruction | Normal scope exit joins children. Early exit, failure or scoped timeout requests cancellation and joins terminal completion before releasing storage, including normal completion after an operation's commit point. |
| Propagation | Child failures become explicit resource-indexed results at join. No unobserved child failure disappears; simultaneous results follow a declared stable order. |
| Cancellation awareness | Before commit, a task observes cancellation at checkpoints and enters typed cleanup, with a terminal result accounting for retained resources. After commit, the existing protocol returns `too_late`. |
| Direction | A request marks the scope's descendants; cleanup joins children before parents release captured storage. Child failure causes a request through the owning scope's failure rule. |
| Persistence | Accepted cancellation stays pending through cleanup. A bounded masking region may delay delivery, never clear the request or restart ordinary work. A `too_late` operation may finish its committed protocol. |

Task frames, child capacity, completion records and continuation storage come from explicit bounded regions in the offline memory plan. Exhaustion returns the existing typed capacity verdict, retaining the unstarted computation's resources. Specialization lowers the bounded task family to a reaction graph with explicit state and syntactic poll sites under the [run-to-completion contract](../spec.md#r-07-037a) and [schedulability proof](../spec.md#r-11-006). Spawning binds a declared frame; it creates no internal thread, run queue or inner scheduler. A pending join records state and returns at a poll site; it never blocks the reaction. Ready results follow the graph's admitted order. Suspension safety requires stable represented storage, preserved loans, closed invariants and a proved restoration/cleanup path. References into a moved or expired frame fail that condition. Inter-core transfer separately needs a transfer/sharing theorem.

Cancellation uses the existing [typed cancellation protocol](../spec.md#r-12-097): generation and request identity, declared cancellation and commit points, `cancelled`, `too_late` and `not_live`, cleanup and DMA quiescence bounds. A request cannot undo a committed operation. Specify the maximum interval to a checkpoint and terminal completion, including cleanup and joins. A scoped timeout requests cancellation and joins within that bound; a client wait timeout only stops waiting and leaves the operation and its authority owned by the enclosing scope. Hardware overrun or compartment restart follows the existing fail-stop/revocation discipline and cannot depend on executing source cleanup. Device-held loans return only after the HAL's completion/quiescence obligation. Test ready and pending awaits, cancel-before-start, cancellation on both sides of commit, parent/child failure, racing completions, capacity exhaustion and cleanup failure. Timing and progress claims name their environmental assumptions.

### Interior Mutability Guardrails

Interior mutability is an encapsulated, proved state-transition interface. Sharing a handle permits only its declared operations, never an unrestricted mutable reference to its contents. The library author supplies a representation invariant, operation contracts, interference conditions and the proof that clients cannot bypass them. The design distinguishes these cases:

| Interface | Required guardrail |
| --- | --- |
| Local cell | Confined to its execution domain; reads return owned values or stable snapshots. Temporary access cannot escape or overlap an incompatible access. |
| Checked dynamic borrow | An explicit borrow-state machine, with alias exclusion and a typed conflict result. Guard release is accounted on every exit, including cancellation. |
| Concurrent cell | Atomic operations or a proved synchronization/ownership-transfer protocol over the actual memory model, with race freedom, publication ordering and invariant preservation. Atomicity alone supplies no multi-step functional contract. |

An open invariant or exclusive dynamic-borrow guard cannot cross suspension, an effect operation that may capture control, or a reentrant call into unknown code. Close it first; a more permissive interface must prove a specific stable, non-reentrant protocol. This restriction includes apparently harmless getters and callbacks with hidden transitive effects. A saved dependent refinement describes a justified snapshot; it cannot be reused as a claim about current contents after another operation may write them.

Derive transfer and sharing eligibility structurally through fields, closures, tasks and stored continuations. A local cell cannot become concurrently shared through a wrapper. Proved representation primitives sit behind sealed interfaces and retain compiler/TAL checking. No mutable global or thread-local state bypasses the declared authority graph. The [exclusive-access theorem](../spec.md#r-05-096a) still forbids conflicting non-atomic accesses, and the [atomic profile](../spec.md#r-15-026) permits no capability in shared mutable memory and no CAS consumer. Capability-bearing continuations therefore need private storage or a separately proved ownership transfer, never a shared mutable capability cell. No general blocking mutex, retry loop or scheduler fairness assumption is imported by naming a synchronization abstraction. Race freedom, functional correctness, bounded progress and information-flow preservation remain separate obligations.

Q25's composed qualification client must combine a runtime-dependent packet length, a graded callback, a handler and a cancellable child updating a bounded shared service cell. Its rejection neighbors include a duplicated owner captured through that cell, stale content indices, cancellation while an invariant is open, early parent storage reuse, ignored child failure and a protocol relying on an unavailable atomic instruction. Qualification constructs witnesses satisfying every entry, representation and resource premise, and independently reviews the behavior contract; basic type inhabitation alone does not exclude vacuity.

### Assurance Presets Over One Surface

One authoring surface carries three assurance presets, **Safe**, **Contracted** and **Complete**, which select which claims a build must establish and never which language it is written in. Safe requires the supported fragment's baseline type, ownership, initialization and effect obligations and no optional theorem; Contracted makes selected functional, protocol or resource contracts mandatory in their declared scope; Complete discharges a declared requirement set over a declared implementation closure and endpoint. One rule generates the rest: **omitting a claim is permitted, and treating an unestablished claim as established is not.** A preset is neither a class of programmer nor a dialect: a systems author may build under Safe, and an application over previously verified libraries may reach Complete without an authored lemma.

The preset is one of four separated concerns and answers only the second: the authoring surface decides which concepts a developer manipulates, the assurance policy which properties the build establishes, the target profile which operations and resource behavior are permitted, and the endpoint which representation a claim is about. A single strictness number is therefore not an assurance report; the expanded obligations, the scope, the assumptions and the endpoint are. Presets share the meaning of every construct they share, so a stronger selection cannot move overflow behavior, evaluation order, ownership, exits or cancellation semantics, and every exported claim carries a status distinguishing established, checked on success, conditional and unestablished, with an unestablished claim unavailable to proof search, elimination and proof-based optimization alike.

These names are deliberately disjoint from [the platform's three admission tiers](../spec.md#r-13-011), which classify what the device admits at install over a binary and its certificate. A preset is decided off device about a source closure, is no admission input, and leaves the refuse-uncertified-code policy untouched. [The preset dossier](assurance-profiles.md) states the obligations, claim statuses, mixed-assurance composition rules, coverage dimensions and qualification stages; its stages are priced with Q25d's other implementation decisions rather than scheduled there.
...
The [component foundation map](../hardware/cheri-foundation-map.md) supplies Q2c's current
source inspection and missing-connection record. The
[workflow comparison contract](workflow-contract.md),
[bounded core design](core-design.md) and
[compiler-route contract](compiler-route-contract.md) record Q20a's accepted
protocol and Q21's retained designs. Q2c and Q21 reject commissioning their
present experiments on the missing foundation and full-cost evidence. Their
source inspections and proposed acceptance cases provide no executed trial,
checked primitive instance or new admission authority; each decision names its
reopening conditions.

Q19a/Q19b start with [CopyRingService.v](../../proofs/CopyRingService.v)'s existing helpers and invariant proofs. Q19c's conditional protocol uses parser destination initialization and reserves copy-service staging for independent reuse. Q2c's rejection leaves that pilot unopened; Q2b owns device lowering, Q3c specifies the protected-sequence fault model and Q3b owns its slice evidence. The mandatory machine and contextual proof obligations retain their owners and priority.
...
Q21 records the bounded design and pricing decisions; Q25 commissions the expanded language design. A new executable trial needs its own implementation owner and budget. Positive and well-formed negative neighbors must exercise the same supported mechanisms; parser failure or unsupported syntax does not count as a safety rejection. The [detailed protocols](#detailed-experiment-protocols) retain the full tests. [Break-even accounting](#where-time-might-be-saved) charges shared work once, separates measured savings from projections and never compares source-only evidence with a final-byte baseline.

## Decision

Proceed with Q19a/Q19b's available proof-tooling experiments and the current compiler/device work. Keep Q19c, Q20's dependent implementation trials and Q21's proposed core/compiler trials unopened until their actual prerequisites and reviewed reopening decisions exist. Q2c/Q21's reasoned rejections concern commissioning these experiments on present evidence; they establish no impossibility result or compiler winner. Retain the current compiler requirements until equivalent substitute evidence supports an explicit amendment.

The main risks are composition failures across dependency, mutation, captures, exits and erasure; displaced compiler or general-logic proof work; common-mode contract/frontend errors; and insufficient reviewer capacity. General handlers, structured async/cancellation, controlled concurrent interior mutation, extensible graded dependent typing and the assurance-preset policy they are selected under belong to the full-language design under Q25. Kernel equality changes and external target deployment retain separate research/admission boundaries. Frontend/backend implementation requires explicit scope, checked prerequisites and pricing; the bounded pilot supplies no qualification of the full language.
...
A peer-reviewed theorem, a maintained tool, an archived artifact deposit and a pen-and-paper metatheory are different kinds of evidence, so an entry names the kind it read and never infers mechanization from a venue.
A licence is what the artifact's own licence file says, read at the milestone that would incorporate it and never inferred from an ecosystem, a sibling project or a forge badge; where an entry below records terms as unread, that is a statement about this survey and the reading belongs to the incorporation.
...
| [VerusBelt](https://iris-project.org/pdfs/2026-pldi-verusbelt.pdf), PLDI 2026 | Semantic soundness in Iris/Rocq for a subset of Verus's proof-oriented types: cells, invariants, resource algebras and storage protocols, with full Rust lifetimes, concurrency and mutable borrows, licensing the type-check-then-spec-check discipline Verus performs with a solver. | It narrows the gap the Verus row names without closing it. The paper is explicit that Verus's front end, its verification-condition generation, its ghost-code erasure pass and its solvers stay trusted, and that standard-library specifications, decreases measures and invariant credits are outside the model. It is the grounding [the manual-proof path for HAL-adjacent `unsafe`](../spec.md#r-18-018) names, and that use reads it correctly: the grounding is of the type system's meaning, never of the tool's implementation. Its subject is a simplified modeled language. |
| [RefinedRust](https://plv.mpi-sws.org/refinedrust/) | Refined ownership types for safe and unsafe Rust, interpreted in Iris; generated proofs checked in Rocq. Particularly relevant to verified implementations behind safe APIs. | The foundational theorem concerns the embedded Rust model. Translation from actual Rust and subsequent compilation remain distinct boundaries. Best-aligned Rust proof infrastructure for the existing Radium anchor, not proof of all Rust or rustc. That project page carries the PLDI 2024 work only; its traits, closures and iterators extension is the separate OOPSLA 2026 paper, with a largely different author list. |
| [Corten](https://arxiv.org/abs/2609.04372) | A deep embedding of Rust's THIR in Rocq, with a weakest-precondition calculus and an interaction-tree denotation, ingesting Rust through hax and the authors' own extension; its case study proves memory safety of a buddy allocator's allocation and deallocation. | The fact that decides its disposition is that the paper names no repository, artifact deposit or code URL anywhere, so there is nothing to pin, inspect or licence and no reproduction is possible from it. It is also a plain September 2026 preprint whose template carries an unfilled venue line. The transpiler is unverified against rustc, the borrow checker is used as an untrusted oracle, and the memory model is single-threaded. It shares its name and nothing else with CortenMM, the Asterinas kernel's memory manager verified in Verus (Zhang et al., SOSP 2025), a different project with a different author list, prover and subject. |
...
| [Creusot](https://github.com/creusot-rs/creusot) | Rust contracts and logical models, including reasoning about mutation; translates to Coma within Why3. | Active implementation, but its ordinary Why3/solver route is not a foundational Rocq proof of Rust lowering or compilation. An interactive prover backend alone does not validate the frontend. A useful donor for model views and post-borrow specifications. Its licence is LGPL-2.1, copyleft where the permissive terms of its neighbors here are not, which an incorporation reads before a reuse. |
| [RustHornBelt](https://people.mpi-sws.org/~dreyer/papers/rusthornbelt/paper.pdf), PLDI 2022 | The machine-checked foundation under that prophecy encoding: typing judgments carrying first-order specifications built with parametric prophecy variables, proved sound in Rocq over lambda-Rust, with proofs for Vec, IterMut, Cell and Mutex. | It verifies no Rust verification tool. The subject is lambda-Rust, not rustc, not surface Rust and not Creusot's actual translation, and the development's own notes record standard-library APIs still unverified. What transfers is that the borrow-end encoding is a sound proof principle, which is the claim the restoration contracts here rest on. |
| [Flux](https://github.com/flux-rs/flux) | Refinement types integrated into Rust; lightweight value invariants can make bounds and API constraints part of signatures. | A practical refinement-checking candidate, not a general dependent proof assistant or established Rocq-to-binary chain. Borrow predictable inference and concise refinement diagnostics, while measuring the supported fragment. Its soundness theorem is about a core calculus and is conditional on the Stacked Borrows discipline, which is itself a proposed rather than normative aliasing model. |
| [Thrust](https://www.riec.tohoku.ac.jp/~unno/papers/pldi2025.pdf), PLDI 2025 | Prophecy-based refinement types whose loop and recursion invariants are inferred by reducing refinement inference to constrained Horn clauses, so mutable borrows receive first-order specifications without aliasing reasoning. | Nothing is machine-checked: the CHC and SMT solvers are trusted oracles, as is the translation from Rust, and the reported results are benchmark outcomes. Its repository is effectively unlicensed, promising a dual grant in a README while shipping neither licence file; only the artifact deposit states MIT, over a container that also bundles MPL-2.0 components. Inference is the transferable part, and an inferred bound is a proposal for Rocq to prove. |
| [Tree Borrows](https://iris-project.org/pdfs/2025-pldi-treeborrows.pdf), PLDI 2025 | An aliasing model for Rust mechanized in Rocq inside Simuliris, with intraprocedural optimizations proved sound against it, retaining most Stacked Borrows optimizations and enabling read-read reordering. | Not a normative Rust semantics, and neither rustc, LLVM nor the separately written Miri implementation is proved to correspond to the Rocq model. Its headline acceptance figure over published crates is a measurement rather than a theorem. It bounds what an aliasing discipline may assume; it supplies no capability provenance rule for this target. |
...
RustHornBelt and VerusBelt extend that lineage, the first to functional specifications of safe APIs implemented with unsafe code, the second to the proof-oriented types a verification tool adds; each proves a model sound, and neither proves a verifier correct.
...
| [Granule](https://github.com/granule-project/granule) and [Gerty](https://github.com/granule-project/gerty) | Graded, linear, and dependent types for resource use, effects, and permissions. The project lists 2024 fractional-uniqueness work and the ICFP 2026 result relating graded-base and linear-base designs. | Research foundations, not an off-the-shelf systems compiler, and each discharges its grade constraints through an SMT solver that is a trusted oracle in both. General grades belong in source proofs until justified by an existing admission obligation; they do not authorize new TAL grade axes. They are also two repositories with two licences: Granule is BSD 3-Clause, while Gerty carries its own dual file whose MIT half covers parser and syntax files derived from Agda, and the project site states no licence at all. |
...
#### Effect Handlers, Sharing and Protocol Logics

The handler, sharing and protocol rules the full-language design proposes have mechanized upstream logics, each about its own modeled language. They supply proof rules and the counterexamples that motivated them. [The elaboration comparison](#type-theory-and-elaboration-reuse) retains the type-and-effect sources beside them, and the [handler](handlers-async.md) and [mutability](interior-mutability.md) dossiers own what the proposed rules must establish locally.

| System | Mechanized contribution | Boundary to retain |
| --- | --- | --- |
| [A Relational Separation Logic for Effect Handlers](https://iris-project.org/pdfs/2026-popl-blaze.pdf), POPL 2026 | Relates two programs with handlers without requiring their effects and handlers to correspond, adding relational bind, frame and concurrency rules, and relating effect-based asynchronous library implementations to genuinely concurrent ones. Soundness is mechanized in Rocq on Iris. | Its subject is an Iris-modeled language with handlers, not OCaml, Links or any implementation, and each equivalence is hand-written per case study rather than decided by a checker. Its Rocq development is LGPL-3.0, a copyleft obligation the permissive entries beside it do not carry. |
| [Iris-WasmFX](https://iris-project.org/pdfs/2026-pldi-iriswasmfx.pdf), PLDI 2026 | Type soundness for typed continuations mechanized over WasmCert, and an Iris program logic and logical relation above it verifying a coroutine library and a generator modularly. The closest mechanized account of stack switching as an explicitly represented resource. | The Wasm extension it tracks is a proposal under revision, not a ratified feature; no engine is proved to implement the semantics and no compiler into it is verified. Its continuations live in a store, not in the preassigned immovable frames this profile requires. |
| [Leaf](https://doi.org/10.1145/3622798), OOPSLA 2023 | A guarding operator making one proposition a shared version of another, with its rules proved sound inside Iris/Rocq and used for a reader-writer lock and a hash table above it. It is the temporary-sharing half of the combination VerusBelt proves. | A proof library over a HeapLang-style model: no machine code, compiler, allocator or OS claim. Its own repository states that the published artifact is superseded by a simplified core, so the paper text and the current tree do not correspond line for line. The [arXiv item](https://arxiv.org/abs/2309.04851) is that paper's extended version and not the publication of record. |
| [Iron](https://iris-project.org/pdfs/2019-popl-iron-final.pdf), POPL 2019 | Fractional resource accounting in a higher-order concurrent separation logic whose adequacy statement is that a verified program accounts for its resources rather than leaking them, including examples delegating reclamation to a forked thread. The closest precedent for disposal as a proof obligation. | The result is about the logic's model and a HeapLang-style semantics, not a real allocator or an OS, and it infers nothing: it is a discipline for discharging obligations the author states. Its development's licence could not be read during this survey, so its terms are unread and an incorporation opens that file first. |
| [Nextgen](https://iris-project.org/pdfs/2025-cpp-nextgen.pdf), CPP 2025 | A modality for non-frame-preserving ghost-state updates across generations, used to build a separation logic for a language with explicit stack allocation whose return clears whole frames. Generation change is the shape revocation and incarnation reuse take here. | The stack-allocation result is about the paper's own language, not C, Rust or this platform's frames, and nothing is established about a compiler or machine code. Its upstream tree states no terms at all; only an archived snapshot carries a licence, and a content licence on a deposit is not a grant to use the development. |
| [Mixtris](https://iris-project.org/pdfs/2026-oopsla-mixtris.pdf), OOPSLA 2026 | Higher-order separation logic for mixed-choice multiparty message passing mechanized in Rocq/Iris, extending the Actris lineage already deferred in the tooling map to protocols offering non-deterministic choice among concurrent sends and receives. | Partial correctness in its own model language. Leader election is verified as functional correctness and not as liveness, so no termination, progress or deadlock freedom follows, and nothing addresses real distributed execution, failures or compiled code. Its artifact is an archived deposit under a content licence with no maintained repository, so it is a frozen snapshot rather than an upstream to track. |

Each is a theorem about another modeled language with its own trusted base and its own prover edition, so what a reading takes is the rule and its side conditions. None supplies this platform's frames, cells, capabilities or memory model, two of them grant no usable terms at all, and citing one closes no obligation in the dossiers that consume these shapes.
...
| [Peregrine](https://types2026.cse.chalmers.se/abstracts/61.pdf), TYPES 2026 | A standalone middle-end on Rocq's extraction intermediate language, adapting the existing verified pipeline, adding verified deserialization of that intermediate from a textual form, and contributing a formal Rocq specification of a large subset of CakeML. | A four-page contributed abstract, so a conference-branded preprint whose post-proceedings status cannot be established. Its own figure draws the new frontends and the CakeML backend as unverified, the CakeML specification is offered as something that may later verify that backend, and the middle-end is itself extracted through the legacy unverified extractor, so the running tool is not bootstrapped through a verified pipeline. Its organization's repositories are not uniformly licensed and one frontend carries no licence file at all. |
...
| [Cogent](https://github.com/au-ts/cogent), with [Dargent](https://people.eng.unimelb.edu.au/rizkallahc/publications/dargent.pdf), POPL 2023 | Systems-oriented code/proof co-generation and refinement to C, with Isabelle proofs and file-system case studies. Dargent adds a declarative bit-level layout language whose generated getters and setters ship with generated Isabelle proofs that they realize the declared layout, so the refinement chain survives a custom representation instead of ending at hand-written marshalling. | The closest published result to this document's own promise of high-level specifications with representation control, and the one to read before designing representation glue. The compiler is not verified: each compilation carries its own certificate, so a defect is a failed proof for that program rather than an excluded class. Isabelle, the C parser, AutoCorres and the C compiler are trusted, bit-field semantics are simulated rather than modeled, hand-written foreign C is outside the certificate, and there is no concurrency. The tree has no single licence: it is per-file BSD 2-Clause or GPLv2 by tag, with the compiler itself GPLv2, which is a copyleft obligation for a downstream that vendors rather than reads it. |
...
| [Verifying Peephole Rewriting in SSA Compiler IRs](https://doi.org/10.4230/LIPIcs.ITP.2024.9), ITP 2024 | A Lean mechanization of a core calculus for SSA IRs, generic over the IR and covering the nested-region scoping MLIR uses, whose central theorem is that applying an already-proved peephole rewrite at a program point preserves the program's denotational semantics. | Evidence that a rewrite family can be proved once and applied many times, which is the shape a validated optimization interface wants. Neither MLIR nor LLVM is verified: the dialect semantics are the authors' own formalization, nothing connects it to the C++ implementation or to the language reference, and each rewrite still needs its own proof. As an admission route it fails [the translator conditions](../spec.md#r-05-020) at the first, checking in Lean rather than Rocq and retiring no interim here. |
| [RustCompCert](https://arxiv.org/abs/2602.07455) | A Rust front end layered on CompCertO that lowers to Clight and reuses the existing backend, with Rocq proofs aimed at semantics preservation and at treating borrow checking as verifying compilation. | Self-described ongoing work in a February 2026 preprint: no end-to-end theorem is claimed finished, no artifact evaluation or review has occurred, and the scope is a sequential subset with no concurrency and no unsafe code. It builds a front end above CompCert rather than compiling Rust to it. Decisively for any incorporation, the fork inherits CompCert's non-commercial terms rather than an open-source licence. |
...
Reading it is the only option its terms allow. The public implementation is a descendant of the paper's compiler rather than a frozen paper artifact, and it carries no licence of its own: the sole licence text in that tree is CompCert's, so its README restricts use to educational, research, personal or evaluation purposes. That is not free software, and it cannot be vendored into a tree whose [commercial rebuildability](#licensing-and-commercial-rebuildability) is a stated deliverable. A design reading of its generator is unaffected.
...
The [CHERI instance and scope limitations](https://arxiv.org/html/2508.15576v2), stated in that publication's extended version, prevent overreading that result. The language is a sequential demonstrator; the CHERI instance is an idealized architecture-independent assembly memory model with a proved over-approximation soundness result for verification. Its under-approximation result for bug finding and implementation in Gillian are future work in this version. The work does not supply a compiled CHERI-C component or a connection to this checkout's Sail term. Inspect its memory-action, resource-production/consumption and framing premises under Q2b before inventing equivalent interfaces. Adopting its separate semantics directly would still face the semantic-anchor gate; a reusable proof pattern or proved connection is the useful output of that comparison.
...
| [Quiver](https://plv.mpi-sws.org/quiver/), PLDI 2024 | Completes a user-supplied separation-logic specification sketch for C code by abductive inference, and emits a machine-checked Rocq/Iris proof relating the code to the completed specification, so the inference engine itself need not be trusted. | The closest published shape for the proposed typed-hole tier: a sketch plus a checked completion, with the search outside the trust boundary. No theorem says an inferred specification is the intended or the strongest one and there is no completeness guarantee, so a completed sketch can be weaker than a reader assumes, and the result is relative to the RefinedC trust base and its C semantics rather than to a compiled binary. Its repository terms are unread here: only the artifact deposit's content licence was reachable, which is not the code's licence. |
...
**SuSLik: synthesize the program and reconstruct its proof.** [SuSLik](https://github.com/TyGuS/suslik) synthesizes heap-manipulating programs from separation-logic specifications. Its [certification component](https://github.com/TyGuS/suslik/blob/master/certification.md), accompanying [Certifying the Synthesis of Heap-Manipulating Programs](https://doi.org/10.1145/3473589) at ICFP 2021, emits correctness certificates for HTT, Iris or VST. This is directly relevant to the proposed proof-producing builders: the search procedure need not be trusted when the emitted program's certificate is checked. Coverage differs by backend, and the HTT path defaults to leaving pure lemmas `Admitted` unless its proof option is enabled; advanced examples still require manual pure proofs. A generated certificate is therefore a candidate, not evidence of a closed proof. Qualify an actual certificate and its assumptions, preserving the intended contract; neither the existing allocation primitives nor an ordinary VST/HeapLang certificate establishes a CHERI connection. Its terms are not single-valued either: the synthesizer is BSD 2-Clause, while the three certification backends ship no licence file and declare GPL-3.0 only in their package metadata, so the producer and the certificate machinery do not travel together, and the paper's own footnote records that the memory model assumes allocation never fails.
...
**Fully in-place programming: functional descriptions with a resource theorem.** [FP2](https://www.microsoft.com/en-us/research/publication/fp2-fully-in-place-functional-programming/), by Lorenzen, Leijen and Swierstra, identifies a linear functional calculus whose programs can execute without allocation or deallocation and with constant stack space, under its restrictions. The document that link serves is the extended Microsoft Research technical report MSR-TR-2023-19, not the ICFP 2023 proceedings paper, which is the separate PACMPL 7(ICFP) Article 198; an audited citation names whichever is read. Nothing in it is mechanized: its theorems are pen-and-paper appendices, and the report itself says only that the key proof may suit mechanization, so in this corpus it is a design reference rather than a checked result. It discusses static uniqueness and dynamic reference-counting embeddings; the implemented Koka route uses the latter, and that route's own Perceus result is likewise pen-and-paper and conditions its garbage-free claim on cycle freedom. This is a stronger resource-design reference than an effect annotation alone, but it does not make arbitrary Koka programs allocation-free or supply a verified CHERI compiler. Investigate static reuse laws for bounded components without importing a reference-counting runtime. [creditmonad](https://github.com/anfelor/creditmonad) is a separate Haskell DSL for testing amortized costs in persistent data structures. Its executable tests can falsify a proposed accounting argument; they neither prove a universal client bound nor connect that bound to target WCET.
...
**Resource inference: RaRust.** [Automatic Linear Resource Bound Analysis for Rust via Prophecy Potentials](https://arxiv.org/html/2502.19810v1), OOPSLA 2025, infers symbolic linear bounds for a safe Rust fragment using shared and prophecy potentials for borrowing. Its soundness is proved on paper against its resource-aware borrow calculus, with no mechanization and with costs assigned by explicit `tick` events. The stated limitations include unsafe code, generics, closures, trait objects and nonlinear bounds. This is a closer inference reference for a future named resource obligation than inventing a general grade engine first. An untrusted analyzer may suggest potentials and bounds for Rocq to prove; its printed answer supplies neither a certificate nor a target WCET bound. Source events, peak memory and machine time retain distinct interpretations. No analyzer port is scheduled by this reading.

**The precedent behind that inference is RaML.** [Resource Aware ML](https://www.raml.co/about/) infers polynomial bounds for a subset of OCaml by reducing potential-annotated typing to linear programming, covering higher-order and polymorphic code, user-defined inductive types, side effects and, in its current release, expected cost for probabilistic programs. It is the older and broader instrument, and it is also the limiting case of this kind of evidence: its soundness is pen-and-paper for the core calculi, no proof assistant accompanies the tool, and nothing connects the implementation to those theorems, so a printed bound is again a proposal rather than a certificate. Two facts decide how it can be used rather than read. Its newest release is a 2020 source tarball distributed from the project site rather than a version-controlled repository, so pinning it means pinning an archive nobody is maintaining. And while RaML's own files are BSD 2-Clause, that tarball bundles an OCaml compiler under other terms and a CompCert-derived example tree under the non-commercial licence, so vendoring it as it ships would import a restriction the deliverable that removes such restrictions exists to avoid.

**Time, progress and schedulability have three separate mechanized donors, each admitting its timing as a premise.** [A Separation Logic for Parallel Time Complexity with Work and Span Credits](https://iris-project.org/pdfs/2026-icfp-parcas.pdf), ICFP 2026, proves in Rocq/Iris that a fork-join program verified with given work and span credits is safe and that both quantities bound every execution, with span credits duplicated at fork and tagged so that only the admitted task may spend them. Its bounds are over computation graphs and not wall-clock time on a given number of processors, which needs a scheduler argument outside the mechanization, and it is worst-case with explicit constants rather than asymptotic. This profile has no fork-join parallelism; what transfers is the credit discipline for a cost that must compose across a branch. [Lilo](https://iris-project.org/pdfs/2025-oopsla-lilo.pdf), OOPSLA 2025, mechanizes termination-sensitive fair refinement in Rocq using progress credits, obligations and promises, and is worth reading precisely for the premise it needs: its liveness holds under scheduler fairness over an interleaving semantics with explicit yields, and this platform's cooperative reaction graph grants no fairness assumption at all, so a bound here comes from admitted activations rather than from that theorem. It is also not an Iris development, deliberately avoiding the step-indexing Iris rests on, so its rules do not compose with Iris ones by assumption. [RefinedProsa](https://iris-project.org/pdfs/2025-pldi-refinedprosa.pdf), PLDI 2025, is the closest published shape for the seam this platform's schedule obligation needs: RefinedC proves the C of a fixed-priority non-preemptive scheduler emits only traces satisfying a protocol invariant, and Prosa's analysis turns that trace into a machine-checked response-time bound. Its boundary is exactly where timing enters, and it enters as an assumption: worst-case execution times for every basic action and client callback are parameters to be obtained elsewhere, the arrival curve is assumed, one system call is axiomatized, and nothing is proved about compiled code, the compiler, the architecture or caches. Its artifact also states two different licences, a BSD 3-Clause file over the code and a content licence on the deposit record, which is the ordinary case for an archived artifact and is read before use rather than after.
...
**Certified search and proof-logging machinery is read against a standing rule rather than adopted.** Constraint and pseudo-Boolean solvers now emit proofs and checkers exist for those formats, so an offline search returning a certificate is an available shape, and the register already decides which shape is admissible. An untrusted producer whose output the *existing* checks re-validate is admissible and free to be as aggressive as it likes: [the rule behind the CryptOpt deletion](../spec.md#r-05-066) states that it is aimed at minting a checker and not at that asymmetric-trust pattern, so an offline search proposing a placement the checks already in the toolchain re-validate needs no certificate-checking import at all. Importing one of these systems as a shipped certificate checker is the refused shape: it mints a checker, several of the candidates check in Lean or HOL4 rather than Rocq and so fail [the translator conditions](../spec.md#r-05-020) at the first, none retires an interim, and where the only yield is a smaller span over a plan that is already sound, [the no-tightening rule](../spec.md#r-05-105) refuses it on its own terms. One argument is not already refused and is recorded here as an open question rather than a recommendation: a checked infeasibility certificate changes a verdict from a search that found no placement into a proof that none exists, which is a yield other than tightness. It would still owe every condition of R-05-020, and the only Rocq-side candidate's trusted base is its roughly 200-line Rocq specification, which carries the input grammar, together with the Rocq toolchain, extraction, the OCaml compiler and the execution environment. That candidate's own input parser sits outside the proof by design and costs no soundness, the certificate being an opaque object in its soundness statement.
...
The theory is Atkey and McBride's, with [Idris 2: Quantitative Type Theory in Practice](https://arxiv.org/abs/2104.00480), ECOOP 2021, describing that language's realization of it; crediting the design to Idris 2 misattributes it, and the paper is not evidence that Idris lets a program replace its grading algebra.
...
**Gerty implements GrTT's distinction between computation use and type use.** In [Graded Modal Dependent Type Theory](https://arxiv.org/html/2010.13163v2), the preprint of the ESOP 2021 paper, a dependent function binder carries a pair of grades, with further bookkeeping for dependencies in the typing context.
...
That metatheory is also pen-and-paper: no machine-checked development accompanies it, and the prototype is a research implementation whose constraint solving is a trusted external oracle and which is not verified against the calculus.
...
**A mechanized erasure reference complements Gerty.** Abel, Danielsson and Eriksson's [A Graded Modal Dependent Type Theory with a Universe and Erasure, Formalized](https://research.chalmers.se/en/publication/537991), ICFP 2023, reports Agda proofs of consistency, normalization, decidable definitional equality and grade preservation, with extraction soundness for specified modalities and context restrictions. Its dependent pairs and erasure side conditions are a concrete comparison for Q25. These results do not transfer automatically to CIC or to a calculus with mutable state and handlers.

That article is the peer-reviewed half of a pair, and the current development is its [extended version](https://arxiv.org/abs/2603.29716), revised 30 April 2026, which adds unit and graded dependent pair types. Read it for what its extraction theorem actually says: soundness holds for a restricted class of modalities, the value-preservation statement is about natural-number-typed programs, and open programs need every context variable erasable, a consistent context and a ban on erased matching for the weak pair. Nothing is proved about an implementation, and Agda's own checker remains the trusted base. Its Agda development is MIT-licensed and its repository is shared with several other papers, so naming this paper's artifact means pinning the tag that version renders under, not the repository.

**Design consequence:** make [graded dependent typing](#graded-dependent-foundation) part of Vela's foundation while keeping runtime representation, erasure, permission ownership, protocol state, effects and quantitative bounds separately interpreted. Library-level proofs qualify the relationships needed by initial clients. Q25 owns general grades and their combined metatheory; the pilot's narrow budget does not set the full language's scope.
...
The full core is a capability-oriented graded dependent language with explicitly interpreted modalities. Its programmer-facing question is local: which authority permits this operation, which values retain it, and what state must be returned at each exit? The foundation remains universe-stratified dependent values in CIC, explicitly sequenced computations over the selected semantics, and ownership interpreted in separation logic. Q21a records the synchronous qualification slice; Q25 owns the wider combined design and proof-obligation map. Q19's library supplies candidate proved rules, not evidence that this combination already has a sound metatheory.
...
Q21a selects its synchronous cases; Q25 extends the design to async, handlers and controlled shared interior mutation with [explicit interaction obligations](#full-language-design-commitments). Revocation retains its existing owner and must compose with retained task authority. A linear cleanup construct is useful only if it elaborates through these rules. Aeneas, Creusot and Prusti inform borrow-end translation, Polonius informs flow-sensitive acceptance, and Affect informs continuation use. Their separate soundness results do not supply the combined theorem. Explicit recovery is reserved for genuinely ambiguous resource or protocol states, not imposed on every ordinary loan.
...
| [Soundly Handling Linearity](https://arxiv.org/abs/2307.09383), POPL 2024; [Affect: An Affine Type and Effect System](https://iris-project.org/pdfs/2025-popl-affect.pdf), POPL 2025 | Continuations that discard or duplicate their captured context interact with ownership. Affect provides an affine type/effect system with Iris-based semantic soundness mechanized in Rocq, built over the Hazel handler logic and covering continuations stored in mutable references. The other is pen-and-paper with a Links implementation, so it supplies a calculus and the soundness bug that motivated it rather than a checked development, and its artifact is GPL-3.0-or-later. Q25 uses these resource rules for general handlers and suspension; their language and runtime are not the selected CHERI instance, and the synchronous Q21a slice supplies no qualification of them. |
| [A Mixed Linear and Graded Logic](https://drops.dagstuhl.de/entities/document/10.4230/LIPIcs.CSL.2025.32), CSL 2025; [Same Coeffect, Different Base](https://arxiv.org/abs/2606.28042), ICFP 2026 | Modal decomposition and translations between graded-base and linear-base presentations help identify when two annotations encode the same information. Keep a simple ownership surface plus separately interpreted resource contracts; these results do not supply the full dependent, mutable frontend or authorize a generic on-device grade interpreter. The translations are pen-and-paper and preserve typing, grades and equations in each direction; they are not full abstraction, no artifact accompanies them, and they establish nothing about any implementation of either lineage. |
...
Q21a records a proposed synchronous trial combining these interactions; the pilot's callbacks and isolated examples do not implement them. Q25 owns the wider design and its connection to this contract; execution follows separately scoped implementation.
...
The [full-language contract](#full-language-design-commitments) adds suspension, cancellation, handlers, concurrent interior mutation and graded dependence under Q25. Its combined client and negative neighbors must qualify their interactions; no synchronous row above supplies that evidence. Q25 must connect the wider judgments to this slice without weakening its ownership, erasure or source-correspondence contracts.
...
**The current Q19c library pilot is blocked on its concrete laws.** Its starting point is accessible CHERI-aware semantics with usable primitive rules, not an abstract signature asserted to be sound. The required Iris-over-Sail foundation and machine connection remain open, much of [the crown-jewel inventory](../assurance/crown-jewels.md)'s specification work remains unauthored, and M1.6's verified exit supplies no capability-aware backend. [Q2c's decision](../hardware/cheri-foundation-map.md#review-decision) rejects commissioning that foundation for the bounded experiment without responsibly bounded cost and named-consumer justification; Q19c opens only when the actual checked constants exist.

The retained core design and bounded emission-lemma alternative remain available for a reviewed reopening. Q21b's proposed experiment asks whether a generated family can reach a compilation theorem with less prerequisite work; its commissioning is rejected on the present full-cost evidence. Finite emission does not remove the general logic needed by other guarantees. A changed library entry condition must identify checked substitute laws and clear affected requirements; until then the pilot does not build an interface that nothing can instantiate.
...
**Restricted frontend.** Use Q21a's synchronous contract and Q25's full-language rules when extending syntax; implement a frontend only after the requisite APIs and composition laws work. Parse its supported subset into proved construction with the [mechanical source package](#mechanically-produced-evidence), not merely source locations and a correspondence narrative.
...
Q21's retained contracts cover the bounded core and compiler comparison; Q25 extends the language design and combined qualification plan. Q20 may consume an available mechanism under its existing scope, but these rows do not require it to build one. Each executable trial needs a separately priced owner before it opens; unsupported features remain untested rather than counted as rejected programs.
...
The [proof reuse inventory](../assurance/proof-reuse.md) qualifies source evidence as of 2026-09-10; qualification alone does not make a foundation locally usable. A usable starting point has a selected licensed edition, a compatible dependency closure, checked assumptions and the required theorem over the local semantics. The [integrated Stdlib references](../assurance/proof-reuse/foundations.md#f01-rocq-stdlib-arithmetic-integrated-by-reference) replace existing proofs without closing an open obligation, so they supply no new effort discount. The bands already assume library reuse. More precise theorem locations reduce repeated discovery work, but no measured saving or newly demonstrated foundation justifies lowering these bands.
...
The initial library bands include checking the selected theorem interfaces and assumptions, narrowly factoring already usable primitive rules into interfaces, proving the corresponding instance wrappers and replaying the resulting clients. Their lower ends assume direct use of those rules; their upper ends allow additional local representation, framing and client-proof repair within the stated scope. This interpretation of the bands is an engineering judgment, not a measured distribution.
They exclude acquiring unavailable source rights, migrating an incompatible prover or program-logic dependency closure, establishing missing primitive soundness, connecting a new machine or program logic, or engineering a universal backend API. Those prerequisites need separately scoped owners and estimates before a dependent row can start; they are not absorbed by its upper end. In particular, [Cerise and Cerisier](../assurance/proof-reuse/hardware.md#hw-05-cerise-universal-contract-and-sentry-encapsulation) use different machines and prover/library editions, while [RustBelt and RefinedRust](../assurance/proof-reuse/languages.md#ownership-initialization-and-type-soundness) supply ownership precedents rather than the missing local capability laws.
The [foundation map's re-entry evidence](../hardware/cheri-foundation-map.md#re-entry-evidence) decides whether the library conditions hold. Until then, its pilot and dependent rows are conditional comparisons, not estimates for delivering their results from the present checkout.

| Deliverable | Effort judgment | Starting point and exit evidence |
| --- | --- | --- |
| `VerifiedComponents` feasibility pilot | 4-8 person-weeks, conditional on checked local primitive laws | Start after the foundation map's re-entry gate. Generic fill and bounded update, narrow interface laws and initial instance wrappers; replay, mutation rejection, and assumption audit. A Cerise source theorem over its own machine does not meet this entry condition. |
| Reusable bounded-component library | 4-9 person-months, including the pilot | Start from the same checked foundation. Parameterized initialization, disjoint copy, scoped update, documented combinators, independent client reuse, and instance/refactoring tests. Selected [stdpp/coqutil laws](../assurance/proof-reuse/foundations.md#f02-stdpp-finite-maps-sets-and-sequences) can support the representation proofs; finite capacity and iteration order still need local proofs. One concrete instance; source assurance only. |
| Scoped borrow-restoration extension | 1-3 person-months | Working buffer predicates and frame rules. Reuse [RustBelt's ownership proof structure](../assurance/proof-reuse/languages.md#rustbelt-semantic-ownership-and-lifetime-soundness) for lexical element/slice borrowing, with local restoration proofs and negative alias/escape tests. Excludes porting lambda-Rust or Iris, general Rust borrow inference and concurrent interior mutation. |
...
| Descriptor-backed codec integration | 2-4 person-months, conditional on a usable synthesis path | One reviewed bounded descriptor, concrete representation, parse/reject/consumed-length proofs and the required admissible-byte canonicity proof. [Narcissus](../assurance/proof-reuse/parsers.md#narcissus-primary-same-language-starting-point) is the source, but its available source targets do not establish local compatibility. Excludes backend porting, a standards-wide descriptor campaign and [VUPER's unresolved acquisition and correspondence work](../assurance/proof-reuse/parsers.md#vuper-unusually-close-uper-research-candidate). |
...
| Reviewed restricted frontend | 9-18 person-months, including its syntax prototype | Working library and a fixed small source semantics. Checked elaboration/correspondence for that subset, useful diagnostics, negative tests and reproducible builds. [MetaRocq](../assurance/proof-reuse/languages.md#metarocq-a-verified-pcuic-checker-with-explicit-normalization-premises) supplies checking and erasure theorems; local frontend correspondence and the checker's normalization premises remain separate obligations. Reuse a qualified backend and runtime discipline; exclude repairing their missing proofs. |
| Native integration of one library family | 2-6 person-months, conditional on the complete target route | The required CHERI compilation, source-correspondence and final-artifact checking routes must work first. Connect representations and obligations, bind exact bytes, and measure resources. Existing [Rupicola/Bedrock2 derivations](../assurance/proof-reuse/languages.md#rupicola-proof-producing-relational-compilation) do not include the printer, purecap bridge or final-byte theorem; those remain separately owned prerequisites, not work hidden inside this band. |
...
| New verified optimizing CHERI compiler route | 10-30 or more person-years | A fixed source/target subset and substantial reuse of verified compiler infrastructure. Include source qualification, dependency adaptation, missing semantic bridges, representation, erasure/lowering, optimization preservation, ABI/link integration and required security-property transport. [CompCert/SECOMP](../assurance/proof-reuse/languages.md#compiler-correctness-and-resource-planning) and [Bedrock2](../assurance/proof-reuse/languages.md#bedrock2-a-proved-compiler-pipeline-to-a-specified-risc-v-machine) supply components with different licence, target and completeness limits; no closed CHERI chain is imported. Excludes hardware proofs and a full general-language ecosystem; feasibility may require narrowing the scope. |
...
The existing [implementation checklist](../implementation/implementation-checklist.md) remains the owner of scheduled work and its measured/calibrated estimates; these ranges neither replace it nor subtract claimed savings from it.
The graded dependent foundation is selected for expressiveness and resource reasoning. Gerty's substitution-skipping optimization remains a separate performance question: its toy-program result supplies neither a measured bottleneck here nor a justification for new speed-only verified machinery.
...
Reuse the inventory's selected source readings when they still apply. Re-price an affected scope only when a concrete adaptation removes named implementation or proof work, or a failed qualification adds it; record the bridge, review and maintenance effort as well as the reused theorem. Discovery of a source, successful kernel replay and reduced client-authoring time answer different questions.
