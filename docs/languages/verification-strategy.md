...
Keep **`VerifiedComponents` as a bounded, target-parametric Rocq library pilot**, with one CHERI instance. Pursue **Vela** as a clean-slate language with graded dependent types, general effect handlers, structured async/cancellation and controlled concurrent interior mutability, alongside flow-sensitive loans and explicit representations. These are [full-language design commitments](#full-language-design-commitments), with Q25 owning their combined design and qualification plan. Successful pilot combinators establish reusable proof engineering; they do not establish Vela's calculus or compiler.
