...
An inhabited entry candidate fixes `N = 4`, `n = 2`, private initialized backing `[1,2,0,0]` with packet prefix `[1,2]`, a separate immutable callback environment invoked twice by the pure handler, unused declared frame/completion slots, live private counter grants, counter value zero, and one allowance of two. Two concurrently authorized children may instead have lengths three and two, disjoint private packets/frames, and allowances totalling five. Either AMO order gives a final value of five after both joins; the intermediate observation may be three or two. This is a proposed entry witness for the chosen representation law, not a claim that those concrete CHERI resources are currently constructed in Rocq. Q25d constructs none of them: [its composition review](full-language-review.md#the-representation-the-composed-duties-are-stated-over) names the carrier each missing constant belongs to and states that it builds no inhabitant of that carrier, so the private grants and schedule premises stay inside `GF-resource` and `MUT-ATOMIC`, both `unpriced`.
...
| --- | --- |
| Cancel before registered child starts | No AMO occurs; retire the allowance and place the returned input resources in the reserved `cancelled` completion record. Retain the registered frame until terminal collection and its release gate; the collecting owner examines the result. |
...
| Cancel observed at a precommit checkpoint versus after commit | First retires allowance with no add; second produces `too_late` and completes exactly once. Arrival during the bounded checkpoint/commit region is decided afterward. Parent reuse before terminal join fails lifecycle ownership. |
...
Upstream precedents exist for several of these debts and none constructs the laws they name. The [handler and sharing logics](verification-strategy.md#effect-handlers-sharing-and-protocol-logics) record a mechanized guarding operator for temporary sharing, which is the closest published shape for `MUT-CELL`'s sealed opening and closing, and a resource-accounting logic whose adequacy statement is that a verified program does not leak, which is the precedent for the release obligations `MUT-EXIT` states. The [Rust and low-level survey](verification-strategy.md#rust-and-low-level-verification) records the two ownership donors: a semantic soundness proof for a subset of a proof-oriented tool's cells, invariants and storage protocols, built by combining that sharing operator with a lifetime logic, and machine-checked specifications of Cell and Mutex over a model of Rust's core. Each is a theorem about another modeled language with its own trusted base and prover edition, and each leaves its own tool's erasure, solver or compiler trusted. The concrete representation, the atomic law over the pinned Sail term and an inhabited cell instance remain debts below; citing a donor closes none of them.
...
No register amendment is applied by this dossier. The chosen scalar counter, private frames and immutable publication are intended to fit the present requirements. Q25d reads the register and prose pair for each proposed change; the duties below state what each still owes before dependent implementation opens.
...
| R-05-020 and R-05-026 | [Q25d takes the nonduplication and retirement disposition](full-language-review.md#r-05-009-r-05-020-and-what-holds-the-frontend-rows), arm-dependent and holding the frontend in the arm this design sits in; the exact source-to-artifact obligations stay owed at Q2c's `artifact-admission`. A cell dossier or Rocq output is no disposition. |
