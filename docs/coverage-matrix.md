...
| --- | --- | --- | --- | --- |
| `B-01` | `P-1` | No speculative, out-of-order, or predictor state exists to carry history across a domain; islands are confidentiality domains and `fence.t` clears what is time-shared. The spatial term is graded: disjointly bound macros or tiers leave no shared path and no arbiter to reach, while two low-sensitivity islands sharing a macro keep a periphery and power coupling that arbitration cannot schedule away, and the isolation model both are stated against is unauthored, so neither is proved | absent / residual | R-15-100, R-15-211, R-15-222, R-17-003b |
| `B-01` | `P-2` | Every SRAM array is ECC-protected and corrected, the validity tag bits take the stronger code, and no sub-granule write reaches the array; interleaving and scrubbing keep a latent error from accumulating and an uncorrectable one is a fail-stop rather than a returned value. The *deliberately induced* fault is the same pair carried further, since it lands outside the model rather than inside it: the mechanism catching each injection is enumerated, a compiler-emitted control-flow signature and a fail-safe decision encoding cover the three sequences where one skipped instruction is the whole decision, and the sentinel is lockstepped because a detector cannot report its own corruption. Over the protected-sequence fault model the signature-and-token construction carries a theorem, the model's single-fault hypothesis an axiom about the silicon; beyond that model the half is coverage rather than a theorem and is booked as such | detected / proved / residual | R-15-175, R-15-177, R-15-178, R-15-181, R-16-008b, R-16-008c, R-16-008d, R-16-008f, R-17-058b |
...
| `B-01` | `P-6` | `Zkt` and `Zvkt` are the architectural data-independent-timing contract, and bare self-exclusion from that list is not a pass; the two named exclusions are variable-latency divide and the vector accesses whose element addresses distribute over banks at runtime, each admitted only on the flow discipline proving no secret reaches the operand or the address. That contract is the *observation* the cell carries: instruction latency and emitted addresses. The analog observation is outside it and is answered one layer down: the crypto core's secret-handling datapath is masked, with *d*-probing and composition theorems verified on the artifact against the probing-model statement, itself an axiom about the silicon whose faithfulness and order are the booked remainder, so the pair is carried architecturally, proved at the analog half's model, and booked for what stands outside that model. The instruction encoding adds a case to the contract without adding a mechanism to it: fetch latency is fixed per bundle and carries no data-dependent term, and the balancing obligation on secret-dependent arms is restated over encoded bundle count rather than instruction count, discharged statically at composition | hardware-enforced / admission-rejected / proved / residual | R-05-004a, R-15-011, R-15-036e, R-15-036f, R-15-053, R-15-053a, R-15-085, R-15-085a, R-17-058a |
| `B-01` | `P-7` | Exactly one Sail model, one capability encoding, and one instruction-fetch format exist, and a second entropy root is refused rather than reconciled; decode names an instruction by exactly one name, being a function of a bundle's contents and a slot index with no dependence on where decoding started, so no byte sequence carries a second reading | admission-rejected | R-15-005, R-15-036a, R-15-036b, R-15-037, R-15-241 |
...
| `B-02` | `P-4` | A binary enters only through a signed generation, an install being a generation rather than an amendment to one, the anti-rollback floor bounds which generations may boot, and no system component fetches code at runtime | hardware-enforced | R-09-028, R-11-005, R-13-001a, R-13-002 |
...
| `B-05` | `P-4` | The monotonic counter is spent only on low-rate security-critical state, so the bulk mutable volume is deliberately not freshness-protected and a physical adversary can roll it back to an authenticated earlier state. The class whose staleness is a security event rather than a lost edit is not left in that surrender: it is declared `Fresh`, carried by an epoch root the counter seals once per epoch rather than once per commit, admitted at composition against the counter's rated endurance, and refused rather than returned when it does not verify | hardware-enforced / admission-rejected / residual | R-10-011, R-10-013, R-10-013c, R-10-013d, R-10-013e, R-17-043 |
...
| `B-05` | `P-6` | Filesystem compression is out of scope, deleting the compression oracle, and deduplication never crosses a confidentiality domain. The one encoding that is compressed, the §15 instruction format, is outside this boundary by requirement rather than by reading: it is composition-time only, reaches no stored extent, leaves no compressor on any runtime path, and takes no secret as an input | absent | R-10-016, R-10-018, R-15-036g |
...
| `B-06` | `P-4` | Mutual authentication is required with no silent downgrade, the legacy generations a downgrade would reach are absent from the silicon, and each protocol has one admissible configuration with nothing to negotiate, its sequencer proved to refine a formal model of the standard's own state machine so that a transition the model does not carry is unreachable; that model is itself curated from the machine-checked symbolic security analysis of the protocol, so the refined machine and the analyzed machine are one artifact, the analysis entering as upstream evidence rather than trust | absent / proved | R-12-041, R-12-042, R-12-043a, R-12-043b, R-12-043e |
| `B-06` | `P-5` | Radio deadlines ship as admitted hard tasks, and a stack crash costs connectivity rather than platform integrity. What one crash costs is not what an adversary able to cause them on demand costs, so the pair carries a second term: entries into the degraded subset are budgeted as a rate and recorded as a countable event class, the accumulation being invisible in a per-incident window budget and reported otherwise as containment succeeding | proved / detected | R-11-007, R-16-002, R-16-009, R-17-030d, R-17-030m |
...
| `B-07` | `P-1` | Non-interference holds modulo robust delimited declassification, and the powerbox is the sole runtime declassifier; what the modulo releases is decided by the consent act, so the mechanism is proved and the judgment is the user's | proved / transferred | R-06-016, R-08-024, R-08-026, R-17-013, R-17-013e |
...
| `B-09` | `P-2` | The base image is bit-for-bit reproducible, so a relying party reproduces the reference values instead of being told them, and the artifact that becomes silicon is held to the same discipline rather than left as the one unreproduced input: the tapeout artifact is a deterministic function of the RTL of record and a pinned flow, regenerated and compared instead of trusted. What reproduction does not establish is the semantic step from RTL to layout, whose equivalence checking is a commercial tool's verdict at the same evidence rung as the imported cores' netlist audit, so this boundary is proved down to the handoff and evidence below it | proved / detected | R-09-027, R-13-026, R-17-060a, R-17-060b, R-17-060c |
...
| `B-09` | `P-7` | The design is named by exactly one object: the tapeout artifact carries a digest signed at handoff by the parties that reviewed the layout, and a part's attested identity names the mask-set digest it claims to be, so a substitution between review and handoff is a mismatch rather than an invisible event and the boot ROM's *attested mask set* has a referent. Correspondence between that named mask set and the fabricated die stays evidence rather than proof, claimed for the logic tier at a stated resolution | proved / residual | R-17-060, R-17-060d, R-17-061, R-17-063 |
