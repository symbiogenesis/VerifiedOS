...
· Raised: M1.2b
· Disposition: closed by the one-line deletion; the predicate's clause wants rewording to admit it, the pin's own comment at the same site already recording the wildcard as a workaround for a float case it could not express.
...
· Raised: M1.2b
· Disposition: open, reported and not closed; the clause is rewordable to the relation that holds, which is equality of the instruction sequence rather than of the bytes, and the register permutation is a witness that the allocator changed and not that the code did.
