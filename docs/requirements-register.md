...
· Accept: each tier is identified by its vehicles rather than by adjective, so which tier a core stands at is read off the artifacts admitted for that core; the rungs R-18-010 stages split at the unbounded one, everything below it evidence and the Kami/Kôika Coq refinement alone theorem, with Isla's obligations on the evidence side and the netlist audit beside the rungs as R-18-012's own gate rather than among them. The interval is the hardware premise's alone and buys the software premise none: the TAL producer and the on-device checker are hard prerequisites with no trusted-toolchain fallback (R-18-020), the pipeline that supplies every certificate is external by exclusion (R-02-008), and no capability-degraded interim exists (R-18-002), so no composition ships a unit against a reduced software tier while that pipeline is built, and a unit at evidence tier on its hardware premise still admits nothing its certifier has not certified.
...
**R-02-003** IS: There is no fixed form factor: laptop, phone, IoT, workstation, and server instantiations share the principles; for the laptop, phone, IoT, and workstation only the physical particulars vary, and a server is the ensemble instantiation R-02-003a states. The reference instantiation is a mobile/laptop device, and R-18-004a carries what *mobile* obliges of the first release in capability and capacity, its physical envelope reaching no pre-silicon gate.
...
**R-02-003a** IS: A server instantiation is an **ensemble**: a composition of machines, each one die carrying its own 36-bit address space (R-15-002a), its own memory in both R-15-247 classes, its own root of trust and measured chain (R-09-002), its own kernel instances and admission checkers, its own clock spine (R-15-195), and its own signed generation (R-13-001a), sharing no memory, no capability, no authority, and no clock with any other, and joined pairwise by ensemble links (R-15-228b) that are a network-class boundary between two machines and never a memory boundary inside one; the composition is one act producing one image per member and the ensemble schedule (R-13-001d, R-11-017a), and every constant of the ensemble, its membership, each link's two endpoints, each link's leader end, and each peer's expected unit identity and ensemble identity, is carried in every member's attested devicetree (R-09-007).
· Accept: every member of an ensemble meets every entry of this register alone, an ensemble adding no exception to any of them: the 64 GB ceiling (R-15-002c), the population wall (R-17-004), and the one-die commitments (R-15-111, R-15-162) each hold per member, and no entry is read as holding over an ensemble what it states of a machine.
· Accept: application payload crosses a link only inside R-12-015d's session carrying R-12-007a's payloads; frames outside an established session are that construction's scheduled establishment traffic or fixed public padding, neither authorizing application delivery or an origin leap, so no member's address space, capacity ceiling, frame, or authority is a function of a peer's; a claim that an ensemble holds a workload one machine cannot is a claim about member count and is stated as one (R-15-171a); an ensemble is not R-15-173's fallback, that fallback bending one machine's roster and never adding a peer; and no member forwards a frame, what a member passes on passing as new ring traffic under its own schedule, so no member is a switch and a link needs no address.
· Accept: the ensemble is a later instantiation than the first release R-18-004a fixes and no member of its floor depends on a second machine, and it is admitted only when five artifacts exist: the ensemble link contract stating the endpoint and the frame descriptor under R-15-228b through R-15-228e; the link session's reference model in the inventory row R-12-015c confers (R-12-015d); the admission artifact's ensemble output (R-11-017a); the composed non-interference statement R-17-014a books; and the endpoint block authored under R-15-092 with its absence predicate under R-15-102 and a two-member measurement of the skew bound and the link's latency constants (R-15-196a). An ensemble composed ahead of any of the five is a review-gate finding against this requirement, and the five are priced as children of one plan item, none of which sits on the M8a roster.
· Accept: laptop, phone, IoT, and workstation stay single machines under R-02-003, the ensemble being a server's form and no other's; an ensemble link is no peripheral, so the cutoffs R-15-145 provides reach a member's USB data lanes where present and never a link (R-15-228b).
...
**R-02-005** MUST: Instruction-level parallelism is bought only from static, exposed mechanisms (the shared scalar front end's in-order single or dual issue, decoder-stage macro-op fusion, and the vector or matrix datapath a class carries), never from mechanisms creating hidden speculative microarchitectural state.
· Accept: consistent with R-15-010 and R-15-031; the scalar front end is one shared microarchitecture at single or dual issue on every class (R-15-112, R-15-108a), so what a class carries beyond it is the datapath R-15-113 lists and never a wider issue width.
· Trace: CJ-SAIL
...
**R-03-001** IS: The defended set is enumerated: remote network attackers including hostile radio infrastructure; hostile web content; malicious or compromised apps, servers, and drivers; malicious DMA peripherals and counterfeit or spoofed wired peripherals and cables; interposition on, peer substitution at, or replay across an ensemble link; evil-maid physical access; forensic extraction of a device seized after at least one unlock; coerced unlock (duress); and software supply-chain attack in both its tampering and subversion forms.
...
· Accept: a component that fetches and executes instructions outside this discipline does not go on the die or the board; a peer machine of an ensemble (R-02-003a) is another copy of the one computer under this same discipline, attested to be so before its link carries a payload (R-12-015d), reached as a device edge through R-15-228c's endpoint, and is not a foreign computer whether it sits on the same board or another.
· Trace: CJ-CERISE
...
**R-05-126f** MUST: Physical quantities carry their dimension as a phantom parameter, the dimension slot the language grammar gives its integer former ([typed-assembly-language.md](languages/typed-assembly-language.md)), decided by syntactic type equality and erased before code generation.
...
· Accept: the checker evaluates a fixed attribute set bottom-up over the already-typed CFG with no fixpoint over open terms anywhere, and the shipped source meets the counting rule [typed-assembly-language.md](languages/typed-assembly-language.md) states, cited here rather than restated. A checker that met the figure by moving decisions into a generated table fails the claim, the category fact being what the budget asserts.
· Trace: CJ-TAL-SOUND
...
· Accept: the half of this that is decidable runs in the same build gate as R-05-163: every record a shipped artifact's theorems quantify over is inhabited, in that artifact or in one it Requires, by a closed top-level definition named `witness_<Record>` and ascribed at that record, so the prover decides inhabitation by type-checking the ascription and the gate decides only that the constant is present; it refuses an artifact that quantifies over a record it does not so inhabit, and counts those witnesses beside its constants. A construction recognised by pattern rather than by type, a constructor named or a record literal opened anywhere in the artifact's text, is not admitted: either can stand inside a hypothesis or inside an instance the artifact exists to refute, so neither establishes an inhabitant, and a criterion that accepts one decides less than this entry requires. Vacuity is not decidable in general, so the rest is per-theorem and constructive rather than a further checker: that a witness satisfies the hypotheses of the theorem it inhabits, and that a distinguishing instance is one the specification rejects rather than one that merely differs, are judgements discharged in the artifact and read at the §5 review gate, and their residue is booked under R-17-016 rather than claimed here. The reference composition serves as the inhabitation witness wherever one exists.
· Trace: CJ-T
...
· Accept: readiness is a posted receive and nothing else, because the peer is never executing when the caller runs: an endpoint belongs to one kernel instance (R-07-004, R-07-007), both peers sit on that instance's core, and one partition runs at a time (R-07-037a), so the only way a peer can be ready is to have left its half of the meeting at the endpoint before it yielded. The endpoint holds that half and at most one of it, so a second send before the receiver has taken the first is the case *no ready peer* and refuses; the landed message occupies the receiving context, one of R-07-027a's three classes, so the transfer adds no object and no queue, and the landing area is one message deep in a context the composition already placed rather than a buffer between the two register files; a capability that lands there stands in the receiving context and is reached by the sweep and the per-load filter there like any other slot (R-08-004), so a revocation between the send and the receive invalidates it where it sits; and the receive that takes it is the invocation R-07-031b numbers, not a kernel act on the receiver's behalf.
...
· Accept: the residency term is a term of the boundary interval and not a fifth term of the switch, R-15-220's three and R-15-220a's context term being cited and not restated; the boundary constant is what R-11-009 charges per visit, so the interval a boundary actually takes is the interval admission divides by the poll period; and R-15-073c's latch is not a state the timer reaches, the only trap that meets a live path being a synchronous fault on it.
...
**R-08-004** MUST: The kernel layer provides object capabilities with first-class revocation through one protocol: bitmap publication, the R-08-006 containment barrier, a monotone kernel epoch, the per-load filter, and budgeted reclamation sweep. There is no capability derivation tree. Time-to-containment is a composition-bounded constant, including the distributed case, where capabilities delegated over cross-core grant edges revoke through a verified bounded-round protocol of local publication and barrier completion, with proxy notification and acknowledgement along those same static edges.
· Accept: the bound includes every local, proxy, outstanding-call and device term of R-08-006 and R-15-208, is stated per composition and enters the §11 schedule; exactly one revocation protocol appears in the kernel spec and its proof.
...
· Accept: the indirection and current-bit validation are priced against the cross-domain call they serve and not against the load path; admission rejects a composition whose simultaneous live grant count exceeds the table, so exhaustion is a build-time rejection and never a runtime failure; and which cross-domain edges are grant-mediated is fixed at composition by the manifest and the R-05-159 linear-capability discipline, never chosen by a principal at runtime. Invocation validates the slot's current live bit even when its handle already resides in a register; publication closes new loans, and an outstanding call must return or be cancelled with its borrowed authority cleared before R-08-006 reports containment. The no-capture proof covers every copy of the yielded bare capability, since the slot bit does not revoke the underlying object shared by other grants.
...
**R-08-005** MUST: A capability-width load applies the revocation bitmap to the loaded capability and clears its tag when revoked. This prevents stored stale authority from becoming usable on reload; it does not revalidate an authorizing capability already resident in a register at every dereference. The complete *freed ⇒ unreachable* postcondition begins at R-08-006's containment completion.
· Accept: the load check is deterministic and architectural, with the fixed added island-local bitmap-read term R-08-005a prices and no added NoC or memory-controller transaction. A register-held capability cannot be declared revoked merely because this load rule exists.
· Trace: CJ-CERISE, CJ-LEAK

**R-08-005a** MUST: The load filter is backed by a dedicated ECC-protected revocation bitmap over a composition-fixed union of 64-byte-aligned revocable main-memory intervals, with one architectural revocation bit per 8-byte capability granule: `0` is live and `1` is revoked. The union is partitioned by island: each island holds one island-local array carrying the bits of its own intervals alone, read by the loads that island issues and by no other. For covered intervals *I*, the payload is exactly Σ|*I*|/64 bytes summed over the islands; the static memory plan charges that payload, its ECC bits, and macro periphery against the §15 SRAM capacity budget, and admission rejects a composition that does not fit. The interval map and each island's resulting bitmap size are attested-devicetree constants; ordinary compartments cannot address any bitmap, and only the owning island's kernel revocation path may update it.
· Accept: this is the CHERIoT non-MMU realization, not RVY `Svyrg`: the loaded capability's base selects the bit and a set bit clears its tag before architectural writeback. RVY's four-bit PTE state machine has no PTE in which to live here and is not silently compressed into one bit; epoch, sweep, and quarantine state remain in their separately specified protocol, while this array carries only the load-time live/revoked predicate. The covered union includes the R-08-004a grant table, whose per-slot bits carry the subtree case.
· Accept: the bit read is a second, dependent access and is priced as one: the base is a field of the value the load returns, so the bit sits at an address the data/tag return decides rather than beside the granule being read. The array is reached inside the island with no NoC or memory-controller request, and it is read in a per-core slot fixed at composition, the read completing at the padded worst case of one slot rotation and never early, so every tagged capability load carries one fixed added term, a constant of the island's core count and of nothing another core does, and a base outside the island's covered intervals takes the same term with a constant live result. That term is inside the single fixed latency R-08-005b states, and the timing-annotated model carries it in the tagged capability load's class.
· Accept: restricting the union to the loading island loses no revocation: across islands there is no shared mutable memory (R-15-223), the shared ring region's windows are composition-fixed (R-15-223a) and outside the covered union, and a capability delegated over a cross-island grant edge revokes through the proxy protocol R-08-004 states, whose proxy slot sits in the delegate island's own grant table and so under that island's own array. An array keyed by base and shared by every island would be a path between islands R-15-223 and R-17-003b say does not exist.
...
· Accept: every capability-width load in the Sail model carries the revoked case as one of its defined results, at the single fixed latency R-08-005a states, so the bank-side filter realizes an architectural result rather than acting outside the instruction that provoked it; a load definition admitting only an unmodified tag or a trap cannot express R-08-005's check at all. Both address-keyed cases rest on this clause and have no second mechanism, the object case and R-08-004a's delegation case each clearing the tag on the load with the revocation colour declined at R-08-004b.
· Trace: CJ-SAIL, CJ-CERISE

**R-08-006** MUST: Containment completes only after the kernel publishes every affected revocation bit, prevents new invocation or derivation of the retired authority, and establishes a bounded barrier over every execution context that can retain it. Each affected core clears live register authority or spills and restores it through the load filter before resumption; saved contexts cannot restore it unfiltered. Every R-08-004c outstanding loan returns or is cancelled with its borrowed authority cleared, each static remote proxy acknowledges the same local postcondition, and every accepted transfer under retired authority reaches R-15-208a's device completion boundary. From this completion event until authorized reuse, retired authority can neither be exercised nor recreated by an untrusted context. The epoch names the protocol event; advancing it alone does not establish containment.
· Accept: the kernel proof connects the postcondition to the actual Sail load, dereference, invocation and context-restore semantics. The composition inventory covers live and saved registers, trusted-stack and sentry roots, grant loans, proxy slots and device-held authority; it either includes narrowed capabilities and every base from which retired authority can be derived in the revocation set or proves them unusable after completion. An omitted root or an outstanding unbounded call rejects the composition. Functional and mutation cases retain a capability in a register across publication, spill and restore it, retain an interior-base capability, revoke a loan in progress, delay a proxy acknowledgement and leave a device transfer in flight; none may report containment before the corresponding postcondition holds.
· Accept: the complete publication, scheduled barrier, bounded-call cancellation, proxy and device bound is derived per composition and charged to its existing admitted slots. No wait depends on untrusted cooperation, no sweep quantum is added to a partition-switch cost, and no containment term depends on sweep progress or the measured teardown rate. The incremental sweep remains reclamation; physical reuse additionally requires R-08-007a.
· Trace: CJ-CERISE, CJ-WCET
...
· Accept: the per-domain sweep-completion constant of R-08-007, its contribution to the quarantine interval, and the background slot budget derived from it, are set by the group width rather than by per-granule issue; the walk stays software and the instruction ends at its group boundary, so R-08-009's refusal is undisturbed.
· Trace: CJ-WCET, CJ-CERISE

**R-08-007a** MUST: Nothing in the revocation protocol wraps, and no revocable resource is reused before it is dead. The epoch is a 64-bit monotone kernel counter naming containment events, advanced only on kernel-mediated teardown. A revocation bit and the granule under it return to service only after R-08-006 completes and a full sweep pass begun after that barrier covers every capability-bearing location that can retain the retired authority.
...
· Accept: three clauses decide it, and it is met when all three hold. (1) The expectation is booked with its owner and its scope rather than absorbed into a guarantee, R-08-004c's shape applied to a rate instead of to a cost: it is stated as a premise at every site that argues from it, including the temporal-trade objection of [Evaluated Architectural Alternatives](background/architectural-alternatives.md), so a corpus sentence reading the rate as settled is a review-gate finding. (2) No bound reads the rate. Containment is R-08-006's completed barrier and waits on no sweep pass, and the answers to a higher rate are a wider R-08-007 slot class, a longer R-08-007a interval against a larger composition-sized quarantine pool, and a composition that does not fit at admission, each of them a slot or a byte R-08-045 already charges at composition; an entry making a §11 bound or a time-to-containment constant a function of the rate fails this clause. (3) The rate is not R-15-100b's A-16 trigger, which reads the cost of the load filter and the sweep rather than the traffic feeding them, so a measurement is an input to that judgment and neither fires it nor disturbs R-15-035.
...
**R-08-043a** MUST: User-initiated retraction is a fourth revocation trigger beside grant expiry, session close, and restart, retiring a grant on the one mechanism: the kernel sets the R-08-005a revocation bit for the grant slot's granule and the per-load filter kills every stored copy of the handle at its next load; retraction completes only at R-08-006's register, loan, proxy and device barrier.
· Accept: retraction reuses the complete R-08-004 protocol, including its containment barrier, so R-08-004's requirement that exactly one revocation mechanism appear in the kernel spec and its proof holds.
· Trace: CJ-CERISE, CJ-NI
...
**R-09-022** MUST: A distinct duress credential, presented in place of the ordinary one, commands the RoT to crypto-erase rather than unlock: it destroys the sealing root wrapping the per-profile volume keys and the device-identity secret, rendering the AEAD-sealed user data unrecoverable through those keys in the time to zeroize a key.
· Accept: data-at-rest is answered by destruction of its complete key custody, not a bulk ciphertext overwrite. Erase completion also requires the first-class plaintext and key-state erase construction qualified under R-15-200 and the second-class discharge R-09-022a orders; first-class power-down alone does not discharge this criterion. Loss of power before those constructions complete remains R-17-058's physical residual.
· Trace: CJ-CRYPTO-SPEC
...
**R-10-004** IS: The VeriBetrFS B^ε buffered-update refinement is the incumbent L1 design, selected for message-log batching's potential NAND-endurance gain and re-proved in Coq/Iris. A plain CoW B+ tree over the same L0 journal is the candidate with fewer index-refinement obligations, not an automatically selected replacement.
· Accept: the index comparison fixes its required endurance, useful capacity, latency and proof-scope criteria before measuring either candidate, under one journal, recovery policy, bounded workload and storage-device contract. It reports NAND write amplification, reserved buffer bytes, worst-case query and flush work, and the recovery/refinement obligations of each; amortized throughput alone establishes no §11 bound. Select the B+ fallback only if it meets the composition's required endurance, capacity and timing with a demonstrably smaller proof obligation; retain B^ε only with its own criteria met, and reject a composition for which neither fits. A different index requires a register decision, not an attribution change in the prior-art document.
· Accept: the B^ε design is re-proved in Coq/Iris and supplies no non-Coq proof anchor; bcachefs's internally logged B+ nodes are separate prior art for filesystem organization, not evidence that this buffered-update refinement is already proved.
· Trace: CJ-T
...
· Accept: a carried migration runs as admitted code at the successor's first boot before the health gate reads anything, and writes the new schema beside the old and never over it, so the retained predecessor root and the index it names are byte-identical before and after and a health-gated return (R-11-002) lands on a store nothing rewrote; a generation carrying none ships whole, the set difference of R-13-008 degenerating to the image and the old schema's structures collected after health passes.
...
**R-11-005b** MUST: A successor generation is composed for the generation that installs it: every descriptor the trust base reads across a generation boundary (the pack, the manifests inside it, the update directive, and the store's key schema) carries its version in its fixed-layout header; each generation declares, per descriptor, the versions its reader installs and the versions it boots on; a successor is emitted in a version the installing generation installs and the successor boots on, and its proofs are stated against the installing generation's spec-set and Sail-model versions, whatever checker, spec-set, model pin, or descriptor the successor itself carries.
· Accept: the installing generation's reader, checker, and transactor perform the whole of R-11-005's admission, so the artifact they admit is decidable against their own versions alone; a generation shipping a new checker, spec-set, model pin, or descriptor is admitted under its predecessor's, and the re-admission R-11-005 forces on a rev lands one generation after the rev.
· Accept: the composer (R-13-001c) targets the generation the device's request names beside its roster, a composition emitted for any other fails admission rather than installing, and the reference reader shape is own version plus predecessor's, so every adjacent step is composable and a composition may skip steps exactly where no break intervenes; the boot header the ROM reads (R-09-005) is outside the rule because the ROM never changes.
· Accept: the update directive is a table from each generation it reaches from to the next root to install, the target where one composition reaches it and an intermediate generation where none does, each step a whole generation under R-11-002's health gate and the floor R-09-030 bounds bootability by, so what a long absence costs is R-17-035a's boot once per step and never a second grammar in any reader; the backward half, what a successor reads at first boot that its predecessor wrote, is R-10-036a.
· Fail-closed: a pack, manifest, or directive in a version the installing reader does not install, or proofs stated against a spec-set the installing checker does not carry, stop the **install** and never a boot; the running generation is untouched, and what is spent is the ability to update until a composition for this generation arrives (R-17-030v).
· Trace: CJ-FORMAT, CJ-ADMIT-IMPL, CJ-DEVTREE
...
**R-11-009** MUST: Admission counts the switch-duty ratio σ = C_switch / T_poll explicitly instead of absorbing it into slack: a task's real cost is its in-slot WCET plus the partition-switch constant times its visit rate, and the constant charged is R-07-040's padded boundary constant, which covers R-15-220a's switch budget together with the trap-path residency the timer's hold can add at any boundary.
· Accept: the ratio is recorded per task in the admission artifact, and the constant it divides is the boundary constant rather than the switch budget alone, so no boundary is admitted against a figure smaller than the interval R-07-040 releases the successor at.
· Trace: CJ-WCET
...
**R-11-014b** MUST: The admission artifact carries a derived worst-case end-to-end bound for every cross-core chain the composition declares, computed from the frame offsets and the per-hop slot periods over the R-07-007 edges, a pinned core (R-11-011) contributing its poll-loop period in place of a frame offset, and on a member of an ensemble an ensemble link hop being one more edge kind whose per-hop term is its slot period plus its guard band as the ensemble schedule states them (R-11-017a); a chain whose derived bound exceeds the deadline its declaration carries fails admission.
...
· Accept: one artifact, three outputs on every machine, and a fourth on a member of an ensemble (R-11-017a).
· Trace: CJ-WCET, CJ-ISOL

**R-11-017a** MUST: On a member of an ensemble (R-02-003a) the artifact R-11-017 emits carries a fourth output, that member's **ensemble link slot tables**, one per link endpoint it holds: for each slot its instant in the member's own spine cycles against the member's own origin (R-11-014a), its direction, and its guard band, every slot of a link carrying the link's one frame length (R-15-228d), and for the member's leader link one **leap slot** per re-alignment cadence (R-15-196a), a kernel-owned idle window the executive reserves on every core of the member at the same absolute instant so that no partition's slot straddles the step, placed after the crypto core's slot that verifies the frame whose landing it reads; one composition act emits every member's artifact and the ensemble schedule together (R-13-001d), so the two tables of one link are two views of one table and agree by construction, and no runtime exchange establishes or amends a slot. The tables are non-work-conserving in R-07-036's sense, an established-session slot with nothing to send carrying an authenticated idle frame (R-15-228d) and never being reassigned, and the link server's slots, the endpoint's encode and decode latencies, the crypto core's per-frame operations, and the endpoint's window grants enter the member's R-11-006 interval arithmetic as ordinary tasks and grants.
· Accept: each member's artifact is admitted alone under R-11-006 with its links as device edges, so an ensemble adds no global schedulability argument, R-11-014's reduction to per-core analyses holding across members as it holds across cores; a chain the composition declares across a link is bounded on R-11-014b's arithmetic with the link hop contributing its slot period plus its guard band as the per-hop term the ensemble schedule states, so a cross-member bound is derived and never asserted.
...
**R-12-004** IS: Every function on the machine falls into one of four classes: software on cores (TCB), software on cores (non-TCB), matter, and excluded foreign computers; the class table is exhaustive. A peer machine of an ensemble is those four classes on another die and adds no fifth (R-02-003a): what reaches this machine from it is a frame the matter tier lands and decodes, the crypto core authenticates, and a contained link server validates (R-15-228c, R-15-228e).
· Accept: the matter tier is driven through capability-checked MMIO/DMA and carries no instruction fetch and no firmware; every function a conventional platform would delegate to a firmware-running coprocessor is dissolved, reduced, or banned. The contained non-TCB class is one residue in the table and two in price, and no per-device obligation is minted for it: what has a specification to refine against is carried by the generic contracts already booked, the control-plane state machines by the Lustre plane R-12-002 and R-12-039 keep them in, the decoders' algebra by R-12-030, whose ECC composes with the integrity layer above it rather than replacing it, and the codec pixel kernels by R-15-238d's conformance, so CJ-VELUS and CJ-HAL stay the crown-jewel targets carrying the control plane and the HAL, with no row grown beside them; for the three approximation stacks with no specification to refine against, the radio PHY's estimation and equalization, the ISP (R-12-067), and software rasterization, the integrity half is carried downstream, the radio's by the FEC the matter tier decodes and by the framing, CRC, and link-layer encryption R-12-039 keeps in software, and the quality half is a measurement and no theorem, so the class's standing residue is quality rather than integrity. The one contained consumer that turns a sample into what the machine accepts is the biometric matcher R-12-016 places in this class, and its false-accept half is carried as a bound on authority rather than as a quality figure or a proof, R-12-019 keeping it a secondary factor and R-17-054 booking the limit.
· Trace: CJ-CERISE
...
**R-12-007a** MUST: The application payload crossing an ensemble link (R-15-228b) carries ring descriptors and payload bytes and nothing else: its descriptors name indices into the **receiving** member's own per-link session table (R-12-006), an offset, and a length, validated by the receiving link server against that table before anything becomes eligible (R-12-092); no capability, no address of either member, and no authority of any kind is encoded in a frame, and an index carries no meaning on the member that sent it.
· Accept: a link endpoint's windows are ring windows mapped without capability-store permission (R-12-007), so what the endpoint lands is untagged bytes; the table an index selects is delegated at composition to the receiving link server alone (R-13-006) and is revoked as any compartment's authority is (R-08-004), with no protocol reaching across the wire because the peer holds nothing to withdraw; and a peer compromised inside an accepted session reaches exactly what a compromised local ring peer behind the same table reaches, the ring proof's Byzantine peer (R-12-008) being the wire's peer.
...
**R-12-015c** MUST: Every protocol that authenticates a session using this service's attestation evidence names a construction and model binding the appraised measured state to the authenticated holder of that session's keys, with a defined freshness condition, protocol context and relying-party appraisal policy. Software identity and unit identity are separate policy choices under R-09-025a; a software-only appraisal does not authenticate a particular die, and no globally linkable unit identifier is required merely to appraise software. Each selected construction's attestation-session reference model is a crown-jewel specification: a protocol proof establishes the stated relation under its assumptions, not that the model captures the intended endpoint or compromise scope.
· Accept: the construction specifies how evidence, fresh challenge or equivalent anti-replay state, peer/role and transcript context, and the protected ownership of the session keys join. A machine-checked protocol argument establishes exactly that relation under named cryptographic, key-compromise and endpoint assumptions; a valid measurement quote or generic signature over a public key alone discharges no session-binding claim. Its connection to the implemented protocol remains owed under the platform's refinement obligations, and an unauthored model or theorem is recorded as open rather than credited as an existing guarantee.
· Accept: qualification includes replayed evidence, concurrent sessions, identical software on another unit, relayed handshakes and the model's key-compromise cases, with each outcome judged against the declared software-only or unit-specific appraisal. A relay that preserves the intended authenticated key holder is distinguished from unauthorized endpoint substitution; neither arbitrary relay prevention nor unit identity is inferred from software appraisal. R-12-015a's bounded credential operations and key non-export remain binding.
· Fail-closed: missing or failed evidence, freshness, session binding or appraisal refuses the attestation-authenticated session; it supplies no fallback claim about the peer, and the running generation is unchanged (R-17-030y).
· Trace: CJ-ATTEST, CJ-CRYPTO-SPEC, CJ-DEVTREE, CJ-T

**R-12-015d** MUST: The ensemble link session is a selected construction under R-12-015c and its reference model a named member of the inventory row that entry confers: two members of an ensemble (R-02-003a) authenticate each other by attestation evidence appraised on **both** of R-09-025a's registers, the generation register carrying the ensemble identity the composition fixed (R-13-001d) and the unit-specific appraisal binding the quote to the RoT-rooted device identity R-09-022 names and R-10-033 re-derives, each side expecting the identity and the ensemble identity its own attested devicetree names for that endpoint; each side supplies a fresh challenge, key establishment takes the hybrid post-quantum hedge R-17-049 states in the one composition-fixed configuration R-12-043a requires of every admitted protocol, the session keys reside in the crypto core (R-15-202) and every established-session frame is encrypted and authenticated there (R-05-070), the endpoint holding no key and no cipher; and a frame's associated data is its link, its session epoch, and the schedule's own count of that link's slots since the epoch began, a monotone value both ends derive from the schedule and neither from the wire, so a frame verifies only in the slot named for it and a frame delayed, reordered, or replayed into a different slot fails its tag; the receive window admits at most one frame per slot, while delay within that slot can preserve a valid tag, and the session's context binds the digest of the link's slot table each end holds, so an end holding a different table is refused at establishment (R-11-017a). Establishment and re-establishment use fixed-size handshake frames in composition-reserved slots at a composition-fixed cadence and never on demand. Before the session is established, only its bounded verified establishment handler consumes those frames; they authorize no application payload and no origin leap. The link contract states their grammar, cryptographic checks and public fields. Authenticated-encryption and slot-tag claims apply to established-session payload and idle frames. A peer whose evidence, freshness, ensemble identity, unit identity, or key binding does not match the composition's constants is refused the link, the member booting and running its own generation regardless.
· Accept: the construction names its evidence, freshness, context, and key-ownership binding as R-12-015c requires, its model states its endpoint and key-compromise assumptions with the wire untrusted, and an unauthored model is recorded as open rather than credited (R-17-049d); the appraisal is unit-specific by choice and stated as such, a second unit running the same image being the substitution an ensemble exists to exclude, and the binding between the quote's signing identity and the device-identity secret is the model's first stated obligation rather than a claim made here; R-15-132 is read against it and unmoved, the identity being attestation evidence processed by the session construction, never a persistent link-layer address or sender field in the link header.
· Accept: qualification adds two cases to R-12-015c's list beside its own, a peer of the right software on another unit and a peer at another ensemble identity, each refused, and an ensemble is updated as one composition (R-13-001d), so a member booted into a new composition refuses a peer still on the old one until that peer boots too; the link's line rate is bounded above by the crypto core's authenticated-encryption throughput as a composition constant the link slot tables respect (R-11-017a), stated as a cost and never absorbed.
· Fail-closed: a peer failing attestation, freshness, appraisal, or key binding, or attesting an ensemble identity or a unit the composition does not name, is refused the link and never the machine, which boots and runs its own generation (R-17-030z); the cost is every function whose shards cross that link for as long as the mismatch lasts.
· Trace: CJ-ATTEST, CJ-DEVTREE, CJ-CRYPTO-SPEC, CJ-T
...
**R-12-076** MUST: Consent for a powerbox grant is owned by a separate, small, verified trusted-path agent, not the compositor: it renders the consent surface into a second framebuffer it alone holds by capability, placed inside the bank-group binding and static DMA window the scanout reservation already covers (R-15-229, R-15-230), and that the scanout controller scans in place of the compositor's while the prompt is up, is attested by an RoT-driven hardware secure-attention indicator the compositor cannot draw, and takes the response over the input front-end itself.
· Accept: the compositor can deny service, an availability fault, but cannot spoof a grant, draw over the prompt, or capture a response; no compositor-written pixel is on the panel while the indicator is lit, the scanout source select being held by the R-12-078 latch and moving only the base the controller reads from, its window, TDM slice, and line-period deadline unchanged, which is what makes a tap inside the rendered region the agent's rather than a tap harvested on the compositor's artwork, and the cost is the screen for the prompt's duration, the applications' surfaces leaving the panel as their touches do.
...
**R-12-078** MUST: The switch is RoT-latched and does not depend on the driver yielding: the front-end carries the RoT-latched ownership register R-15-144 requires, and the scanout controller's source select is latched by the same signal, so the display half of the path takes the latch the input half takes.
· Accept: the property is a hardware bi-implication (the indicator cannot be lit while the driver owns the front-end or the compositor's framebuffer is on the panel, and the agent can neither own the front-end nor be scanned out without the indicator being lit) that the compositor, the driver, and a compromised kernel alike cannot separate; the display leg is what makes the input leg worth having, a compositor free to draw over the agent's region under a lit indicator turning a genuine tap on its own artwork into the agent's consent.
· Trace: CJ-DEVTREE, CJ-NI
...
**R-13-001d** MUST: An ensemble is composed as **one act**: the composer (R-13-001c) takes one ensemble manifest naming every member, the unit expected at each of its link endpoints, each link's two endpoints and its leader end, and each member's roster, and emits one generation per member (R-13-001a) together with the ensemble schedule (R-11-017a), each member's generation carrying the **ensemble identity**, the content hash of that manifest, as an extension its source determines and therefore in the generation register (R-09-025a); each member installs and boots its own generation through its own transactor and its own admission (R-11-001, R-11-005), no transactor and no admission spanning two machines, and a link comes up only between two members whose generation registers carry the same ensemble identity and whose device registers name the units the manifest placed there (R-12-015d).
· Accept: the ensemble identity is a hash over composition inputs and never over any member's image, so no member's image contains another's digest and the composition has no circular dependency; a relying party reproduces every member's reference integrity manifest from the one ensemble manifest and source (R-09-026, R-09-027); a partial rollout is a set of links held down and never a member running two generations, R-11-001's health gate being per member and reading no link state, so a member that reverts keeps its links down until its peers stand at the same ensemble identity, an availability cost and nothing else (R-03-009); and no reachable state has two members exchanging frames under different ensemble identities.
· Trace: CJ-DEVTREE, CJ-ATTEST
...
**R-13-023a** IS: Witnessed public commitment is a third supply-chain property beside R-13-023's correspondence and confinement. It establishes that the base image and package identities admitted under R-13-023b are committed to a public log, with resistance to inconsistent histories under R-13-023c's witness assumptions; it establishes neither uniform release selection nor the absence of targeted software. An adversary can log two admissible source closures and deliver the malicious one only to a target, with deterministic compilation and valid inclusion, correspondence and confinement evidence throughout.
· Accept: the two-logged-variants construction is retained as a counterexample to any claim that inclusion alone prevents targeting; public commitment, detection by a monitor and prevention by release-selection policy are distinct claims. A content-addressed name makes later comparison possible but supplies neither the committed source nor a monitor's review, so no source-availability, monitoring or non-targeting guarantee is inferred from the log's existence.
· Accept: holder-built packages and private roster composition remain admitted on their existing terms, with no mandatory vendor catalogue added. A stronger targeting claim requires its own defined package/channel/version identity, legitimate-variant policy, selection rule, available audit material, monitor responsibilities and response to a suspicious release; absent that construction, targeted selection remains a residual. Separately, R-13-026 permits a nondeterministic composer to produce different source-permitted bytes for the same roster; reproducible or local composition can remove that producer variation and cannot remove the logged-source-variant counterexample.
· Trace: CJ-T

**R-13-023b** MUST: What is published to the append-only log is what the population can share and never a generation's root: the base image's content-addressed root, the reproducible object R-13-026 names, and the content-addressed name of every admitted package, computed over the parts a composition does not touch: its exact content-addressed source closure and the hash of its capability manifest with its interface descriptor (R-13-001). The R-13-003 manifest is not that name, its image bytes, derivation, correspondence theorem, and wiring table being re-emitted per roster by R-13-028, so a hash over it would be as roster-specific as the generation root this entry refuses. Admitting a generation requires an inclusion proof for its base image's root and every package its roster names against a checkpoint satisfying R-13-023c's witness policy, with the device pinning the last checkpoint it verified and refusing one inconsistent with it.
...
· Accept: a promise of future inclusion is not inclusion, and none is accepted in place of a proof. Witness signatures satisfy the declared policy rather than merely reaching an arbitrary count; R-13-023c states the assumptions under which that policy prevents inconsistent accepted histories.
· Accept: a generation's root is not a logged object because it is roster-specific and per device (R-13-001a, R-13-001c), so its inclusion would record one device's composition and publish a roster the log otherwise never sees (R-17-035a). The log carries names rather than source bytes or rosters; that commitment adds no claim of source availability or semantic review, and neither strengthens nor substitutes for R-13-023's correspondence and confinement.
· Fail-closed: a package or base-image root with no inclusion proof, a checkpoint that fails the declared witness policy, or one inconsistent with the pinned checkpoint stops the **install**; the running generation is untouched (R-17-030v).
· Trace: CJ-T, CJ-DEVTREE

**R-13-023c** MUST: An admitted witness policy states its faulty-witness assumptions and carries a machine-checked safety argument that, within each policy epoch, every pair of acceptable quorums shares an honest witness whose authenticated history prevents inconsistent accepted checkpoints. The policy covers the witness identities and keys, persistent state, recovery and policy transitions rather than treating operational independence as a quorum theorem. The witness-policy reference model is a crown-jewel specification: a proof can establish its safety predicate without establishing that its fault, persistence or trust-transition assumptions match the intended deployment.
· Accept: for a fixed flat K-of-N policy admitting every size-K subset and tolerating at most f Byzantine witnesses, the argument establishes honest intersection, equivalently 2K - N > f. The 1-of-2 counterexample, two honest witnesses signing different extensions of a common checkpoint for different fresh clients, is rejected under that model. Other policies establish their own intersection condition rather than borrowing this formula.
· Accept: a witness verifies consistency with its authenticated persistent checkpoint and atomically persists the accepted successor before releasing its co-signature. State loss or rollback cannot silently restart the same witness identity on a fresh history; recovery supplies authenticated continuity or the identity is retired under the policy's transition rule. A key or witness-policy change supplies checked continuity from an accepted checkpoint, or an explicitly authenticated rebootstrap with its replacement trust assumptions and scope stated. Disjoint trust replacement is not claimed to preserve the old population-wide guarantee merely because both policies are individually valid, and no rule here requires two enrolled generation keys for a holder's R-09-036a change.
· Accept: the policy states availability separately from safety, including which outages can prevent its signatures being obtained; withholding required witness evidence spends install availability under R-17-030v and never permits a lower threshold at admission. Qualification covers fresh clients, conflicting forks, restart, state rollback and policy transition against the declared assumptions. External witness behaviour remains an assumption of the local proof, not an implemented service this platform claims to verify.
· Trace: CJ-WITNESS, CJ-T, CJ-DEVTREE
...
· Accept: the base image is the selected fixed boundary; no second fixed package layer is composed across rosters. Roster-wide re-lowering is the accepted cost of preserving R-13-010b's merge and specialization reach over the package closure. This is a default, not a measured optimum: reconsidering it requires R-15-036p's bytes-versus-WCET comparison from the R-18-014a backend, with the additional layer's composition and reproducibility duties included.
· Trace: CJ-NI, CJ-KERNEL
...
· Accept: the delivery-path cost stays booked as accepted in [performance-estimates.md](performance/performance-estimates.md) rather than claimed recovered, and the install path is the only exit, taken by a product decision and not by a platform mechanism.
· Trace: CJ-TAL-SOUND
...
· Accept: one Wasm interpreter artifact exists in the platform image; both theorems check against the pinned semantics at build; binding the engine grants an embedding no authority beyond its own manifest, and a hostile guest module reaches nothing its embedding did not import to it. What the embedding exposes to its guest, guest-observable timing inside the embedding's own slot, and object-level spatial and temporal safety within a guest's linear memory are excluded from the default claim. The boundary cases include adjacent-object overflow and stale-pointer reuse that core Wasm permits while confining the guest to its arena. A concrete consumer requiring stronger internal protection reopens comparison of safe-source compilation, finer module separation and MSWasm-shaped handles; any selected extension must account for its mechanized subset, compiler, host-call lifetime rules, metadata, memory and WCET costs before the pinned dialect changes.
...
· Accept: the ceiling is quoted as a product limit wherever the roster's capacity is claimed, so a reader meets it where the claim is made and not only at R-15-002a where the width is defined, and no goals-adjacent text claims a workload class the ceiling cannot hold on one machine, a claim that an ensemble (R-02-003a) holds a larger model being a claim about member count under R-15-171a and stated as one, no member's space growing; no per-class budget stated anywhere in the corpus exceeds its class's ceiling, and a composed map places each class inside its ceiling as it places every region inside the 36 bits (R-15-002b); what the first release commits to is R-18-004a's enumerated members rather than an open-ended capability. What would falsify the bet is a roster member the design wants and the space cannot hold, and its consequence is fixed rather than open: refusal at admission with no swap and no overcommit (R-15-171), and a fallback that bends capacity and never mechanism (R-15-173). This entry adds no obligation to the address path; it fixes where the number must be repeated.
· Trace: CJ-SAIL, CJ-DEVTREE

**R-15-002d** MUST NOT: No region of the physical address map is both a device region and executable: every region whose `mem_type` is `IOMemory` carries `executable` clear, and no composition of this platform, and no amendment to a composed map, gives one of them execute.
· Accept: an emptiness check taken where the map is authored: no region of any composition under `model/config/` declares `IOMemory` together with `executable` set, the CMake template there included, and the SoC map package the RTL reads carries the same per-region bits, being generated from the composition rather than authored beside it. That the compositions agree with one another on the region list is `tools/check.py`'s already and decides only that they agree; what this entry decides is the value they agree on, in each of them and in the template. A composition declaring an executable device region is a composition failure of the kind R-15-002b already states for a region that does not fit inside the space, and never a trap the machine takes at its first fetch.
· Accept: the model decides the access and this entry decides the declaration, and neither stands in for the other. `pmaCheck` (`model/model/sys/mem.sail`) reads the region's `executable` attribute on an instruction fetch and returns an access fault where it is clear, and an address inside no declared region is the same fault, `matching_pma_region` returning nothing, so for any map satisfying this entry the fetch path is closed by the model already; what the entry adds is that a map which does not satisfy it is never composed. The obligation runs one way and only one: a main-memory region that is not executable stays declarable, that being the shape of the admitted datapath scratchpad (R-15-165, R-15-167) whose executable form is the question R-18-009 leaves open, so a check reporting one of those is deciding something this entry does not state.
· Accept: it is not R-14-003 or R-14-004 restated and neither implies it. Those quantify over capability authority, execute authority being derived at install time over the read-only extents of the content-addressed image alone, and the per-page no-execute bit R-14-003 names as subsumed is a translation-structure attribute a platform with no MMU (R-15-002) does not carry. This quantifies over an attribute of the composed map that the fetch check reads whether or not any capability over the region exists, so the two are met separately, and a composition meeting §14's wiring rule while declaring an executable device region fails this one.
· Trace: CJ-SAIL, CJ-DEVTREE
...
**R-15-007** MUST: The capability format is **re-parameterized CHERI Concentrate at 64+1 bits**, not a bespoke format: the bounds algorithm and sentry and instruction semantics follow the pinned lineage, and the capability algebra is unchanged apart from the permission lattice (R-15-007b), and what changes is the field widths and one packing decision that carries the same information as upstream's internal-exponent flag: the exponent field is explicit, a zero field being the denormal case at effective exponent zero and a field of *k* + 1 the normalized case at effective exponent *k*. The frozen parameterization is a 36-bit address (R-15-002a), a 4-bit object type, 5-bit encoded permissions (R-15-007b), a 5-bit exponent, and 8-bit base and 6-bit top mantissas with the top's high bits derived as CHERI Concentrate derives them, plus one validity tag bit outside the 64 (R-15-203). The object-type and permission space, the sentry mechanism, and the capability instruction set are frozen with the profile.
· Accept: encode, decode, bounds derivation, and every capability instruction are a re-parameterization of `sail-cheri-riscv`'s capability functions rather than a rewrite, so what the change owes is R-15-007a's representation-correctness proof and not a re-proof of CHERI. A ratified RVY base remains a **conditional re-pin option**. Ratification triggers a recorded comparison with this baseline, preferably before the final freeze (R-15-014a); the comparison may propose changing the capability format and field widths rather than requiring RVY to fit the current 64+1-bit representation. It records the exact specification and model revisions, instruction and CSR differences, permission and sentry semantics, memory and stored-authority compatibility, footprint and timing costs, compiler and RTL work, proof obligations, and the independent oracles gained or lost (R-17-048a). Adoption requires an amendment that updates every affected obligation and reruns R-18-034; the present pin and format govern until that amendment passes. Ratification alone changes neither.
· Trace: CJ-SAIL
...
**R-15-007d** MUST: The format width is **fixed for the finally frozen profile**, not a composition parameter: every capability in the immutable image and every sealed blob is stored in it, so a later format break invalidates stored authority wholesale rather than costing a recompile.
· Accept: the current width and its SRAM-capacity margin remain governed by R-15-002a. R-15-007 permits a pre-final-freeze RVY amendment to revise the format and widths, with the affected model, layout, compiler, RTL, and proofs updated together. A format break after final freeze requires a separate approved transition accounting for every affected immutable image and sealed blob, including authority invalidation or reprovisioning; stored bits must never acquire authority by reinterpretation. No runtime widening path or automatic conversion is admitted (R-15-014, R-15-014a, R-18-034).
· Trace: CJ-SAIL, CJ-DEVTREE
...
· Accept: the ground is the derived constant R-15-007q names, made shorter rather than merely cheaper to walk: the sweep's pass time is the post-barrier component of R-08-007a's quarantine, so one instruction per group in place of a capability load per tagged granule shortens the pass by roughly the group width, and the quarantine interval, the composition-sized quarantine pool (R-08-008), and the §11 background slot budget shrink with it; capacity and admission headroom, never the containment bound, whose R-08-006 barrier waits on no sweep progress.
...
**R-15-009** IS: The bespoke matrix extension is fork-and-frozen until a ratified RISC-V matrix extension supersedes it, at which point it re-pins; its Sail semantics land with the instruction surface rather than ahead of it. The re-pin candidates comprise the attached AME (`Ztt` family), the integrated IME (`Zvvm` family), and the vector-coupled VME (`Zvt` family, the track of the earlier `Zvopmm` proposal). A track's draft name or review milestone admits none of its instruction or state surface.
...
· Accept: the target is recorded as the set of tracks, and a re-pin record names each candidate's specification revision and ratification status. The choice is made at the re-pin, which discharges R-15-010's five-part test and reruns the amendment review gate (R-18-034). The comparison accounts for each candidate's register and CSR state (R-15-001b, R-15-214), memory-access and restart semantics, dependence on the M-class VLEN (R-15-113), and arithmetic and rounding semantics (R-15-083, R-15-117); a claim of lower integration cost without that surface inventory is incomplete. Ratification does not by itself discharge those constraints or the sustained-GEMM measurement (R-15-116, R-15-116a).
· Trace: CJ-SAIL
...
· Accept: the amendment class recorded as closed in [Evaluated Architectural Alternatives](background/architectural-alternatives.md) reopens for this item and resolves in favour, on the gate as written.
· Trace: CJ-SAIL, CJ-FORMAT
...
· Accept: execute-only capabilities are derived over the read-only content-addressed image, never over written memory; the one instruction-memory writer is the measured-boot loader (R-09-006), on either memory class and on a bulk domain's refill as on a cold boot (R-15-190a), each of its loads ordered by the boot sequence and a plain `fence` ahead of the release that lets a core fetch it.
· Trace: CJ-SAIL, CJ-CERISE
...
**R-15-053b** MUST: The architectural leakage model names two observations, instruction latency and the memory addresses an execution emits, and the `Zkt`/`Zvkt` contract covers the first alone: a banked SRAM serves one access per bank per cycle, so the requests one core puts to one bank in one cycle (a load beside a draining store, R-15-015) serialize, and a scalar access's latency is a function of its address relative to those requests exactly as an off-list vector access's is of its element addresses (R-15-085a). No scalar load or store is on the data-independent-latency list by virtue of its address, and the timing-annotated model carries the scalar access class at its bank-conflicted bound, R-15-085b's shape, so the WCET entry stays sound. The obligation on addresses is R-05-070's for every access, scalar or element, and the constant-time claim is made over admitted secret-typed code alone: a secret-indexed scalar table lookup does not type at the address site under R-05-067 and is either hardened and re-typed or dispositioned at R-05-072, and a compartment outside that discipline holds no platform secret and is quoted with no such claim (R-17-003a).
· Accept: the leakage-model statement R-15-053 fixes names addresses as an observation beside latency, with the address term stated for the scalar path here as R-15-085a states it for the vector path; where the corpus names the model by the extension's letters it is the model that observes both, and no entry, row, or cell grounds an address obligation in the ratified extension's own data-independent-timing contract; and every constant-time claim (R-05-004, R-05-062) names the R-05-070 discipline it is scoped to. The cross-partition observer is closed by the schedule and not by the discipline, the term staying inside the issuing partition's own slot under R-07-036, so what the flow obligation covers is intra-partition observation, the scope R-15-085b states.
· Trace: CJ-LEAK, CJ-WCET, CJ-CT-SOUND
...
· Accept: a re-pin obligation is recorded, as for the matrix extension (R-15-009); CHERI instead carries R-15-007's conditional RVY review.
· Trace: CJ-SAIL
...
· Accept: the frozen fork's semantics are diffed against that shape at each PQC TG milestone (stabilization, freeze, public review, and ratification), with completed milestones distinguished from scheduled targets in the profile's status record, and any divergence is booked as a re-pin delta before ratification rather than discovered after it. The freeze is named among them because it is the milestone the two booked deltas change character at: before it a diff is taken against a moving draft; after it the frozen candidate supplies a stable comparison point, while any subsequent approved change still requires a fresh diff. R-15-053's keystone contract is thereby discharged architecturally for this instruction, not only locally.
· Trace: CJ-SAIL, CJ-LEAK
...
**R-15-059a** MUST: The C-class Keccak throughput and register-pressure figures are stated at the C-class **VLEN=256**, where the 2048-bit element group is LMUL=8 and one permutation occupies one LMUL=8 vector register group; they are not extrapolated from the V-class VLEN=4096.
...
· Accept: its entry point is itself a sentry entered by the same instruction. Before analogous switcher-proof authoring, a bounded qualification reads Griotte's Rocq/Iris call/return contracts, stack and register-clearing arguments and adversarial examples. It replays selected proofs with an exact assumption report and maps one known-to-unknown call onto this profile's permissions, finite otypes, ABI and Sail primitives, recording timing and interrupt differences. Source qualification may precede local primitive proofs; transfer requires the actual proved connection and grants no upstream theorem credit over this dialect. Read the exact source licence before incorporation; no second machine model or unproved axiom enters the proof chain by this comparison.
· Trace: CJ-KERNEL
...
**R-15-092** MUST: The net-new blocks with no legacy RTL to preserve (the capability- and tag-carrying DMA fabric, the TDM NoC, the fixed-function sequencers) are authored directly in Kami/Kôika, from which SystemVerilog is generated for synthesis.
· Accept: each elected version records the proved source or circuit object, the emitter, external primitives and wrappers, and the digest of the exact synthesis input. Theorem-tier RTL ⊑ Sail requires a checked correspondence from that proved object to that input; generation and byte reproducibility alone do not establish it. An unchecked emission seam remains at evidence tier under R-01-002b, with its validation and trusted tools named, and a circuit-source theorem is not reported as a theorem over the emitted RTL. The generated SystemVerilog is not an additional semantic anchor by default; any proposed anchor remains subject to R-05-020 and R-15-093. The plan's per-block election keeps Kami the default.
· Trace: CJ-RTL-SAIL
...
**R-15-100a** MUST: The absence-contract register exists as a single enumerated artifact, [absence-contract.md](hardware/absence-contract.md), carrying one row per claimed absence with its ground, the netlist evidence sought, and its discharge form. It is a *derived view* in R-15-001a's sense.
...
**R-15-100b** MUST: A removal whose ground is that no consumer exists holds only while no consumer exists, so where [Evaluated Architectural Alternatives](background/architectural-alternatives.md) defers a mechanism rather than declining it, the removal it would retire is named against it. Static code overlays, deferred until a measured composed roster exceeds its executable SRAM budget after R-13-010a, R-13-010b, R-13-010c and R-15-036a, would retire R-15-047's ground, a fill component writing an executable bank at run time being the runtime write-then-execute consumer that exclusion rests on the absence of, and R-13-010's deleted on-device loader with it. The refill of an OFF second-class domain carrying cold code (R-15-190a) is not that consumer: it is the R-09-006 loader writing under the R-15-247h hold and ahead of release, on the measured chain, where an overlay fills a bank at a phase transition with cores running. A deterministic generation-tag plane, preserved as the admissible form of identity-based temporal safety should the R-08-005 load filter and the R-08-006 sweep prove too costly, would retire R-15-035's exclusion of a second metadata plane. R-15-100's caches are retired by neither: the overlay disposition excludes a demand-filled instruction cache, a miss path and application-directed code loading by name, and a statically scheduled bank carries no tag or valid array for R-15-103's audit to find.
...
· Accept: no die-to-die link exists anywhere in the machine (R-15-162); an ensemble link joins two machines and is not a link inside one (R-02-003a, R-15-228b), each member of an ensemble running its own kernel instances and its own image under the one parameterized model and meeting this entry alone.
· Trace: CJ-SAIL
...
· Accept: CHERI stays a single-front-end problem; vector data carries no tags and checks land on scalar-issued vector memory ops, per-element for gather/scatter. A proposed core-issued arithmetic operation over registers alone is evaluated under R-04-010a and the profile's ordinary ISA admission rules, rather than classified as an autonomous texture engine by its workload. This distinction admits no new instruction: addresses and capability-checked loads remain ordinary core operations, and any candidate still owes exact semantics, fixed timing, no hidden state, and a measured benefit over the admitted RVV lowering or fusion.
· Trace: CJ-CERISE, CJ-WCET
...
· Accept: the legacy turbo and convolutional decoders are absent from the die; belief propagation on a fixed graph is grammar-free arithmetic and does not breach the codec-block ban; and the decoders' instruction surface carries no measurement conditioning, its scope being categorical rather than a margin, so R-15-014b owes it at the provisional freeze rather than at the second act. The storage path's fixed-function LDPC ECC engine (R-12-025, R-09-004) and the ensemble link endpoint's encoder and decoder stages (R-15-228e) are matter of one of these two families carrying no instruction surface, each a stage of a device datapath whose latency is that device's own row in the timing-annotated model and not a third entry of R-15-119c's, and neither is a unit of this entry; the link's code is this design's own, stated in the link contract between two copies of this design and owing no table to any standard, so R-15-119a's carve-out does not reach it.
· Trace: CJ-SAIL
...
· Accept: admitted under the defense-in-depth clause because a physical cutoff adds no modeled mechanism, no Sail surface, and no proof obligation, while covering a domain the gates' own verification does not reach. An ensemble link takes no cutoff, being no user-facing port: its authority is the attested session R-12-015d states, and its two ends are placed by composition rather than plugged in (R-15-228b).
· Trace: CJ-T
...
· Accept: the enable is driven by grant liveness rather than software claim, and therefore carries an in-use indication no compromised component can suppress. The ensemble link endpoint is enabled by the same grant liveness under a composition-fixed grant, the link server's delegated window grant standing in for a consented one because a server has no consent act for it (R-15-228c).
· Trace: CJ-CERISE
...
**R-15-150** IS: The microphone, radio, and USB-data cutoffs are sealed, gasketed Hall-effect (or reed) switches read from the position of a detented user-held actuator carrying a coded pole pattern, never toggled by a magnet passing a sensor: paired sensors of opposite polarity inside the sealed cavity read the pattern, the ingress-protection rating is preserved with no chassis penetration, and whether the break is a contact or a dominant FET is an implementation choice, auditable by IRIS inspection.
· Accept: the cut is a dominant load-switch on the power/bias rail, or the data-lane mux for USB; the state is held by the actuator's detent and by no electrical latch, so a sensor glitch or a power cycle re-reads the position rather than remembering a toggle; and *enabled* is the one reading in which both sensors report the pattern's opposed poles, every other reading (a single external field of either polarity, a saturating field, no field) being *cut*, so a stray magnet can throw a cutoff and cannot restore a peripheral the user cut. What a shaped field at the chassis can do, and what a stray field costs, is R-17-058g's.
· Trace: CJ-T
...
· Accept: multi-tier capacity is taken monolithically by sequential 3D, sharing one mask set, one fab lot, one package, and one attested identity, with no die-to-die link anywhere. An ensemble link (R-15-228b) joins distinct physical dies with separate packages, address spaces, memories and attested unit identities, each a copy of the same mask-set design (R-17-061b), and is not the interface this entry declines (R-02-003a).
· Trace: CJ-DEVTREE
...
**R-15-171a** MUST: On an ensemble (R-02-003a) the fit R-15-171 decides is decided **per member**: a model larger than one member's bulk class is admitted only sharded at composition, every shard and every expert of a mixture resident on the member the composition names, each member's shard fitting that member's own budget under R-15-171 with no swap and no overcommit, the exchange between shards laid out as declared ring exchanges over the link slot tables (R-11-017a, R-15-223a) with the frames per step fixed by those tables and never by the tokens, and no weight, expert, or activation fetched across a link on a routing decision, R-15-171's storage-boundary rule holding at the link boundary unchanged; a sharding that does not fit the members it names or whose per-step exchange does not fit its link slots is refused at composition on the same pass R-15-171 refuses an unfitting roster on.
· Accept: residency and work per token stay composition-time constants on every member exactly as on one machine, a routed expert on another member being computed in that member's own slot on a frame its table carries whether or not the route selected it; the admitted token rate of a sharded model is a constant of two grants and not one, the bank grant R-15-247p fixes on each member and the link slots the ensemble schedule grants its exchanges, together with the crypto core's per-frame share those exchanges spend (R-12-015d), so what an ensemble adds is capacity and aggregate throughput by member count and never a single session's rate above what one member's grants fix; and a model that fits an ensemble makes no claim about the ceiling of any member (R-15-002c).
· Fail-closed: a sharding that does not fit its members or its slots is refused at composition on R-15-171's pass (R-17-030q); the cost is delivery of that model.
· Trace: CJ-MEMPLAN, CJ-WCET
...
· Accept: the fallback is stated as a composition change, not an architectural one; an ensemble (R-02-003a) is not a fallback either, relieving no member's budget.
...
· Accept: the class is normative ahead of R-15-247m's qualification, and the ground for adopting it there is structural rather than array evidence, recorded as a judgment and not presented as a derivation: adopting the class deletes the second roster the tier-count grading would otherwise demand (R-18-030a), the grading itself standing unresolved for the first class (R-15-173a), so the specification carrying the class has one roster to show closing rather than two, which is a decision about specification size taken as one; what qualification then supplies is every per-class constant, R-15-247m admitting no density figure as an architectural input ahead of its measurement, so a macro that falls short at qualification moves the capacity the plan places and never the boundary it places against.
· Trace: CJ-MEMPLAN, CJ-WCET
...
**R-15-247h** MUST NOT: No instruction is added for discharge, refresh, second-class tag maintenance, or class migration. The mechanism is §12 matter, one fixed-function register slave with no instruction fetch and no firmware, and its two phases run under two conditions: **discharge** is sequenced by the RoT while every application core, DMA engine, and capability-bearing fabric initiator that can address the domain is held in reset, and **refresh** is the same slave's **fixed share of each second-class bank's composition-time TDM schedule**, taken on ON and RETAINED domains alike (R-15-247k) at the cadence the hot-corner floor R-15-247m measures sets. No refresh rides an instruction and none rides a core: `cbo.scrub` (R-15-177a) remains the ECC verify-and-correct scrub, restores no charge, and is not the refresh under another name.
· Accept: R-15-010 test 5 admits both phases on the ground that refused the Sv39 walker, Itanium's RSE, the TBRE engine, and the STKZ engine: neither is autonomous in the sense the test refuses, the discharge firing only on an RoT-sequenced transition and the refresh advancing only with the schedule's own slot counter, both decisionless and data-independent, and neither reading a runtime measure (R-15-247r). TBRE was refused for a walker whose traffic followed what it read and contended with execution for the array; the refresh share reads nothing and contends with nothing, a bank in its refresh slot being granted to no requester exactly as a bank in another island's slot is, so it enters every access's §11 term as the constant the bank schedule already is and never as an interference term, and its issue stream is the slave's and not a core's. There is no instruction stream in flight across a discharge to carry an opcode, so no opcode is spent. `cbo.zero` is undisturbed: R-15-182's eager zeroize remains the granule and block mechanism during execution, and discharge is whole-bank and only across a power transition.
· Accept: **a software refresh loop is refused on arithmetic and not on taste.** Its issue count is the class's capacity over the block width and its rate the inverse of the floor R-15-247m measures at the hot corner, so it is a background term that grows with the capacity the class is bought for and shrinks with the corner the qualification returns, where a background slot is sized once at composition and a slot sized against a room-temperature figure is a slot that loses data at the trip temperature. The share is instead the arithmetic [the bank-count contract](hardware/bank-count-dse-contract.md) computes per candidate, refresh phases times phase cycles against the deadline the floor sets, and a candidate whose sweep does not fit inside the floor is refused there rather than made up by a core.
...
**R-15-247k** MUST: RETAINED for a second-class domain is **maintenance-accessible and requester-inaccessible**: the refresh sequencer reaches it on the composition-fixed cadence, taking the same share of the bank schedule it takes on an ON domain (R-15-247h), nothing else does, and its rail is set by refresh-write margin rather than by latch hold margin, a different and generally higher floor.
...
**R-15-247m** MUST: Per-class usable density, the retention floor, and the read, write, refresh, and discharge latency constants are measured on a **repaired megabit-class macro carrying complete tag, ECC, refresh, discharge, and routing overhead**, and enter the attested devicetree as composition-time constants at part qualification. The retention floor is measured **at the hot corner and at end of life**: at the junction temperature at which R-15-193's thermal trip fires, the hottest the machine runs at without halting, and after the bias-stress drift the service life the release declares accumulates, the qualification stating the corner and the life beside the figure. The refresh cadence is derived from that floor, and the refresh share of each bank's schedule (R-15-247h) and the refresh power at capacity from that cadence, so the per-mode power figure R-18-004c declares carries refresh at the hot-corner cadence and at no milder one. No density figure is an architectural input ahead of that measurement.
· Accept: the corner is named because retention in an oxide-semiconductor gain cell is thermally activated (R-15-247c), moving by an order of magnitude over a few tens of degrees, so a room-temperature, time-zero figure overstates the floor at the corner that decides refresh by orders of magnitude; a figure stated at room temperature, at time zero, or at any corner milder than the trip and the declared life is not the floor, decides nothing here, and quoted as the floor is a review-gate finding against this requirement. The retention corner is stated as a measured interval rather than a point, on the calibration and attestation footing: its floor is consumed by the refresh deadline and by nothing else (R-15-247c), its cold-corner ceiling by nothing but the honest width of the R-17-058f window, and the spread between them decides refresh power, droop admissibility at capacity (R-15-247g), the refresh share of the bank schedule, and that window at once. A macro that misses one of these is a part that does not qualify, not an architecture that reverts: the fallback bends capacity and never the mechanism (R-15-173).
· Trace: CJ-DEVTREE
...
· Accept: there is no die-to-die interface to protect; every hop of the memory path is on-die, and an ensemble link (R-15-228b) is a device edge outside this chain whose frames carry their own code, established-session frames also carrying an authentication tag verified at the consuming member and re-encoded at no hop (R-15-228e). The one re-encoding point on the path, the memory controller's sub-granule read-modify-write stage (R-15-181), does not mask a transit fault, because the existing codeword's check is verified *before* the merge, so the stage catches a corrupt granule rather than laundering it into a fresh codeword.
· Trace: CJ-SAIL
...
· Accept: R-15-178 fixes the code's strength and leaves its grouping open, and the grouping decides both the cost and the independence. Independence is the security property: a combined or unequal-protection code would make the categorically-worse failure's guarantee conditional on the ordinary failure's distribution, which is refused in [Evaluated Architectural Alternatives](background/architectural-alternatives.md); the area it would recover is taken instead from R-15-181a's codeword width.
· Trace: CJ-CERISE
...
· Accept: no edge in the lattice is traversable by an access, a timer, a counter, or a temperature; a domain's power state is a function of the mode schedule and the current mode index, never of access history, occupancy watermark, idle timer, or observed leakage. First-class destruction is a required postcondition of the qualified R-15-189n construction, not an inference from rail voltage; second-class OFF has the separately stated R-15-247k meaning. Neither mode-state definition guarantees erasure under uncontrolled power loss (R-15-200).
· Trace: CJ-NI, CJ-WCET
...
**R-15-189n** MUST: The RoT gates that transition on a one-bit discharge confirmation. A first-class physical-erasure claim requires the qualified construction to establish that confirmation entails destruction of the data, validity tags and check bits under its declared corners; a rail-voltage observation alone is not that implication.
· Accept: the confirmation is one bit read once, never a retry loop and never a feedback path into timing; a domain that does not confirm does not enter the mode. R-15-200's qualification records the storage-node discharge path, the confirmation's relation to its erase postcondition, and the covered supply and temperature conditions. A construction lacking that evidence fails qualification; R-15-189j's clear before reuse still applies and supplies no power-loss erasure claim.
...
· Accept: an OFF bulk domain is refilled by the measured chain and by nothing else: its data extents return through the capabilities their owners already hold at the quiescent points R-15-171 admits, and an executable extent R-15-247s places there is written by the R-09-006 loader alone while the R-15-247h hold is in force, each extent verified against the content-addressed image before that hold releases, so a bulk OFF→ON carrying code is the boot-chain variant R-09-006 already takes and not a runtime fill; no running component writes an executable extent on either class, which is what keeps R-15-047's ground standing across the transition (R-15-100b).
· Trace: CJ-ISOL, CJ-DEVTREE
...
· Accept: every theorem touching the RoT path uses one-sided bounds only; external interface clocks terminate in a bounded FIFO behind the capability-checked DMA window, contributing only a line-rate bound to the §11 bandwidth reservation; reset is synchronized per domain at its boundary. An ensemble link's recovered line clock is one of the external interface clocks and terminates as they do, the count of three unmoved (R-15-196a).
· Trace: CJ-WCET, CJ-ISOL

**R-15-196a** MUST: Each member of an ensemble keeps its own clock spine (R-15-195) and an ensemble link's recovered line clock is one of R-15-196's external interface clocks, terminating in the endpoint's bounded FIFO behind its capability-bounded window with the fabric side on the island clock, so no modeled path crosses between two members' clocks and no fixed-latency claim holds at cycle cadence across a link; what holds is slot cadence, and alignment across members is a **scheduled leap and never a loop**: the ensemble's members form a composition-fixed leader tree carried in every attested devicetree, a member's endpoint on its leader link exposes a one-sided arrival-phase reading the landing of the leader link's frame in the slot its table names for the leap updates, consumed only after the crypto core has verified that frame's tag in its own slot (R-15-228e), a slot whose frame failed its tag yielding no leap, and at the leap slot its tables name (R-11-017a), a kernel-owned idle slot between rotations that belongs to no partition and that the executive reserves on every core of the member at the same absolute instant, the follower steps its own platform-wide origin (R-11-014a) by at most the composition's skew bound, so every table stated against that origin, the executive, the TDM NoC schedule, the window grants, and the link slot tables, moves together and nothing stated relative to the origin moves at all. The origin is one **frame-origin register** in the clock spine (R-15-195) that every core's boundary handler reads as its common zero when it programs `mtimecmp` for the next table instant (R-07-040), as the memory controller's TDM arbiter and each window's grant do (R-15-208), loaded by the reset table (R-15-198), leaving `mtime` the free-running counter R-09-015 keeps it, and stepped by exactly one kernel instance, the boot core's (R-09-006), under a capability the composition grants to it alone, in the leap slot and at no other instant; that register, which a single machine loads once and never writes, and its one write path are the only structure an ensemble adds to a machine's timing, so no partition's slot straddles the step and R-07-007's three cross-core mechanisms gain no fourth, the step being one kernel's capability-checked store to a spine register every core reads as a constant. Every receive slot of a link table carries a guard band at least the skew bound plus the link's latency bound, the bound being derived from the declared oscillator tolerance of both members and the leap cadence and never asserted, and the leap is a measured step and not a decision: which slot leaps, from which link's reading, and by at most how much are composition constants, and only the magnitude is read, from a physical quantity no partition's data reaches and no unverified frame steers.
· Accept: the leap moves the origin's phase alone, never a divider, a rail, or a PLL (R-15-195, R-15-188), and no partition's slot instant or width relative to the origin moves, R-07-032, R-07-040, and R-08-027b holding as stated; what a partition measuring across a leap under an R-08-031 grant can read is oscillator drift, a physical quantity independent of every partition's data, together with the bounded walk a compromised leader or a wire adversary delaying a valid frame within its accepted slot can impose, which R-17-025a books; no write path reaches the frame-origin register from anything but the boot core kernel's capability-checked store, and no leap path exists from a landing the crypto core has not verified; the root member of the tree takes no leap and every other member is follower on exactly one link, so a member holding several links steps once; the skew bound is measured on a two-member pair at qualification and entered in the attested devicetree beside the link's latency constants, a pair whose measured skew exceeds the bound failing to qualify rather than leaping more often; the endpoint holds no phase accumulator, no latency estimator, and no write path to any origin or launch instant, its clock-data recovery being line-interface matter reaching no launch instant (A-21); no theorem quantifies over a metastability resolution at the link (R-15-197); and the fixed-latency claim across members holds at slot cadence and not at cycle cadence, which R-17-025a books.
· Trace: CJ-WCET, CJ-ISOL
...
**R-15-200** IS: The absent external memory interface removes the bus-interposer case; orderly zeroization and uncontrolled power loss are separate cases. SRAM volatility alone establishes neither negligible first-class remanence nor cold-boot closure. First-class sensitive state and key storage remain subject to construction-specific qualification, and the second class's confidentiality remanence is booked open (R-15-247m, R-17-058f). R-15-199's memory-cryptography exclusion remains a scope and proof-surface decision, not a claim that every physical residual is closed.
· Accept: part qualification records the actual first-class macros, registers and key-storage construction under abrupt power loss, cooling, supply discharge, retained power states and the declared service-life corners, alongside the second-class measurements. The record identifies all sensitive plaintext and key copies, expanded keys and derivation state, and distinguishes a completed erase construction from remanence characterization. A missing record or an erase claim resting on the medium's name fails qualification; measured decay characterizes the physical residual and supplies no sanitization guarantee (R-15-247c). Authority remanence on the second class is discharged by the confirmed exit-path construction (R-15-247d, R-15-247e, R-15-247q), not by volatility. No relative key-versus-data lifetime is asserted without this complete record (R-17-059a).
· Trace: CJ-T
...
**R-15-208** MUST: In-flight-DMA revocation is an obligation with one mechanism, bounded per-window re-authorization against the revocation bitmap: an autonomous streaming engine's delegated window capability is held in a fabric-side window register at the owning island's memory controller, and at the start of each TDM grant of that window the controller reads the revocation bit at the window capability's base from the island's R-08-005a array in its own slot, and no request of that grant issues when the bit is set; a core-issued mover's operand capability undergoes the ordinary architectural capability checks, not a fictitious capability reload at issue, and its transfer is one bounded transaction inside the core's slot. R-08-006's barrier stops further issue under retired register-held authority and waits for already accepted transfers to complete, so no such operand or transfer survives containment. Time-to-containment for a device-held capability is therefore a composition constant derived from the schedule artifact and from nothing else: the bitmap-publication step of R-08-006, plus at most one rotation of the window's TDM schedule until the next grant re-authorizes, plus one granted slot for the requests already accepted in the grant in progress, which R-15-208a's completion boundary lands. That constant is the device term of R-08-004's bound, and its worst case enters the §11 budget.
· Accept: the constant is derived from the attested schedule's rotation period and slot width and never from device behaviour, and the §11 admission proof derives it per composition; no window stays authorized across a grant boundary without the bit read; and no revocation-epoch comparison exists on the request path, an epoch advance on any teardown reaching a device window only through its bit.
· Trace: CJ-CERISE, CJ-WCET
...
· Accept: a partition's timing behaviour is independent of another's activity, decided against the isolation model's non-interference statement rather than against the list of mechanisms above it; until that model is authored the obligation is stated and not discharged (R-17-003b, R-17-003c).
· Trace: CJ-ISOL, CJ-NI
...
**R-15-222** IS: Islands are the unit confidentiality is partitioned at: a memory, NoC, and power partition (bound SRAM banks, macros, or tiers, TDM-NoC slots, and at the top rung its own clock and power domain), not a coherence domain. A high-assurance domain holds its island exclusively; a low-sensitivity island may host more than one domain, in time on one core under `fence.t` and concurrently on distinct cores under R-15-222a's per-core slotting, and never two in one granule (R-08-012c).
· Accept: no cache-coherence protocol and no coherence directory exist within or across islands; which domains an island hosts is a composition constant of the attested devicetree, and an island hosting a high-assurance domain hosts no other.
· Trace: CJ-ISOL

**R-15-222a** MUST: Where an island hosts more than one domain concurrently, its shared structures are slotted per core at composition: the memory controller's per-island arbitration is sub-divided into a fixed per-core rotation at every bank the island owns, the island's NoC injection slots are assigned per core, and each slot is sized to the longest single transaction a bank serves (R-15-087's atomic RMW), so no transaction spans a slot and no core's access waits on another core's. The rotation runs whether or not a core issues, so a core's memory and fabric latency is a function of the schedule and of its own addresses and never of a peer core's activity, and each core's bandwidth quantizes to its slots and feeds §11 admission as R-15-228's island ceiling does. A domain reaches the array only through the cores it is composed onto, so per-core slotting is per-domain slotting.
· Accept: no arbitration decision inside a shared island depends on another core's requests, decided against the isolation model's non-interference statement once it is authored and stated until then (R-17-003c); the rotation and its slot widths are entries of the attested schedule artifact the §11 proof emits, non-work-conserving in R-07-036's sense; and a composition placing two domains on the cores of one island with no per-core rotation is refused at admission.
· Trace: CJ-ISOL, CJ-NI, CJ-WCET
...
**R-15-228b** IS: An **ensemble link** is a point-to-point wire between an endpoint of one machine and an endpoint of another (R-02-003a), and it is a network-class device edge and not a memory: it carries one fixed-size frame per slot of the ensemble schedule (R-11-017a, R-15-228d) and nothing else, an established-session payload frame carrying R-12-007a's payload inside R-12-015d's session and establishment traffic following that construction's bounded path, so no load, no store, no capability, no coherence message, and no access into either member's address space crosses it. It is not the die-to-die link R-15-162 declines inside one machine, its two ends being distinct dies of the same mask-set design (R-17-061b), with separate packages, address spaces, memories and attested unit identities, and it expands no member's space (R-15-002a) and relieves no member's roster (R-15-173); it is not the memory path R-15-199 and R-17-059 scope, so both are unmoved; it is not the wired data port R-15-145 names, admitting no foreign device, its authority to carry a payload being the attested session and its enable the liveness of a composition-fixed grant rather than a consented one (R-15-228c); and it is not a LAN port under R-15-135 through R-15-139, carrying no Ethernet PCS, no autonegotiation, and no adaptive canceller, so R-15-139's decline is unmoved by it.
· Accept: no entry, cell, or artifact reads a link as a memory hop, a growth axis, a capacity relief, or a peripheral; every byte a link carries lands in the endpoint's receive window, establishment traffic reaches only R-12-015d's bounded handler, and application payload reaches any other memory only by the link server's copy after decode, authentication, and validation (R-15-228c, R-15-228e, R-12-007a); each machine of the pair meets R-15-111 and R-15-162 alone, the link joining two machines and sitting inside neither; and an ensemble adds no fourth cross-core mechanism to R-07-007 and no fifth class to R-12-004, the link server being contained non-TCB software on cores, the endpoint matter, and the leap one kernel instance's capability-checked store to the frame-origin register that every core reads as a constant and no kernel-to-kernel act (R-15-196a).
· Trace: CJ-ISOL, CJ-DEVTREE

**R-15-228c** MUST: The ensemble link endpoint is matter under R-04-010a's three conditions and a device of R-15-206's second shape: a line interface of the kind the USB PHY already is, whose recovered clock is one of R-15-196's external interface clocks terminating in the endpoint's bounded FIFO and whose equalization is trained at link-up and frozen per link epoch on R-15-137's terms; a transmit engine that encodes the frame its window holds under the link's code, a fixed linear map at fixed latency, and serializes it, and a receive engine that decodes what the wire delivers at R-15-228e's fixed latency and lands the result in the receive window's entry for that slot, each engine holding a delegated, bounds-checked, revocable window capability into a ring region of the endpoint's island (R-15-223) for the lifetime of its window; and a slot timer of R-15-122's kind that launches and lands at the instants of the table core-issued software loaded, stated against the member's own origin. The endpoint decides nothing: it holds no retry buffer, no acknowledgement or credit state, no arbiter, no frame parser, no cipher, no key, no verdict register, no queue whose depth varies with a peer's activity, no state a received frame writes except the landing itself and the one-sided arrival-phase reading R-15-196a's leap consumes, and no leap path from a landing the crypto core has not verified; its configuration MMIO and its two windows are one ownership granted and revoked together (R-15-143), held by the link server compartment alone, and the endpoint is electronically enabled by the liveness of the link server's delegated window grant, R-15-146's mechanism with the composition's grant in place of a consented one, a server having no consent act for it, so a link whose server is torn down is cut as any unused peripheral is.
· Accept: the block is sorted by R-04-010a's conditions and by nothing else: its transition relation is welded over no writable program store, its code and every schedule parameter are declared configuration the composition supplies, every launch instant and every window is core-issued under explicit capability operands, and it runs to one slot's bound holding nothing across slots but the frozen equalization, which is the per-epoch reading R-15-137's canceller already takes of the third condition; a codeword the wire delivers is reacted to under a relation this composition welded and never decided on, which is the PD sequencer's precedent; the absence contract carries its four rows, A-18 through A-21, discharged by R-15-102's structural predicate as authored RTL under R-15-092; and the revocation of its windows is R-15-208's per-grant re-authorization with R-15-208a's completion boundary, so a torn-down link leaves no engine authorized past the next grant. A link server that does not drain a receive entry before the schedule lands on it again is that partition's overrun, restarted by R-07-040 and reaching no other partition's slot.
· Trace: CJ-CERISE, CJ-HAL

**R-15-228d** MUST: Every ensemble link carries one fixed-size frame per slot of its table and the table is non-work-conserving in R-07-036's sense: the link server places a frame in every transmit slot, an **authenticated idle frame** where an established session has nothing to send, so the wire carries the same frame count and the same frame size whatever the payload; no slot is reassigned, donated, or skipped, no frame is acknowledged, retried, or reordered, and no arbitration, flow control, or back-pressure exists on either end, a slot a peer fills with an idle frame being that peer's burned slot and never a stall of this member's frame. A link's rate is the slots its tables grant, quantized as R-15-228's island ceiling is and fed to admission the same way. Before establishment, the same enabled-link slots carry only R-12-015d's scheduled handshake frames or fixed public padding, neither eligible as application traffic or a leap input.
· Accept: no entry, register, or signal in either endpoint or either link server makes a launch or a landing depend on a peer's readiness, a receive entry's occupancy, or a frame's contents, decided against the isolation model's non-interference statement once it is authored and stated until then (R-17-003d); the frame count and size are per-link composition constants of the attested schedule artifact; and an idle frame is indistinguishable on the wire from a payload frame except by a type field read after the crypto core has decrypted and authenticated it, which is the wire-activity half of R-08-027b read across a link and the reason slot occupancy is not a channel on an adversary-held cable.
· Trace: CJ-WCET, CJ-ISOL

**R-15-228e** MUST: A frame is one codeword of a fixed-geometry block code in one of R-15-119's two families, this design's own and stated in the link contract, encoded and decoded by the endpoint's engines (R-15-228c) at **one fixed latency** on R-15-119c's discipline, the decoder run to its iteration bound and never stopped early so that its latency is independent of the frame's symbols and of whether it was correctable and is one entry in the endpoint's device row of the timing-annotated model; an established-session frame carries the session's authentication tag over the whole frame under R-12-015d's keys, verified by the crypto core before any field is read by the frame's verified copy-once parser (R-05-042), and a frame whose tag does not verify, an uncorrectable frame included, is delivered to nothing and fail-stops the link. The tag decides integrity against an adversary and the code decides it against noise: no decoder verdict, syndrome, or CRC decides a frame, on R-15-119b's ground, and no frame is re-encoded at any hop, R-15-176's chain ending at each member's edge and the frame carrying its own checks across the wire.
· Accept: the established-session frame grammar is a descriptor of R-05-042's inventory read after the tag verifies and canonical under R-05-051a; the decode, the encode, and the tag operation are entries the member's admission check charges per frame in the endpoint's, the link server's, and the crypto core's slots (R-11-017a); no path delivers an established-session frame's payload or any of its receive-window bytes to a consumer ahead of the tag; scheduled handshake frames take only R-12-015d's bounded establishment path, and the code alone authorizes no handshake, payload, or leap; the decoder holds no verdict register and reports nothing but the decoded bits, so an uncorrectable frame is met at the tag and never at a decoder flag; and re-establishment after a stop is a fresh session at the composition-fixed cadence R-12-015d names, never a resumption and never on demand.
· Fail-closed: a frame that fails its tag, a corrupted frame included, stops the link, whose session is torn down and re-established only at the scheduled instant R-12-015d names (R-17-030z); the cost is every function sharded across that link until re-establishment.
· Trace: CJ-FORMAT, CJ-WCET, CJ-CRYPTO-SPEC
...
**R-15-244** MUST NOT: Analog compute-in-memory (memristor/RRAM/PCM crossbars) is excluded: no deterministic ISA semantics to Sail-model, non-volatile analog weights add persistent plaintext custody requiring their own erase construction, and it presents the richest possible data-dependent power channel with a reactive calibration loop.
...
**R-17-001b** MUST: The coverage matrix exists as a single enumerated artifact, [coverage-matrix.md](assurance/coverage-matrix.md), carrying the boundary enumeration, the property enumeration, and one row per pair of them with its construction, its discharge mode, the requirements that carry it, and its standing, the crown-jewel inventory's status of the specifications those requirements constrain. It is a *derived view* in R-15-001a's sense.
· Accept: the artifact exists; every requirement it cites resolves here; and the cell conditions of R-17-001a hold over its own two enumerations, so the coverage argument is computed from the register rather than restated beside it. Each row's discharge mode reports how its cited requirements carry the pair, and each row's standing is computed by `tools/check.py` from those requirements' traces and the inventory's status column, `authored` where every specification the row reaches exists, `not authored` where none does, and `partial` between, rewritten under `--fix` so that no row's standing is hand-maintained; the view introduces no obligation and no vocabulary of its own.
· Trace: CJ-T
...
**R-17-003c** IS: The seam's spatial term has a third case below the macro, and it is a schedule rather than an absence: two domains concurrent on two cores of one low-sensitivity island share that island's NoC slots, bank ports, and RMW serialization point, and R-15-222a's per-core rotation schedules the contention away rather than deleting it. What survives is the bank port and the RMW stage themselves, shared structures whose separation is a rotation the admission proof emits and not a path that is absent, plus the physical coupling of the shared array R-17-003b books one rung up, and the non-interference statement the rotation is stated against is the same unauthored NoC/island isolation model.
· Accept: the inventory row and the coverage cell read residual and not absent for a domain inside a shared island, and neither reads proved while the model is unauthored (R-15-211, R-15-222a); no high-assurance domain sits in the case, exclusivity being mandatory for it (R-15-222, R-15-226); and the case is narrowed by scheduling and never by throttling (R-15-227).
· Trace: CJ-ISOL, CJ-NI

**R-17-003d** IS: The seam's spatial term has a fourth case, across a wire, and it is a schedule and a session rather than an absence: two members of an ensemble share nothing but the ensemble link (R-15-228b), whose separation is R-15-228d's slot table and R-12-015d's session, and the non-interference statement both are stated against is the same unauthored NoC/island isolation model extended by one boundary class, the untrusted wire.
· Accept: the inventory row and the coverage cell read residual for the separation across the link and absent only for the arbitration, flow control, and retry the link has no vehicle for, and neither reads the separation proved while the model is unauthored (R-15-211); no high-assurance domain's authority crosses a link, a frame naming indices alone (R-12-007a); and the case is narrowed by scheduling and by the session, never by a regulator or a retry (R-15-227, R-15-228d).
· Trace: CJ-ISOL, CJ-NI
...
**R-17-013h** IS: The enrolment residual: R-09-036a puts the largest authority on the machine, which root may admit a whole generation, behind one consent act, and R-17-013's comprehension ceiling reaches that act without being stated over it, that entry's subject being the powerbox's runtime declassification and enrolment being an RoT act on the same trusted path. A holder who enrols a root they should not has authorized what no proof reaches, and the consequence is larger than any powerbox grant's, being every future generation rather than one object.
· Accept: the residual is the comprehension half alone and is named here because R-17-013's own scope does not carry it; what it is not is the coerced case, which is answered rather than booked, enrolment riding the credential R-12-016 gates so a holder compelled to enrol presents the duress credential (R-12-018) and the device erases as it does at any other coerced unlock. Nothing is claimed to close the comprehension half, and no mechanism that would decide on the holder's behalf which roots to trust is admitted, that being the policy oracle R-17-013e refuses.
· Trace: CJ-NI
...
**R-17-014a** IS: An ensemble adds a fourth dimension none of seL4's proof reaches beside R-17-014's three: the composition of per-member non-interference theorems across the ensemble link as an untrusted-wire boundary, each member's theorem taken whole and the link's contribution stated as R-15-228d's schedule and R-12-015d's session and nothing more, fresh in method and in premise, and carried by no per-member theorem.
· Accept: booked as freshness, not trust, exactly as R-17-014 is: the composed statement is authored beside the seam lemmas of R-18-031 as a lemma of its own and folded into none of them, and no cell reads the composed statement as proved, the proved term of B-10/P-1 being the session's cryptographic protection R-12-015d states and the composition standing residual beside it (R-17-003d); its silent failure modes are a too-weak boundary-class statement in the isolation model, which is the crown-jewel risk row 8 already carries and not a new one, and a wrong endpoint entry in the composition, which fails closed at the link (R-12-015d) rather than silently.
· Trace: CJ-NI, CJ-T
...
**R-17-023** IS: Seam: **revocation ⋈ the schedule**. Containment is the bounded publication and register, loan, proxy and device barrier of R-08-006; reclamation uses the sized background slot class and R-08-007a reuse gate.
...
**R-17-025a** IS: Seam: **the ensemble link ⋈ clock domains, the schedule, attestation, and the memory path**, four meetings named so the mechanism is not admitted alone: the link's recovered clock is one more member of R-15-196's second class and alignment across members is a scheduled leap of the follower's origin under a composition-time skew bound (R-15-196a); its slot tables are emitted by the one composition act with the NoC schedule (R-11-017a); its session is bound to the peer's ensemble identity and unit as composition constants (R-12-015d); and it touches the memory path nowhere, landing in a device window and carrying no load, store, or capability (R-15-228b).
· Accept: discharged by R-15-196a, R-11-017a, R-12-015d, and R-15-228b through R-15-228e, with the three asynchronous boundaries R-15-196 counts unmoved and the fixed-latency claim held at slot cadence and not at cycle cadence across a link; the residual the leap keeps is stated and bounded: a compromised leader, or a wire adversary delaying a genuine frame within the same accepted slot, can walk a follower's origin by at most the skew bound per cadence inside the guard band, visible to a time-granted partition (R-08-031) as R-17-007's class of channel and to no other, a frame displaced outside its accepted slot being refused and stopping the link (R-17-030z); the tag does not authenticate the arrival instant within a slot.
· Trace: CJ-WCET, CJ-ISOL
...
**R-17-030v** IS: Fail-closed seam **the log and the directive ⋈ installing at all**: a generation lacking an inclusion proof for its base-image root or a package identity, or whose checkpoint fails the witness policy, is refused (R-13-023b, R-13-023c), as is one offered under directive metadata whose expiry precedes the monotonic time floor (R-11-005a), and one composed in a version the installing reader does not install or proved against a spec-set the installing checker does not carry (R-11-005b), so a party able to withhold log material, to stop refreshing directives, or to compose only for a step the device is not at holds a denial of *updating* for as long as the withholding lasts.
· Accept: what the seam spends is the availability of the **install path** alone and never of the running generation, which continues on the composition it has; the denial is visible rather than silent, R-11-005a obliging the age of the newest verified directive to be surfaced, so a withheld update and an absent update are distinguishable to the holder.
· Accept: refusal is the correct direction on this seam because each alternative is worse in the property its requirement exists to establish, installing an artifact whose public commitment or log-history consistency is unestablished, installing one whose currency is unknown, and installing one its reader was not generated to read, which is the grammar R-13-009 deletes; the member is therefore retained as a refusal rather than repaired into a degraded mode, and it costs availability alone as R-03-009 requires of every member.
· Trace: CJ-T, CJ-DEVTREE

**R-17-030y** IS: Fail-closed seam **attestation authentication ⋈ the session**: R-12-015c refuses a session whose evidence, freshness, session binding or appraisal fails, so an adversary able to withhold or disrupt that exchange can deny the attestation-authenticated service without changing what the running generation executes.
· Accept: the refusal yields no peer-authentication claim and no weaker fallback; evidence of measured software alone is not substituted for the missing session property. Its availability cost is the affected session, and the model states the endpoint and key-compromise assumptions under which the accepted-session guarantee holds.
· Trace: CJ-CRYPTO-SPEC, CJ-DEVTREE, CJ-T
...
**R-17-030z** IS: Fail-closed seam **the ensemble link ⋈ the sharded function**: a peer that fails attestation, freshness, appraisal, or key binding, or that attests an ensemble identity or a unit the composition does not name, is refused the link (R-12-015d), and a frame that fails its tag, a corrupted frame included, fail-stops the link (R-15-228e), each stopping the link and never the machine, which boots and runs its own generation regardless; so a party able to withhold a peer, hold it at another composition, cut the wire, or degrade it past the code holds a standing denial of every function sharded across that link for as long as the withholding lasts, re-establishment being a scheduled act at a composition-fixed cadence and never an on-demand one.
· Accept: what the seam spends is availability alone, per R-03-009, and the link's alone: no member yields a weaker peer claim, delivers an unverified frame, or changes what any machine executes, and no member's schedule, memory, or authority moves, a burned slot being what the non-work-conserving table already spends (R-15-228d); the denial is a server's, reaches no emergency path, and so leaves R-17-030l's composition unmoved; and its rate is countable under R-17-030m through R-16-027, a link stop being an attested event class of the same kind as a containment entry.
· Accept: refusal is the correct direction because each alternative is worse in the property its requirement exists to establish: a link carried at an unmatched composition is an ensemble nobody admitted, a frame delivered on the code's verdict alone is an integrity claim resting on noise statistics against an adversary, and a retry is the reactive element R-15-228d deletes; the member is therefore retained as a refusal rather than repaired into a degraded mode, and it is standing rather than incidental, held open for as long as the peer, the wire, or the composition is withheld.
· Trace: CJ-DEVTREE, CJ-ATTEST, CJ-WCET
...
**R-17-035a** IS: The generation-boundary seam is that same window on the install path, and it has two halves. **Latency:** because an install is a generation (R-13-001a), adding a package costs a boot, and the atomicity, the whole-roster memory plan, the schedulability proof, and the cross-roster merge are all bought at the price of immediacy, with the reboot additionally discarding everything outside the enumerated mutable volumes (R-10-026). What the boot pays is the check R-13-028 bounds by what the composition re-lowered off-device (R-13-001c): every object of the package closure outside the base image, re-emitted at composition with its derivation and its correspondence theorem, is re-checked on the device, while the base image is mapped and never re-lowered (R-13-026), so as that boundary now sits an install's cost tracks the roster and not the change: the same act is cheap on a bare device and expensive on a full one, and what bounds it on the device is R-06-015a's declared budget on each proof the install checks, taken over that closure, never the size of the change. **Disclosure:** a generation is roster-specific, so composing one off-device (R-13-001c) tells the composer what the device has installed.
...
**R-17-049d** IS: The ensemble link session is a construction this design authors: it has no upstream symbolic analysis in a prover to curate from as the four radio protocols have (R-12-043e), so its composed session security stands where R-17-049 leaves TLS 1.3's and WireGuard's, unnarrowed above the primitive reductions, carried by the reference model R-12-015d names once authored and by nothing until then, and it inherits the hybrid hedge and the hardness assumptions unchanged (R-17-049b).
· Accept: no requirement claims the link's session security from the primitive reductions alone or from the radio protocols' analyses; the model, its theorem, and the implementation connection are recorded open in the inventory row R-12-015c confers, as that entry requires; and the hedge's response to a broken assumption is a generation of every member and never a negotiation on the wire.
· Trace: CJ-REDUCTION, CJ-ATTEST
...
· Accept: discharged by R-15-137 through R-15-139; the ensemble link (R-15-228b) is not a wired port of this entry, carrying no Ethernet PCS, no autonegotiation, and no canceller, its code bought once per link of a server rather than for one port of a handset (R-15-228e).
· Trace: CJ-ISOL
...
· Accept: the first two are stated in [performance-estimates.md](performance/performance-estimates.md)'s media rows and the third is a product ceiling this design accepts rather than a gap it intends to close.
· Trace: CJ-NI, CJ-WCET
...
**R-17-058** IS: Physical residuals: Rowhammer stays listed only as the per-class residual R-15-184 states, never as a live remote primitive; first-class cold-boot remanence is unresolved pending R-15-200's construction-specific qualification, and second-class confidentiality remanence is not closed (R-17-058f); TEMPEST-class emission is attenuated but not closed against a near-field probe; and macro-internal and sequential-3D thermal coupling are narrowed rather than eliminated. Four are not consequences of the invasive-attack scope line and are stated in their own right: the analog channel out of the crypto core (R-17-058a), which is a decision about that core, the two the R-15-247 second class adds, its continuous refresh signature (R-17-058e) and its confidentiality remanence (R-17-058f), and the sealed cutoffs' magnetic actuation (R-17-058g), which is a decision about how the switches are read.
· Accept: each names the mechanism that narrows it, if any, and the scope line that bounds it; a completed orderly erase is not evidence that abrupt power loss erases either class.
· Trace: CJ-T
...
**R-17-058f** IS: **Confidentiality remanence scales with the capacity the second class buys.** The property making refresh cheap leaves a power-off plaintext window whose measured interval R-15-247m qualifies, covering second-class domains live at power loss. The first class's interval is separately qualified under R-15-200, not assumed to be seconds. Authority remanence is discharged by R-15-247d, R-15-247e, and R-15-247q; confidentiality remanence after uncontrolled power loss is discharged by no construction in this design.
· Accept: R-15-247q's mode-exit discharge narrows the exposure from every domain ever written to those ON or RETAINED at the instant of power loss and does not close it, an adversary cutting power while the machine runs. Ceasing refresh is a power action and never a sanitization action, and R-15-247c forbids treating decay as one. Neither class's OFF state establishes erasure by its name; any comparison uses the qualified constructions and their measured physical residuals. The design accepts the remanence cost to buy capacity and states the trade rather than resolving it.
· Trace: CJ-T, CJ-ISOL

**R-17-058g** IS: **The sealed cutoffs are actuated by a field, and a field has no hand on it.** The microphone, radio, and USB-data cutoffs read a magnetic pole pattern through the chassis (R-15-150), which is what keeps them sealed and what leaves two cases open: an adversary at the chassis with a shaped field reproducing the pattern at both sensors restores a peripheral the user cut, and a stray field of any shape throws one, the cellular ring included where it lands on the radio switch. The first is a physical-access adversary holding the device, on the far side of every guarantee R-15-145 admits the cutoffs for, which are stated against software, firmware, and the RoT and not against a hand on the case; the second is an availability cost of the kind R-17-030l composes into its life-safety case, one more way a device behaving correctly does not ring.
· Accept: no requirement claims the cutoffs against an adversary holding the device, and none claims a peripheral the user cut stays cut against a shaped field at the chassis; the coded pattern is what narrows the first case to a shaped field rather than any magnet, and the cut reading on every ambiguous input is what keeps a stray field from restoring anything, so the second case runs one way only (R-15-150). No mechanism is proposed against either, each candidate spending the property the switch exists for: an electrical latch that remembers a toggle is the pass-actuated reading R-15-150 forbids, and a penetrating mechanical switch is the ingress path the sealed cavity exists to close.
· Trace: CJ-T

**R-17-058h** IS: **The ensemble link is a conductor that leaves the die and the enclosure, and what it carries is protected by a session rather than by absence.** Interposition, splicing, and substitution on the wire are defeated by R-12-015d's attested session and R-15-228e's tag, not by there being no wire to reach, which is the one place this design defends a wire by a mechanism where it defends the memory path by a scope line (R-17-059); the established session shows ciphertext at instants the schedule fixes, with establishment exposing only R-12-015d's declared public handshake fields, and the SerDes current and the frame edges are R-17-058's near-field class read on a conductor outside R-15-153's shell, attenuated by nothing that entry counts.
· Accept: no claim rests on the cable, the connector, or the enclosure for a link's confidentiality or integrity, a claim resting on any of them being a spec defect of R-05-153's kind as at R-17-058a; what the wire shows an observer is established-session ciphertext, the construction's declared public handshake fields, and the schedule's public instants, the same frame count and size in every slot (R-15-228d), so the exposure adds no secret-dependent term; the memory path's scope line is unmoved, the link being no memory path (R-15-228b); and the exposure is stated per link, so a wire an adversary holds costs that link and nothing another member holds.
· Trace: CJ-T, CJ-LEAK
...
· Accept: there is no defence-in-depth layer beneath the package boundary on the memory path, so the invasive-attack scope line is load-bearing rather than conservative, and if it is ever judged wrong there is no second mechanism behind it. **This is the sharpest instance of *verify rather than hedge* applied to the design's own threat model.** The ensemble link is not on the memory path and takes the other posture, a session defending an interface that exists (R-15-228b, R-17-058h), which leaves this line where it is.
· Trace: CJ-T
...
· Accept: **powered-die access does not settle key custody after power loss.** A construction that destroys all usable key material before bulk data becomes recoverable can leave an attacker reaching data without its key. Whether this die supplies that separation is unresolved until R-15-200's complete key and data lifetime record exists; first-class SRAM alone supplies no such ordering. Asserting equal reach for every power state, or asserting that the key necessarily disappears first, is a review-gate finding. The bulk-class cipher remains declined under R-15-199: R-15-247q narrows the exposure through completed exit-path discharge, R-17-058f books the remaining physical residual, and a new encrypt-and-key pipeline would spend the R-17-039 refinement workstream on that residual. This is a scope and proof-surface choice, not a claim of ineffective cryptography or variable cipher latency; R-15-201's hit-or-miss term belongs to its tree.
· Trace: CJ-T
...
· Accept: mitigations are open RTL, multi-sourcing, and IRIS backside optical verification, none complete; an ensemble multiplies copies of that one mask set and adds no second one, the relation between two attested copies being R-12-015d's construction and R-17-061b's booking rather than a fabrication residual.
· Trace: CJ-T
...
· Accept: the bottom logic tier is the inspection target, not a claim that every transistor or alteration on it is resolved; the evidence states the visible structures and the R-17-063 resolution ceiling. Upper-tier defect evidence does not substitute for inspection of the acting logic tier.
· Trace: CJ-T
...
**R-17-061b** IS: An ensemble is N attested copies of one mask set: the supply-chain object count multiplies per member and the mask-set, fab-lot, and inspection residuals R-17-060 through R-17-063b book do not, each member being one die of the one design imaged as R-17-061 states. What the ensemble adds and a machine does not carry is the two-machine attestation relation the second-die ledger named as one more thing to state and prove: it is stated at R-12-015d, its reference model is a member of the inventory row R-12-015c confers, and it is carried no further than that model until the model and its argument exist.
· Accept: R-15-162's disposition stands per member and no member's trust structure acquires a second die; a fabrication defect on one member reaches its peers only as a peer whose attestation the composition still names, which is the case the session model must state; and no artifact quotes a link's authentication as more than the session model's standing in the inventory.
· Trace: CJ-T, CJ-ATTEST
...
· Accept: each inspection claim names the process, preparation, imaging resolution, resolved structures and complementary electrical checks. A macro outline or capacity bound is not verification of individual bitcells or transistor connectivity against RTL; evidence from another process does not qualify R-17-063b's stack. None of the three mitigations is complete.
· Trace: CJ-T
...
**R-17-063b** IS: The process stack is an **axiom of this design and an offering of no foundry**: a frontside-powered fully-depleted SOI logic tier on an infra-red-resolvable node (R-15-160, R-15-161, R-17-061a), sequential 6T SRAM tiers built above it at the count R-15-170 budgets, oxide-semiconductor 2T0C decks in the back end of line above those (R-15-247), all patterned High-NA on one die inside the usable half field R-15-170 fixes, with no chiplet and no bonded stack (R-15-162). No foundry offers the combination and no process design kit carries it, and the axiom that engineering is free (R-01-006) prices effort and not wafers, so the stack enters the physical/fab class of R-17-001's index as a named assumption beside the fabrication residual R-17-060 names rather than as a supply anyone has.
· Accept: every capacity, density, area, yield, or unit-cost figure an artifact states above the single planar tier names this assumption or cites an entry that does, and a figure stated as if a foundry supplied the stack is a review-gate finding against this requirement; the assumption is discharged element by element and only by a process design kit read under R-18-001a's dating rule at the milestone that would spend it, never by a laboratory demonstration and never by the phrase *sequential 3D*.
· Accept: the assumption is one and not a graded set: an element withdrawn, a tier count not reached, a deck not manufacturable above a finished logic tier, a half field not patterned, bends capacity under R-15-173 and returns no declined mechanism (R-18-008), which is what makes it an axiom and not a design fallback point; and no proof rests on it, T's constituents being unmoved by who patterns the die (R-18-008a), so what it moves is R-18-004b's yield-and-unit-cost line, which is where the stack is priced.
· Trace: CJ-T
...
**R-18-001a** MUST: Every maturity claim this specification makes about an upstream is a reading with a date, taken from that upstream's own repository, release page, licence file, or specification build, re-taken at the milestone that would spend it on exactly the rule a licence is read under, and never inherited from the reading before it or inferred from a project's lineage or reputation. The substrate this design starts from is accordingly a set of start-froms of unequal maturity and not an ecosystem: a vendored model and its capability oracle at one end, an unverified backend prototype and an unratified standardization draft in the middle, and an unauthored soundness metatheorem at the other. The realization's schedule position is a reading of the same kind and is stated as a computation rather than as a date: the remaining estimate is the sum `tools/check.py` holds over the plan's open cells, the attended rate is the one the plan states, and the horizon is the quotient the same tool re-derives.
· Accept: no maturity claim about an upstream stands anywhere in the corpus without the artifact that read it and the date it was read on, [the version matrix](hardware/cheri-version-matrix.md) carrying the date for the three CHERI lineages and the plan carrying it per pin at the item that took it. An unowned or undated maturity claim is a review-gate finding against this requirement exactly as an unbuilt prerequisite is one against R-18-003b, and a claim contrasting this substrate with an alternative's is read against the same measurements rather than against the phrase naming it. The realization carries no calendar horizon, in a heading or in a sentence: the plan's basis states the attended rate and the tool-held remaining estimate and derives the horizon from them, so a month or a year named anywhere in the corpus as a delivery date is a finding against this requirement.
· Trace: CJ-COMPCERT, CJ-TAL-SOUND
...
**R-18-004b** MUST: R-18-004a's floor is priced in capacity, bandwidth, and area rather than in throughput, because at a fixed node this machine's binding scarcity is on-die memory and not compute (R-15-158, R-15-170), and each of its five comparisons is between a demand the composed roster produces and a supply an artifact measures, failed by either side alone, with a sixth line beside them declared on both sides. **First-class capacity**: the composed roster's resident bytes on the first class, summed over R-15-247s's first list as the whole-program static memory plan places them (R-08-010, R-08-012a), at most (1 − τ) of that class's usable capacity, τ being its own declared exclusivity fraction, and that capacity being at least the pessimistic end of R-15-173a's ungraded-branch budget stated as **payload**, the SECDED floor R-15-175 puts on every array and the tag plane's own DECTED code (R-15-178) being carried above the payload figure and never inside it, which is the convention this entry fixes once for every capacity figure the floor is scored against and which R-15-181a prices. **Second-class capacity**: the same comparison over R-15-247s's second list, against that class's own τ, with a usable second-class capacity of at least 4 GB of payload on the same convention. **Exclusivity**: each class's declared τ (R-15-172, which nets it out of the budget) at most 20%, so the pooling tax is inside each comparison rather than reported beside it. **Bandwidth**: at least 8 GB/s of sustained second-class read granted to the M-class island, and at least 20 GB/s of sustained aggregate read plus write across all islands, that aggregate covering concurrently the scanout reservation at the enumerated maximum mode (R-15-236b), the compositor's per-frame surface traffic, the decode frame pool at the declared ceiling, the camera burst ring, and the M-class grant. **Area**: the first-class array at that capacity and the roster's whole logic tier together inside the usable High-NA half field R-15-170 fixes, on the one die R-15-111 fixes with neither of the two reliefs R-15-162 declines, the fit shown at both ends of R-15-170's own density band on its own per-gigabyte arithmetic with R-15-181a's metadata above the payload figure, so a candidate closing only at that band's optimistic end fails. **Yield and unit cost**: the composition declares, at the frozen configuration, a die-yield model over the tier and deck count, a per-tier defect density, and the repair coverage the R-15-247m macro carries, and the unit cost per good die that model and a declared wafer cost derive, against a unit-cost ceiling the release declares; the model is stated for the process stack R-17-063b books and for no other, a composition declaring no model, no figure, or no ceiling fails this requirement, a derived cost above the ceiling fails at the R-15-108 report on declared operands exactly as an area fit closing only at the optimistic end does, and the model itself is decided against the first production lot's measured yield and never ahead of it.
· Accept: the sixth line is declared on both sides on the ground R-18-004c gives the power figure: no pre-silicon artifact reaches a yield and R-15-247m's megabit macro says nothing about the yield of a reticle-scale multi-tier die, so the line obliges the model's, the figure's, and the ceiling's existence and derives no value any of them must meet, the ceiling being a product decision recorded rather than a threshold this entry computes. What it forecloses is a die priced by area alone, the case in which a candidate closes every comparison and is built at a cost nobody declared.
...
**R-18-030** IS: Memory staging is capacity-limited, not availability-limited: first parts are planar single-tier at order 0.5–1 GB, with the 4–8 GB phone envelope arriving if and only if sequential-3D tier counts reach 8–16.
...
**R-18-030a** MUST: The multi-tier envelope is planned as upside on the **first class**, the single planar tier being the case that class must be shown to close on: the budget R-15-173a states for that tier carries the base system, servers, and compositor in hundreds of megabytes and the scalar working set beside them, which is what the class holds once bulk moves onto the R-15-247 second class, and the closure is owed at that budget's pessimistic end, which is where R-18-004b reads the first-class floor. Framebuffers, origin arenas, media buffers, and model weights are second-class regions and are budgeted against that class's own capacity, not against the planar tier.
