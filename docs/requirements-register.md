...
**R-15-208** MUST: In-flight-DMA revocation is an obligation with one mechanism, bounded per-window re-authorization against the revocation bitmap: an autonomous streaming engine's delegated window capability is held in a fabric-side window register at the owning island's memory controller, and at the start of each TDM grant of that window the controller reads the revocation bit at the window capability's base from the island's R-08-005a array in its own slot, and no request of that grant issues when the bit is set; a core-issued mover's operand capability passes the issuing core's load filter at issue (R-08-005) and its transfer is one bounded transaction inside the core's slot, so it holds nothing across a rotation. Time-to-containment for a device-held capability is therefore a composition constant derived from the schedule artifact and from nothing else: the epoch flip (R-08-006), plus at most one rotation of the window's TDM schedule until the next grant re-authorizes, plus one granted slot for the requests already accepted in the grant in progress, which R-15-208a's completion boundary lands. That constant is the device term of R-08-004's bound, and its worst case enters the §11 budget.
...
· Accept: refusal is the correct direction on this seam because each alternative is worse in the property its requirement exists to establish, installing an artifact whose population is unknown, installing one whose currency is unknown, and installing one its reader was not generated to read, which is the grammar R-13-009 deletes; the member is therefore retained as a refusal rather than repaired into a degraded mode, and it costs availability alone as R-03-009 requires of every member.
· Trace: CJ-T, CJ-DEVTREE
...
· Accept: thirty-six requirements confer a refusal and nineteen seams collect them, both figures recomputed rather than maintained here.
· Trace: CJ-T
