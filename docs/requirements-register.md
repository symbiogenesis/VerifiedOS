...
· Accept: the parameter set is VLEN per class, core count per class, issue width and pipeline depth, SRAM bank/macro/tier-to-island assignment, scratchpad sizes, the CBO block size, the FEC decoders' realization geometry on the classes carrying them (R-15-119a), and the TDM-NoC schedule; there is no cache, way-colouring, or integrity-tree parameter to choose. Naming the FEC geometry here settles nothing about the per-class bank count, which R-15-247p places in R-15-014a's frozen set while this enumeration does not name it: whether that is the tier-to-island assignment under another name or an entry this list is short remains a reported question rather than one this row answers. The block size is one parameter for `cbo.zero`, `cloadtags`, `creclaim`, and `cbo.scrub` together (R-15-007q welds the group to the block), so no block instruction's width is tunable alone, and what enters R-15-014a's enumerated final-freeze delta at item (vii) is that one welded parameter rather than four instruction widths, scored against a first-class SRAM macro geometry and a second-class deck row and page geometry together.
· Trace: CJ-RTL-SAIL
