...
**R-15-040b** IS: The scalar-FP fork books a **second** accepted cost beside the calling convention, and it is a standing tax rather than a single act: RVV's scalar-operand forms architecturally name an `f` register, so every future vector floating-point extension arrives with scalar-operand forms of its own and is adopted only by re-homing them and restating the divergence against an upstream Sail model that has moved under the profile since the last restatement. No other row of this profile pays a surcharge of that shape.
· Accept: the cost is recorded beside R-15-040's own and is a **term in the arithmetic** of every subsequent vector-FP amendment rather than a note beneath it: R-15-067p prices `Zvfbfa` and `Zvfofp8min` against it by name, and an amendment adopting a vector floating-point extension without pricing the re-homing is a review-gate finding (R-18-034). It reverses nothing, R-15-040's ground being R-15-039's register file, and it is bounded by the fact that makes the fork affordable at all, that the platform curates its own profile and its own model rather than tracking one.
· Trace: CJ-SAIL
...
**R-15-067p** IS: `Zvfbfa` (native bf16 arithmetic) and `Zvfofp8min` (OFP8 conversion) are carried as **candidates priced against R-15-040b's surcharge** rather than adopted with the profile, because each arrives with scalar-operand forms and each therefore pays the re-homing and the restatement against a moving upstream model that no other adoption on this profile pays. The two halves are not worth the same: the OFP8 conversion half has a live consumer in R-15-117's software de-quantization on the M-class vector unit, and the native bf16 arithmetic half has none, `Zvfbfwma` with `Zvfbfmin` (R-15-067i) supplying the widening multiply-accumulate and the conversions the matrix lane consumes.
· Accept: the disposition is decided per half and not per extension, which is what a surcharge priced per adoption requires: an amendment proposing either states the re-homing it performs, the divergence it restates, and the consumer it serves, and one proposing the arithmetic half without a consumer is refused on R-15-048's ground rather than weighed. Neither is a standing adoption under R-15-067k, whose criterion is an admission argument discharged in the entry, and the argument here is exactly what is open.
