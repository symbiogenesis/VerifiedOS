...
**R-12-076** MUST: Consent for a powerbox grant is owned by a separate, small, verified trusted-path agent, not the compositor: it renders the consent surface into a framebuffer it alone holds by capability and that the scanout controller scans in place of the compositor's while the prompt is up, is attested by an RoT-driven hardware secure-attention indicator the compositor cannot draw, and takes the response over the input front-end itself.
· Accept: the compositor can deny service, an availability fault, but cannot spoof a grant, draw over the prompt, or capture a response; no compositor-written pixel is on the panel while the indicator is lit, the scanout source select being held by the R-12-078 latch, which is what makes a tap inside the rendered region the agent's rather than a tap harvested on the compositor's artwork, and the cost is the screen for the prompt's duration, the applications' surfaces leaving the panel as their touches do.
