...
· Accept: a composition whose two members' tables for one link name different slots, whose guard band on any link is below the composition's skew bound plus that link's latency bound, whose re-alignment cadence times both members' declared oscillator tolerance exceeds the guard band, or whose link server's slot does not cover the frames its table grants it, fails admission rather than shipping a table one end does not hold; the tables are entries of the attested schedule artifact, public constants of the generation as the widths and offsets are (R-11-014a).
· Trace: CJ-WCET, CJ-ISOL
...
**R-15-196a** MUST: Each member of an ensemble keeps its own clock spine (R-15-195) and an ensemble link's recovered line clock is one of R-15-196's external interface clocks, terminating in the endpoint's bounded FIFO behind its capability-bounded window with the fabric side on the island clock, so no modeled path crosses between two members' clocks and no fixed-latency claim holds at cycle cadence across a link; what holds is slot cadence, and alignment across members is a **scheduled leap and never a loop**: the ensemble's members form a composition-fixed leader tree carried in every attested devicetree, a member's endpoint on its leader link exposes a one-sided arrival-phase reading the landing of a frame updates, and at the leap slot its tables name (R-11-017a), a kernel-owned idle slot between rotations that belongs to no partition, the follower's kernel reads that register and steps its own platform-wide origin (R-11-014a) by at most the composition's skew bound, so every table stated against that origin, the executive, the TDM NoC schedule, the window grants, and the link slot tables, moves together and nothing stated relative to the origin moves at all. Every receive slot of a link table carries a guard band at least the skew bound plus the link's latency bound, the bound being derived from the declared oscillator tolerance of both members and the leap cadence and never asserted, and the leap is a measured step and not a decision: which slot leaps, from which link's reading, and by at most how much are composition constants, and only the magnitude is read, from a physical quantity no partition's data reaches.
· Accept: the leap moves the origin's phase alone, never a divider, a rail, or a PLL (R-15-195, R-15-188), and no partition's slot instant or width relative to the origin moves, R-07-032, R-07-040, and R-08-027b holding as stated; what a partition measuring across a leap under an R-08-031 grant can read is oscillator drift, a physical quantity independent of every partition's data; the root member of the tree takes no leap and every other member is follower on exactly one link, so a member holding several links steps once; the skew bound is measured on a two-member pair at qualification and entered in the attested devicetree beside the link's latency constants, a pair whose measured skew exceeds the bound failing to qualify rather than leaping more often; the endpoint holds no phase accumulator, no latency estimator, and no write path to any origin or launch instant, its clock-data recovery being line-interface matter reaching no launch instant (A-21); no theorem quantifies over a metastability resolution at the link (R-15-197); and the fixed-latency claim across members holds at slot cadence and not at cycle cadence, which R-17-025a books.
· Trace: CJ-WCET, CJ-ISOL
...
· Accept: booked as freshness, not trust, exactly as R-17-014 is: the composed statement is authored beside the seam lemmas of R-18-031 as a lemma of its own and folded into none of them, and no cell reads proved for the ensemble link's confidentiality while it is unauthored (R-17-003d); its silent failure modes are a too-weak boundary-class statement in the isolation model, which is the crown-jewel risk row 8 already carries and not a new one, and a wrong endpoint entry in the composition, which fails closed at the link (R-12-015d) rather than silently.
· Trace: CJ-NI, CJ-T
...
· Accept: discharged by R-15-196a, R-11-017a, R-12-015d, and R-15-228b through R-15-228e, with the three asynchronous boundaries R-15-196 counts unmoved and the fixed-latency claim held at slot cadence and not at cycle cadence across a link.
· Trace: CJ-WCET, CJ-ISOL
