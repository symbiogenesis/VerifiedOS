...
· Accept: the leakage-model statement R-15-053 fixes names addresses as an observation beside latency, with the address term stated for the scalar path here as R-15-085a states it for the vector path; no entry, row, or cell quotes `Zkt`/`Zvkt` as covering a memory address; and every constant-time claim (R-05-004, R-05-062) names the R-05-070 discipline it is scoped to. The cross-partition observer is closed by the schedule and not by the discipline, the term staying inside the issuing partition's own slot under R-07-036, so what the flow obligation covers is intra-partition observation, the scope R-15-085b states.
