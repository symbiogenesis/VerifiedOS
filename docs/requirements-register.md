...
· Accept: four requirements confer freshness, the figure recomputed rather than maintained here, and a fifth conferral is not admitted until R-10-013 names the state it adds.
· Trace: CJ-DEVTREE
...
· Accept: the parameter set is VLEN per class, core count per class, issue width and pipeline depth, SRAM bank/macro/tier-to-island assignment, scratchpad sizes, the CBO block size, and the TDM-NoC schedule; there is no cache, way-colouring, or integrity-tree parameter to choose. The block size is one parameter for `cbo.zero`, `cloadtags`, `creclaim`, and `cbo.scrub` together (R-15-007q welds the group to the block), so no block instruction's width is tunable alone and none of them enters R-15-014a's enumerated final-freeze delta.
· Trace: CJ-RTL-SAIL
...
**R-15-247p** MUST: Bank granularity on the second class is admitted against three quantities jointly, the island bandwidth ceiling §11 consumes, the read energy per bit that bitline capacitance sets, and the R-15-247g simultaneous-activation envelope, with the droop envelope a **hard admission constraint** and the other two objectives; the per-class bank count is in R-15-014a's frozen parameter set. Banks are whole-bound to islands: no address interleaving crosses an island boundary, no bank is dynamically allocated, stolen, or donated, and no bank count varies with occupancy or load.
...
**R-15-184** IS: Rowhammer and its RowPress variant have no charge-disturbance analog in SRAM, so the refresh-management apparatus (RFM cadence, PRAC counters, alert and back-off) is deleted, not tuned. What the R-15-247 second class deletes is that same **reactive** apparatus and not refresh itself: its refresh is a composition-time schedule (R-15-247g) carrying no activation counter, no alert, and no back-off, so no mechanism there reads an access pattern.
· Accept: no activation-counting or back-off mitigation is carried or re-tuned per disturbance variant on either class; the first class's residual is SRAM's own read/write-disturb and half-select modes, covered by the pervasive ECC and cell-level margin, with an uncorrectable event a fail-stop sentinel event. The second class's own disturb behaviour is a device property carried by the R-15-247m qualification measurement and is asserted nowhere ahead of it; what is normative here is that whatever that measurement returns is answered by cell margin, ECC, and the fixed schedule, never by a counter reacting to observed activity (R-15-247r).
· Trace: CJ-SAIL
