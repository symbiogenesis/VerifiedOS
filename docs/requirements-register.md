...
· Accept: multi-tier capacity is taken monolithically by sequential 3D, sharing one mask set, one fab lot, one package, and one attested identity, with no die-to-die link anywhere. An ensemble link (R-15-228b) shares no mask set, package, address space, memory, or attested identity between its two machines and is not the interface this entry declines (R-02-003a).
· Trace: CJ-DEVTREE
...
**R-15-228d** MUST: Every ensemble link carries one fixed-size frame per slot of its table and the table is non-work-conserving in R-07-036's sense: the link server places a frame in every transmit slot, an **authenticated idle frame** where it has nothing to send, so the wire carries the same frame count and the same frame size whatever the payload; no slot is reassigned, donated, or skipped, no frame is acknowledged, retried, or reordered, and no arbitration, flow control, or back-pressure exists on either end, a slot a peer fills with an idle frame being that peer's burned slot and never a stall of this member's frame. A link's rate is the slots its tables grant, quantized as R-15-228's island ceiling is and fed to admission the same way.
...
· Accept: discharged by R-15-196a, R-11-017a, R-12-015d, and R-15-228b through R-15-228e, with the three asynchronous boundaries R-15-196 counts unmoved and the fixed-latency claim held at slot cadence and not at cycle cadence across a link; the residual the leap keeps is stated and bounded: a leader the composition names and an adversary has compromised can walk a follower's origin by at most the skew bound per cadence inside the guard band, visible to a time-granted partition (R-08-031) as R-17-007's class of channel and to no other, a walk past the band stopping the link by its tag (R-17-030z).
· Trace: CJ-WCET, CJ-ISOL
