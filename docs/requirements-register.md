...
**R-02-003** IS: There is no fixed form factor: laptop, phone, IoT, workstation, and server instantiations share the principles and only the physical particulars vary. The reference instantiation is a mobile/laptop device, and R-18-004a carries what *mobile* obliges of the first release.
...
· Accept: signing on the masked datapath uses the hedged (randomized) ML-DSA variant, the deterministic variant being materially harder to mask; and where the secret path is served by the R-15-055 vector-crypto instructions the masked instance is original authorship rather than curation, no masked realization of a vector crypto extension existing to import and the scalar first-order datapath being the only precedent, so the profile freeze decides between a masked instance of those units and a secret path narrowed to a dedicated masked datapath, and an admitted binary reaches neither unmasked; and the datapath carries the R-17-058d ineffective-fault countermeasure, so the fork's candidates are evaluated with its permutation-structured redundancy included rather than retrofitted.
...
**R-15-009** IS: The bespoke matrix extension is fork-and-frozen until a ratified RISC-V matrix extension (AME/IME lineage) supersedes it, at which point it re-pins; its Sail semantics land with the instruction surface rather than ahead of it.
...
· Accept: the parameter set is VLEN per class, core count per class, issue width and pipeline depth, the **per-class bank count** (R-15-247p), the **bank/macro/tier-to-island assignment** (R-15-228, which R-15-228a fixes as an input to the §8 plan and never an output of it), scratchpad sizes, the CBO block size, the FEC decoders' realization geometry on the classes carrying them (R-15-119a), and the TDM-NoC schedule; there is no cache, way-colouring, or integrity-tree parameter to choose. The count and the assignment are **two parameters and not one under two names**: an assignment distributes objects and arrays across the banks a map already grants, so it presupposes a set of banks rather than fixing how many exist, which is why R-15-228a can make the map an input to the plan while R-15-247p still admits the count against a droop envelope and two objectives. Neither is scoped to one class: the second class is not SRAM (R-15-247n) and its banks are whole-bound to islands exactly as the first class's are (R-15-247p), which is why the assignment carries no medium in its name here or in the three entries that own it. The block size is one parameter for `cbo.zero`, `cloadtags`, `creclaim`, and `cbo.scrub` together (R-15-007q welds the group to the block), so no block instruction's width is tunable alone, and what enters R-15-014a's enumerated final-freeze delta at item (vii) is that one welded parameter rather than four instruction widths, scored against a first-class SRAM macro geometry and a second-class deck row and page geometry together.
· Trace: CJ-RTL-SAIL
...
**R-15-109a** MUST: R-18-004a's workload floor and R-18-004b's five quantities are hard constraints on this search in the classification R-15-109 fixes, binding every candidate whatever parameter moved: a candidate failing one is pruned rather than priced, and the frozen configuration is taken from among candidates meeting all of them. They are cited here and never restated. Three parameters carry them where R-15-109's rule puts them: core count per class against R-18-004a(ii)'s rates and (iv), (v) and (vi)'s software pipelines and bound; the per-class bank count against R-18-004b's two bandwidth floors (R-15-247p); and core count and issue width together against R-18-004b's area comparison, which is the figure R-15-108a's Pareto axis is weighed against.
· Accept: R-15-109's own list is not grown, that entry fixing classification rather than membership and forbidding a list grown one report at a time; what lands here is the constraint's home and not a second copy of the obligation, which R-18-004a and R-18-004b state.
· Accept: the exploration's report carries, beside the frozen configuration, each of R-18-004b's five comparisons with the side that decided it and the area fit stated at both ends of R-15-170's density band, so a candidate meeting the floor only at that band's optimistic end is visible rather than absorbed. A report omitting a comparison is a finding against this requirement.
· Trace: CJ-RTL-SAIL, CJ-WCET
...
· Accept: three grounds, and the area one is the weakest. One validity bit per 64-bit granule is 15.6 MB per GB of data and about 20–30 MB of SRAM per GB once the tag plane carries its own DECTED code, so a 40 GB bulk tier's foreign sidecar would consume 40–100% of a 1–2 GB first class, in precisely the case the bulk class exists to serve; native tags cost 1.5625% plus that code, some 2.5–3.5% of the bulk array. Atomicity is the second: data, tag validity, and both ECC planes must commit together at the granule (R-15-247b), which is materially harder across two media with different write timings, the same reason R-15-203 puts tags in the row rather than in a table. The cost model is the third: R-15-007q admits `cloadtags` on the ground that native tags are read in parallel with the data, so the saving is issue and not traffic, and a foreign sidecar would give one instruction two cost models and a §11 term that is no longer one constant.
· Trace: CJ-CERISE, CJ-MEMPLAN
...
· Accept: the order is tag discharge confirmed, then requester addressability, then data sanitization confirmed, then measured execution, with the discharge taken at mode exit (R-15-247q); no path admits a partially sanitized bank, and the two boundaries are separated by name rather than reconciled (R-17-058f).
· Trace: CJ-DEVTREE
...
**R-17-053b** IS: The mobile floor books nine product forfeits against a current handset, each a consequence of a deletion this register already takes and none a gap intended to close: no general web browsing in the first release (R-18-004), the content member standing where it is not; the three media costs, which R-17-053a states and this entry cites rather than restating; no adaptive or variable refresh, the frame period being a constant of the selected rate rather than a function of what changed (R-15-236c); the background at the share R-17-005 states, so a tenant set held at a high rung is held as state and not run (R-17-004, R-17-005, R-17-008); cellular at LTE class through reduced-bandwidth NR, scaling by core count and never by a firmware processor (R-15-134); the classical handshake's accepted cost where R-05-064a leaves it standing (R-05-064); no parity on general interactive code, which no clause of the floor asks for and which the goal ordering already subordinates (R-02-004, R-01-006); a part at or near the High-NA half field for a handset, with neither chiplet nor bonded relief available (R-15-170, R-15-162, R-15-111); and battery life as a declared per-mode figure scored at part qualification rather than a quantity the floor can fail ahead of silicon, no pre-silicon artifact reaching joules (R-18-004c).
· Accept: each forfeit cites the entry that takes the deletion, so none is a new cost and none is a claim about a mechanism this register does not carry. A forfeit whose citing entry is retired without this list moving is a spec defect in the sense R-05-153 defines, and a floor clause meetable only by re-opening one of the cited entries is a finding against R-18-004a.
· Accept: the list is the floor's own ceiling statement and not a second reading of §2's: R-02-004 states that there is no parity and this states what the release forfeits at the floor it does meet, which is why the ninth member is the one the floor itself creates rather than one a deletion does.
· Trace: CJ-NI, CJ-WCET
...
· Accept: no member is met over a mechanism this register deletes, so (v) may not be met with a codec block (R-15-238), (vi) with variable refresh (R-15-236c) or with a frame yielding idle slack across a confidentiality boundary (R-07-036), (vii) with routed expert fetch across the storage boundary (R-15-171), or (viii) with content sniffing or runtime handler registration (R-12-024b). No member requires the browser, whose deferral R-18-004 fixes, and (viii) is the member that stands where it is not.
· Accept: no member is met by a lithography advantage, the node being held constant across the comparison the form-factor claim invites. Every threshold above is an absolute user-visible quantity or a bound derived from this machine's own declared cadences; none is a ratio to a reference part and none is read off one's datasheet; and R-18-004b's capacity and area comparisons are evaluated at both ends of R-15-170's own density band, so a candidate closing only at that band's optimistic end fails.
· Accept: the floor bends where the two requirements R-18-033 holds above it do not, so a member meetable only by widening the verified TCB or by admitting an authority beside capabilities moves the member and not them; and a member still failing after the levers it names are spent renames the release the fork's other arm, a fixed-capacity secure appliance, rather than reopening the deletion it endangers, R-15-162's refusal being priced by this entry and reopened by none of it.
· Trace: CJ-WCET, CJ-MEMPLAN, CJ-DEVTREE

**R-18-004b** MUST: R-18-004a's floor is priced in capacity, bandwidth, and area rather than in throughput, because at a fixed node this machine's binding scarcity is on-die memory and not compute (R-15-158, R-15-170), and each of its five quantities is a comparison between a demand the composed roster produces and a supply an artifact measures, failed by either side alone. **First-class capacity**: the composed roster's resident bytes on the first class, summed over R-15-247s's first list as the whole-program static memory plan places them (R-08-010, R-08-012a), at most (1 − τ) of the measured usable first-class capacity, that capacity being at least 1 GB of **payload** at R-15-173a's ungraded branch, the SECDED floor R-15-175 puts on every array and the tag plane's own DECTED code (R-15-178) being carried above the payload figure and never inside it, which is the convention this entry fixes for every capacity figure the floor is scored against and which R-15-181a prices. **Second-class capacity**: the same comparison over R-15-247s's second list against a measured usable second-class capacity of at least 4 GB of payload on that same convention. **Exclusivity**: each class's declared exclusivity fraction τ, which R-15-172 nets out of the budget, at most 20% on this comparison's supply side, so the pooling tax is inside the comparison rather than reported beside it. **Bandwidth**: at least 8 GB/s of sustained second-class read granted to the M-class island, which is one pass over R-18-004a(vii)'s resident weight set and its per-token key-and-value extent at the floor's token rate, and at least 20 GB/s of sustained aggregate read plus write across all islands, covering the scanout reservation at the enumerated maximum mode (R-15-236b), the compositor's per-frame surface traffic, the decode frame pool at the declared ceiling, the camera burst ring, and the M-class grant concurrently. **Area**: the first-class array at that capacity and the roster's whole logic tier together inside R-15-170's ~400 mm² usable High-NA half field, on one die with no second die and no die-to-die link (R-15-111, R-15-162), the fit shown at both ends of that entry's own density band on its own per-gigabyte arithmetic with R-15-181a's metadata above it.
...
· Accept: the demand side is measured and not declared here, so no per-consumer split is stated in this entry and none can go stale: the roster's per-class resident bytes are summed over R-15-247s's two lists by the whole-program plan, and a split asserted here would be a derived fact no artifact owns. What this entry states is the supply floor, the tax bound, and the bandwidth demand the floor's own members generate.
...
**R-18-004c** MUST: Every clause of R-18-004a and R-18-004b names the gate and the artifact that decides it, and a clause is **not deemed met at a gate whose instrument cannot reach it**. Roster completeness, the two per-class residency comparisons, the declared ceilings read against their floors, the format-and-descriptor enumeration, and the emergency compartment's zero-authority and Before-First-Unlock properties are decided on the composed system with the two-class static memory plan beside it, which is the earliest gate at which the floor can fail. The schedule-derived clauses, R-18-004a(i)'s deadline class, (ii)'s rates inside the admitted slots, (v)'s worst-case cycles per frame against the media server's slot (R-15-067h, R-15-238d), and (vi)'s bound at every rung, are decided on the one artifact R-11-017 emits, and (iii)'s and (iv)'s bounds are cycle-valued and decided on the timing-annotated model. Usable density, the retention corner, the four latency constants, and the per-island bank grant are decided at the qualification R-15-247m names and never ahead of it. The area comparison is decided by the R-15-108 exploration, which is the act that owns area, and never by the profile-freeze measurement, which owns bytes and cycles.
· Accept: four limits are stated rather than absorbed, and each names what would lift it. The timing-annotated model's latency magnitudes are unmeasured (R-17-041), so no cycle-valued clause is decided until they are authored; the emulator is functional rather than cycle-accurate, so it decides completeness and footprint and nothing timed; R-11-017's artifact is itself unauthored, so no schedule-derived clause is decided until it exists; and the die is staged class by class (R-18-005), so no clause running on the V-, M-, or FEC classes is decided at scalar bring-up. What stays falsifiable ahead of all four is the capacity fit, the roster's completeness, the declared ceilings, and the descriptor enumeration, which is what the floor exists to make fail early.
· Accept: **the floor carries no energy clause, and what stands in its place is a declaration rather than an omission.** No pre-silicon artifact reaches joules and R-15-247m measures latency constants and a retention corner rather than per-bit energy, so what this entry obliges is that the composition declare an average-power figure per admitted global mode (R-11-018) against a declared battery capacity (R-12-065), carried in the attested devicetree beside the mode schedule. The figure is well-posed because a domain's power state is a function of the mode index alone (R-15-189m), and it is a composition-time declaration and not the reactive power management R-15-243 excludes. A composition declaring no such figure fails this requirement; the figure is scored where R-15-247m is scored, so a part missing it is a part that does not qualify rather than an architecture that reverts, and what the declaration does not buy is booked in R-17-053b.
· Trace: CJ-WCET, CJ-DEVTREE
...
· Accept: it is the artifact [userspace-porting.md](userspace-porting.md) declines to be, that document's roster being elective and stageable by design so that nothing on it can fail to arrive, and this set is the one demonstration the release owes that can. Naming capabilities rather than targets is what leaves that roster's sequencing undisturbed while bounding it below.
· Trace: CJ-WCET
