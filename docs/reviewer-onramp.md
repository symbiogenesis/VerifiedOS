...
- **The derived views state nothing of their own.** [coverage-matrix.md](coverage-matrix.md), [crown-jewels.md](crown-jewels.md), [absence-contract.md](absence-contract.md), [isa-profile.md](isa-profile.md), and the contracts and the macro architecture under §15 are computed from the register and are defective, never authoritative, where they disagree with it (R-15-001a). A finding against a view is a finding against the entry it derives from.
...
- **The letter-suffix density is high, and that is a signal rather than a defect.** 452 of the register's 1405 entries are post-hoc insertions, which is evidence about edit rate against review stability. [critique.md](critique.md) reads it that way and does not charge it as an error.
