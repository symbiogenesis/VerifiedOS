...
| [`cap-trap`](../../corpus/cap-trap.s) | nine traps through MTCC and back through MEPCC: six capability violations with their `mtval` detail, an alignment exception, an unallocated CSR address, and an unallocated instruction encoding whose two low bits are not `11`, whose `mtval` is the whole 32-bit word |
...
| [`block-authority`](../../corpus/block-authority.s) | generated block-device authority campaign: valid full-block write/readback, untagged and underbounded loads/stores, missing-load and missing-store permissions; exact trap cause/detail/site, control and staging preservation, and complete medium readback through valid authority |
...
The block-authority member is generated by `python tools/run.py block-authority emit`
from the composition and the profile's Sail exception and permission owners.
`block-authority check` and `test --only block_authority` reject stale output.
The generator bounds its input fixture to 256 bytes as a corpus resource limit;
configuration admission remains the emulator's decision. The campaign verifies
every modeled medium byte, using separate refused-command trials to observe
unchanged medium and preserved staging. Its trap handler rejects unexpected
causes, wrong fault sites and continuations reached without refusal.
`test --only block_authority --slow` in the provisioned guest additionally executes
...
- [tools/vos/dialect.py](../../tools/vos/dialect.py) is one row per mnemonic the curated model decodes, **generated** from the model's own `mapping clause encdec` and `assembly` clauses rather than transcribed from them and never read out of the RISC-V manuals. It was transcribed until M1.4′, which inverted it: [tools/vos/dialectgen.py](../../tools/vos/dialectgen.py) writes [the table](../../tools/generated/dialect-table.json) out of the bundle the model emits about itself, `dialect.py` loads it, **K-88** holds the bytes against the generator, and `fence` is the one row still written by hand, its `assembly` clause being the one the emitter leaves as unstructured text. Where the model and the manuals differ the model is what the corpus must run on, and they do differ: `auipcc`, `cjal` and `cjalr` hold the base encodings of `auipc`, `jal` and `jalr`, `lc` sits at MISC-MEM `funct3` 010 beside `cbo.zero`, and `sc` is the capability store rather than a store-conditional.
...
**Two absences in the encoder are scope and not oversight.** The dictionary bundle format of [isa-profile.md](../hardware/isa-profile.md) §1.1 is a fetch container the model does not implement yet, so an image here is a stream of canonical 32-bit instructions, which is what the curated model fetches. And the matrix surface is absent because the profile names no instruction for it, which is a profile act and not a datapath's arrival: that surface is a second-act item R-15-014a carries at (ix), conditioned on the dense-GEMM margin R-15-116 admits the unit against. Nothing enters the profile by inheritance (R-15-007j), so there is no mnemonic to transcribe until a row names one, and the corpus version that follows that act adds its rows. The FEC pair is no longer among them: its scope is categorical rather than measured, so it was owed at the provisional freeze and the profile now names it, and `ldpcdec` and `polardec` are rows in the table like any other (R-15-119b).

**The vector rows the corpus writes are the memory surface and what feeds it**, which is the surface **M0.8b** puts the per-element capability check on: every form whose element addresses are made differently, at all four element widths, plus the configuration instruction that says how many elements there are and the four moves that put a value into the vector file and read one back. **The encoder table is no longer scoped to them, and that is M1.4′'s doing rather than a decision this document takes.** M0.8b encoded no vector arithmetic on the ground that a row nothing in the corpus writes is a row nothing checks; that ground was the cost of transcribing one, and generation removes it, so the table now carries every form the admission admits and the arithmetic with it. What the corpus writes is unchanged and this section's reading of it stands. Nothing in the register, the profile or the plan decides whether the table should carry the whole admitted surface or a declared scope, so the widening is an open finding of M1.4′'s rather than a scoping this corpus chose.
...
- **The encoder cannot express it.** A field the row holds constant is the point of the check, so the encoding is outside what any operand assignment reaches. Both of [`memory-model`](../../corpus/memory-model.s)'s words are here: the `fence` row emits `fm` as the literal zero and `rd` and `rs1` as zero, which is what makes a reserved `fm` and a set-but-ignored register pair unreachable through it. [`cap-trap`](../../corpus/cap-trap.s)'s second word is the same ground over a field *every* row holds constant: the two low bits of the opcode are `11` in every encoding this machine allocates, so a word without them is outside what any row and any operand assignment reaches, and the reading beside it is the instruction it is that word with those two bits set.
...
| [cap-trap](../../corpus/cap-trap.s) | `0x00100290` | the encoder cannot express it | `addi t0, zero, 1` |
...
**The rig explores instruction streams beyond the fixed corpus campaigns.** A corpus member exercises its declared cases; a verification engine generates instruction streams, consumes execution traces, adjudicates two executors against each other, and **shrinks a counterexample automatically**, which no corpus can do at all. The instrument is [tools/run.py testrig](../../tools/vos/cli/testrig.py) over [vos/rvfi.py](../../tools/vos/rvfi.py), the wire format, and [vos/vengine.py](../../tools/vos/vengine.py), the generator, the socket and the shrinker; adjudication is [vos/trace.py](../../tools/vos/trace.py)'s, unchanged, so there is one adjudicator here and not two.
...
Two exclusions narrow what a stream may contain, and each is stated rather than left to be discovered. The **block operations** are the format's and not the machine's: `rvfi_write` stops the emulator on them, which is §9.3's last row seen from the side that has to generate around it. The **words whose low two bits are not `11`** are neither, being the generator's own arithmetic: such a word is a perfectly ordinary unallocated 32-bit encoding, both emitters report it whole and it traps with the whole word in `mtval` ([`cap-trap`](../../corpus/cap-trap.s)'s ninth check decides that), but three quarters of a uniform 32-bit draw are words this machine allocates no encoding to, so a stream that did not force those two bits would spend most of itself on one trap.
