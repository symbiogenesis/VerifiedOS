...
The predicate this record uses instead is textual and checkable at two named revisions: **a source is added where its own heading, or its bolded lead line under an existing heading, is present under `docs/assurance/proof-reuse/` at `f528852f680dd00d599b73feb69258e214a1385a` and absent at `e694a548e0c31520814eba0d5d1d981fd0571bb5`**, the last revision before the sweep commit `2ea8d61`. A section heading is not a source record and is not tabled, so a section heading the sweep adds carries no row of its own. On that predicate [hardware](proof-reuse/hardware.md), [parsers](proof-reuse/parsers.md) and [protocols](proof-reuse/protocols.md) add no source, and the tables below enumerate the sources the other four add.
...
Every `uncelled` row above points at an obligation the register owes and no cell prices, and three of those obligations are bullets of [the proof map's mandatory work](unassigned-proof-map.md#8-mandatory-work-outside-the-priced-total): the HAL's implementation in certifying Rust, which the VerusBelt row reaches through the manual-proof path; R-18-014's certifying Rust compiler, which RustCompCert, Lean-MLIR and Tree Borrows reach; and the full Iris refinement proofs for the kernel and the storage stack, which the Quiver row reaches. The fourth is R-05-119 and R-05-121 at the RustHornBelt row, which the map books in no bullet of that section by id and which no cell and no slice names either; where that pair is booked is the map owner's question, and this record reports the gap rather than closing it. What this record does add is that the sweep's strongest language sources point at obligations no cell prices: a source aimed at one is a reading, and pricing a cell because a source now exists for it would be the survey opening a cell, which the plan refuses.
