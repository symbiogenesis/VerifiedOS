...
**The corpus reaches the adjudicator through a frame and not through the rig's socket, and that departs from the plan's wording.** The implementation checklist has R2 connect the curated scalar RTL to the existing RVFI-DII rig, with one protocol across the golden model and the RTL, and [the rig's completion note](../implementation/completion-log.md#s11-join-the-testrig-ecosystem-over-rvfi-dii) names the rig's socket as what the RTL's port connects to. Two things keep the corpus off that socket. Direct instruction injection replaces the fetch source (section 6), so over the socket the core executes the words the host injects rather than fetching the member's image, and exercises none of the fetch path's capability checks. And the version-2 packet has no cause field, so a packet stream written from a run with real fetch would compare a trap as a boolean; carrying the cause makes it a different grammar rather than a longer packet, on [rvfi.py](../../tools/vos/rvfi.py)'s own ground for the dialect. The frame is therefore the version-2 packet's field set with the cause added, written as fixed-width text with a header and a counted trailer so that the decoder can refuse a torn or partial file line by line. What stays one is the comparison: the golden side, the projection and `trace.adjudicate` are the rig's and the corpus's, and generated streams still reach the core over the socket (section 6). Whether the plan's wording moves to name two routes is a plan act, reported as a finding and not decided here.
...
| `insn` on a fetch fault | the scoreboard entry's instruction word, whatever the fetch returned | zero where the fetch faulted and no word was fetched, as the golden emulator records it; otherwise a fetch-side fault diverges at the `I` record for a reason that is the port's and not the core's |
...
**For each corpus member in the declared scope:** the golden run reaches a HTIF verdict of success and its trace matches the digest the manifest records; the RTL harness loads the same image, enters at the same first program counter with the same reset grants in place, the code root in `PCC`, `MTCC` and `MEPCC` and the data, seal and unseal roots in `c1` to `c3`, and stops after the retirement whose store reaches `tohost`, which is where the emulator stops; the frame it wrote decodes; `rtltrace.compare` against the golden trace is complete; and the RTL's HTIF verdict is the golden one. For one member, `python tools/run.py testrig adapt FRAME TRACE` decides the third and fourth clauses, and through completeness the entry and stop points of the second; it decides neither the image's identity nor the reset root pair, and it does not hold the reference trace to the manifest's digest.
...
**Predicate.** For the curated scalar core under a riscv-formal wrapper, with instruction and data memory responses unconstrained and reset held for the first cycle, no safety check of the declared set finds a counterexample within its declared depth; under the further assumption that every instruction and data memory request is answered within a bounded number of cycles, the liveness check finds none within its depth; and riscv-formal's cover check reaches a retirement of the in-scope forms within the instruction depth. The fairness assumption is liveness's alone: every other check holds or fails under any memory responses, while a memory that never answers keeps any instruction from retiring and would fail liveness for a reason that is not about the core. The last clause is what keeps the others from being vacuous: a pipeline deeper than the bound retires nothing inside it, and every instruction check then holds for no reason.

**Plan.** [tools/vos/bmc.py](../../tools/vos/bmc.py) declares the check kinds (`insn`, `reg`, `pc_fwd`, `pc_bwd`, `causal`, `unique`, `liveness`, `cover`), the assumption liveness adds, and a depth for each, and `python tools/run.py testrig bmc` prints the plan with its instruction scope read out of the generated dialect table. **The depths are provisional placeholders.** No source states one for this core: R-15-094 sets no bound, and riscv-formal, whose own configurations would be the precedent, is not pinned or read. The first run sets each depth and records it with the time its check took, and the cover clause decides whether the instruction depth admitted a retirement at all. The scope's rule is a set of Sail constructors, the integer computational forms of the base and M extensions whose semantics riscv-formal's RV64IM model shares with this dialect, and every other base or M constructor is excluded with its reason: loads and stores are authorized by a capability, branches trap on PCC's bounds, fences are collapsed, and traps and returns go through the capability special registers. A constructor the model gains that is neither in scope nor excluded is a finding the command reports.

**Wrapper obligations.** The wrapper presents riscv-formal's signals at `XLEN` 64, with register data as the memory encoding the frame's `rd_wdata` carries, and an `order` the port states (section 3). It states the fairness assumption as an assumption active only when the liveness check runs, and how riscv-formal expresses that is re-read at the pin. riscv-formal checks no tag; the tag's correctness is the co-simulation's to decide.
...
| `python tools/run.py testrig framesim --controls --corpus` | guest | that the SystemVerilog writer's frames decode field for field as its stimulus stated, that each seeded writer mutant is reported by that stimulus, and that every member `carry` holds whole comes back whole through it |
| `python tools/run.py testrig bmc` | either | the smoke's plan and its instruction scope; it runs nothing |
| `python tools/run.py test --only rtltrace` | either | the protocol refusals, the projection, the seeds, `carry`'s totals and their scopes, the writer mutants' anchors, the stimulus layout against its bench and this document's field table against the decoder |
...
- The bounded model-checking smoke's four inputs of section 7, and the depths its first run sets.
- The plan's disposition of the corpus route of section 1.
