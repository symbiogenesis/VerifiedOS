...
- **Members that exercise a unit the scalar core does not carry.** The corpus's own account of its members places the vector memory surface, the class-table geometry and the Keccak permutation on the vector file, and the plan puts the vector and FEC units at M10 after scalar bring-up.
