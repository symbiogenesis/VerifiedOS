...
· Raised: M1.2b
· Disposition: closed, contained commit `5b4e1cc335476f98506f01b5b8beaf29220c61b2` repairs the proof branches and hypotheses; the focused Recomposition target passes with unchanged theorem declarations and admit census. M1.2b records a successful clean full proof/extraction/compiler sequence at the same revision; assembly-byte acceptance remains open.
...
· Raised: M1.2b
· Disposition: open; equal line counts and opcode multisets do not establish ordered instruction or dataflow equivalence. The original byte-identity clause remains unsatisfied. A replacement needs an authored acceptance decision and a qualified comparison preserving instruction order, operands, control flow and the calling convention; the recorded register permutation alone supplies none of those checks.
...
· Raised: M1.2b
· Disposition: open; either the chunks lower to integer loads and stores of the same width, which is a small edit this item could have carried had it been ruled, or M1.3's VL=1 vector lowering absorbs the whole of it, which its cell should then say.
