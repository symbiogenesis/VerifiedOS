...
*An index of what the build has found, and not a second copy of it. The completion notes in [the completion log](../implementation/completion-log.md) and the cells of [the implementation checklist](../implementation/implementation-checklist.md) own every finding's content; this register owns its id, its type, the item that raised it, and its disposition, and owns nothing else.*

## How to read this

A finding is something the build learned that the plan would otherwise learn again. The plan records 755 of them across 130 items and nothing read them, which costs three ways: the same fact found twice at two items, an owed act with nowhere to live until somebody assembles S1's rows out of prose by hand, and a methodological finding that never becomes a rule. This register is what reads them.
...
**F-328** method: R-15-013's criterion states that both its rosters are read off the citations, every declined hedge citing the clause at the point of decline, but no standing decline carries that citation in its own register entry, the citation standing in the prose specification which R-05-152 makes commentary rather than the artifact the gate audits, and the register reaches four of the five only through R-15-200's criterion naming PMP, the IOMMU, MTE and the Harvard split in passing, while the shadow-stack decline is reached by none of them
· Raised: S29
· Disposition: open, reported and not closed; the admissions do carry the citation in their own criterion lines, so the defect is one-sided, and what would close it is a citation added at each declining entry rather than a roster kept somewhere. **The shadow-stack decline is the member an act built on R-15-200's roster would miss**, that roster naming four mechanisms and not it, so a closing act enumerates the declines from R-15-013's own subject rather than from the one criterion that happens to list some of them
...
**F-281** owed-act: docs/implementation/placement-search.md quotes the tool's candidate-set predicate verbatim in its §2 and restates the check-to-entry pairs of `CONSTRAINTS` in its §4 table, and no rule holds either restatement against tools/vos/memplan.py, so a change to the tool leaves the document stale with no finding
· Raised: Q5a
...
**F-282** owed-act: docs/background/critique.md:154 says nothing in the corpus prices packings, and this landing prices a packing on span used, unused reservation and padding over a witness plan, so the sentence is a claim to re-read on landing while the paragraph around it (critique.md:144-159) names no item and is not answered by a witness-plan search
· Raised: Q5a
...
· Raised: M1.2b
· Disposition: closed, contained commit `5b4e1cc335476f98506f01b5b8beaf29220c61b2` repairs the proof branches and hypotheses; the focused Recomposition target passes with unchanged theorem declarations and admit census. M1.2b records a successful clean full proof/extraction/compiler sequence at the same revision. The later F-318 campaign supplies assembly-byte acceptance, which the focused proof repair alone does not establish.
...
· Raised: M1.2b
· Disposition: closed; independent source-boundary review accepts exactly this unreachable wildcard deletion against contained starting commit `92b19055def735196c31911be7d8b7a8def72eb2`. M1.2b's predicate names that sole exception, preserves the zero-diff common/ boundary, and the rebuilt compiler retains a positive `-dcapasm` smoke outcome.
...
· Raised: M1.2b
· Disposition: closed; the allocator repair is retained in the contained compiler and a fresh campaign at `a826d5f17bb09742940cb66f7c986c55ccfbf6de` passes all nine source-identified pairs under the reviewed [assembly comparison contract](../implementation/comparisons/compiler-assembly.md), which admits only decimal metadata changes on exact standalone or named-label annotations. Fresh producer receipts, source/output hashes, a clean build and the scalar-FP refusal are retained. The three missing historical sources have explicitly reviewed replacements; the lost outputs are not claimed equal.
...
· Raised: M1.2b
· Disposition: open; M1.3 owns the VL=1 lowering of float loads, stores and spills. M1.2b's accepted refusal and register-only handling do not supply that implementation or its target evidence.

**F-320** measurement: M1.2a measured 64 of `riscV/Asm.v`'s 169 instruction constructors as naming a float register, and the deletion removes 63 of them, not 64: the sixty-fourth is `Pld_arg`, whose destination was `ireg + freg` and is now `ireg`, so the constructor stays and only its argument narrows. By the predicate "lines matching `^  \| P` between `Inductive instruction` and the next top-level declaration", the file carries 106 constructor lines over the span 150 to 289 against the pin's 169 over 147 to 367, and the set difference of the two constructor-name lists is exactly 63 names removed with none added
· Raised: M1.2b
· Disposition: closed by the measurement; the cell's figure is right as a count of lines naming `freg` and wants one clause saying which of them survives.

**F-321** measurement: the pin typed an integer machine register `if Archi.ptr64 then Tany64 else Tany32`, and the deletion replaces the whole match with `Tany64`, because a float-typed value now lives in an integer register and `Conventions1.loc_result` returns `Tfloat`, `Tsingle` and `Tany64` in `R10`, which `loc_result_type` needs `Tany64` to admit. Three sites then compose into an unsoundness on a target nothing else refuses: `mreg_type` is `Tany64` unconditionally, `loc_arguments_rec` routes `(Tfloat | Tany64)` through `int_arg`, which emits `One (R r)`, a single integer register, and `riscV/Asmexpand.ml`'s `args_size` was widened to match. On RV32 that is four bytes for an eight-byte value, and `mreg_type`'s widening is exactly what removes the typing obligation that would have caught it, so a 32-bit tree would configure, build and mis-pass a `double` rather than refuse it
· Raised: M1.2b
· Disposition: closed by refusing the target: `configure`'s `riscv32-*|rv32-*` arm exits 2 with a message naming R-15-002a and R-15-040, the usage text drops the `rv32-linux` line, and the refusal is verified at `e640991` (`./configure rv32-linux` exit 2, `./configure rv64-linux -ignore-ocaml-version` exit 0 with `Makefile.config` byte-identical to the one the build used).

**F-322** method: `riscV/Machregs.v`'s `destroyed_by_op` loses its one clobbering arm, the `F6` scratch of the float-to-integer conversions, and becomes the constant `nil`. That is the same value on every operation the pin gave `nil`, but not the same term: a constant reduces where the pin's `match` on a variable operation did not, so `auto` began closing by conversion what `backend/Stackingproof.v` proved by hand, and three tactic lines of its `Lop` case had to go, the rewrite by `transl_destroyed_by_op` among them. The same mechanism, a proof script sensitive to a definition's shape or a constructor's arity rather than to its meaning, is what makes the recomposition proof's repair expensive
· Raised: M1.2b
· Disposition: closed for `Stackingproof.v`, where the three lines are deleted with the reason stated at the site; recorded because it is the cheap instance of the class the recomposition finding reports as the expensive one.
...
**F-273** measurement: registering the eighth derived view moves K-24's view count, README.md:348 and docs/background/critique.md:3 asserting seven where the artifact gives eight at 28e1897, and the selftest's baseline refuses on that same finding so no mutant decides anything on the lane; in a scratch clone at 29bdfd8 with only that count repaired the selftest kills 114 of 114 mutants with 0 stillborn over all 94 rules
· Raised: R4
...
· Raised: I16
· Disposition: closed, the two declared apart with the difference stated at both, a substituted source meeting the elaboration's own compile where an error refuses the arm and no lint gate at all
...
· Raised: I16
· Disposition: closed, the declaration refused per substitution and per imported source rather than applied best-effort, in all three ways it can mean nothing, the sharpest case being a line the unreached filter already dropped
...
· Raised: I16
· Disposition: open, the instrument reporting the two apart so the column stays true of the rows carrying it, and a sentence in the record's §5 naming the partition owed at the first row that displaces a module kind rather than at the first row declared
...
**F-225e** measurement: a package declares no module kind, so a substitution whose authored side is a package is invisible to the introduction and inert members of the diff, and where its imported side is a package too it is visible to no member at all, which is what R1b's own first row measures as
· Raised: I16
· Disposition: standing, the partition first deciding something at a row whose imported side declares modules, and it is why the inert check reads module declarations rather than substitution rows
