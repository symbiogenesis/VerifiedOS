...
2026-09-13. The code and decoder research handoff is checked on 2026-09-24. A published mechanization is evidence about its stated
...
### Research handoff for future code and decoder proofs

The [coding survey](../../background/open-math-conjectures.md#optimal-binary-code-rate-versus-distance) informs the concrete ECC obligations below and Q23g's still-unselected link code. These are evaluation leads under the existing contracts, not new code selections. The 2026 improvements to asymptotic binary rate-distance upper bounds constrain possible designs; they supply neither a construction nor a short-block decoder. The survey records an inspected Lean theorem for a strict improvement below the second MRRW rate, with the same bound as Alrabiah-Guruswami's channel argument. Reusing it would require replay and an imported-assumption audit, then a separate derivation of any claimed finite-length constraint; an asymptotic limsup theorem alone supplies no bound for a selected short block. A proposed memory-code proof still needs the actual generator/parity relation, distance and correction/detection claims at the selected widths, then the RTL, atomic data/tag and fixed-latency joins. Representative acceptance witnesses exercise a correctable pattern, a required detected pattern and an out-of-model pattern without claiming the latter is covered. The physical fault distribution remains a separate input.

The [folded and ordinary Reed-Solomon advances](../../background/open-math-conjectures.md#deterministic-fully-polynomial-list-decoding) can inform a separately scoped bulk-recovery proof. Keep the folded-code fixed-gap result distinct from ordinary RS over sufficiently large prime fields with `q >= C(R,delta)*n`; neither establishes deterministic polynomial complexity jointly in length and inverse gap. The [structured factorization result](../../background/open-math-conjectures.md#deterministic-polynomial-factorization-over-finite-fields) is another decoding-specific lead, not a general finite-field factorization algorithm. The missing bridge fixes a field representation, evaluation set, block length, rate, radius, list cap and all finite constants, proves candidate-list completeness within that radius, and bounds the cost of authenticated selection. An ambiguous received word with two valid candidates is a necessary distinguishing example: list recovery alone must not be credited as unique delivery.

The [small-workspace decoder](../../background/open-math-conjectures.md#capacity-approaching-decoding-with-small-workspace) offers a separate scratch-space comparison for its supported lengths and fixed parameters. Its `N^tau` workspace excludes read-only random-access input and sequential output. Charge those stores, code representation, candidate buffering or repeated authentication, and the `N^(1+tau)` work when building a target bound. A useful resource witness separates scratch from total resident bytes and exercises the maximum admitted candidate count. It provides no streaming-input theorem or fixed-latency hardware decoder. In particular, none of these list-decoding leads satisfies the [current endpoint selection predicate](../../hardware/ensemble-link-contract.md#21-the-selection-predicate) merely by reaching capacity; its admitted families and decoded-bits-only interface still govern Q23g.

The [VT construction](../../background/open-math-conjectures.md#finite-length-optimality-of-single-deletion-vt-codes) is retained for a future synchronization-loss channel, not the current substitution/erasure obligations. The June 2026 TMLR publication proves that the LLM-derived rule generates the VT family; it still leaves finite-length maximality open beyond the established small lengths. Before using it, select a deletion model and prove the finite encode/decode relation; exercise deletion at the first and last positions and distinguish two-deletion or substitution inputs from the theorem's domain. No matching physical channel is presently supplied.

The [seven-cycle construction and checker](../../background/open-math-conjectures.md#shannon-capacity-of-the-seven-cycle) likewise await an actual `C_7` confusability model. A proof experiment would first verify base-certificate independence and cardinality and then the recursive arithmetic; the surveyed package's default supplied counts are not a substitute for its `--recomputed` path. Refusing a deliberately confusable pair or corrupted count distinguishes a certificate check from trusting the reported bound. The new heterogeneous recursion is not supplied as a Lean proof; the separate formal predecessor proves a weaker lower bound. Even successful replay would supply a lower bound in that graph model, not its exact capacity, a platform channel model, authentication or an admitted decoder.
