...
The [VT construction](../../background/open-math-conjectures.md#finite-length-optimality-of-single-deletion-vt-codes) is retained for a future synchronization-loss channel, not the current substitution/erasure obligations. The LLM-derived rule's identification with the VT family does not prove finite-length maximality. Before using it, select a deletion model and prove the finite encode/decode relation; exercise deletion at the first and last positions and distinguish two-deletion or substitution inputs from the theorem's domain. No matching physical channel is presently supplied.

The [seven-cycle construction and checker](../../background/open-math-conjectures.md#shannon-capacity-of-the-seven-cycle) likewise await an actual `C_7` confusability model. A proof experiment would first verify base-certificate independence and cardinality and then the recursive arithmetic; the surveyed package's default supplied counts are not a substitute for its `--recomputed` path. Refusing a deliberately confusable pair or corrupted count distinguishes a certificate check from trusting the reported bound. Even successful replay would supply a lower bound in that graph model, not its exact capacity, a platform channel model, authentication or an admitted decoder.
