...
This inventory qualifies published proof developments against this project's actual obligations. Research and source inspection date: **2026-09-10**, except the EverParse front-end scope recorded below, which is read on **2026-09-13**. A strong published example is not an already discharged VerifiedOS requirement. No external parser, interpreter or proof term is incorporated by this document, and no candidate below is rebuilt in the project's Rocq switch. Source inspection, upstream claims and local qualification are distinguished explicitly.
