...
| [P equals BPP](../background/open-math-conjectures.md#derandomization-p-equals-bpp) | Q20b may compare a deterministic candidate finder with the same bounded randomized search, frozen goals and kernel acceptance. For the September generator result, first represent the relevant test by the stated low-degree F_2 polynomial or alternating parity-circuit class. Record degree, seeds, unsuccessful searches and total cost. | Checked output already permits an untrusted randomized producer. A generator for a restricted circuit class does not fool an arbitrary verifier. Derandomization neither supplies secret entropy nor makes randomized fingerprints accepted proofs. |
