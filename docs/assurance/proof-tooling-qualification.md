...
| Python proof client | [Pytanque `4092b1238b56468fdc1b3d100e078791c9690fd4`](https://github.com/LLM4Rocq/pytanque/tree/4092b1238b56468fdc1b3d100e078791c9690fd4), package 0.2.2; its own LICENSE is Apache-2.0, copyright 2024 Inria. Archive SHA-256 `3f2f394858086e106ae1ec58f4d694a2de8e17066bce87df8331d73eec1b066d`. It uses the selected rocq-lsp revision's `pet` executable. External trial only. |
...
| Standalone proof tactic | CoqHammer tactics 1.3.3+9.2 at [revision `16f8d646bc5e30d685cff6a49da5cf3cd4f9fa88`](https://github.com/lukaszcz/coqhammer/tree/16f8d646bc5e30d685cff6a49da5cf3cd4f9fa88), its own LICENSE LGPL-2.1, copyright 2017 Lukasz Czajka and Cezary Kaliszyk, University of Innsbruck. Official archive SHA-512 `6547d6824795a7f3d0281c0931be2c63571443a28df7467dfe7caa0380845653a3f1a61c71550ff3bd64c18b2694336c2e04a412e65ea9bddcdb3d2b5aa940d9`. Only the standalone tactics package was installed in the candidate switch. |
...
The separate Pytanque probe drives the same selected Rocq server through `pet` in stdio mode. Its native Python invocation is `<render-env>/bin/python <receipt-directory>/pytanque-trial.py`, with the candidate switch's `bin` first on `PATH` and the qualified native project as its working directory. The preserved launcher calls `Pytanque(mode=PytanqueMode.STDIO)`, `set_workspace(False, project)`, `start(source, theorem)`, `goals(state)` and `premises(state)` for **both** named invariants. The first publisher query and premise retrieval took 29.32 s; the subsequent take query took 0.106 s. Running `intros v H.` exposed each invariant's actual local context and remaining goal with `proof_finished=false`. `exact I.` was refused with the expected invariant type. The receipt binds the source, imported RingContract source/object, project configuration, Rocq executable and `pet` executable hashes. The LSP trial above supplies the precise broken-bound source range; Pytanque does not convert an unfinished state into acceptance. These observations qualify the bounded communication probe, not the cache invalidation or human-review clauses that remain unsatisfied below.
...
The standalone `sauto` comparison replays this same dependent module with only three proof scripts changed: the two generated-equation clients use `sauto dep: off use: generated_equation_1` or `_2`, and the implementation-agreement proof destructs the Boolean tag then uses `sauto dep: off`. The concrete computation definitions and decidable-index transport proof remain unchanged. A first `Require` without `Import` did not activate the tactic grammar and was refused in 0.32 s; the repaired `From Hammer Require Import Tactics.` candidate audited in 1.07 s. All 19 native constant types and exact assumption sets equal the handwritten-proof baseline, with every assumption set empty. Its kernel recheck passed in 11.05 s. No external prover/search service, full Hammer plugin or dependent automation mode participated. The scripts replay using only the installed standalone tactic package; the compiled proof terms recheck in the kernel. The short explicit baseline remains the retained implementation because this trial shows no client maintenance benefit from adopting the tactic dependency.

An additional existing-record trial imports the actual `AdmissionPath.Package D`, `Reading D` and `read_cert` constants, rather than substituting a new record for them. An Equations definition reads that package's `option D` payload and is proved equal to the existing `read_cert` for every `D`, reading and package. Its generated equation is replayed with standalone `sauto dep: off`. Both the existing and generated functions compute by `eq_refl` on a present natural-number certificate and an absent Boolean certificate. The unchanged AdmissionPath source compiles in 1.63 s; all 18 constants of the experimental record module pass the native assumption audit in 1.25 s with empty assumption sets, and kernel rechecking passes in 12.25 s. These are computation and definition-preservation witnesses for the existing record task, not proofs of certificate acceptance or serialization. `record-trial.py` and its receipt preserve the complete experiment and dependency digest.

## Cost accounting and scope of the verdict

| Cost category | Observed work |
| --- | --- |
| Setup | Candidate editor, renderer backend and Equations switch installation: 703.48 s, two jobs. Alectryon Python environment: 4.95 s; Pytanque installation: 0.82 s; standalone CoqHammer tactics installation: 8.62 s. Exact solver attempts and package receipts are retained separately from proof replay. The switch/package exports before and after the added trials are preserved. |
