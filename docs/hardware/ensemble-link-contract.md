...
> It states the frame grammar as a Narcissus descriptor, the link's code and its selection predicate, the endpoint's register description with every register's owner, the per-link constants the attested devicetree carries, R-04-010a's three conditions decided against the block clause by clause, the netlist evidence an auditor searches for, and the link server's IDL interface.
...
A frame on the wire is one codeword of the link's code (R-15-228e). **The descriptor's language is the decoded message block and never the wire symbols.** The encoder produces the parity symbols and the decoder strips them, neither reads a field, and no parser sees a symbol. That separation is what makes *no decoder verdict, syndrome, or CRC decides a frame* a structural property here rather than a rule somebody enforces: the code's output is bits, the grammar's input is bits, and there is no path by which a decode outcome reaches a field.
...
| 3 | `step_payload` | opaque bytes, the construction's | `H_step(s)` for the decoded step `s`, and the remainder of the block declared zero fill under `IDL-024` | yes | R-12-015d's bounded establishment handler alone |
...
| --- | --- |
| one admissible length form | every form is `M` bytes; no field carries a length except `descriptor_count`, whose width `IDL-023`'s ladder fixes from `N_desc_max` alone |
...
| no free padding, no reserved bits | every fill byte is declared and checked, `FF-1` and `FF-2` to zero under `IDL-024` and `FF-4` to `F_pad`; the descriptor's flag set is `WF-10`, where a set bit above the declared count is a decode failure |
...
`RW` and `RO` describe direction for the named owner; a direction that owner does not hold faults, and so does every offset this table does not admit. The offsets themselves are composition constants and are not stated here, the memory map being the attested devicetree's (R-09-007).
...
| `EPOCH` | the link server compartment, as configuration MMIO | `RO` | which link epoch is in force and whether its equalization training has completed. Stepped by link-down and link-up alone, and by no software write |
| `EQ_COEF[j]` | the RoT | RoT-write, cleared on link-down; the link server neither reads nor writes it | the equalization coefficients, trained at link-up and frozen for the epoch on R-15-137's terms (R-15-228c) |
| `ARRIVAL_PHASE` | the endpoint writes it; the boot core kernel's leap path reads it | `RO` to all software | the one-sided arrival-phase reading of the most recent landing in the slot this endpoint's table names for the leap (R-15-196a). Holds one reading and no history, no accumulator and no estimator |

**That is the whole set**, and the absences are as much of the description as the rows. There is no receive status register and no landed flag, because R-15-228c admits no state a received frame writes except the landing itself and the arrival-phase reading; **the schedule and not a flag tells the link server when to read a receive entry**, which is the same non-work-conserving discipline the table already keeps (R-15-228d). There is no FIFO level register, a level being state the wire's timing writes. There is no enable bit, the endpoint being electronically enabled by the liveness of the link server's delegated window grant and by no software claim (R-15-146, R-15-228c). There is no software-writable equalization path, and no coefficient is retrained inside an epoch.
...
`EQ_COEF[j]` sits in the RoT's aperture and not in the link server's, so R-15-143's indivisible ownership is over the configuration MMIO and the two windows, and the coefficient store is neither. That placement is a reading taken here: R-15-143 makes a front end's configuration MMIO and its window one ownership, and R-15-137 makes the coefficients RoT-loadable, and the two are compatible only if the coefficient store is outside the ownership R-15-143 binds. A reviewer taking the other reading would have the link server able to reshape what the line delivers while holding the window, which is the control R-15-143 exists to prevent.
...
The recovered line clock is one of R-15-196's three external interface clocks and terminates in the endpoint's bounded FIFO of depth `D_fifo`, with the fabric side on the island clock (R-15-196a). **The FIFO is on the receive path alone.** The transmit serializer runs off the member's own clock, so the transmit path crosses no domain, and this is why an ensemble link leaves R-15-196's count of three asynchronous boundaries unmoved rather than adding a fourth: the link contributes one more member to the second class and no new class.
...
`R_line` carries two predicates at once and both bind. It contributes a line-rate bound to the member's bandwidth reservation, which is all R-15-196 lets an external interface clock contribute; and it is bounded above by the crypto core's authenticated-encryption throughput as a composition constant the link slot tables respect, which R-12-015d states as a cost and never absorbs. A declared `R_line` above that ceiling is inadmissible at composition and not a degraded mode.
...
| --- | --- | --- | --- | --- | --- |
| `M` | code message length, one frame's message block | bytes | equals `L_tag` plus the sum of §1.3's plaintext body; the same for all four forms | the composition (R-15-228d) | the attested devicetree; Q23d |
...
| `code_family` | LDPC or polar | n/a | §2.1's nine clauses, then the lower decode latency at the target residual frame error rate | **owed**; operand is the wire's raw error rate at the declared reach and `M` | first measured at Q23g |
...
| `N_desc_max` | descriptors per payload frame | count | `w_count` is the width `IDL-023` fixes for it | the composition | §7's declaration |
...
| `T_enc` | encode latency | cycles | fixed, independent of the symbols (§2.2) | **owed**; R-17-041's magnitudes, [crown-jewel](../assurance/crown-jewels.md) row 15 | measured at Q23g; entered in the devicetree fixture |
| `T_dec` | decode latency | cycles | fixed, independent of the symbols and of correctability (§2.2) | **owed**; the same | the same |
| `T_slot` | slot period of this link | spine cycles | the table's own, one frame per slot (R-15-228d) | the ensemble schedule | Q23d |
| `G_band` | guard band, per slot | spine cycles | at or above `B_skew` plus this link's latency bound (R-11-017a) | the ensemble schedule | Q23d |
| `C_leap` | re-alignment cadence on the leader link | slots | `C_leap` times both members' declared oscillator tolerance lies inside `G_band` (R-11-017a) | the ensemble schedule | Q23d |
| `B_skew` | skew bound between two members' origins | spine cycles | **owed**; measured on a two-member pair at qualification, a pair exceeding it failing to qualify rather than leaping more often (R-15-196a) | measured at Q23g | entered in the attested devicetree |
...
Every term above is a row of §4 and every one is pending, so the arithmetic evaluates to nothing today. That is the honest state and it is the useful one: a consumer cites a row rather than a sentence, and a row that acquires a value moves every figure derived from it.

### 4.2 One declaration this contract cannot make, and who owes it

R-09-007's list of what the static devicetree declares names core classes, islands, the NoC schedule, OPP tables and radio calibration, and it gained one explicit Accept clause for the inference socket's declared constants under R-15-228h. **The ensemble link's per-link constants are not named in that list.** They are decided elsewhere and decisively: R-02-003a carries the membership, each link's endpoints, each link's leader end and each peer's expected unit and ensemble identity in every member's attested devicetree; R-11-017a makes the slot tables entries of the attested schedule artifact and public constants of the generation; and R-15-196a has the skew bound and the link's latency constants entered in the attested devicetree at qualification. So which artifact declares each constant is not in doubt, whatever §4 still says about its value, and §4 cites those three entries rather than R-09-007.

Whether R-09-007 owes an Accept clause naming this second class of declared constants, on the precedent of the one R-15-228h already earned there, is **a register act this document reports and does not take**. It is recorded here so that a reader comparing R-09-007's list against §4's table finds the answer rather than the gap.
...
| the one admitted carry | the equalization frozen per link epoch, which R-15-137's canceller already takes of this condition per epoch (R-15-228c) | equalization retrained inside an epoch, or adapted while traffic is in flight |
...
The link server is contained non-TCB software on cores (R-15-228b), and its clients are the sharded-function compartments on the same member. This section states its world in prose rather than as a second `interfaces/*.json` declaration beside [the ring reference](../../interfaces/ring-reference.json), and the reason is mechanical: `run.py ring emit` reads that one declaration and K-89 binds its emitted Gallina artifact byte for byte, so a second JSON world would be an authored artifact no emitter reads and no rule holds, which is the shape this corpus calls a costated holder. **The declaration and its generated interface artifact are owed**, with `IDL-064`'s three parts and R-18-037's conformance campaign, and they are owed to the item that stands the link server up rather than to this contract.
...
| an R-09-007 Accept clause naming this class of declared per-link constants | **reported, not taken**; §4.2 names the entries that decide them instead |
