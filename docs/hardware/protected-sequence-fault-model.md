...
The two rows about the datapath are separated on purpose, because they are the two cases a reader most often merges. A fault in **stored** state is R-16-008b's, it has a mechanism, and the mechanism is ECC with a fail-stop where correction fails. A **transient** strike in the datapath itself has no mechanism here at all: R-16-008b says the transient datapath strike is reached by none of its enumerated mechanisms and is booked at R-17-058b, and R-17-058b says it is unclaimed at consumer grade. Neither is in this relation, and the reason differs in each case.
