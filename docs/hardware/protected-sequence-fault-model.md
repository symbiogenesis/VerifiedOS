...
Two perturbations are defined, each at a named site of `run_hart_active`. Write `step_f(s)` for the state after one call `try_step(n, true)` from `s` with the perturbation `f` applied at the site it names, so `step_f` differs from `step` at that site and nowhere else.
...
An **execution of the activation** is the maximal run of these rules from `(s_in, 1)`, where `s_in` is a state with `PC = a_in`: the finite derivation ending at the first configuration whose committed `PC` is outside `R`, or, where no configuration commits such a `PC`, the infinite sequence of configurations the rules generate. The relation is the set of those runs. The second case is not a formality: [the classification below](#5-fault-free-in-model-faulted-and-excluded) names it and says where it is absorbed.
...
The witnesses are stated over one concrete region instance, `W`, a credential comparison at the RoT gate reduced to the smallest shape carrying every element R-16-008c's construction sentence names. Mnemonics and the `c10`/`c11` capability spelling are [the differential corpus](../assurance/differential-corpus.md)'s, as its purecap programs write them; `s1`, `s2`, `a0` and the temporaries are ABI register names, named here for their roles rather than cited from a program. The compose-time constants are twelve bits wide so the traces read; that width is itself a finding, recorded below.
