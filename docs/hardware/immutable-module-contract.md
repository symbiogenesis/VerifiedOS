...
| `SOCKET_STATE` and `ISOLATE`, the endpoint's lifecycle and isolation control | public | host endpoint | nothing clears them, and they retain no client or session material: the RoT is their only writer and sets each at the declared section 6 transition it runs | the state section 6 names, `isolated` after any transition into it; re-enable is `T12` and never an autonomous recovery | R-15-228h, R-15-228i |
