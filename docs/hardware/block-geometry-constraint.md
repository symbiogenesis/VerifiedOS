...
This is not a second freeze-measurement instrument. [The freeze-measurement contract](../implementation/freeze-measurement-contract.md) records in its own §10 that this item and the per-class bank count are re-derived from realized macro geometry rather than from a composed image, so no corpus, no composition recipe, and no byte threshold reaches them, and a freeze report quoting either from that instrument is a finding against it. This document therefore carries none of that machinery, and acquiring any of it would make it the thing that contract already refuses.
...
**Two of the derivable rows bind and the rest are implied by them.** The floor is C5's codeword and the ceiling is C3's integer destination, so **the block is 32, 64, 128, 256, or 512 bytes**, which is 4, 8, 16, 32, or 64 granules. C2 is implied by C5, a codeword being a whole number of granules; C4 is implied by C3; and C6 is vacuous rather than merely unbinding, which is a stronger statement than one about the set. R-08-005a keys one revocation bit to each 8-byte capability granule and aligns only the *covered intervals* at 64 bytes, and `creclaim` applies the bit a granule at a time (R-15-007s), so there is no interval-level object for a block to straddle: a block that crosses an interval boundary is a run of granules each with its own bit, or with none where it lies outside the covered union, exactly as one that does not. No candidate, in the set or outside it, is excluded on that ground, and the row stands so that a bitmap keyed at a coarser unit than the granule would have a row to become binding in. Under R-15-181a's 128-bit fallback codeword the floor halves and the set gains 16 bytes at two granules.
