...
| `core/cva6_rvfi.sv` | 106, 107, 257, 258, 259, 296, 373, 380 | width | the commit trace's read and write masks are `CLEN/8` wide. This is the port [the differential corpus](../assurance/differential-corpus.md) reads, so its width change is visible to the co-simulation gate rather than internal |
...
The [scalar transform contract](scalar-width-transform-contract.md) records the
implemented width/metadata slice. [The transform registry](../../tools/rtl-width-transforms.json)
