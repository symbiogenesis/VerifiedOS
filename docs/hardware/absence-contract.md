...
The semantic content of the host microarchitectural removals in this document is **one hyperproperty** (R-15-101):
...
A-22 records a separate optional-device structural predicate under R-15-228k. It is no assertion that instruction-stream timing semantics cover a module with no instructions: its own circuit, state and interface contract supplies that statement, and its physical qualification is separate. No module RTL, netlist enumeration or physical candidate is authored or qualified here.
...
Each row is a structure whose **absence** is claimed, with the discharge form that closes it. Discharge form is fixed by who authored the block, not by the structure (§4). The numbered tests in the ground column are R-15-010's five admission tests: (1) deterministic architectural semantics, (2) data-independent timing, (3) no hidden microarchitectural state surviving a partition switch un-flushed by `fence.t`, (4) no authority path outside capabilities, (5) no autonomous behaviour. A row that stands on a decline under the defense-in-depth clause carries one recorded disposition per function the declined structure performs in the architecture it is taken from, and not only for the one the row is filed under (R-15-013a); A-16 is this section's instance, the initialization-tag plane being a hedge declined under R-15-013.
...
| **A-12** | Reactive frequency control (DVFS): any load-, activity- or temperature-driven control of a clock or rail | fails test (5); power states are schedule artifacts, never control loops: divider, rail and PLL state is a function of the composed schedule alone, an OPP change being the island's divider ratio and rail set-point reprogrammed at a partition boundary from the composed assignment (R-15-188), and the PLL hierarchy locking under the reset table alone (R-15-195) | no activity counter, load monitor, or temperature reading on any path to a divider, rail, or PLL control; no frequency-scaling state machine with an input other than the composed OPP assignment and the mode index; no PLL retune reachable from anything but the reset sequencer | R-15-100, R-15-103, R-15-188, R-15-195 |
...
| **A-18** | Ensemble link retry buffer or acknowledgement state | a frame is launched once at its slot and a frame that fails its tag stops the link, so no copy outlives the slot it launched in (R-15-228d, R-15-228e) | no retransmit buffer, no acknowledgement tracker, no timeout counter in the endpoint | R-15-228c, R-15-228d, R-15-228e |
| **A-19** | Ensemble link credit or flow-control counter | fails test (5), and would put a peer's readiness into this machine's timing: every launch and landing is a slot of the table | no credit register, no ready or stall input from the wire reaching the launch path, no back-pressure output | R-15-228c, R-15-228d |
| **A-20** | Ensemble link arbiter or virtual-channel multiplexer | one point-to-point link per endpoint pair carries one frame per slot, so there is nothing to arbitrate | no arbiter, no virtual-channel state, no per-flow queue, no queue whose depth varies with a peer's activity | R-15-228b, R-15-228d |
| **A-21** | Autonomous phase or latency tracker in the ensemble link endpoint | re-alignment is a core-issued leap of the follower's frame-origin register at a scheduled slot; the endpoint exposes a one-sided arrival-phase reading and holds no loop, and its clock-data recovery is analog line-interface matter reaching no launch instant | no phase accumulator or latency estimator; no write path from the endpoint to any origin or launch instant; no write path to the frame-origin register from anything but the boot core kernel's capability-checked store; no leap path from a landing the crypto core has not verified; no endpoint state a received frame updates except the landing and the arrival-phase reading | R-15-228c, R-15-196a |
| **A-22** | Immutable inference module execution/update paths and host-mastership paths | the separate device category permits only fixed logic, fixed weights and declared private state; no host authority is delegated over the socket | search the complete module and board for instruction fetch, executable ROM, writable program or microcode, firmware controller, mutable weights/adapters, user-data NVM, debug overrides, undeclared state or host mastership; search the host endpoint for card-supplied addresses, interrupts, adaptive retry/credit or arbitration paths; bind each retained store to its owner and clearing/reset contract | R-04-010b, R-12-085k, R-15-228f, R-15-228g, R-15-228k |
...
| **A-16** | Second tag plane (initialization-tag plane) | declined hedge under the *verify rather than hedge* clause; one tag bit per granule, not two | R-15-013, R-15-013a, R-15-035 |
| **A-17** | Second-class memory maintenance opcodes (discharge, refresh, tag maintenance, class migration, tier promotion) and any decode path reaching them | no instruction exists to remove: maintenance is §12 matter, discharge sequenced by the RoT while every requester that can address the domain is held in reset, refresh the same slave's fixed share of each bank's composition-time schedule, ECC scrub the existing `cbo.scrub`, and class assignment the static memory plan's composition-time act | R-15-247h, R-15-247, R-15-177a |

**Two removals are held against a deferred alternative, and this register says which** (R-15-100b). A removal whose ground is that no consumer exists holds only while no consumer exists, so where [Evaluated Architectural Alternatives](../background/architectural-alternatives.md) **defers** a mechanism rather than declining it, the removal it would retire is named against it here with the trigger that would fire it, instead of leaving an enumeration that states its members unconditionally. Naming admits nothing: each alternative stays deferred, and R-15-035, R-15-047 and R-13-010 stand as written until an amendment takes a trigger (R-18-034).
...
The imported-core check covers, by name: predictor arrays, reorder buffer and reservation stations, prefetch engine, cache data/tag/valid arrays, a second hardware thread context, and load-, activity- or temperature-driven frequency control paths, the divider and PLL inputs read for any source other than the composed schedule (R-15-103).
...
Its admitted range is a separate issue-time constraint (R-15-104a): every byte read by a run-ahead request, including alignment or burst rounding, must lie within the currently executing tagged, unsealed, execute-permitted PCC and within declared executable memory under the composed map's existing attributes, without address wraparound. A failed or unresolved check issues no traffic and raises no architectural exception solely for that run-ahead refusal. Checking only the first byte fails the rule. A PCC authority change, including one at an unchanged cursor, invalidates queued authorization; buffered or in-flight responses are discarded or revalidated against current authority and the current static path before use, while issued requests remain timing and drain obligations until completion or cancellation. Device regions receive no run-ahead request because their executable attribute is clear (R-15-002d). A bundle crossing the PCC boundary waits for architectural demand under the existing fetch checks, so the rule changes no capability alignment. The register imposes no independent depth cap: every finite implementation still owes the stream-dependence, timing and pipeline-drain obligations, and a range check supplies no outstanding-fetch completion bound. This is a constraint on the retained structure, with no claimed absence and no added row in §3.
...
| `fence.t`-flushed | **a single structure: the store buffer**, drained rather than merely fenced, and holding **SRAM-space stores only**, which is what keeps the drain's bound a function of the class rather than of an endpoint's accept latency or a second-class refresh wait | R-15-213, R-15-217, R-15-015b, R-15-218 |
