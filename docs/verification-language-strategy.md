...
The [implementation checklist](implementation-checklist.md#q-assessment-actions) schedules Q19's tooling and conditional library qualification and Q20's integrated authoring and optimized-artifact trials. **Q2c and Q21a/Q21b close their present commissioning decisions by reasoned rejection**: the concrete foundation and its machine/contextual connections cannot be responsibly priced for these consumers. Their [foundation map](cheri-foundation-map.md), [core design](language-core-design.md) and [compiler comparison](compiler-route-contract.md) retain the contracts and reopening evidence. The Clight/CHERI-CompCert route remains required; the proposed direct explicit-memory capability IR is unselected, and no frontend, foundation or paired compiler trial opens.
...
The [proof reuse inventory](proof-reuse.md) qualifies source evidence as of 2026-09-10; qualification alone does not make a foundation locally usable. A usable starting point has a selected licensed edition, a compatible dependency closure, checked assumptions and the required theorem over the local semantics. The [integrated Stdlib references](proof-reuse/foundations.md#f01-rocq-stdlib-arithmetic-integrated-by-reference) replace existing proofs without closing an open obligation, so they supply no new effort discount. The bands already assume library reuse. More precise theorem locations reduce repeated discovery work, but no measured saving or newly demonstrated foundation justifies lowering these bands.
...
They exclude acquiring unavailable source rights, migrating an incompatible prover or program-logic dependency closure, establishing missing primitive soundness, connecting a new machine or program logic, or engineering a universal backend API. Those prerequisites need separately scoped owners and estimates before a dependent row can start; they are not absorbed by its upper end. In particular, [Cerise and Cerisier](proof-reuse/hardware.md#hw-05-cerise-universal-contract-and-sentry-encapsulation) use different machines and prover/library editions, while [RustBelt and RefinedRust](proof-reuse/languages.md#ownership-initialization-and-type-soundness) supply ownership precedents rather than the missing local capability laws.
The [foundation map's re-entry evidence](cheri-foundation-map.md#re-entry-evidence) decides whether the library conditions hold. Until then, its pilot and dependent rows are conditional comparisons, not estimates for delivering their results from the present checkout.
...
| Reusable bounded-component library | 4-9 person-months, including the pilot | Start from the same checked foundation. Parameterized initialization, disjoint copy, scoped update, documented combinators, independent client reuse, and instance/refactoring tests. Selected [stdpp/coqutil laws](proof-reuse/foundations.md#f02-stdpp-finite-maps-sets-and-sequences) can support the representation proofs; finite capacity and iteration order still need local proofs. One concrete instance; source assurance only. |
| Scoped borrow-restoration extension | 1-3 person-months | Working buffer predicates and frame rules. Reuse [RustBelt's ownership proof structure](proof-reuse/languages.md#rustbelt-semantic-ownership-and-lifetime-soundness) for lexical element/slice borrowing, with local restoration proofs and negative alias/escape tests. Excludes porting lambda-Rust or Iris, general Rust borrow inference and concurrent interior mutation. |
...
| Descriptor-backed codec integration | 2-4 person-months, conditional on a usable synthesis path | One reviewed bounded descriptor, concrete representation, parse/reject/consumed-length proofs and the required admissible-byte canonicity proof. [Narcissus](proof-reuse/parsers.md#narcissus-primary-same-language-starting-point) is the source, but its available source targets do not establish local compatibility. Excludes backend porting, a standards-wide descriptor campaign and [VUPER's unresolved acquisition and correspondence work](proof-reuse/parsers.md#vuper-unusually-close-uper-research-candidate). |
...
| Reviewed restricted frontend | 9-18 person-months, including its syntax prototype | Working library and a fixed small source semantics. Checked elaboration/correspondence for that subset, useful diagnostics, negative tests and reproducible builds. [MetaRocq](proof-reuse/languages.md#metarocq-a-verified-pcuic-checker-with-explicit-normalization-premises) supplies checking and erasure theorems; local frontend correspondence and the checker's normalization premises remain separate obligations. Reuse a qualified backend and runtime discipline; exclude repairing their missing proofs. |
| Native integration of one library family | 2-6 person-months, conditional on the complete target route | The required CHERI compilation, source-correspondence and final-artifact checking routes must work first. Connect representations and obligations, bind exact bytes, and measure resources. Existing [Rupicola/Bedrock2 derivations](proof-reuse/languages.md#rupicola-proof-producing-relational-compilation) do not include the printer, purecap bridge or final-byte theorem; those remain separately owned prerequisites, not work hidden inside this band. |
