...
- **What is read.** A witness set: a set of row positions fixed at composition, at least one per subarray per deck of every bank in the phase, read through the ordinary read port after the dwell and compared against the landing state. The indication is positive where every witness reads the landing state and negative otherwise.
...
| `T_ret_ceiling` | the longest interval at which any cell still reads its written value, at the cold corner | the same | cold corner, time zero | R5, R-15-247m, R-17-058f | pending |
