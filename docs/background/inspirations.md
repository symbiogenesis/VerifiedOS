...
Promoting the 2024 CHERI-seL4 work in-scope would supply the purecap *bring-up evidence and design* to verify against; it does not supply the proof, and after M4.1b it does not supply the implementation either, that release's kernel half carrying a licence this repository cannot combine with.

**Honest residual: a Coq re-proof is fresh (§17).**
What transfers from seL4's maturity is the **retained object-model design, the ABI, the long-scrutinized abstract specification, and the in-scope multikernel and CHERI engineering**, not the proof. **The C implementation is what stopped transferring**: the kernel-C provenance fork (M4.1b) forecloses carrying that code the same way the specification fork forecloses carrying the Haskell model, so what crosses is the design read and re-stated here rather than the source.
...
The search for an existing Coq artifact to inherit comes back empty: seL4 is **Isabelle-only, and actively so**, with the `l4v` proof and every current extension (MCS, multicore, the seL4 Core Platform) staying in Isabelle/HOL, so the route is genuinely greenfield in Coq: design and spec carry across, the C does not, and there is no proof to port.
...
**This is therefore the entry where the import is tooling rather than technique** (§18): the composition tool and its system-description-to-capability-distribution path, and the sDDF interface set and driver templates, are the start-from for §7's composition tooling and §12's ring library, a substantial body of engineering the design does not author. **The CHERI Alliance's already-purecap CHERI-seL4 / CHERI-Microkit port is not on that list**, M4.1b's read having found that release's kernel half GPL-2.0-only with its CHERI work contributed under the same tag, so the composition tooling and the ring library this entry pointed at it for are authored here instead.
...
**What does not transfer: CryptOpt's additional admission route.**
...
**CryptOpt's additional translation-validation route is rejected**; its randomized search methods and CompCert-CT's leakage-aware lowering examples remain producer references under R-05-066. Applicable C/IR candidates use the ordinary compiler and final-artifact checks, retaining functional correctness, source correspondence, CT, masking and resource obligations. Neither an unchecked assembly leaf nor a new checker is admitted, and target or proof-cost improvements are measured before a candidate is retained.
...
The masked crypto core (§5, §15, §17) reuses gadget designs and verification techniques; the plan's post-M10 cells price selected Coq objects over separately qualified foundations, not the complete hardening program. The gadget theory is settled: domain-oriented masking (Groß et al.), the robust probing model (Faust et al.), PINI composition (Cassiers/Standaert), and the HPC gadget families give glitch-robust, trivially composable construction at arbitrary order, with COMPRESS and AGEMA as generators and SMAesH as the parametric-order masked-AES existence proof. The verification tooling is mature and layered: SILVER exact at gadget scale, PROLEAD statistical at full-cipher scale under glitch- and transition-extended probing, MATCHI compositional over full masked hardware, and the one published end-to-end pipeline (formal verification, then tapeout, then physical evaluation: the TCHES 2026 prime-field-masking-on-silicon study) executes exactly the verify-then-characterize discipline R-15-053a mandates. Two results anchor the axiom's narrowing: Coco-class execution-aware verification checks masked software against the executing core's gate-level netlist, the discharge R-15-053a's accept requires, and the share-slicing and MIRACLE measurement campaigns are why nothing weaker is accepted, masked software on ordinary cores being refuted at ISA-level reasoning. OpenTitan's masked AES and KMAC carry the production randomness architecture this design inherits with its RoT lineage: a stream-cipher expansion (Bivium-class, hundreds of fresh bits per cycle) between the entropy root and the gadgets, which is where R-05-004a's named DRBG assumption comes from, an LFSR's output cancellation being a proven way to void a probing proof. The [proof inventory](../assurance/proof-reuse/crypto.md) establishes no compatible reusable Coq proof of the complete required masking and combined-fault obligations; that is a bounded qualification result rather than an absence claim over every library. **The vector-masking question is a disposition rather than an implementation obligation**: R-05-004a narrows the secret path to a dedicated masked datapath of the parametric-order lineage, so what this repository owes is a realization of an arm with published prior art rather than a first of its kind, and the vector crypto units stay in the profile for work that carries no secret. The [Lean arithmetic source](../assurance/proof-reuse/crypto.md#prime-field-masking-in-lean-4) proves a preimage-cardinality statement for single-wire observations with fresh stage masks. Its probability interpretation is informal and independent replication is unestablished. The MIT grant permits adaptation, but the required probabilistic connection, *d*-order generalization and correspondence to the glitch/transition model remain authored work in the plan's arithmetic-composition cell. Boolean gadget arguments still require their own model match, and neither source supplies R-17-058d's ineffective-fault reduction.
...
- **It makes the checker-size commitment honest by splitting the checker, not by shrinking a CIC checker.**
  ...
  The full **CIC proof kernel** (MetaCoq-lineage, honestly tens of kLoC) does not vanish, and it does not leave the device: it keeps one per-install act, checking each package's artifact-local source-correspondence theorem beside the type check (§6, §13), while the deep Tier-0 and hyperproperty proofs it also validates move to where those proofs actually live, **release-time, over the fixed base-image TCB**, their result bound into the signed measured-boot root (§9), not a per-install on-device cost.
...
Its dependency set is a machine semantics and a type theory and nothing else, which makes it the one part of this platform whose correctness argument mentions no operating system, so it is specified separately, in [typed-assembly-language.md](../languages/typed-assembly-language.md), with this platform depending on its `cheri-rv64` instantiation and pinning a version (§5).
...
The shape outlives the decade, and its clearest modern instance is a deletion rather than an adoption: Intel's **x86S** proposal is this dependency at the instruction set, retiring real mode, VM86, rings 1 and 2, 32-bit ring 0, and the 8259 for real specification surface at no cycles and no shed security property, which fails none of the technical gates the *rejected profile simplifications* table in [architectural-alternatives.md](architectural-alternatives.md) sets; it reached version 1.2 in June 2024 and was abandoned that December, because firmware, hypervisors, operating systems, and an installed base had to move together and no one party could move them.
What stopped it is a gate no technical rubric contains, which is the reason that table needs only five: this design owns every layer whose consent x86S had to ask for, and pays for the ownership in the porting bill [userspace-porting.md](../implementation/userspace-porting.md) itemizes rather than in a negotiation it could lose.
...
Here composition fixes the interfaces through which authority can be delegated (§4, §13), while the user's runtime act designates an object (§8.6). A clipboard snapshot is such an ordinary object, not a new grantable privilege class or one of §7's kernel-object classes. Designation still has to be witnessed on this platform's trusted display and input path; an app-rendered gesture alone supplies no witness.
...
It is the largest-scale existence proof that a share-nothing, coherence-free, message-passing manycore is buildable, the role Barrelfish (above) plays for the model itself, SemperOS (above) for its distributed capabilities, and Groq's TSP (below) for the same share-nothing, statically scheduled substrate taken across machines as well as across cores.
...
## Groq's Tensor Streaming Processor: static scheduling carried across chips, and the shared address space an ensemble does not take

Groq's Tensor Streaming Processor (TSP, marketed as the LPU; licensed non-exclusively to NVIDIA in December 2025, Groq's founder and part of its team joining NVIDIA while Groq continues as an independent company, and shipped by NVIDIA as the Groq 3 LPU) is the deployed existence proof that a statically scheduled, cacheless, arbiter-free chip scales to a rack by adding chips under one compile-time schedule, and it is cited here for the chip-to-chip half rather than for the chip.
The single chip, described at ISCA 2020 ([Abts et al., "Think Fast: A Tensor Streaming Processor (TSP) for Accelerating Deep Learning Workloads"](https://cdn.sanity.io/files/chol0sk5/production/16e7e13197dfe5f52d3b0ed62e29620662c38743.pdf), DOI 10.1109/ISCA45697.2020.00023), guarantees determinism "by eliminating all reactive elements in the hardware (e.g. arbiters, and caches)", holds "220 MiBytes of globally shared SRAM" on a 14 nm die at 900 MHz with no DRAM interface of its own, leaves it to the compiler to "precisely schedule instructions" with "cycle-accurate control", and carries sixteen chip-to-chip links on that first part, so the on-chip convergence is the Cerebras entry's (above) once more.
What this design takes is the link.
The multi-chip system, described at ISCA 2022 ([Abts et al., "A Software-defined Tensor Streaming Multiprocessor for Large-scale Machine Learning"](https://cdn.sanity.io/files/chol0sk5/production/81d0c45ed70a10f1804bbd82f2d0076af1186ff0.pdf), DOI 10.1145/3470496.3527405), carries the same discipline onto the wire: "the hardware is disallowed from asserting back pressure which would disrupt deterministic operation of the network. Instead, software explicitly schedules vectors on each physical link in the system"; "the interconnection disallows dynamic arbitration since it would result in non-determinism", with no queues or virtual channels beyond shallow clock-crossing buffers; "all contention for both functional units and network links are resolved at compile-time"; the model partition "induces the traffic pattern for the parallel workload"; "an individual TSP binary is compiled and prepared for each TSP in the system" and every program is launched simultaneously; each chip keeps "an independent clock source", the links are plesiochronous, link latency is measured at start and then held, and drift is cancelled by a `RUNTIME_DESKEW` instruction "scheduled to be executed on each TSP within the network at the same time" against a spanning tree of hardware-aligned counters; and every link carries "forward error correction (FEC) on every link", an uncorrectable burst flagging the runtime to "replay" the inference rather than a link-level retry.
[Groq's RealScale document](https://cdn.sanity.io/files/chol0sk5/production/e46afbbba8b35a206499c165eb457502a52e710f.pdf) states the product form: "a communication fabric of point-to-point links, eliminating the need for network interface cards, intermediate layer switches", links "explicitly flow-controlled by software, such that they are scheduled as first-class functional units", vectors "transmitted at prescheduled times so as to be present at the receiving end when needed", and links that "are phase-aligned and act like a high bandwidth, fixed latency wire between the chips".
The [GroqCard](https://cdn.sanity.io/files/chol0sk5/production/ec9c0e1b3f1e07f7389df879af71529abd66d662.pdf) and [GroqNode](https://cdn.sanity.io/files/chol0sk5/production/7a32f5b5843eb1c12279dbf23fa23e3da5f07f62.pdf) briefs (v1.5, 2022) put one chip on a PCIe card with "230 MB of on-die memory" and "no run-to-run variation", up to eleven chip-to-chip connectors per card and eight cards in a 4U server; [NVIDIA's Groq 3 LPX description](https://developer.nvidia.com/blog/inside-nvidia-groq-3-lpx-the-low-latency-inference-accelerator-for-the-nvidia-vera-rubin-platform/) (2026-03-16) keeps the shape at 500 MB of compiler-managed SRAM and 96 chip-to-chip links per chip with 256 chips per rack, and states the reason for scale-out plainly: "Because on-chip SRAM capacity is finite, larger models are scaled across many interconnected LPU accelerators using parallel execution strategies such as layer-wise partitioning".
The corporate state is Groq's own ([newsroom, 2025-12-24](https://groq.com/newsroom/groq-and-nvidia-enter-non-exclusive-inference-technology-licensing-agreement-to-accelerate-ai-inference-at-global-scale)): a non-exclusive licence with a transfer of staff, and not a sale of the company. All readings are of 2026-09-12; the 2020 part's node, clock, and capacity are cited as that part's, no later node figure being published, and no deployment count is taken from press.

**What transfers is the evidence that the ensemble (§2, §15) is a shape that ships and scales.**
A statically scheduled chip with no caches and no arbiters, its weights wholly resident in on-chip SRAM, joined to its peers by point-to-point links whose every transfer instant the compiler fixed, with no back-pressure, no arbitration, and no queues on the wire, one binary per chip from one compilation, an independent clock per chip re-aligned by a scheduled instruction against a fixed tree rather than by a reactive protocol, and models larger than one chip's SRAM partitioned across chips at compile time with the traffic pattern that partition induces: every one of those is a commitment this design already makes inside one die and now takes across the link, as the slot tables the admission proof emits (§11), the non-work-conserving frame with its authenticated idle frames (§15), the per-member image from one composition act (§13), the scheduled leap of a follower's origin under a composition-time skew bound along a composition-fixed leader tree (§15), and the sharded model with every shard resident (§15). Groq's own careful phrase for what this obtains across a wire is "the illusion of a synchronous, lock-step system", and its measurement that 99% of 24,240 BERT-Large inferences on four chips returned inside 1225 microseconds and all inside 1300 is the evidence that the illusion holds at scale.

**What does not transfer is the trust boundary, and every divergence is where the ensemble parts from the rack.**
Groq exposes every chip's SRAM as "logically shared, but physically distributed" global memory addressed across chips, "more than 2 TeraBytes" across up to 10,440 chips, so a Groq rack is one shared-memory machine sharded across dies, which is exactly the memory expansion across a link this design declines (§15, and the second-die entry in [Evaluated Architectural Alternatives](architectural-alternatives.md)); an ensemble's members share nothing, each keeps its own 36-bit space, and a link carries ring descriptors and never a load, a store, or a capability.
The TSP "acts as both a processing element (endpoint) and network switch", routing tensors over a software-defined Dragonfly with up to five hops; an ensemble link joins exactly two named machines, no member forwards a frame, and what a member passes on it passes as new ring traffic under its own schedule.
Groq's published material describes no cryptographic protection, authentication, or attestation of the chip-to-chip link and nothing resembling a capability, a delegated table, or a typed payload, the frames being raw 320-byte vectors; an ensemble link carries ciphertext inside a session between two mutually attested machines whose unit and composition the ensemble names (§12), which Groq's setting does not need and this design's threat model (§3) does.
Groq's system reliability is a hot-spare node per rack and whole-inference replay on an uncorrectable error, a runtime policy; an ensemble link stops and re-establishes a fresh session at a scheduled cadence (§15, §17), which is stricter and is this design's own rule rather than Groq's.
And the TSP is one statically scheduled logical core with SIMD functional slices whose discipline is the compiler's alone; this design keeps its multikernel, its capabilities, and its admission proof on every member unchanged.
What transfers is therefore **evidence, not code**: the deployed demonstration that static link scheduling, no back-pressure, no arbitration, FEC without retry, one binary per chip, and scheduled deskew compose into a working multi-chip machine, and the clean contrast on the one axis the ensemble refuses to follow, memory pooled across a link. Nothing of Groq's is imported: no compiler source is among the artifacts read, and the link carries no protection to curate.
...
**Angel** (City University London) is kept as the negative example: it was first, in 1992, and it had **no explicit protection system at all**, resting on names an attacker was assumed unable to guess, which is the shape [absence-contract.md](../hardware/absence-contract.md) exists to make impossible to ship by accident.
...
**The critique has a modern restatement, and it reaches one thing Opal's does not.**
Kamp puts the flat model itself in the dock (*Linear Address Spaces: Unsafe at any speed*, ACM Queue 20(2), 2022), on the observation that the first thing software does with a flat memory is impose a subdivision structure on it, which makes CHERI a reaction against flatness rather than a feature added to it.
Kell sharpens that into the objection this line's own proponents never state ([*Flat vs segmented memory: it's recursive*](https://www.humprog.org/~stephen/blog/2026/08/25/#flat-vs-segmented), 2026-08-25): subdivision is **recursive** where hardware is finite-state and prefers bounded depth, so the naive reconciliation fixes the depth the hardware knows about and leaves the rest to software, which is the hardware washing its hands of what the programmer is doing; CHERI escapes that by making every narrowing a software act, at the price that what code can address becomes an emergence of the software, the transitive closure of the capabilities that have flowed within its reach, with an auditing difficulty attached that the post leaves to *"the right tools"*.
Both halves land where this design has an answer, and neither answer is CHERI's.
The recursion is **resolved before the hardware sees it**: sub-compartments nest and nesting is not a second mechanism, a sub-compartment being one more node in the same flat machine-checked graph (R-04-006), so there is no level-*N* cutoff to bound and no depth carried at run time.
The closure is **enumerable at composition, and is enumerated**: no integer, address literal, or stored bit pattern ever becomes a capability (R-05-136), allocation is composition-time so nothing narrows at run time (§7, §8), a compartment's initial capability set is its whole authority under a one-pass scan of the image (R-05-089), and `ambient.rooted-mutability` is decided as a whole-image predicate whose certificate enumerates every mutable root and every initial capability with a provenance chain per store site (TAL-059, [typed-assembly-language.md](../languages/typed-assembly-language.md)).
That is the tool the objection supposes could exist, carried as an admission obligation rather than as an analysis someone runs afterwards.
The scope limit is the one this section already states of itself: every part of that answer is a property of a **composed** image, so none of it answers the objection for a system that loads code at run time, which is the population the objection is written about.
...
Two obligations are genuinely new relative to a translation-IOMMU and are booked in §17: (1) **in-flight-DMA revocation**: a capability held by a running transfer must honour the §8 revocation so time-to-containment stays bounded, by one mechanism, bounded per-window re-authorization against the revocation bitmap at each fabric grant of the window (§15), with Cornucopia Reloaded's load barrier the lineage the core-side filter takes rather than a second device-side check; (2) a **capability- and tag-carrying fabric**: the interconnect must propagate capabilities, tags, and revocation state to the DMA blocks (new Sail / RTL ⊑ Sail surface, §15/§18).
...
**The quiescent point: its register lesson is retained.**
Oberon's collector runs only when no command is executing, so no procedure activation remains and module-level variables are its roots. This removes collector-mutator overlap by scheduling, at the cost of waiting between commands.
The platform keeps its own bitmap-driven sweep in incremental background slots: it performs no reachability trace, and binding each quantum to a partition switch would add work to the admitted switch path. The narrower safety premise still matters. A per-load filter clears a capability reloaded from memory, not one already held in a register. R-08-006 requires a bounded live-authority barrier before containment completes, including saved contexts, outstanding grant loans, remote proxies and device work; R-08-007a requires a post-barrier sweep over every possible holder before reuse. The [CHERIoT register-revocation explanation](https://cheri-alliance.org/cheri-enabled/cheriot/) supplies the direct capability-system precedent. [Evaluated Architectural Alternatives](architectural-alternatives.md) separates this retained obligation from the declined placement of sweep quanta.
...
What imports is the method, the bounded register-barrier lesson, and the module key.
...
A fifth answer is the one GrapheneOS gives the owner rather than the adversary: it installs onto a Pixel and **relocks the bootloader under its own verified-boot key**, so the owner's OS boots with the same rollback protection and attestation the vendor's had, which is the only shipping instance of an owner holding the root that admits their device's software while the silicon's own root stays the manufacturer's. That split is taken as §9's **enrolled root set**: the ROM root verifies the two verified firmware images alone, the root that admits a generation is a set the holder edits on the trusted consent path, and the vendor's key is one member the holder may remove.
...
GrapheneOS's [secure-paste implementation proposal](https://github.com/GrapheneOS/platform_frameworks_base/pull/435) contributes **designation as authorization** to the clipboard: in its Paste-only mode, an explicit action through the system selection toolbar, input method, accessibility service or keyboard shortcut grants short-lived access to the current item for the destination app. Apps retain access to items copied by their own identity. The proposal is open and planned for merge, not a shipped guarantee; it also carries global and per-app compatibility controls, with ordinary access allowed by default. The contribution taken here is the item-and-recipient-bound designation, not those policy controls. The ordinary snapshot grant (§8.6) uses the existing powerbox and trusted-path agent, and its fresh on-device confirmation follows the RoT latch. The whole panel and application touch remain unavailable during that act; the untrusted app and input stack cannot supply the witness that GrapheneOS obtains through its trusted system services. No clipboard server, standing future-item grant or persistent consent record is needed.
...
**The same project's image-based half is the nearer ancestor of §9, §10 and §11, and it is cited here rather than left to the Fedora Atomic and OSTree entries below.** Poettering's *stateless systems* proposal (2014) is the first statement of the discipline §10 carries to its endpoint: the vendor tree is the only thing shipped, `/etc` and `/var` are regenerable from declarations the packages carry (`tmpfiles`, `sysusers`), and a factory reset is a boot with neither populated. The 2022 *fitting everything together* design assembles the mechanisms: a unified kernel image signed as one object, `/usr` under dm-verity with its root hash pinned by the signed kernel command line, A/B `/usr` partitions with boot-assessment counters, TPM2-bound volume keys generated on first boot rather than provisioned, per-user encrypted homes whose keys are flushed on suspend, and a `systemd-repart` first boot that instantiates rather than installs. ParticleOS is that design as a buildable distribution whose defining premise is that the owner builds the image and signs it under their own keys, and Amutable (2026) is the same group commercializing it under the triad *build, boot, runtime* integrity. Each of those is here in a stronger form and the entries name where: dm-verity's on-access check is §10's per-read Merkle verify against the boot-attested root; the regenerated `/etc` is the compiled config generation nothing parses at runtime; homed's key flush is the Before-First-Unlock state; boot assessment is RoT boot counting with automatic revert; and the *runtime integrity* phase, which is that design's own stated gap since nothing after boot is measured, is closed by construction here, static composition and admission by proof leaving no runtime code to measure. **Two things are taken rather than surpassed.** The first is *systemd-measure*'s move of sealing a volume key to a **public key over signed expected measurements** rather than to the measurement itself, which is what keeps a sealed secret openable across an update without re-enrolling every volume; §10 and §12 take it as the FDE key's policy, the signing root of the reference integrity manifest over the generation register plus the anti-rollback floor, and it is cheaper here than there because every expected value is reproduced from source (§9) where a firmware PCR is brittle. The second is the **partition of the measurement space by stakeholder**, PCR 11 for the vendor image, 12 for the administrator's configuration, 13 for extensions and 15 for local identity, so a policy or a relying party can name the software without the unit; §9's generation and device registers are that split with the vector unchanged. And ParticleOS's premise is taken whole, with GrapheneOS's relocking under the owner's key beside it (below): §9's enrolled root set makes the root that admits a generation the holder's, the vendor's key one member the holder can remove, so signing decides who may push and proof decides what may run. What is refused is the substrate under all of it: UEFI, a firmware PCR bank, a discrete TPM, a Linux kernel, and a package manager at image-build time (§2, §9).

---

## greetd: the privilege-separated login surface, and the greeter nobody here may replace

greetd contributed the *shape* of a minimal login manager: a privileged daemon owning authentication and session start, an **unprivileged greeter** owning nothing but the conversation surface, and a small IPC protocol between them; it enters this design's orbit as a member of the COSMIC closure (`cosmic-greeter` is a libcosmic greetd greeter, its companion daemon driving PAM), where [userspace-porting.md](../implementation/userspace-porting.md) gives it its disposition.
...
## Clear Linux, Serpent OS, and AerynOS: stateless as shipped practice, the package manager that upgrades itself, and the live swap that ran out of ideas

Clear Linux (Intel, 2015 onward) is where *stateless* first shipped as a rule a package build is held to rather than a proposal: the vendor tree under `/usr` is the whole of what ships, `/etc` holds only what the administrator wrote, and deleting it is a factory reset, the discipline the systemd entry above traces to Poettering's 2014 proposal and that the same group now calls *hermetic /usr*. Ikey Doherty, on that team, carried it into Solus and then into Serpent OS, renamed AerynOS in March 2025, which is the nearest shipping instance of G4 (§1) on a Linux base: a package may place files under `/usr` alone; every transaction builds a whole new `/usr` from a content-addressed store by hardlink and swaps it in with one atomic exchange; the last transactions are retained states the boot menu offers, each boot entry naming the transaction it boots; and a declarative system model names the wanted package set, from which moss recomputes the whole system on every transaction rather than tracking what is installed. Each of those is here in a stronger form: the last is R-13-001a's *an install is a generation, not an amendment to one*, reached there from the practical side; the retained states are §10's retain-K-plus-pinned history, with the signed, reproducible diff a hardlink tree cannot give; the boot entry naming its transaction is §10's root chosen by verifying its copies rather than by reading a pointer; and the two trigger classes moss keeps for what statelessness could not delete, `tmpfiles`, `sysusers`, and the caches, are composition outputs here (§10, §13), so no post-install code survives at all (§13).

**Two things are taken.** The first is what AerynOS shipped in August 2026 under the phrase *install once, update forever*: a root index at the repository root tells an older client which upgrades to apply, in what order, before the new repository and package formats can be read, so the package manager upgrades itself across a breaking on-disk change with no reinstall. §11 takes the shape as a rule rather than a mechanism: a successor is composed for the generation that installs it, in that generation's descriptor versions and against its spec-set, and a device several steps behind takes the steps its directive names in order (R-11-005b), with §10 giving the backward half the durable-state rule already had, the store's own schema migrating or shipping whole and never being read as what it is not (R-10-036a). The second is evidence rather than mechanism. AerynOS's headline is the live atomic swap, applied with no reboot, and its own November 2025 post-mortem records where that ends: after the exchange, running daemons keep inodes into the archived tree, the desktop shell watching `/usr/share/applications` never sees a newly installed application, and "the Linux kernel does not offer any mechanism to forcibly 'revoke' inodes", so the authors conclude they are possibly stuck with reboots. That is the cost R-13-001a chose on purpose and §17 prices at R-17-035a: a generation takes effect at the next boot because a running system is never edited, and this design, which has the revocation Linux lacks (§8), still declines to spend it there, the memory plan and the schedule being whole-image quantities no live swap preserves.

**What is declined** is the rest of the pitch: the erofs-over-overlay immutability plan, which is the same live swap one layer down; the per-feature-level optimized builds, which R-10-034's profile-free layout refuses by construction; the hardlink tree itself, there being no directory tree to blit where a compartment's namespace is manifest-backed (§14) and an image maps in place (§13); and the dependency solver over provided sonames, where a roster names content-addressed closures and nothing is resolved on the device (§13). Barney, Doherty's September 2026 note toward a source-first successor, adds hash-keyed builds with full chain invalidation and an automated multi-stage bootstrap that breaks dependency cycles, both of which the NixOS and bootstrappable-builds entries already carry.

---

## Unison: canonical structural identity, with executable correctness checked separately

[Unison's hash definition](https://www.unison-lang.org/docs/language-reference/hashes/) derives identity from a definition's structure while excluding names. This makes permitted renames metadata changes and allows references to identify exact definitions rather than resolve names at runtime. It does not decide arbitrary semantic equivalence, and two different algorithms computing the same function need not share an identity.

The platform takes the naming discipline: state the exact normalization equivalence and its version, including which name and binding changes it permits, before using a normalized digest as a definition identity. The existing canonical-encoding requirements (R-05-051a/R-05-051b) and content-addressed closure provide the relevant boundaries. A permitted rename preserves identity; a selected behavior-changing edit changes it. No requirement asks all semantically equivalent programs to share a hash.

A compilation cache can store the wrong executable under the correct source key. Deriving that key from structural identity supplies no compiler-correctness theorem. Cached artifacts therefore retain the existing correspondence and admission checks against their exact executable bytes and relevant dependency and toolchain identities (R-13-023, R-13-026). A wrong cached executable under a correct source key must fail those evidence checks, without introducing a trusted cache or a second admission path.

The same rule applies to an interpreter acceleration: it lies within the verified semantics-preserving implementation or carries a checked equivalence to the code it replaces. Merely sharing an identity key supplies neither. Unison's structural naming is the adopted lesson; its cache is not evidence discharging this platform's execution theorem.

---

## Transparency logs: public commitment and witnessed history, with targeting left explicit

Proof-carrying admission establishes properties of the artifact in hand. A transparency log makes the admitted base-image and package identities public commitments. These are different properties: a source-level backdoor can satisfy the admitted specification, and a valid proof says nothing about how widely that package was distributed.

**What the platform takes.** R-13-023b logs the reproducible base image's root and each package's content-addressed source-and-interface identity. It does not log the generation root or its private roster. Inclusion and consistency proofs with the co-signed checkpoint travel in the pack and are checked offline; a promise of future inclusion is insufficient. Certificate Transparency supplies the Merkle proof structure, and [C2SP's witness protocol](https://c2sp.org/tlog-witness@v1.0.0) supplies the pattern for persistent witness checkpoints. Tile-based serving permits an untrusted mirror to deliver the material without a live log call at admission.

**A quorum needs a policy theorem.** Each witness follows its own history, so K-of-N operational independence alone does not establish a shared history. R-13-023c requires honest intersection of accepted quorums under a declared faulty-witness model, persistent state before co-signing, authenticated recovery and a justified policy transition. Its 1-of-2 counterexample has two honest witnesses signing different extensions for different fresh clients. Stronger thresholds trade install availability for safety; missing signatures never authorize weakening the policy. A disjoint trust replacement states its rebootstrap assumptions rather than claiming continuity by default.

**What publication does not establish.** Log two admissible source variants, then deliver one only to a victim: all inclusion and correspondence checks still pass. The log can support discovery but supplies neither uniform release selection nor inspection of source bytes that it does not publish. [Software Distribution Transparency and Auditability](https://arxiv.org/html/1711.07278v1) separates source availability, correspondence, monitoring and hidden-version detection. [Apple's release-transparency design](https://security.apple.com/documentation/private-cloud-compute/releasetransparency) adds binary publication and monitoring, while its [non-targetability analysis](https://security.apple.com/documentation/private-cloud-compute/attacks) uses further authentication and routing mechanisms. Their guarantees are not consequences of an inclusion proof alone.

**Honest residual.** R-13-023a retains targeted selection of logged variants. A stronger claim needs explicit package/channel/version identity, legitimate-variant and selection rules, available audit material, monitor responsibilities and a response to suspicious releases; none follows from publishing a digest. Holder-built packages and private composition remain supported without a mandatory vendor catalogue. Reproducible composition removes a separate source-permitted producer variation and cannot prevent selecting a different logged source.

---

## TUF and Uptane: freshness and corroboration at the update boundary

**The Update Framework** separates root, targets, snapshot and timestamp roles to address distinct update threats. Its expiry mechanism addresses a stale but correctly signed view; a valid artifact proof likewise does not establish that an offered generation is current. The platform takes that distinction in R-11-005a, using its authenticated time source and persisted monotonic floor, refusing expired directives and surfacing the age of the newest verified directive. The signed generation root already binds one coherent image rather than a mix of separately selected package bytes.

**Uptane** contributes a second distinction: image authorization and a device's update directive are independently corroborated. Its image and director repositories separate approval of an image from selection for a device. Here the holder's enrolled keys authorize generations, proof checks establish admissibility, R-13-023b establishes witnessed public commitment, and the directive's expiry establishes its stated freshness. None establishes uniform selection or harmless source intent, and a proof does not replace the holder's authorization. [The Update Framework specification](https://theupdateframework.github.io/specification/latest/), [Uptane Standard](https://uptane.org/docs/latest/standard/uptane-standard)

What is declined is process or signature evidence standing in for artifact proofs, including a partial-verification tier that omits required admission checks. Naming, authorization and freshness remain useful separate properties. The platform's per-roster composition does not import a director with authority over the holder's enrolled roots, and unavailable fresh metadata stops an install rather than a boot.
...
The dissertation's [assumptions and environment-diversity discussion](https://dwheeler.com/trusting-trust/dissertation/html/wheeler-trusting-trust-ddc.html) include the infrastructure surrounding compilation. [Malka et al.](https://arxiv.org/html/2607.24888v1) demonstrate why that scope matters: a compromised bootstrap `strip` rewrites its clean-source successor and propagates through the distribution build. Their implementation targets particular ELF layouts; it supplies an attack pattern, not evidence of an attack on this platform. R-06-014 owns the resulting bootstrap diversity scope and evidence, including transformations after compilation, while R-13-010a already includes stripped bytes in admitted-artifact correspondence.
...
Its family is the entire L0–L3 methodology of §10: **SFSCQ / DiskSec** contribute machine-checked **data non-interference** (the L3 confidentiality method); **RefFS** contributes concurrent **linearizability and liveness** under fair scheduling through its **MoLi** *dynamically layered definite releases* framework. The [OSDI 2024 paper](https://www.usenix.org/system/files/osdi24-zou.pdf) excludes crashes and leaves mechanized soundness of MoLi as future work, so RefFS supplies no crash theorem. **VeriBetrFS** contributes the write-optimized **B^ε-tree** index design (L1); **Perennial / GoJournal** contribute the concurrent crash-safe write-ahead **journal** (Iris/Coq: the L0 trunk), with **DaisyNFS** the top-half-transaction-over-journal *layering template*. The [source-qualified proof inventory](../assurance/proof-reuse/systems.md) records the edition and assumption limits, including DiskSec's static-permission proof and its incomplete dynamic-permission revision.
...
§9 is that chain, sharpened: an **OpenTitan-class RoT** provides measured boot, key storage, TRNG, monotonic counters, and boot-attempt counting, with the chain RoT → verified M-mode firmware → per-core kernels → static image, every stage measured and the post-quantum schemes split by verifier: SLH-DSA for every ROM-verified object, ML-DSA on the replaceable paths above it (R-05-058c), plus A/B images with RoT boot-counting auto-revert and a monotonic anti-rollback floor (§9, §11).
§15 then goes past ChromeOS by **placing the bespoke OpenTitan-class root on-die** as the platform's *only* management processor (removing the discrete-RoT interposer/probing surface and making attestation coverage total), the "no foreign computers" mandate (§4) applied to the root of trust itself.
ChromeOS supplies the verified-boot product template and OpenTitan the open reference lineage. The local ISA, verifier split and proof obligations are this platform's own design, not claims about an unmodified upstream block or an implemented local root.
...
That is the same trade §15 makes, reached independently and paid for at the same price, and it is the citation [absence-contract.md](../hardware/absence-contract.md) wants beside its own refusals, because a firmware project that gave up an entire architecture rather than accept a blob is stronger evidence than any argument that the refusal is coherent.
...
Ported userspace (UI layout, coordinate math, general numerics; §12/[userspace-porting.md](../implementation/userspace-porting.md)) uses scalar float pervasively, and the standard RISC-V `lp64d` ABI passes it in `f` registers; folding it onto VL=1 vector ops means a **soft-float-register ABI** and per-operation `vsetvli`/`vmv` setup.
...
## Cerebras and all-SRAM machines: the refresh-free precedent and the two-class divergence

Cerebras supplies a large-scale precedent for an all-SRAM, cacheless machine; embedded tightly-coupled-memory systems supply the smaller one. This platform takes **on-die main memory on the same die as the cores**, native capability tags and explicit placement, accepting lower capacity and a custom physical program to keep the memory path statically analyzable.

The current design departs from that precedent: **first-class 6T SRAM** holds the scalar working set and **second-class oxide-semiconductor 2T0C gain-cell decks** hold bulk data (R-15-247). Placement is fixed at composition; no cache, migration or promotion moves data between them. Each class has fixed operation latencies, not the same latency, and bulk refresh and scrub consume fixed schedule reservations. [imec's capacitor-less IGZO result](https://www.imec-int.com/en/press/imec-demonstrates-capacitor-less-igzo-based-dram-cell-400s-retention-time) supports the long-retention cell lineage; it does not qualify the complete integrated macro.

**What is deleted, and what remains.**
First-class SRAM stores bits in bistable latches and needs no refresh while powered; it still has static leakage and local stability and disturbance constraints. The second class retains charge and requires scheduled refresh. Both omit access-reactive RFM/PRAC feedback and conventional DRAM activate/precharge and row-buffer state. R-15-184 owns the per-class disturbance residual and R-15-247h the bulk maintenance schedule. The timing gain is a known per-class access and service relation, including those reservations, rather than a universal SRAM constant or an absence of all maintenance.

**Physical media and placement policy are separate choices.**
An SRAM/DRAM hybrid can use static placement; heterogeneity does not require reactive migration. The current design still declines conventional DRAM because its actual controller, refresh, disturbance, interface and proof obligations must earn their place under the admission tests. Q6 compares those concrete costs on one workload. Non-volatile working memory additionally makes power-off retention a designed property; first-class volatility does not itself establish erasure, and second-class remanence is already a named residual. These are medium-specific questions, not consequences of the word hybrid.

**Density and leakage are qualified physical costs.**
SRAM's larger cell and idle leakage motivate static cell and process levers: high-Vt storage, low-leakage periphery choices, fixed assist and the qualified sequential tier structure. Gate-all-around and CFET density levers are confined to passive upper tiers. **Backside power remains refused** because opaque metal occludes the IRIS optical path; no low-voltage benefit is credited to a mechanism the design excludes (R-15-160). Chiplets and bonded stacks remain excluded (R-15-162).

Capacity claims use the current budgets in R-15-170, R-15-173a and R-18-004b, with metadata above payload and actual overheads at qualification. They are not extrapolated from an all-SRAM stack at a leading-edge node. The adopted bulk class removes the superseded choice between a mature complementary backend and a different single-planar release roster: one roster must meet the floor on the admitted two-class configuration. The complete SOI, sequential-SRAM and gain-cell process stack remains an assumption no foundry supplies (R-17-063b); a cell result or a more mature component process does not discharge it.

**What the adoption does not establish.**
On-die placement deletes the external memory interface, which is the main scope argument for declining memory cryptography, but it does not close power-loss remanence. R-15-200 qualifies the actual first-class memory, registers and complete sensitive-state and key lifetime alongside the bulk macro. A completed zeroize and uncontrolled power loss are different cases; natural decay provides no sanitization guarantee (R-15-247c). Native tag bits remove a tag-table/cache mechanism, while density, power, disturbance, remanence and physical-inspection limits remain governed by their own contracts.

The transferred architecture is therefore static placement, native tags, no cache, no autonomous DRAM power policy and no access-reactive refresh controller. It carries the bulk class's fixed refresh and scrub schedule, qualification costs and physical residuals explicitly.

---

## Cacheless RISC-V and explicit scratchpads: explicit placement without a reactive hierarchy
...
The two-class design retains a latency and bandwidth difference between first-class SRAM and bulk gain cells. Caches could hide some of that difference, but would add history-dependent timing, flush rules and coherence. The design instead admits the slower bulk constants and places data explicitly.

**What the deletion buys, on the scarce axis.**
On-die placement does not make every memory access as fast as a core or eliminate differences between classes. The scarce-axis gain is removing a hit-or-miss distribution and its hidden state, while charging each operation's fixed class-specific latency and scheduled service in the timing model.
...
The cache hit-or-miss term leaves the WCET analysis. The remaining memory terms include the target class's operation constants, TDM wait and scheduled maintenance; fixed latency does not mean immediate issue acceptance (§11, §15).
...
CHERIoT supplies the more natural object model for a statically composed purecap machine: capabilities live directly in registers and tagged memory, object types are sealed, and temporal safety combines a load filter with a software protocol that closes live-register authority before reclamation. This platform uses address-keyed bits, a kernel epoch and a bounded barrier; R-08-004b admits no capability colour.
...
The CHERI-side obligations are explicit: the per-load filter blocks reloaded stale authority; the bounded register, loan, proxy and device barrier establishes containment; the sized background sweep and complete holder map gate reuse; and the quarantine pool prices forced-sweep denial of service to the aggressor (R-08-005 through R-08-009). The epoch labels the protocol event and does not replace any of those premises.
Its required proof includes userland capabilities in registers and tagged memory that a kernel derivation tree does not record; the hardware load filter alone does not establish that coverage.
...
The residual kernel proof includes the novel work §7 names and the explicit R-08-006/R-08-007a containment and reuse obligations: the purecap CHERI-C refinement, the multikernel non-interference composition, and the switcher and sentry verification against the Sail model.
...
Confinement was CHERI's before this change (R-07-006, R-08-003); revocation requires the end-to-end CHERI-side barrier and reuse proofs, and the grant slot plus its loan discipline covers the ancestry case; typing and rights move from CNode fields to sealing and permissions, which are unforgeable in hardware rather than in a proof.
...
The containment barrier and holder-map proof remain explicit software obligations; deleting the tree does not discharge them.
...
The extrapolation assumes that the CHERIoT lineage's answer (capabilities in registers and tagged memory, objects named by sealed capabilities, revocation through an address-keyed load filter and a bounded live-authority barrier) is sufficient for a kernel whose object graph is fixed at composition, and that nothing in seL4's dynamic object machinery is load-bearing once that graph cannot change.
...
Kernel objects are placed by §8's composition-time memory plan and installed by the §9 boot firmware under R-07-028's existing initialisation-refinement obligation; they are named by sealed CHERI capabilities over a composition-fixed otype set; revocation is the address-keyed bitmap, bounded barrier, kernel epoch, sweep and load filter, without a capability colour.
...
## Griotte: qualified switcher-proof prior art

[Griotte](https://arxiv.org/html/2609.01110v1) (September 2026) gives a Rocq/Iris model of a CHERIoT-like machine, a verified switcher and modular reasoning about calls into unknown compartments. Its stack-safety and no-capture reasoning is close to the switcher boundary this platform owes. The model is idealized: it omits concurrency and exceptions/interrupts, and supplies neither this platform's confidentiality theorem nor physical reclamation correctness. The [artifact](https://github.com/logsem/griotte) also declares machine/layout premises and functional extensionality; those assumptions must be read, not hidden by the shared prover name.

**What the platform takes.** R-15-069 requires bounded source qualification before analogous fresh proof authoring: replay selected proofs, report exact assumptions, and map one known-to-unknown call across actual permissions, finite otypes, calling convention, stack/register clearing and Sail primitives, with timing and interrupt differences explicit. Read the exact licence before incorporating code. Classify reusable lemmas, adversarial examples and new local obligations separately. An example that exposes a semantic mismatch is useful evidence even when no proof transfers.

**Honest residual (§17).** A successful upstream replay proves its own machine statement. Local transfer still needs the proved primitive and semantic connections required by the existing proof-reuse qualification; source reading creates neither those prerequisites nor a second trusted machine model. No local switcher or revocation proof is marked complete by this lead.
...
The platform requires the CT judgment on every admitted binary regardless of producer. A compiler's preservation theorem can establish a useful property, but does not replace this common admission path. Its lowering techniques and regression cases remain usable as untrusted producer inputs under R-05-066.
...
- **Coverage.**
  ...
  A key-handling server or PIN-handling app that touches secret-labeled material therefore owes its own binary CT evidence in addition to memory safety (R-05-074, R-13-020). A non-exportable key handle does not expose the key bytes to its holder; the component operating on those bytes remains subject to the secret-material obligations.
...
- **Scope is a labeling obligation, not a blanket tax.**
  CT is required of every artifact that touches **secret-labeled** material, regardless of arrival channel (R-05-074, R-13-020); ordinary apps that touch no secrets carry no CT obligation.
...
[userspace-porting.md](../implementation/userspace-porting.md) states the consequence on the side that meets it, the browser's.
...
- **Iris-Wasm: the confinement statement worth citing.**
  ...
  [Iris-MSWasm](https://iris-project.org/pdfs/2024-oopsla-iris-mswasm.pdf) (OOPSLA 2024) mechanizes handles for finer spatial and temporal protection inside the guest. That is a distinct boundary from CHERI confinement of the interpreter's arena: ordinary Wasm can still overwrite adjacent objects in linear memory ([WebAssembly security model](https://webassembly.org/docs/security/)). Core Wasm remains admitted; a concrete consumer needing internal object safety reopens the R-14-013a comparison with safe-source compilation and finer module separation, including host-call lifetime rules and bounded metadata, memory and execution costs.
...
**What the platform takes.** One pure-interpreter engine as a §13 library compartment, its soundness and confinement theorems consumed from the WasmCert/Iris-Wasm lineage against the R-14-013b pinned semantics; SpecTec as tracked upstream; conformance and differential execution as untrusted producer-side evidence; the MSWasm handle model as relevant prior art with an explicit consumer-driven reopening condition.
**Honest residual (§17).** The fidelity of the pinned semantics to the Wasm the world compiles to is the R-17-016b agreement gap on a language, booked and never claimed closed; the theorems say nothing about what an embedding chooses to expose to its guest or object-level spatial and temporal safety inside a linear-memory arena, which remain explicit boundaries of the offer; and no verified JS engine exists to extend the offer to the browser's other half, a declination R-14-013c records.
...
They are recorded here rather than imported because §15's TDM fabric already *is* that answer inside one die, and across an ensemble link (§15) the slot tables are emitted by the same composition act rather than obtained by a standard's negotiation over a switched bus this design does not have; the link is point-to-point and statically slotted, so the standards' arbitration has nothing to arbitrate there either, and what they supply is the precedent that a scheduled wire holds its instants under a bounded skew, beside the Groq entry (above) as the industrial one.
...
§7's single address space takes the same position deliberately, and [userspace-porting.md](../implementation/userspace-porting.md) is where the bill is itemized: every entry a source-level re-target rather than a binary that runs, with ambient POSIX authority re-expressed as explicit capabilities one obstacle at a time.
...
Static structure decays into dynamic API wherever nothing forces it to stay static, which is the argument for keeping the composition rules in [requirements-register.md](../requirements-register.md) and [absence-contract.md](../hardware/absence-contract.md), where a proof obligation and an auditor's search hold them, rather than in a runtime that can be extended by anyone in a hurry.
...
R-15-115 forbids an autonomous texture engine, and [architectural-alternatives.md](architectural-alternatives.md) books the lower peak throughput of the no-JIT software renderer. That engine-level decision does not preclude evaluating a bounded register-only arithmetic operation under the ordinary ISA admission rules; it admits none by itself.
...
The first-release work includes compositing, 2D and text, codecs and the ISP; whether a particular operation dominates is Q8's measurement, not a conclusion from the 2008 workload. [Intel's Larrabee paper](https://www.intel.com/content/dam/develop/external/us/en/documents/larrabee-manycore-164179.pdf) attributes its filtering gap to several features together, including arithmetic width, gathers, register bandwidth and decompression; its sampler also carries a texture cache and memory machinery. No quoted speedup isolates the value of a register-only operation on this profile. Q8 tests RVV lowering and fusion first, then a remaining hotspot may justify comparing a core-issued register primitive with fixed timing and exact semantics. Ordinary capability-checked loads still supply its operands, and no hidden memory access, cache or scheduler is admitted.
...
A second radio die is not used because it would add a package, SerDes protocol, clock-domain boundary, second attestation problem, and second object for supply-chain inspection while sharing the same mask-set risk; an ensemble link (§15) pays each of those once per link and buys a whole second machine (§2), which the bifurcation would not.

Clocking uses the **common-PLL spine and integer-divider hierarchy** of R-15-195. Internal islands retain separate operating points but their modeled crossings use deterministic ratio synchronizers at composition-fixed clock relations. R-15-196 separately bounds the genuinely asynchronous RoT slow-clock, external and reset boundaries; R-15-197 does not assert a constant over metastability. GALS is lineage, not permission to introduce independent internal clocks outside that contract.
...
The residual includes leakage power analysis against arrays held in long retention, a different exposure window from the crypto core's operating analog channel. The same low-leakage and retained-power choices must also appear in R-15-200's power-loss remanence qualification; a retention lever is not evidence of rapid erasure.
...
Baochip-1x provides a production example of packaging for IRIS inspection, not transistor-by-transistor verification against RTL. The [IRIS paper](https://arxiv.org/abs/2303.07406) and [Huang's SRAM imaging analysis](https://www.bunniestudios.com/blog/2026/on-reading-srams-in-ir-images-and-establishing-bounds-on-trust/) distinguish visible macro structures and electrical checks from individual bitcells that the 22 nm images do not resolve. This supports obtainable physical evidence at that resolution. R-17-061 and R-17-063 require the actual process, preparation, resolved structures and complementary checks to be named; the demonstration does not qualify the proposed multi-tier SOI stack or close its below-resolution adversary.
