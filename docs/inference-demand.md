...
**Bits per parameter is not a sufficient statistic for quality, and one row of the ladder proves it.** `Q5_0` costs 13.1% more bytes per token than `Q4_K_M` and buys nothing: its top-1 agreement is **lower** (84.848% against 85.064%), its perplexity is **higher** (33.8962 against 33.6094), and only its mean KL divergence is marginally better. The legacy `q5_0` format carries one scale per block of 32 and no super-block structure, where `q4_K` carries a two-level scheme, so **`Q4_K_M` dominates `Q5_0` on this model at both ends**, on bytes and on quality together, and a reader ordering formats by nominal width would pick the worse one. It is also the file whose block type this backend does not repack, so it is dominated on host throughput as well.
