...
**Two conditions qualify every timed figure and neither qualifies the bytes.** The first is power: one run, `llama-bench` over Q4_K_M with an f16 cache, was taken **on battery, discharging**, and every other timed figure was taken **on AC, charging**, which the guest reports at `/sys/class/power_supply/*/status` and each stage stamp in the run log records; the table in section 6 carries the state per row. The second is contention: nine sibling agent lanes ran on the same twelve cores throughout, one of them a proof build, so every throughput figure here is a lower bound taken under a load that was neither constant nor measured. **The bytes-per-token, resident-bytes, buffer-size and budget figures are power-independent and contention-independent, and they are the load-bearing evidence.**
...
**This table does not order by bytes per token, and that is a measurement about the conditions rather than about the model.** Its eight rows were taken in eight windows spread over four hours beside nine sibling lanes and across a power-state change, one row's standard deviation reaches 37% of its mean, and the widest file is the fastest on five of the eight columns. **So nothing here is offered as evidence for or against the bandwidth-bound shape**; what the rows establish is that a throughput figure taken this way carries no ordering information, which is why the budget below is derived from bytes and not from any of them.

**`llama-bench`, one invocation over every file.** The comparison the shape needs is one process, one window, the files interleaved by the tool's own loop rather than by a script: `-p 256 -n 64 -d 0 -t 12 -r 3` with an f16 cache and every file passed to a single invocation. The last column is the derived quantity the claim is about, generation throughput times the bytes read per token, which is the rate at which the configuration actually moves weight bytes.

| File | Bytes read per token | pp256 at depth 0 | tg64 at depth 0 | Byte rate achieved at tg64 |
| --- | --- | --- | --- | --- |
| Q4_K_M | 2,491,326,004 | 120.65 ± 11.46 | 3.251 ± 0.696 | 8.10 GB/s |
| Q5_0 | 2,817,756,724 | 26.81 ± 2.41 | 2.247 ± 0.138 | 6.33 GB/s |
| Q5_K_M | 2,883,558,964 | 56.88 ± 6.88 | 2.146 ± 0.074 | 6.19 GB/s |
| Q6_K | 3,300,306,484 | 50.54 ± 4.98 | 1.824 ± 0.096 | 6.02 GB/s |
| Q8_0 | 4,274,451,104 | 117.91 ± 11.03 | 1.777 ± 0.130 | 7.60 GB/s |

**Generation is monotone decreasing in bytes per token across all five files, and the byte rate is flat within a factor of 1.35 across a 1.72-fold spread of bytes per token.** That is the bandwidth-bound shape, and it is the one throughput result this report offers as evidence: a configuration reading 72% more per token generates at 55% of the rate, and what stays roughly constant is bytes moved per second rather than tokens produced per second. The prompt-processing column does not behave that way and is not expected to, prompt processing at a 512-token micro-batch reading each weight once for many tokens, and `Q5_0`'s low prompt figure is the same absence of a repacked matmul kernel that section 5 identifies in its buffer sizes. **This run spans a power-state transition and its bias runs the same way as its result**, the guest reporting AC at the invocation's first stamp and battery two minutes after its last, so a monotone slowdown over the run would produce this ordering on its own; the files were passed in ascending order of bytes per token, which is the ordering the transition would fake. That is why the run was taken a second time, whole, inside one power state.

**End to end.** One `llama-completion` run per file at `-c 8192 -n 128 -t 12 --temp 0.7` over a fixed prompt, the first 24 lines of [the spec](spec.md) at this revision ([prompt.json](inference-demand/prompt.json) carries the text and its SHA-256), with `-no-cnv` so no chat template is applied, on AC. The figures are the `common_perf_print` lines the tool prints: `prompt eval time` is first-token latency with tokenization included, `eval time` is per-token generation with sampling included, and `total time` is the whole. The prompt tokenizes to 556 tokens and 127 tokens are generated after the first.
...
| File | Bits per parameter | Mean KL divergence | Top-1 agreement | Perplexity | ln(PPL/PPL base) |
| --- | --- | --- | --- | --- | --- |
| Q4_K_M | 4.955 | 0.095627 ± 0.002759 | 85.064 ± 0.498 % | 33.6094 | 0.10535 ± 0.00876 |
| Q5_K_M | 5.735 | 0.015998 ± 0.000364 | 93.177 ± 0.353 % | 30.2817 | 0.00108 ± 0.00392 |
| Q6_K | 6.564 | 0.008798 ± 0.000544 | 95.073 ± 0.303 % | 30.3026 | 0.00177 ± 0.00292 |
| Q8_0 (the base against itself) | 8.501 | 0.000000 ± 0.000000 | 100.000 ± 0.000 % | 30.3696 | 0.00398 ± 0.00161 |

**The base row is the instrument's self-check and it passes**: the widest file scored against its own logits gives a mean KL divergence of zero and 100.000% top-1 agreement, so the comparator's zero is where it should be. Its non-zero `ln(PPL/PPL base)` of 0.00398 is the one figure of that row that is not zero and is a per-chunk-ordering artifact of the tool's own ratio column rather than a difference between the file and itself, which the KL and agreement columns settle.

**The comparator is monotone in bits per parameter and the four-bit row is where the quality falls fastest.** Between Q4_K_M and Q5_K_M, 0.78 more bits per parameter divides the mean KL divergence by 6.0 and lifts top-1 agreement by 8.1 points; between Q5_K_M and Q6_K, 0.83 more bits divides it by 1.8 and lifts agreement by 1.9 points. So the format the floor names, four bits, is on the steep part of this model's curve, and the format one step above it costs 16% more bytes per token for that difference.
...
$ cmake --build build --target llama-bench llama-completion llama-perplexity llama-cli llama-gguf
...
- `bench-<quant>-kvf16.json` and `bench-<quant>-kvq8.json`, the rows `llama-bench -o json` reported, and [bench-controlled-kvf16.json](inference-demand/bench-controlled-kvf16.json), the one-invocation run.
