...
> Non-normative Q21a design record. This proposed bounded interpretation and qualification contract supplies no implemented frontend, proved metatheory or production qualification. The [requirements register](requirements-register.md) remains authoritative. [The strategy](verification-language-strategy.md#a-compositional-core) states the wider objective; [the implementation checklist](implementation-checklist.md#q-assessment-actions) owns scheduling.

## Decision and boundary

Select a synchronous core with stable dependent values, explicitly sequenced computations, resource-indexed exits, fixed representations and a finite loan tree. Qualification covers fallthrough, ordinary return, `Result` propagation, branch-sensitive returned shared loans, bounded loops with `break` and `continue`, statically disjoint fields and nested reborrowing. These are commissioned design cases, unavailable as a Vela implementation today. Q19's callback pilot establishes none of this flow analysis by itself.
...
`PrepareAndInspect` develops the parser-derived private-copy extension. Its notation is illustrative, with no available API claimed. `Workspace` contains statically disjoint `bytes : Buffer(N)` and `audit : Cell(word)` fields. Capacity `N` specializes to a representable bound; input length `n` remains represented. Input stays immutable during this sequential fixture under its entry permissions; no concurrent snapshot is claimed.
...
```text
PrepareAndInspect(ref workspace, in input[n], runtime keep_view,
...
Inspection controls are separate from header validity. A break never denotes a successfully partial copy. The callback is total and effect-free, with disjoint captured representation; its unused result cannot alter the intended bytes. Audit begins at zero and counts inspected indices, including the iteration that takes a break. Each iteration increments its cursor exactly once before any continue, and the bound `n <= N` proves the audit increment representable. Break/continue controls are supplied in a public immutable vector with length `n`; continue skips no required copy or initialization work, which precedes inspection.

The source trace records `ReadInput(i,b)`, `WriteDestination(i,b)` and callback invocations. On an admitted length, each input index below `n` occurs exactly once, in order, and no other input index occurs. Destination initialization precedes copying; validation/inspection read only the private destination. These are explicit functional/trace obligations in addition to set-valued effects. Transport to machine observations remains the route's proof obligation, and an optimized implementation need not retain source helper-call events as actual machine calls.

On `too_large`, the entire destination is initialized to zero and no input byte is copied. Otherwise it holds the input prefix followed by zeros. `invalid_header` corresponds exactly to the header check's invalid answer on that private prefix; both `Ready` and `view_not_requested` require its valid answer. Audit is zero on pre-inspection failures and otherwise equals the inspected-index count. `Ready` retains real `n` with evidence `n <= N`, with no decoder consumed-length claim. All exits return `PreparationClosed`, preserve input and unrelated storage, and restore workspace ownership except for the returned prefix loan on `Ready`.

| Point | Required resource predicate |
| --- | --- |
| Entry | Exclusive workspace, immutable input loan, disjoint repeatable callback, `PreparationOpen`, effect permissions and satisfiable representation |
...
| --- | --- |
| Make the copy guard inclusive with the same invariant | Read/write bounds at index `n`, instantiated at the boundary |
...
| Continue with a local loan or propagate error with `PreparationOpen` | Loop invariant or error cleanup predicate |
...
The observation relation treats input bytes, length, inspection controls and status as public. Equal public inputs and related frames require equal specified branch/address/termination and external-event observations, including the callback's contract. This modest relation still needs proof; it supplies neither adversarial-context isolation nor target timing. A secret-dependent replacement must fail the relation when that input is designated secret. Reclassifying it public changes the independently reviewed contract and invalidates the comparison.
...
The following owner keys are proposals, not schedule IDs or proof constants. Each selected package needs a reviewed checklist cell and entry predicate before implementation dispatch. Estimates are attended agent-session design judgments, not measured results or conversions of the strategy's person-month bands. They exclude presently unbounded foundation work and therefore do not establish a complete implementation price. The general-logic and machine connections remain [Q2c's prerequisites](implementation-checklist.md#q-assessment-actions); scoping those supplies no law.
...
| `core-repair-trial` | 48 h, range 32-64 | Author and qualify the combined fixture/negative neighbors, then measure semantic/refactor/representation replay and review. Excludes a repair service and Q20's held-out staging adaptation. |
...
| Byte representations, read/write/frame, initialization, loan and cleanup laws | `DescriptorCheck` uses another memory model; `CopyRingService` is a statement artifact. No concrete CHERI instance exists in this record. | Q2c assigns checked foundations; Q2b/M7.1 implement consumers. New packages consume their actual constants. |
...
| Observation, cost and robust guarantees | An annotation or closed effect set supplies no theorem | `core-soundness` composes supplied laws; Q2c/Q21b retain concrete connections. Missing law blocks its requested case. |
...
| Device lowering, optimization, linkage and bytes | Actual producer state and missing connections are Q21b's inventory | Q2b and existing M1/TAL owners, or separately priced route extensions |

Design acceptance requires Tier-A review of the composed fixture, branch/loop predicates, shared source/IR contract and every missing law's ownership, plus integrated document checks. An inconsistent interaction is a reasoned rejection. The document checker decides corpus rules, not metatheory. Execution additionally requires positive cases replaying at the admitted assumption set, well-formed negatives failing at intended premises, satisfiable entry witnesses, independent behavior checks and current diagnostics after edits. Unavailable cases stay unavailable. Production qualification retains Q20c's stronger endpoint and actual prerequisites.
