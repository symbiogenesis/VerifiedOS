...
*Q23e's completed static calculation covers layer-wise and tensor-wise sharding of the declared model over two, four and eight members: bytes per token per link, serial exchanges, frame demand and a symbolic rate formula. Every byte is derived from the declared tensor sizes; no figure is a device measurement and no host time from [the inference demand report](inference-demand.md) enters the calculation. Open Q23h in [the implementation checklist](../implementation/implementation-checklist.md) owns measured crypto costs and admission of a concrete rate against the qualified composition.*

**What this report decides is the ordering of the two terms and not the bandwidth question.** At the floor's rate the busiest shape below moves 6.45 MB/s on its busiest link, and **at any line rate above that figure the exchange count and not the byte count is the binding term**: what separates the shapes by two orders of magnitude is the number of serial exchanges one decode step contains, because R-11-017a charges each of them a slot period plus a guard band and R-12-015d charges each frame of it to the crypto core. Whether a link clears 6.45 MB/s is not decided here and is not decidable here: R-12-015d bounds the line rate above by the crypto core's authenticated-encryption throughput, which section 1 records that no artifact in this tree declares, and Q23b's link contract owes the line-rate bound itself. So the traffic tables below are read as slot-table demands rather than as bandwidth demands, which is what R-15-171a means by *the frames per step fixed by those tables and never by the tokens*.

Every figure is taken on [traffic.json](ensemble-sharding-traffic/traffic.json), which is [shard.py](ensemble-sharding-traffic/shard.py)'s output whole over the tracked [static-Q4_K_M.json](inference-demand/static-Q4_K_M.json), at revision `59ac376`, which is the revision of that static file as the arithmetic reads it and the argument the invocation supplies. Each figure's predicate is stated where it is derived and carried in that file's `predicate` field. Two inputs come from outside the static file, its `n_head_kv` of 8 and its `n_embd_head` of 128, which the file does not carry: they are read from [the inference demand report](inference-demand.md)'s section 3, are named as that section's in `traffic.json`'s `head_counts_owner`, and feed only the cache columns of section 8.
...
- **The encode and decode cost per frame.** R-15-228e makes each one entry in the endpoint's device row of the timing-annotated model, and R-15-119c says outright that *the magnitudes are R-17-041's to fix and not this entry's*. The model's table landed at M0.9 and it carries **core operation classes and no device row of any kind** ([core/timing.sail](../../model/model/core/timing.sail)), its `qualified` field is `false` in the shipped configuration ([verifiedos.json](../../model/config/verifiedos.json)) and every number in it is a placeholder that file says is one, and R-17-041's crown-jewel row over the magnitudes is unauthored. So there is no magnitude to quote and none is invented; what the two constants do have is a **shape**, one fixed latency per direction independent of the frame's symbols and of whether it was correctable, which is what lets them enter the arithmetic below as symbols.
...
| Embedding placement | replicated in residency and column-sharded in the per-token read under tensor-wise sharding, so the input lookup needs no exchange and the head costs each member one column slice; resident on the first member under layer-wise | R-15-171a names the member a shard sits on; R-08-012a places the object once the composition has named it |
| The rate the arithmetic is read at | R-18-004a(vii)'s five tokens per second over its 8,192-token context, read on one member | No entry states a token rate an ensemble owes; the reading is this report's, on R-02-003a |

**Three of those six deserve their reason rather than only their owner.**

**The topology is a declared cycle, and the choice is priced on the binding terms rather than on traffic.** An endpoint is a block and not a port: R-15-228c makes each one matter carrying its own absence-contract rows A-18 through A-21, so a cycle costs each member two endpoint blocks at every member count where a fully connected ensemble costs `M-1`. What the cycle spends for them is serial exchanges. Under tensor-wise sharding at eight members the declared cycle's ring collective takes 1,008 serial exchanges per step where a fully connected ensemble takes 72, a fully connected all-reduce reducing in one serial position because each member sends its whole partial concurrently on its seven links and sums locally; on section 7's arithmetic that is 198 us of cadence against 2.78 ms. The alternative is not free on the term section 8 makes the deciding one: it charges each member `2(M-1)` frames per serial position, **25,200 crypto operations per second at eight members against the ring's 10,080**, and four times the per-member bytes besides. Both sides are in [traffic.json](ensemble-sharding-traffic/traffic.json)'s `alternative_topology` block at the default width and payload. Neither shape is the register's, and what the pricing establishes is that the topology is a trade between the cadence term and the crypto term rather than a question of traffic saved.
...
**The floor's rate is read across to a member, and that reading is this report's.** R-02-003a says in terms that the ensemble is a later instantiation than the first release R-18-004a fixes and that no member of that floor depends on a second machine, so R-18-004a(vii) places no obligation on an ensemble; what licenses reading its rate and context on one member is the same entry's *every member of an ensemble meets every entry of this register alone*. Every figure in sections 6 through 9 rests on that reading, and there is no rate an ensemble owes to read instead.
...
**The head is tied and that is a term of the partition rather than a detail.** The file carries no `output.weight`, so `token_embd.weight` is the language-model head read whole once per token as well as the input lookup's matrix. Under a layer-wise split it sits on one member, which therefore reads 319,065,600 more bytes per token than any other, and that asymmetry is what decides the two-member verdict in section 8. Under a tensor-wise split it is column-sharded by vocabulary and the asymmetry disappears, the rows below charging each member one column slice of `head_bytes / M` per token. Its residency is the other half of that placement and is deliberately not tabulated here: a replicated 319,065,600-byte embedding is a per-member capacity charge R-15-171a's per-member fit would see, and this report costs the per-token read and never the residency.
...
**Eight is the largest tensor-wise member count this model's geometry admits without splitting an indivisible object.** The model has 8 key-value heads, which is [the inference demand report](inference-demand.md)'s section 3 and not this file's, the static file carrying no head count; at eight members each member holds exactly one. Its FFN width of **9,728** divides by 2, 4 and 8 exactly (4,864, 2,432 and 1,216), and that width is derived here rather than read anywhere: layer 0's `ffn_gate.weight` is 14,008,320 bytes at `q4_K`'s 144 bytes per 256 elements, so 24,903,680 elements over a hidden width of 2,560. Both divisions are exact or [shard.py](ensemble-sharding-traffic/shard.py) refuses the file, and the width is carried in [traffic.json](ensemble-sharding-traffic/traffic.json)'s geometry block with that predicate. And 36 layers over 8 members is five layers on four members and four on the other four, which is where the layer-wise slowest shard is.
...
The two columns are the same measurement read two ways, and the second is the one an admission check uses. R-11-017a refuses a member's artifact on three conditions and this table feeds two of them: where a guard band on any of its links falls below the composition's skew bound plus that link's latency bound, and **where the link server's slot does not cover the frames its table grants it**, which is what section 6's frames-per-step table is an input to.

**What the link's latency bound is made of is a reading of this report and not a quotation.** R-02-003a and R-15-196a call the quantities *the link's latency constants*, measured on a two-member pair at qualification and entered in the attested devicetree beside the skew bound, and Q23g owns that measurement; R-15-228e separately makes the encode and the decode one entry each in the endpoint's device row. This report reads those two constants as what the link's latency bound is over, because they are the only fixed per-frame latencies on the path, and says so rather than attributing the identification to an entry. On that reading, at 198 us the guard band, the skew bound and the endpoint's two constants have to fit inside a fifth of a millisecond, and none of the four has a magnitude yet.
...
**Two members bring the weight stream inside the grant floor under both shapes**, which is what the split buys on the member, and with a `q8_0` cache added tensor-wise fits at 0.98 while layer-wise does not, at 1.08 on the member holding the tied head and 0.88 on the other. The 319,065,600 bytes of that head are the whole of the difference, so the asymmetry is a placement question and not a property of layer-wise sharding. **Which member holds the tied head is the composition's act under R-15-171a**, whose rule is that every shard is resident on the member the composition names; where the object then sits on that member is R-08-012a's whole-program memory plan, which colours one machine's islands and decides a class rather than a member. At four members and above every row is inside the floor under both shapes with either cache format. R-15-171a's rule that no single session's rate rises above what one member's grants fix stands unmoved by any of it, and what the split buys **on this term** is grant headroom: section 9's first term is `min over members of G / (W + K)`, whose value rises as the shard shrinks, so at a fixed grant sharding raises that ceiling. Whether the headroom becomes rate is decided by the two terms under it, neither of which this tree can evaluate.
...
with `F` the per-direction frames per step of section 6 and `E` the serial exchanges of section 7, and where the link server's slot covers the `F` frames its table grants it, which is R-11-017a's third refusal condition and the one the section 6 table is directly an input to. The guard band's own floor is the skew bound plus the link's latency bound, which section 7 reads as `t_enc + t_dec` and marks as this report's reading. So:

- **Layer-wise at two members is admitted on every term this tree can decide.** Its cadence bound of 100 ms is the slackest of the six and two orders of magnitude above every tensor-wise row, which is as far as the comparison goes, no artifact stating a slot period to compare it against; its crypto charge is 50 operations per second at a 2,048-byte payload; and its only tension is the head holder's 1.08 grant floors with a `q8_0` cache, which R-18-004b meets by a composition declaring a grant above the stated minimum exactly as the single machine's 1.56 must be met.
...
- **Above two members tensor-wise over the declared cycle and ring is refused on cadence before `t_aead` is reached, and the refusal belongs to those two parameters rather than to tensor-wise sharding.** At eight members the 198 us bound has to contain the guard band, the skew bound and both endpoint latency constants, and an ensemble schedule that meets it is a claim about four magnitudes none of which exists. Neither the cycle nor the ring collective is any entry's, and the other topology section 2 prices reverses the verdict: fully connected, the same shape reduces in one serial position per all-reduce, 72 serial exchanges and a 2.78 ms bound, which is the two-member row exactly, and it is then refused or admitted by `t_aead` like the two-member case, at the same cadence bound and seven times its crypto charge, 25,200 operations per second per member against 3,600. So what this tree decides is that above two members the cadence term and the crypto term cannot both be relaxed, and which of them binds is the topology's to choose.

## 9. The admitted token rate, as a law and not a figure

R-15-171a makes the admitted token rate of a sharded model a constant of three terms, and each is owned elsewhere. The admitted rate is **at most the minimum of the three**, each being a ceiling the decode period must clear, and it is read on the slowest member because a pipeline runs at its slowest stage. **How the three compose is not stated here and not stated by R-15-171a**, which calls the rate a constant of them and supplies no composition function: what composes them is one member's R-11-006 interval arithmetic, into which R-11-017a puts the link server's slots, the endpoint's encode and decode latencies, the crypto core's per-frame operations and the endpoint's window grants *as ordinary tasks and grants*, which is a single schedulability analysis and not three independent ceilings. Q23d's emission decides it; treating the minimum as the answer would be an upper bound read as a value.
...
| The crypto core's per-frame share | `phi / (2 · F · t_aead)` | R-12-015d, the throughput a composition constant the slot tables respect | Q23h's measured crypto costs joined to Q23d's actual reserved share; no qualified value is supplied here |

This report supplies `W`, `K`, `E` and `F` for six shapes and supplies none of `G`, `phi`, `t_aead`, the slot period or the guard band. **No numeric admitted token rate is supplied.** Q23h must bind measured crypto costs and the other producers' qualified operands to one composition before reporting its admitted rate. The simple per-frame expression above describes payload demand; the final reservation must also cover establishment/rekey, authenticated idle/control frames and the admitted refusal/error paths. A symbolic calculation cannot close that measurement item.
...
- [traffic.json](ensemble-sharding-traffic/traffic.json), the geometry, the declared parameters with what owes each, the floors with their owners, the four undeclared terms with what they wait on, the fully connected alternative priced beside the declared cycle, and per shape and member count the exchange pattern, the per-link bytes, the frames and crypto operations at each payload of the sweep, and each member's second-class read against both bandwidth floors.
