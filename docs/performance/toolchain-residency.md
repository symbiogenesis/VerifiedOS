...
| --- | --- |
| [Compact static dictionaries](../background/open-math-conjectures.md#compact-static-dictionaries-and-dynamic-ordered-indexes) | Immutable symbol, dispatch or composition tables are concrete candidates for a resident producer. Prove lookup agreement with the original finite map, including absent keys, and connect the emitted bytes to that map. Charge construction workspace, randomized seeds, values, word width and purecap metadata separately from the succinct index. A static word-RAM lookup bound does not prove the target representation or improve the dynamic router by itself. |
| [Parallel matching and explicit-tree planning](../background/open-math-conjectures.md#p-versus-uniform-nc) | Reduce a concrete assignment pass to the July bipartite matching/linear-matroid result with polynomially bounded weights and explicit field representation, or expose the actual expression tree. Prove returned assignments legal and the transformed pass equivalent. Compare against the sequential baseline with finite processor, memory and communication budgets. The August Mod_p L result concerns modular-counting decisions and supplies no ordinary-logspace witness algorithm. |
| [Approximate rank in parallel](../background/open-math-conjectures.md#p-versus-uniform-nc) | The September NC^3 preprint is a candidate only for a pass accepting a `(1-epsilon)` rank guarantee over a rational matrix space, with fixed epsilon. Freeze that objective, account for rational encoding and parallel workspace, and prove the returned matrix belongs to the space with the required rank guarantee. A checked nonzero minor witnesses only a rank lower bound; it does not by itself prove approximation to the unknown maximum, exact rank, or general PIT. |
...
| [Catalytic graph, matching, sequence and streaming algorithms](../background/open-math-conjectures.md#catalytic-graph-and-sequence-algorithms) | The June polynomial-time catalytic maximum-matching result permits a general-graph assignment candidate alongside the published reachability/sequence algorithms. Freeze the graph, objective and legal-edge relation; prove the result and exact restoration of the original payload, tags, initialization and typed invariants inside one authorized quiescent region. State supported interruptions and restoration behavior. Charge input replay, catalyst capacity and restoration work; test arbitrary catalyst contents and interruption points. Match any streaming/register candidate's pass, passive-output and characteristic restrictions separately. |
...
a fault by itself. Logarithmic clean workspace in the general-matching result
still comes with polynomially many catalytic bits; the sublinear catalyst bound
of the published reachability/sequence algorithm cannot be assigned to it.
The withdrawn almost-logarithmic-total-space tree-evaluation claim supplies no
alternative bound. For every candidate, a lower host peak is exploratory evidence;
