...
The freeze is two acts (R-15-014a). The provisional act fixes everything not conditioned on measurement and is the schedule root R-18-003a names. The final act is the one the proof is taken with, and its delta is closed at nine enumerated items whose common property is that each is re-derived from measurement rather than decided on the page. **Six of the nine are measured against generated output** that does not exist until a backend and a composed image do (R-18-003c), and that set is this contract's subject. The remaining three are re-derived at that same act against quantities no image this recipe builds exhibits: two against realized macro geometry rather than against code, and one against the sustained dense-GEMM margin R-15-116 admits the matrix unit against, which is a throughput ratio this contract's byte and cycle columns cannot express. §10 says where each is decided instead.

A reproducible measurement needs three things fixed in advance: the measured artifact, the comparison baseline, and the acceptance threshold. A byte delta without its corpus cannot support a freeze decision, and an aggregate hit rate violates R-15-036k. Defining thresholds at M0, before the backend produces the corpus, prevents choosing them to fit a candidate's results. M1.8 implements the instrument against this contract's existing criteria.
...
**The Order column above is what a report's order is checked against, and §0 of the implementation plan is not.** That section states the act in one sentence naming recipe steps and decisions together, and its first two clauses name acts rather than decisions: *compose and merge first* is the merging of §3's S3 and the linking of its S5, and *select and report the dictionary* is its S6, and all three run inside every build this contract specifies rather than standing as decisions with an order of their own. Read that way the two texts say one thing, §0's remaining clauses naming FD-3, FD-4, FD-5, FD-6, FD-7 and FD-8 in the Order column's own order. Read the other way, with the dictionary clause taken for a decision, §0 would put FD-1 ahead of FD-3 and select a dictionary against a corpus the outliner had not touched, which R-15-036i forbids: outlining and tail merging are the §13 duplication-removal levers R-15-036o gives an owner, and R-15-036i orders those levers ahead of the selection they are partly substitutive with. So the Order column is the sequence a report records, §9's `G-6` reads that column and no other list, and a report whose recorded order came from §0's sentence read as a list of decisions is refused by it.
...
**Two rows of R-15-014a's delta are instrumented here and named in §0's closing clauses**, because the delta is closed and the report quantifies over it: FD-6, the further code-size candidate R-15-067e names, and FD-8, the fusion set R-15-031a selects. FD-8 is the one row whose *decision* is not this instrument's to take, membership being a composition-time parameter of the §15 design-space exploration; what this instrument owes it is its measured input, the emitted adjacency histogram, because the exploration cannot invent the mix it selects against. §10 states that boundary.
...
- **S6 runs inside every variant.** A candidate instrument changes the histogram S6 selects against, so a variant that inherits the baseline's dictionary has measured the instrument against a dictionary chosen for its absence. This is the contract's single most consequential methodological rule and the easiest one to skip, since skipping it makes every variant cheaper to build. Each variant therefore records whether S6 ran inside it, and §9's `G-13` rejects a report in which a variant that moves a knob records that it did not.
...
**Default.** There is none, and the ground is that no arm of this decision could be one. The candidates are three geometries of one format, so nothing here is carried or dropped and there is no leave-it-as-it-is to fall back on: the format has a bundle, a header and a slot count whichever candidate is admitted, and the item allocates no opcode and adds no instruction. R-15-036a's reference instantiation of 128, 16 and 7 is this item's **provisional** value and not its default: R-15-014a makes every delta item's provisional value a compilation target for the first act and re-derives the item from measurement at the second, so carrying the reference into the frozen profile because the measured deltas came out small is the decision-on-the-page that entry's closed delta exists to prevent. The threshold above already decides every outcome the sweep can reach: the bar admits a set, the smallest realized λ narrows it, the smaller bundle breaks what remains, and §8's three candidates carry three distinct bundle widths, so exactly one configuration survives wherever any clears the bar. The one case the threshold does not reach is that none clears it, and that is FD-1's precondition failure rather than a fallback here, the two being one joint sweep (§1, §3).
...
| `decisions` | one entry per `FD-` row: variants, each carrying the knobs it moves and whether S6 ran inside it, columns as §6 lists them, threshold applied, verdict, and for FD-3 the nil-row treatment of R-15-036n |
...
| `G-13` | a variant that moves a knob records no S6 re-selection inside itself, `n/a` there being a claim that a rule §3 states over every variant does not reach this one | this contract, §3 |

Three of these are worth their own sentence. `G-5` is what keeps the report honest about the strike: R-15-036n's candidate is struck ahead of the freeze, so a byte column against it is evidence that somebody re-opened a closed decision without the review gate. `G-11` is what keeps the realized dictionary from being an artifact of the sweep rather than of the admitted machine, which is the failure the closing pass of §3 exists to prevent. `G-13` is the predicate §3's third binding rule is owed. A variant that inherited the baseline's dictionary reports a number about the instrument and the stale histogram together, which is a measurement of neither, and that number is plausible rather than detectable: nothing in the delta says which dictionary produced it. So the predicate reads the variant's own record of whether S6 ran inside it, and a variant that moved a knob and records `n/a` there is refused exactly as one recording that S6 did not run is, an axis that does not apply being the one thing a rule §3 states over every variant cannot be.
...
- **Three rows in the ordered report act read against §0 of the implementation plan**, which is the text this document instantiates. The single-check multi-register save and restore is a nil row rather than a measurement, R-15-036n striking it ahead of the freeze; and R-15-014a's delta carries two rows beyond the instruments above, the further code-size candidate of R-15-067e and the fusion-set membership of R-15-031a. This contract carries all three in the shape the register fixes, and the plan's §0 states them the same way.
- **Two of R-15-014a's nine delta items are not this instrument's, and are named here so the omission reads as a boundary rather than a gap.** The welded CBO block size (R-15-007q) and the per-class bank count (R-15-247p) are re-derived at the final freeze like the six above, but from realized macro geometry rather than from a composed image, so no corpus, recipe, or byte delta here reaches them. The block size is scored against a first-class SRAM macro geometry and a second-class deck row and page geometry together, and its answer is the interval satisfying both rather than a value satisfying one; the bank count is scored against the island bandwidth ceiling, the read energy per bit bitline capacitance sets, and the R-15-247g simultaneous-activation envelope, with the droop envelope pruning infeasible points as a hard constraint rather than trading against the other two as an objective. Both are decided by the §15 design-space exploration and reported there; a freeze report quoting either from this instrument is a finding against this bullet.
- **A third delta item is not this instrument's either, and on a different ground.** The bespoke matrix extension's instruction surface, its operand sourcing, and its encoding (R-15-009, R-15-116) are re-derived at the final freeze against the sustained dense-GEMM margin R-15-116 admits the unit against, and that margin is neither a byte column nor a worst-case-cycle column: it is a throughput and per-watt ratio against a vector baseline, taken over a dense-GEMM workload no member of §2 carries and in a unit §8 declares nothing for. There is no surface to instrument either, the profile booking the extension as a unit and naming no mnemonic, operand form, or encoding, and R-15-009 landing the Sail semantics with that surface rather than ahead of it. So the item carries no decision row, no corpus member, and no threshold here, and a freeze report quoting one from this instrument is a finding against this bullet exactly as it is against the one above; what would earn it a row is a dense-GEMM member in §2 and a per-watt unit in §8, which are additions to this contract rather than readings of it.
