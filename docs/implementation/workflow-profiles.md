...
**The laptop's desktop supplied that evidence, and the deferral now binds the fixed tier.** The first release is a general-purpose laptop whose applications run in an elastic domain (R-07-037e, R-14-011c): a composition-fixed envelope inside which apps launch at runtime and grow their heaps from one verified, revocation-gated pool (R-08-047a, R-08-047b). Inside such a domain, hibernation remains, as the third rung of the pool's exhaustion ladder (R-08-047d) rather than as a slate swap, and a recipe needs no pre-enumerated companion combinations or slate bindings for its members. Everything below continues to govern every component outside an elastic domain and every composition that declares none.
...
These authored recipes guide composition. Concrete bytes, periods, maxima and latency ceilings come from the selected applications and hardware, with an explicit unavailable result for a missing port or a combination that cannot fit. They are not measurements or promised capacity multipliers. The browser remains deferred under R-18-004; no preset silently enlarges the laptop release floor.
...
| Development | Editor, source view and selected build/proof/composition phase | Only explicitly admitted terminals and services | On the laptop the build runs as elastic members under R-13-027a's elastic form, otherwise in its composition mode; outputs execute only in a successor generation; checkpoint source/diagnostics, not arbitrary compiler stacks |

Audio, radio, display, storage, revocation, consent and required inference reservations remain present wherever the product contract requires them. The ordinary mode must still meet the simultaneous laptop floor; development builds are scored among the desktop's members in the elastic form, or in their separate composition mode under R-13-027a, with that distinction explicit in every comparison. Each recipe names allowed companion apps; unsupported side-by-side combinations are unavailable instead of silently thinning budgets. A user can select a different admitted profile or compose a new generation.
