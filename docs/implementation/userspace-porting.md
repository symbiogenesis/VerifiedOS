...
Collaboration rides the §12 network stack; acquiring file authority and reading another compartment's clipboard snapshot use the §8.6 powerbox. The paste act consumes one delivery grant and briefly takes the panel and application touch through the existing trusted path; it creates no standing clipboard permission.
