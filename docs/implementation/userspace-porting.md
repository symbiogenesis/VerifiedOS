...
Four others need explicit platform choices. `memory64` is on in the upstream's default feature set; R-14-013b makes it eligible within the Wasm 3.0 reference only after curation, concrete bounded backing and both engine proofs. The same default leaves `simd` off, while R-14-013d takes fixed-width SIMD subject to its proof condition. Neither default defines the admitted allowlist. The `wat` text-format front end is a development convenience for which a §13 image has no consumer, every admitted module arriving as binary. The guaranteed native tail calls behind the upstream's `unstable` feature turn on nightly Rust's `explicit_tail_calls`, which is a host-compiler property, separate from admitted guest tail-call instructions and no evidence for this certifying backend's stack bound. And `auto-dispatch` resolves an untested compilation target to the portable fallback rather than a threaded form; the platform chooses its form explicitly below.
...
The official [Wasm 3.0 specification defines the deterministic profile](https://webassembly.github.io/spec/core/appendix/profiles.html). R-14-013b makes it a curation target: exact definitions and both engine proofs must cover the enabled instructions, including concrete numeric operations. The inspected WasmCert input does not establish that complete artifact; Q34g owns the remaining work. Growth can still fail for resource reasons.
