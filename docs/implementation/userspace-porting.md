...
4. **No runtime code generation, anywhere.**
   Compilation is a producer act finished before admission (the certifying toolchain, §5/§18), and that toolchain is resident (§13), so the device can compile: what it cannot do is generate code into a running generation, because nothing here JITs (§14) and what a build emits is data for a successor generation. Any embedded script/Wasm engine therefore runs **pure-interpreter**, and a port that wants a compiler at run time is on this obstacle whether or not a compiler is present on the machine.
