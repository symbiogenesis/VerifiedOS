...
> It is the userland analogue of [Evaluated Architectural Alternatives](../background/architectural-alternatives.md) and carries no normative weight: every target still enters through the Tier-2/Tier-1 admission discipline of §13, and a name here is a *statement of intent*, not a grant of exception.
...
The application roster defaults to **Rust**, with the [compute compatibility path](compute-compatibility.md) also targeting existing OpenCL C and HIP sources under the same admission requirements: safe Rust is memory-safe by construction (§5, §14), so a port whose whole dependency closure is `#![forbid(unsafe_code)]` is the cheapest path to the mandatory Tier-2 memory-safety certificate (§13), the certifying Rust→RV64+CHERI toolchain (§18) discharges it automatically, and rustc/LLVM never enter the trust base.
...
The same rule reaches display clients: GTK, Qt, and the engines beneath Firefox and Chromium could arrive only as source-level re-targets against that capability libc, which exposes the platform's typed display interface and does not speak the Wayland wire protocol. A wire-compatible server would therefore serve no unchanged admissible client; the Rust roster reduces source-retargeting effort without making its upstream protocols part of the platform.
...
- **Servo and the shared web application host**, the contained, per-origin browser engine of §14, also serving standalone web apps and embedded views through one capability-scoped interface (Tier-2 origin compartments). The entire browser-dependent program follows §18's deferral past the first release.
...
1. **`unsafe` needs the existing admission evidence.**
   Each FFI, pointer or synchronization site is removed when unnecessary, routed through the verified HAL, or covered by the manual memory-safety proof R-13-019 permits. Source language and a safe wrapper supply no certificate for the wrapped code.
   A C/C++ library behind an FFI is itself a sub-port. Audit the whole dependency closure, including OpenCL/HIP host adapters and libraries; familiar APIs do not bypass proof obligations.
2. **GPU dependence becomes software compute.**
   There is no fixed-function GPU or AMD/NVIDIA device stack (§15). Rendering, compositing and codecs use V-class software; dense matrix work uses the admitted M-class backend. An optional immutable inference device retains its separately qualified broker and grant (R-12-085f through R-12-085n).
   Preserve existing source interfaces wherever their semantics can map to this substrate. The [compute compatibility plan](compute-compatibility.md) targets OpenCL C/SPIR-V ahead of time and HIP source/API adaptation, sharing the certified native-kernel dispatch under R-13-018a through R-13-018c. No stock GPU binary, runtime shader compiler or new executable-loading path follows from that reuse.
   A toolkit's shaders can use this route when their complete source closure and specializations are available before generation admission; runtime-generated variants remain a concrete porting gap. Prove the transformations and account for host adapters, synchronization and resources instead of assuming a fixed shader set makes the port free. Full Vulkan/GL/Metal/wgpu personalities remain outside the current display contract; their build-time languages and reusable algorithms are assessed separately under R-04-001a and [Evaluated Architectural Alternatives](../background/architectural-alternatives.md). The [graphics API surface](compute-compatibility.md#graphics-api-surface) a port can target is Vulkan SC-shaped: pipelines named by identity from the package's closure, objects reserved at device creation, no runtime shader module, and GLSL or WGSL accepted only as producer input; it also lists the start-froms for that surface and for the renderer beneath it.
...
4. **New native instructions enter through a successor boot.**
   The resident toolchain can emit native artifacts as data for successor-generation admission (§13), or Wasm bundles for same-session interpretation in a precomposed elastic host (R-14-013e). A port can edit, build and run guest applications on-device without reboot. Newly emitted native instructions still cannot run in that session: executable memory is boot-written and no path JITs. The [application-host contract](contracts/application-host.md) fixes guest identity, resource and yield bounds, replacement and native-service acceleration. A port requiring a new native helper still crosses the generation boundary.
...
That is why it is porting discipline rather than a recovery lever ([architectural-alternatives.md](../background/architectural-alternatives.md), the recovery gate): it changes no mechanism, no schedule, and no theorem, so there is nothing for the spec to land.
...
The compositor is the half that needs the work, and its renderer is a backend against the shared substrate on the same terms as the other GUI targets: its shader set, like theirs, is fixed at build time and so AOT-compiles and is certified ahead of admission (§13, obstacle 2).
...
The target does **not speak the Wayland wire protocol**. Its control plane is the fork-and-frozen typed IDL over synchronous capability endpoints (R-07-029, R-12-010, R-12-013), so a Wayland connection is not an alternate server interface. Its delegation model also differs: a descriptor names only a pre-delegated per-session table entry, with no path or ambient reference (R-12-006), and new authority crosses only the explicit control plane; capability-store permission is absent from data-plane ring pages (R-12-007), so Wayland file-descriptor passing has no target equivalent. The ambient global registry disappears under R-12-006; R-12-075 separately excludes ambient observation of surfaces and input. This target disposition leaves host-side development tools outside its scope.
...
Obstacle 4 never bites it: the shader set is fixed at build time, so it AOT-compiles and is certified ahead of admission like any other code (§13, obstacle 2).
The other half of that backend is the platform layer, where GPUI's `unsafe` lives. Windowing, input and frame timing become §12 display-server client work under per-surface and per-input capabilities; clipboard copies and same-source reads operate on the editor's own bounded snapshots, while a cross-compartment paste is the §8.6 powerbox's trusted designation of one immutable item and recipient.

**tree-sitter is a sub-port with prior art, and the tables are why it is bounded.**
Its parsing runtime is C, so the `-sys` crate is inadmissible (closure disposition 3), but two facts keep the replacement small: the grammar *generator* is already Rust and runs at build time, exactly where compilation belongs (obstacle 4), and a generated grammar is overwhelmingly static parse tables rather than code.
...
**An editor here can be an integrated development environment, and the integration point is a generation rather than a process.**
Compilation and proving are producer acts (§5, §13) and the toolchain that performs them is resident (§13), so an editor on this roster can drive a build: what it cannot do is run the result in place, because nothing JITs (§14) and a build's output is admitted as a successor generation and installed through the generation-update path (§11, §13). The inner loop is therefore edit, build, prove, admit, reboot, with the holder's consent act on the signature (§13), and no *run this binary now* step exists at any speed.
That is a property of the platform rather than of this target, and it bounds what any editor on this roster can mean.
Collaboration rides the §12 network stack; acquiring file authority and reading another compartment's clipboard snapshot use the §8.6 powerbox. The paste act consumes one delivery grant and takes the panel and application touch for that designation through the existing trusted path; it creates no standing clipboard permission.

**Disposition:** Tier-2, gated on the shared render substrate and on nothing else, language ecosystems included.
Adopt the editor core (rope, multi-buffer, diagnostics, git integration) as clean safe Rust, write a fourth GPUI backend against that substrate with the shader set AOT-compiled and certified ahead of admission, replace the tree-sitter runtime with a safe-Rust table consumer cross-checked against both the C and ast-grep implementations, rewrite one external scanner per grammar actually shipped, treat language servers as a later addition admitted only where near-free, and bind the agent panel to the agent compartment below rather than to a subprocess.
...
Obstacle 1 clears if the closure is safe Rust; obstacle 2 clears because the agent renders through its client. Obstacle 4 asks nothing of the agent but that it generate no code into the running generation: it may edit source and drive the resident toolchain, and what it builds re-enters through admission.
...
`oo7`, a Rust implementation of the freedesktop Secret Service whose daemon replaces `gnome-keyring-daemon`, is the start-from, and it is a start-from for its **shape**, never its protocol ([inspirations.md](../background/inspirations.md) records why the protocol itself is declined).
...
NuShell parses to an internal representation and evaluates it; nothing is compiled to native, so it clears obstacle 4 by construction, no runtime codegen, no JIT (§14), and the parse-and-evaluate engine ports as ordinary pure-interpreter safe Rust.
**(4) The `unsafe` goes.**
...
Acquiring file authority and reading another compartment's clipboard snapshot are powerbox-only (§8.6, §14); copying the browser's own data and reading its own snapshot mint no authority.
**(4) The engines size their sandboxes by virtual reservation, and there is no address space to reserve.**
Both halves of a modern engine's memory discipline already assume a bounded region rather than an open heap: Wasm's linear memory is one flat array every access is masked or checked against, and the mainstream JavaScript engines have moved intra-heap isolation off the MMU entirely, giving their objects software-computed offsets inside one pre-reserved region because a page is the wrong granularity for a single heap's objects ([inspirations.md](../background/inspirations.md) records that diagnosis as taken and its mechanism as declined, CHERI supplying in hardware what both emulate).
...
Without a large virtual reservation, the interpreter must implement guest memory limits explicitly.
CHERI bounds the native backing allocation, but its rounded extent is not necessarily the guest's current byte length. R-14-013h therefore retains exact guest range, overflow and segment checks unless the interpreter proof establishes a particular check redundant, with the same trap and preceding effects. Guard pages and native hardware faults are not substitutes for this language behavior, and no universal bounds-check saving is credited.
...
#### Web applications: one engine and a capability-scoped host
...
| Approach | Useful idea | VerifiedOS disposition |
| --- | --- | --- |
| Progressive Web Apps | A Web App Manifest describes app presentation; service workers support offline content and event handling. [PWA guide](https://web.dev/learn/pwa) | Primary frontend compatibility target. Supported web APIs, bounded offline storage and standalone windows use the browser engine. A manifest is composition input, not an authority grant, and installability alone promises no offline behavior. |
| WebView2 | An embeddable web surface and a shared serviced runtime; Microsoft's production runtime is distinct from the Edge browser executable. [Distribution model](https://learn.microsoft.com/en-us/microsoft-edge/webview2/concepts/distribution) | Take shared runtime maintenance and an explicit embedding interface. Author the VerifiedOS backend against Servo; do not assume Microsoft's Windows runtime or ABI can run here. Runtime updates join an admitted successor generation rather than an independent evergreen updater. |
| Tauri 2 | Webview UI plus native commands, scoped through capabilities; its engine comes from the host platform. [Capabilities](https://v2.tauri.app/security/capabilities/), [webview backends](https://v2.tauri.app/reference/webview-versions/) | Preferred source-port shape for an app needing native work. Keep portable frontend assets, retarget the Rust backend and replace command/plugin plumbing with typed IDL and powerbox-scoped handles. Tauri permission configuration is not a CHERI capability or an admission certificate. |
| Isolated Web Apps | Signed bundles, distinct application origins and restrictions on subsequently fetched executable content. [IWA design](https://developer.chrome.com/docs/iwa/introduction) | Take the immutable asset closure and separate package identity for packaged apps. Use the platform's existing package/store machinery. Chromium's evolving bundle format, privileged APIs and deployment support are not assumed to be portable standards or promised compatibility. |
| Electron | A web frontend with an application backend; current guidance requires renderer sandboxing, context isolation and careful IPC, and warns about exposing Node to remote content. [Security guidance](https://www.electronjs.org/docs/latest/tutorial/security) | No Electron/Chromium/Node runtime port. Reuse a separable frontend and reauthor necessary backend operations through the same host interface. Secure Electron configurations exist; the incompatibility here is its dependency and authority model, not a claim that every Electron app is unsafe. |
| CEF | Chromium embedding for third-party applications. [Project scope](https://github.com/chromiumembedded/cef/blob/master/README.md) | An embedding reference, not a second engine. Source ports replace CEF-specific calls. Omitting Node does not make a Chromium/C++ closure automatically admissible or its host bridge safe. |

These pieces compose as a design, not by stacking their runtimes. Servo already exposes an [embedding API](https://doc.servo.org/servo/); qualifying the selected revision and replacing its incompatible dependencies remains necessary. Tauri is already a user of system webviews, including WebView2 on Windows, so those names describe different layers rather than mutually exclusive alternatives. The VerifiedOS host occupies the system-webview layer and the capability bridge occupies the native-command layer.

**Two content profiles, one engine.** A hosted PWA loads its HTTPS origin in a standalone window and retains only the offline resources its cache policy and storage budget cover. It has the browser's mediated web APIs, no app-specific native bridge, and no implied access to another browser profile's login or storage. Its mutable site content can update without a reboot because it remains interpreted content within an existing binding. A transient app window or a saved link can be browser state; installing a system-visible app descriptor, handler or changed authority ceiling is a successor-generation act.

A packaged app installs an immutable frontend asset closure and optionally a native backend. Its descriptor binds a stable app identity to a versioned content root, private package origin, permitted navigation, engine/bridge versions, interfaces and budgets. Cookies, IndexedDB, Cache Storage and service-worker registrations are isolated by app/profile and origin; two app scopes on the same HTTPS origin are not separate security principals merely because their manifest paths differ. A package origin never impersonates the website's HTTPS origin or inherits its cookies. Authentication therefore needs a tested web redirect or existing credential-broker flow rather than copied browser credentials.

Packaged JS/Wasm is still untrusted interpreted data. Package scripts, imports and workers come only from the authenticated closure; network data and mutable caches cannot become replacement scripts through `eval`, dynamic imports, script URLs or service-worker responses. Apply an enforced content policy covering those routes, navigation and frames; a policy string alone is not the isolation mechanism. Remote pages open in separate unprivileged bindings and receive no package bridge. Engine, backend and package-asset updates use the normal signed generation and reboot, including when the device builds them itself. Browser caching has no authority to update them. The offline claim includes the exact retained assets and declared durable app data, never a promise that remote services work offline.

**WASI vocabulary and WIT types form the native interop basis.** WASI is a family of system interfaces; WIT is the interface language used to describe them and custom application interfaces. The [WASI project](https://github.com/WebAssembly/WASI/blob/main/README.md) describes modular WIT APIs in WASI 0.2 and native async in its 0.3 preview. [WIT](https://component-model.bytecodealliance.org/design/wit.html) supplies records, variants, results, resource handles and import/export worlds; those types describe an interface, not its authorization or behavioral proof. Reuse that ecosystem's vocabulary where it fits, with custom application interfaces for operations WASI does not define.

VerifiedOS already owns the required adaptation in its [WIT-derived IDL profile](../languages/idl-profile.md#1-the-start-from-and-what-survives-the-read). The web bridge consumes that one frozen profile rather than creating another IDL or claiming complete upstream WIT/WASI compatibility. Its [constructor rules](../languages/idl-profile.md#2-the-type-constructors) and [world/resource mapping](../languages/idl-profile.md#5-worlds-manifests-and-resources) govern narrowing, manifests and handles. A reviewed profile declaration generates the web-facing API shape, native bindings, manifest and proof skeleton; binding generation for a web frontend does not turn privileged browser glue into JavaScript. Optional app-side JS/TypeScript wrappers remain guest content, with validation at the native boundary. The existing ring reference emits a Gallina interface artifact only, so production marshalling, parsers and language bindings are prerequisites still to implement; Q29's pilot does not claim they already exist.

The useful source-port vocabulary is resource-oriented access to selected files, network sessions, clocks and random data. Map each supported operation onto the existing object, network, time and crypto services with scoped grants; do not implement a blanket WASI host or expose its whole interface collection to a page. A file chooser yields an app/profile-scoped object handle, and a native document operation consumes that handle instead of a global pathname. WIT ownership syntax alone does not make a handle unforgeable: the host checks the session table and its generation, and the [profile's borrow and cancellation rules](../languages/idl-profile.md#435-cancellation-generation-and-dma) decide when the underlying capability can be released. JavaScript garbage collection is not the revocation mechanism.

The [profile's transport decision](../languages/idl-profile.md#12-why-the-wire-layer-is-authored-rather-than-forked-and-the-two-grounds-are-technical) remains binding: native RV64+CHERI endpoints and the canonical bounded ring encoding, with no Component Model Canonical ABI, Wasmtime, component loader or Wasm translation in the native call path. Upstream [wit-bindgen](https://github.com/bytecodealliance/wit-bindgen/blob/main/README.md) generates bindings for Wasm components; its native-CHERI backend would be new work, not an existing capability. An interpreted Wasm guest can reach only explicit host functions admitted by its embedding, under the platform interpreter's existing theorem scope; WIT vocabulary does not add Component Model guest support.

WASI 0.3's async design is a comparison input, not an automatic profile upgrade. The current profile excludes `future` and `stream` formers and maps asynchronous work onto bounded request/completion lifecycles. A JavaScript Promise facade can represent such an operation inside the frontend; it adds neither a new wire type nor background execution rights, and its cancellation, navigation teardown and terminal result must follow the ring contract. The profile also owns length bounds, closed variants, flow labels and error-result obligations that stock WIT does not supply. Any consumer needing a missing constructor first needs a separately reviewed profile amendment with resource and proof costs.

**The bridge is small and explicit.** Prefer standard browser APIs implemented over existing services. Add an app-specific bridge only for work that needs it, such as a bounded native document operation or numeric kernel. Generate the frontend vocabulary and native request types from the same profile declaration; messages on the ring use the platform's canonical bounded encoding. Do not expose arbitrary `invoke` targets, filesystem paths, shell strings, native objects, plugin discovery or a localhost HTTP control server. An embedding app's authority does not automatically pass to the content it displays.

Authorize each operation at the native service, using the authenticated app/profile, origin and session epoch and a scoped handle into the session's delegated table. A frontend-supplied origin or window label proves nothing. Check payload bounds, handle lifetime and the user's grant independently of frontend validation. Close the old session on navigation or revocation and reject late replies into a new document; wait for revocation, quarantine and device completion before reusing its pool member. Child frames and popups do not inherit the bridge. The security case assumes a compromised frontend and bounds what it can ask for, while the trusted powerbox surface remains outside all web-rendered chrome. A malicious frontend can still misuse a valid narrow grant; containment does not establish user intent or correct app behavior.

**Budgets and maintenance are platform features.** Windows, frames and workers bind the fixed origin pool. A service worker is bounded scheduled work with declared queues and termination behavior, not an always-running daemon. Persistent stores, caches, checkpoints and outstanding calls have explicit quotas and exhaustion outcomes. Declare how eviction handles focused, consent-bearing, audio-active and non-checkpointable work, including the case where every candidate is protected. Background sync, push and notification delivery need a separately qualified event path under those budgets; none is inferred from displaying a manifest. Durable data survives according to §10, while runtime sessions and undeclared state do not survive a generation change.

One engine version per generation makes the platform responsible for patch qualification across the app corpus. Record the source closure, admitted artifacts, supported web/bridge profile and regression results together. A security fix goes through composition and the reboot path; record that latency rather than promising evergreen immediacy. An incompatible app is updated, deferred or refused for that generation, not given a stale private engine. Share immutable engine code through existing composition where permitted; renderer heaps, inline caches, cookies and grants remain private. Count per-binding memory and dispatch costs as well as the saved executable bytes.

**Compatibility and performance are measured outputs.** Publish a versioned supported-feature profile and a reproducible corpus: Web Platform Tests for the selected HTML/CSS/DOM, navigation, storage and worker surface; test262 for JS; the pinned Wasm suite; and actual hosted, packaged and native-backed app workflows. Servo documents [its WPT runner](https://book.servo.org/contributing/testing.html), but upstream results do not qualify the replacement JS engine or the VerifiedOS port. Report unsupported tests and features explicitly. Node/native extensions, Windows host APIs, Tauri/CEF-specific plugins, proprietary DRM and GPU/JIT-dependent workloads require a source-level alternative or stay unsupported; a wrapper cannot supply them.

Before implementation, freeze per-workflow acceptance thresholds for cold launch, input-to-paint latency, frame delivery, bridge latency, foreground/background progress, resident first/second-class memory, stored bytes and update/reboot recovery. Compare a tab, standalone window and embedded view on the same admitted engine and composition, with and without a native backend where applicable. Host runs supply behavioral and relative-performance evidence; target timing requires the project's model or qualified hardware evidence. Include adversarial navigation during IPC, cross-profile state probes, malformed messages, stale handles, cache substitution, full pools and workers killed mid-update. Passing the corpus is bounded compatibility evidence, not a proof of web correctness or a universal performance claim.

Delivery order is the browser engine and interpreters, then hosted app windows and a bounded offline/storage profile, then immutable packaged frontends, then the typed native bridge and one source-port pilot. Advanced background/device APIs and additional framework adapters follow a named consumer and a separately priced contract. Q29 prices qualification and a bounded pilot; the full browser, interpreter, proof and production API programs remain separate work.
...
Four others need explicit platform choices. `memory64` is on in the upstream's default feature set; R-14-013b requires it for full Core 3.0, with curated semantics, concrete bounded backing and both engine proofs before release. The same default leaves `simd` off, while R-14-013d takes fixed-width SIMD subject to its proof condition. Neither default defines platform compatibility; all Core 3.0 features are required. The `wat` text-format front end is a development convenience for which a §13 image has no consumer, every admitted module arriving as binary. The guaranteed native tail calls behind the upstream's `unstable` feature turn on nightly Rust's `explicit_tail_calls`, which is a host-compiler property, separate from admitted guest tail-call instructions and no evidence for this certifying backend's stack bound. And `auto-dispatch` resolves an untested compilation target to the portable fallback rather than a threaded form; the platform chooses its form explicitly below.

**The deterministic profile needs curation beyond the interim's build flag.**
...
The official [Wasm 3.0 specification defines the deterministic profile](https://webassembly.github.io/spec/core/appendix/profiles.html). R-14-013b selects its numeric choices without removing Core 3.0 syntax: exact definitions and both engine proofs must cover the complete language, including concrete numeric operations. The inspected WasmCert input does not establish that complete artifact; Q34g owns the remaining work. Growth can still fail for resource reasons.
An engine feature flag is therefore not proof of the admitted semantics. Turning it on in the interim changes only that interim until curation and executable refinement close for the platform engine.
...
**Two upstream choices need an explicit platform disposition.**
...
The upstream arrangement retains stable storage for the engine's lifetime. Stability while a module is live does not itself forbid later reclamation, but an unbounded engine-lifetime append store supplies no platform release protocol.
R-14-015 and R-08-046 require bounded occupancy, declared exhaustion and complete retirement before reuse. The [Wasm execution contract](contracts/wasm-execution.md) therefore selects bounded module-owned immutable IR with checked offsets/indices and binding-private host roots. Its release waits for all frames, continuations, calls and permitted sharers before containment, sweep and sanitization. It needs no live capability relocation and does not encode native handler pointers in guest IR. Q34g owns the refinement and lifetime proof; stable storage alone is not that evidence, and the upstream append-only implementation is not adopted.

*The artifact-size knob that disables guest validation.* The release ships `validate` as a crate feature, on in the default set, with the upstream precondition stated plainly in [the release's changelog](https://github.com/wasmi-labs/wasmi/blob/v2.0.0/CHANGELOG.md) rather than in the release post, which names the knob without it: users with full control over the Wasm inputs can reduce binary artifact sizes by roughly 200 to 300 kB by disabling it.
...
It is not selected for the platform guest path, including local and packaged bundles: R-14-013e requires verified validation and R-14-013h permits reuse only of exact immutable results already established by that path. A signature, image residency or producer-controlled input does not alone discharge those obligations. Neither upstream change is adopted outright.

**The threading form is indirect, and that narrows R-14-008a rather than reopening it.**
R-14-008a already admits threaded dispatch by name among its ahead-of-time levers, so the only open question is the form, and the upstream's taxonomy is four rather than two: direct-threaded, embedding handler function pointers in the interpreter's IR and tail-calling from one handler to the next; indirect-threaded, embedding op-codes and mapping each through a jump table at dispatch; a switch-loop, which is the upstream's own 1.0 technique; and a call-loop it recommends against, the last two being the portable fallbacks for targets without tail calls.
Only the first two are threaded at all, so direct against indirect is the whole of the live question and naming four is what makes that narrowing legible rather than assumed.
...
Its IR is a buffer of code pointers; the buffer is data, so it consumes no executable SRAM budget, which is the capacity measurement R-15-100b states the static-code-overlay trigger as, and no component fills an executable bank at run time, which is the runtime write-then-execute consumer R-15-047's exclusion rests on and which R-15-100b names as what would retire that exclusion.
A threading scheme that inlined handler *bodies* rather than pointers is a different mechanism, and what refuses it is R-14-008a's own first clause, no code generation happening at run time anywhere on the path (R-14-002, R-14-004).
R-14-008b's Accept closes the adjacent move separately: no set is admitted on the ground that it is data rather than generated code, so an ahead-of-time inlined set stands or falls on the footprint budget and not on that ground.
...
*The predicate these verdicts were taken with.* The suite is [`wasmi-labs/wasmi-benchmarks`](https://github.com/wasmi-labs/wasmi-benchmarks), read on 2026-09-14, its membership closed there in an execute group and a startup group with `argon2` appearing in both; the table below carries one row per entry of that reading and one further row marked outside the suite, so the enumeration is the table and no count stands beside it to drift.
...
| `fibonacci-rec` | execute | predictor-fed and unread here | the guest call-return chain its own description calls call-intense, which reaches a tail-calling interpreter as dispatch-target variation a target predictor learns rather than as host call/return pairs (A-05) |
...
| CoreMark score run, **outside the suite** | execution score, `wasm-coremark-rs` | predictor-fed and unread here | two dispatch sites carrying separable histories in a target predictor, against the collapsed form's one entry with mixed history (A-05); the conditional branch the separated form adds is a direction predictor's (A-04) |
...
The mechanism is a dynamic predictor getting two entries with separable histories instead of one entry with mixed history, and the entries are a target predictor's: both forms dispatch indirectly, the separated one from two sites and the collapsed one from a single site the `csel` feeds, so A-05 removes the table those entries would live in, while A-04 removes the direction predictor the separated form's added conditional branch would use.
...
The OpenCL/HIP source adapters, where qualified, share this native M-class backend rather than adding a CUDA/AMD GPU stack (R-13-018a). Throughput remains the honest M-class envelope (§15 capacity honesty) on one machine, not datacenter-class, and an ensemble (§2) reaches more only by machine count under the sharding rule of R-15-171a, a single session's token rate staying one member's constant.
**(c) Obstacle 5 lands on the de-quantization code and nowhere else in this target**, which is worth saying because the de-quant path is the one place a framework reaches for the floating-point environment: rounding is round-to-nearest at every vector instruction, so a de-quant kernel that switches modes to match a reference implementation cannot, and must instead be validated bit-exactly against that reference at build time. The conversions themselves are `Zvfbfmin`'s (§15, R-15-067i) and are adopted; what is not adopted is native bf16 arithmetic or an OFP8 conversion form (R-15-067p), so the software path here is the one the spec assigns and its cost is booked at R-15-117a rather than treated as free.
...
- **A software rendering/compositing library on V-class cores**, the substrate COSMIC's compositor, Zed's GPUI, and Servo's WebRender all collapse onto, is built once under the §12 display model and shared across the three GUI targets.
  Its interface dispatches AOT-certified kernels over capability-scoped buffers (obstacle 2). The OpenCL/SPIR-V and HIP adapters share this native execution path where qualified, so standard-facing ports do not grow separate device stacks. Each toolkit still owes its shader/specialization inventory and backend contract before the port can be priced or claimed compatible. The [graphics API surface](compute-compatibility.md#graphics-api-surface) lists the start-froms for this library's 3D half, structural references rather than code, since every working software 3D path outside it JITs (R-12-083), and records that no checklist cell yet prices the renderer.
...
The Tier-2 admission floor (§13) certifies *memory safety*, not full behavioral correctness. CHERI enforces spatial bounds at runtime, while the certificate covers temporal safety, CFI, and the absence of runtime emission into executable memory. They do not establish object-level safety inside an interpreted guest's linear memory (R-14-013a), or that gitoxide merges correctly, Servo's interpreter event loop implements the intended behavior, or Zed orchestrates language servers correctly. The Tier-1 compositor has additional information-flow obligations for surface-to-input mediation (§12, §13).
