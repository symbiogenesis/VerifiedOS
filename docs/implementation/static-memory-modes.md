...
Every charge is one span per arena. Arenas are separate address spaces, so bytes
left free in one never pay for another, and the interference relation carries no
edge between two of them. That premise is what lets the optimality replay refuse
shorter layouts one arena at a time while the witness stands elsewhere, so the
receipt reads it back off each searched relation instead of assuming it, and a
charge that stopped carrying one span per declared arena is a finding.
...
Where all three charges exist, they are ordered inside every arena: the per-mode
charge is at most the binding family's, which is at most one conservative
layout's. The reason is elementary and is the receipt's own check on the declared
families rather than a theorem this artifact proves: a binding family's layout for
one mode is a legal layout for that mode, and one conservative layout repeated in
every mode is a binding family. A family the switch rule refuses has no binding
charge, so it exhibits no ordering at all; the receipt records that absence rather
than a comparison, and a refused family carries neither a charge nor a replayed
optimum.
...
Generic reusable slots with checked binding may change the fixed-identity premise
the baseline's witness states, which is how the baseline and the agenda both hedge
it. Such a change needs its own binding proof, and nothing here supplies one.
...
- **Two arenas.** Six identities in two arenas and three modes, each activating one
  ...
  differ and neither stands for the other. This is the family in which a charge
  summed, collapsed or carried across arenas would show, and in which the replay's
  per-arena decomposition is exercised rather than asserted.
...
the same verdict. Where the bounded search returns no layout, neither reading has
anything to read, and the receipt reports their agreement as untested rather than
as held. What separates the two readings is the focused test that hands them a
layout overlapping only in the second mode; on a searched witness the receipt's
field records that both ran and both accepted it. The binding family's per-mode
legality is the single-trace checker's; the switch rule and the retained
equalities are checked separately, because they are the only part that makes a
family of layouts one claim.
...
a machine-checked general proof.
