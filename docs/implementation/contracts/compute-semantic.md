...
| [Polynomial identity testing](../../background/open-math-conjectures.md#deterministic-polynomial-time-polynomial-identity-testing) | Match the actual representation to a restricted theorem: the read-4 result needs characteristic zero or at least five; Guo's September depth-four result allows arbitrary characteristic but at most three product summands and constant bottom-factor degree. Record degree, extension-field evaluation, field/bit-operation costs and the white-box versus black-box interface. An identity or candidate filter precedes the existing exact correspondence proof; it does not prove FP32 order, overflow, traps or memory effects. General PIT remains open and the frozen FP32 pilots are not automatically such a workload. |
| [Strong Descartes' Rule](../../background/open-math-conjectures.md#strong-descartes-rule-over-finite-fields) | A future exponentiation-circuit filter can compare the June randomized-prime test after establishing its condensed sparse representation, degree/height bounds, admissible prime sampling and subgroup evaluation. The fixed-prime nonuniform root bound supplies no uniform parameter guarantee. Keep filter soundness separate from Q30d's exact source correspondence and Q30e's numerical/effect refinement; neither a randomized test nor the open conjecture supplies them. No filter is selected for the pilots. |
...
| [Hadamard matrices](../../background/open-math-conjectures.md#hadamard-matrix-conjecture) | The independently checked finite matrices, including order 668, are concrete witnesses for a later transform slice selected by Q30g. A local certificate would bind the selected bytes to `H*H^T = n*I`; finite orthogonality supplies no fast transform algorithm. The lowering still needs normalization, scaling, rounding, overflow, padding, layout and WCET proofs against that slice's semantics. The [Bonsai rotation handoff](../../performance/bonsai2-assessment.md#finite-rotation-proof-handoff) already has a power-of-two consumer independent of the universal conjecture; it is outside the current GEMM pilot. |
