...
Bound it by transcribing **mechanically rather than by hand**: seL4's executable model is Haskell, and **hs-to-coq** carries that scrutinized prototype into Gallina without a paraphrase pass, so the artifact that earned the independent eyes is *preserved* rather than re-typed, leaving only the abstract spec and the refinement authored fresh.
That fresh abstract spec is not unmoored: it is **disciplined by the refinement against the hs-to-coq executable model**, since any *divergence* fails the proof loudly, so the sole silent residual is a *too-weak-but-faithful* abstract spec, the generic crown-jewel risk seL4 already carries in Isabelle (§5) rather than one the Coq move introduces.
