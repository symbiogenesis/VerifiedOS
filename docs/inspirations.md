...
What stopped it is a gate no technical rubric contains, which is the reason that table needs only five: this design owns every layer whose consent x86S had to ask for, and pays for the ownership in the porting bill [userspace-porting.md](userspace-porting.md) itemizes rather than in a negotiation it could lose.
