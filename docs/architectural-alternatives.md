...
## Static-slot fine-grained multithreading: the non-speculative throughput candidate the SMT rejection does not reach
...
That rejection is correct and its ground is specific: it reaches *simultaneous* (dynamic-issue) multithreading and nothing wider.
...
**The largest shipping member of this branch is HongMeng, and the vocabulary around it is where the calibration is needed.**
Huawei's HongMeng microkernel, under HarmonyOS NEXT, holds a Common Criteria **EAL6+** certificate issued by NSCIB with SGS Brightsight as the laboratory, for a target of evaluation named as the HongMeng Kernel on the Kirin 9000, announced 15 August 2023, whose public Security Target claims capability-based access control, information-flow control, attestation and residual information protection, and lists **ADV_SPM.1, a formal security policy model**, among its assurance components.
What EAL6 requires is a *semiformal* design description plus that *formal policy model*; what it does not require, and what nothing public demonstrates here, is a machine-checked refinement from the model down to the implementation, which is exactly what seL4 has and exactly the gap §5's rule exists to close.
There is no public source, no proof artifact, no named prover, and the kernel is proprietary and is **not** the kernel shipped in the Apache-2.0 OpenHarmony project it is routinely conflated with; the one substantive public technical document, the OSDI 2024 paper, is about performance and Linux ABI compatibility and presents no verification.
So the accurate description is that HongMeng is almost certainly the largest-scale **certified** microkernel ever shipped, and that "formally verified" applied to it is a certification-scheme term of art meaning something other than what this design means by it. Its compatibility argument is also the clearest available statement of the price of the road not taken, since preserving the Linux API and ABI so an existing ecosystem runs is the source of essentially every complexity cost the paper reports, and a clean-slate design with one pinned guest semantics (§5, §14) does not pay it.
...
## The commercial trusted-execution families: TrustZone, SEV-SNP, TDX, Arm CCA, Apple's SEP, and Caliptra; one structure, declined once

The entry above dispositions the academic enclave line; this one dispositions the industrial families a reviewer actually meets, because they are the mechanisms that ship and their failure record is the strongest available argument for §4's "no foreign computers".

**The structure is one structure, and the abstraction is the only thing that varies.**
TrustZone puts a second, more privileged computer in a Secure World; TDX puts it in SEAM mode; SEV puts it in the platform security processor; Arm CCA puts it in Root and Realm exception levels; Apple puts it on a separate die as the Secure Enclave Processor; Caliptra and Pluton put it in an IP block. In every case the operating system is demoted, and the component demoting it is vendor-signed, closed or semi-closed, and unaccountable to the software it polices. The published criticism is structural rather than anecdotal: Cerdeira et al. (IEEE S&P 2020) find in real vendor firmware an "excessively large TCB", drivers in trusted-OS kernel space, trusted applications able to map Normal-World physical memory, and revocation so ill-supported that vulnerable applications can be reloaded, with the asymmetry that a compromised secure world yields the normal-world kernel *even if that kernel has no bugs*; Pinto et al. (2021) name unauditability itself as a design pitfall rather than a commercial inconvenience; and the empirical hammer is Shakevsky et al.'s "Trust Dies in Darkness" (USENIX Security 2022), where a closed trusted OS on roughly a hundred million devices reused AES-GCM initialization vectors, enabling a downgrade attack and defeating both hardware-backed key attestation and FIDO2, and no outsider could tell for years. The design does not decline these one at a time; it declines the structure, which is what §4 says and what §6's exhaustive trusted-base enumeration is the positive form of.

**Confidential computing's root property is memory encryption, and the interposer era has settled what that buys.**
SEVered established that encryption without integrity is not confidentiality and CipherLeaks that deterministic encryption keyed by physical address is an oracle; then the physical attacks arrived and did not stop at DDR4. **Battering RAM**, an active interposer costing between fifty and a thousand dollars, aliases physical addresses at run time past both vendors' boot-time alias checks and **breaks remote attestation** on Intel Scalable SGX and AMD SEV-SNP; **WireTap** builds a ciphertext dictionary passively; **TEE.fail** carries the whole class to DDR5, defeating TDX and SEV-SNP with ciphertext hiding and extracting ECDSA attestation keys from Intel's provisioning enclave. The common root cause is stated plainly by the analysts: deterministic AES-XTS keyed only by physical address, no per-block randomisation, no tag, therefore no freshness and no replay detection.
The memory-cryptography entry below already declines the mechanism on the ground that it protects an interface this machine does not have; what this entry adds is that **Apple's own design draws the same line from the inside**. The SEP encrypts its DRAM with AES-XEX plus CMAC under an anti-replay tree, and roots that tree in **plain, unencrypted SRAM inside the enclave boundary**, because memory inside the boundary needs no cryptography. This platform is entirely that root, which is the strongest single statement of the argument available and is worth citing exactly that way.

**What remains in scope must not be waved away, and two items deserve naming here rather than in a footnote.**
Non-invasive physical side channels, power, electromagnetic, thermal, and clock and voltage glitching, are wholly unaffected by deleting the memory interface, and belong in §17 as they belong in Caliptra's own stated threat model; invasive attacks requiring depackaging are where Caliptra draws its boundary explicitly, and this design should draw its own boundary explicitly rather than inherit one silently. Supply chain is the residual the transparency import only partly reaches, and Apple's June 2026 answer to it is structural rather than cryptographic: **at least two roots of trust from independent vendors**, where this design has one on-die root and should say what it does about a subverted mask or fab (§17, §18).

**The two most useful calibration points in the whole family are about verification, not isolation.**
First, **Arm CCA is the only commercial security monitor with a serious machine-checked story**: Arm and AWS verified the Realm Management Monitor specification and a prototype (OOPSLA 2023), ESBMC added model checking (SAS 2024), and then an independent Verus-based consistency check found **35 confirmed bugs in that already-verified specification** (ASPLOS 2026). "Formally verified" without a stated property, a stated model, and an independent check of the specification against itself is a weaker claim than it sounds, and that is precisely the failure mode [requirements-register.md](requirements-register.md) and [coverage-matrix.md](coverage-matrix.md) exist to make impossible here; the finding is the argument for not relaxing either. CCA's other import is a discipline for the enforcement core, that the monitor is **mechanism and not policy**, which bounds what has to be proved about it. Intel's contribution is procedural and worth copying: a nine-month adversarial pre-release review enumerating 81 vectors and confirming 10 bugs, published, is the industrial shape of what [absence-contract.md](absence-contract.md) does declaratively.
Second, and this is the most important 2026 result in the survey, **attestation rather than isolation is where the field is breaking.** Sardar's formal analysis of attested TLS (AsiaCCS 2026, ESORICS 2026) demonstrates **diversion attacks**: a connection intended for one attested machine is silently relayed to a different compromised machine running identical software, anywhere in the world, with the client none the wiser; seven ways of binding evidence to the connection were tested and none prevents relay, CVE-2026-33697 rates it 7.5, production systems including Meta's WhatsApp Private Processing and Edgeless's Contrast are affected, and a manual security review by a competent firm had missed what formal analysis found. Two consequences land directly. Attesting *what software is running* is not attesting *which machine you are talking to*, so root-central attestation (§9) must bind its evidence to the channel and to the die, and which binding level it achieves is a statement the register owes rather than a design note. And it is the standing counterexample to any claim that a verified enforcement core suffices, since the protocol above the core is exactly where this failed.

**Caliptra is dispositioned here too, because it is the alternative to the chosen root.**
No rigorous published comparison with OpenTitan exists; on the available evidence OpenTitan is the more auditable choice, carrying uniform Apache 2.0 licensing against Caliptra's split specification terms, a public per-IP design-verification coverage dashboard, and RTL known to correspond to high-volume shipped silicon, and its selection stands. Three things import anyway: Caliptra's **written non-goals section**, which is a specification practice this repository already keeps and should credit; its **peer rather than hierarchical** trust model; and its explicit statement of which physical attacks are in and out of scope. One real gap is recorded rather than argued away: Caliptra 2.x endorses with post-quantum signatures (ML-DSA-87) and the chosen root does not, which is a live question for §9 rather than a settled one.

**Disposition:** no import of any secure world, security monitor, trust domain, realm, or on-die co-resident privileged operating system; the structure is declined at §4 rather than the vendors at §17, and the memory-encryption mechanism is declined at the entry below on the ground this one supplies from Apple's own architecture. Imported as disciplines: mechanism-not-policy for an enforcement core, the enumerated-vector pre-release adversarial review, written non-goals, and an explicit invasive-versus-non-invasive physical boundary. Two findings are recorded as obligations this document cannot itself discharge, the attestation-binding requirement above and the single-root supply-chain question, both of which belong to §9 and §17 rather than here.
Non-normative; no spec-body change.
...
Where the complementary back-end device does not reach array quality and the first class is the single planar tier at 1–2 GB (R-15-173a), it falls to 0.6–1.2 GB, which the bulk class's move onto oxide decks makes survivable for the roster and not for the *scalar working set*: 0.6 GB against hundreds of megabytes of kernel objects, stacks, rings, and server working sets is the same squeeze one rung lower, and it is the class the 1.67× lands on, bulk being on a medium a security bitcell does not reach.
...
**Reject** large-array 2T SOT-MRAM substitution with semantic separation. The ground is not the obsolete claim that all MRAM necessarily needs wear leveling, and within the capacity class it still ranks second on the axes stated; it is that **the class has an occupant** (R-15-247, the next entry), so admitting this candidate would mean displacing an adopted class rather than filling an empty slot. The two acts carry different burdens, and the six measured triggers above are written against the lower one. The oxide gain-cell candidate reaches the same idle-power and stacking atom without non-volatility, a line cipher, or the magnetic fault surface, and its bounded power-off remanence is answered by explicit tag invalidation and pre-admission sanitization rather than by a per-boot cipher.
...
## Oxide-semiconductor gain-cell (2T0C) working memory: adopted as the second static latency class, and retained here as the record of why

> **Adopted, and therefore not an alternative.** Oxide-semiconductor 2T0C decks are normative as the second of two static latency classes (R-15-247, and §15's memory subsystem). What this entry holds is the device evidence and the three-construction reset analysis the adoption rests on, kept because the normative text states the decision and not the argument for it.
...
The published trajectory: imec's [first functional 2T0C cell at >400 s retention](https://www.imec-int.com/en/press/imec-demonstrates-capacitor-less-igzo-based-dram-cell-400s-retention-time) (IEDM 2020), then [>10^3 s retention, >10^11 read/write cycles, and <10 ns writes](https://www.imec-int.com/en/articles/capacitor-less-igzo-based-dram-cell-excellent-retention-endurance-and-gate-length-scaling) (IEDM 2021), then [retention beyond 4.5 hours](https://www.imec-int.com/en/articles/disrupting-dram-roadmap-capacitor-less-igzo-dram-technology); a [dual-gate cell at ION = 1500 µA/µm](https://ieeexplore.ieee.org/document/10019488/) (IEDM 2022); a [stacked two-layer vertical channel-all-around bit-cell at 4F² effective area](https://www.researchgate.net/publication/378064875_First_Demonstration_of_Stacked_2T0C-DRAM_Bit-Cell_Constructed_by_Two-Layers_of_Vertical_Channel-All-Around_IGZO_FETs_Realizing_4F_2_Area_Cost) (IEDM 2023); an [8×8 monolithic multi-deck array](https://doi.org/10.1126/sciadv.adu4323) (*Science Advances* 2025); and, decisive for who funds the maturation, [Kioxia and Nanya's OCTRAM](https://www.kioxia.com/en-jp/rd/technology/topics/topics-77.html) (IEDM 2024), a 4F² DRAM on an IGZO vertical transistor at >15 µA/cell on current and <1 aA/cell off current, extended at [IEDM 2025 to 8-layer-stacked oxide-channel transistors](https://www.kioxia.com/en-jp/about/news/2025/20251212-1.html) formed by a NAND-like stack-and-replace flow (3D OCTRAM).

**What the commodity roadmap does and does not de-risk, stated narrowly because the tempting reading is wider.** **Samsung's and SK hynix's 4F² vertical-channel programs are silicon**, framed as a transitional step toward 3D DRAM, with Micron bypassing vertical channel transistors entirely for direct 3D DRAM; none of the three is an oxide-channel program. **OCTRAM is an oxide *access transistor* for a capacitor cell, not a gain cell**: it is the successor to the silicon access transistor in 1T1C, and the capacitorless 2T0C gain cell is a research device carried by imec, academic groups, and startups (Neo Semiconductor's IGZO 3D-X-DRAM variant among them). So the commodity roadmap is moving onto the oxide channel *as an access device*, which de-risks the deposition chemistry, the thermal budget, the stack-and-replace flow, and multi-deck stacking, while the gain cell rides those same process modules **without being the thing anyone is funding to volume**. The roadmap does not de-risk the cell, and it does not de-risk the macro at all, which is why R-15-247m's gate is a custom macro program rather than a part purchase.

**The array evidence is two to three orders of magnitude short of that gate, and the shortfall is periphery and repair rather than cells.** The largest published *2T0C* array is sixty-four cells; the closest structural analogue to what this design needs, a 1 Mbit monolithic 3D DRAM stacking one planar and two vertical oxide FET layers over silicon CMOS ([Okamoto et al., VLSI 2023](https://ieeexplore.ieee.org/document/10185263/)), is a capacitor cell rather than a gain cell. Nothing published carries repair, ECC, or a tag plane at any scale.

**One calibration consequence follows and is carried into the normative text.** The demonstrated 4F² vertical cell has an effective gate length near 40 nm, where imec's 14 nm figure is a device scalability claim rather than a realized cell, so the deck feature size and not the 4F² is the load-bearing unknown, and it is taken at the relaxed end until a realized cell says otherwise.
...
- **Eventual physical volatility is preserved, but reset semantics never wait for natural discharge.** A 2T0C gain cell stores charge rather than a non-volatile material state, so its contents eventually decay after power loss; at an hours-scale retention corner, however, a short power interruption can restore the machine while old data and old capability-validity bits remain physically readable. “Volatile” therefore names the medium's eventual endpoint, not the reset invariant. No epoch-key or line cipher is owed, but **immediate authority invalidation and pre-admission sanitization are owed**.
  ...
  Four reset constructions are available, and **three of the four are refused**, which is the part of this analysis that decides the mechanism:

  - **Volatile SRAM tag sidecars,** refused: gain cells would retain the data plane while one validity bit per 64-bit granule and its DECTED protection sat in a fast-decaying SRAM tag plane guaranteed clear on loss of the macro supply, so retained stale bytes become inert integers as soon as their authority bits disappear. It is the tempting construction and it does not scale (R-15-247a). One validity bit per 64-bit granule is 15.6 MB per GB of data and some 20 to 30 MB of SRAM per GB once the plane carries its own DECTED code, so a 40 GB bulk tier's sidecar consumes 40 to 100% of a 1 to 2 GB fast class, in exactly the case the bulk class exists to serve; atomicity across two media with different write timings is materially harder than within one; and a foreign sidecar gives `cloadtags` two cost models where a native plane gives it one.
  - **Reset-time tag-plane clearing,** refused as a mechanism and kept as the *ordering*: a fixed-latency sweep clearing and verifying every tag bit while the RoT holds every requester in reset is a sweep sized to capacity, but the order it states, tag invalidation first as the security boundary and full sanitization later as the confidentiality boundary, is exactly what R-15-247d fixes.
  - **Confirmed active discharge through a dedicated bleed path,** refused (R-15-247e): a second path to ground on the storage node is a second leakage contributor and becomes retention-limiting the moment its off current exceeds the write device's, which the literature places near 10⁻¹⁸ to 10⁻¹⁹ A. A cell whose reset mechanism eats its own retention has spent the property the class is bought for.
  - **Confirmed discharge through the cells' existing write devices,** adopted: every write wordline in the bank asserted against grounded write bitlines, each storage node draining through its own write transistor. No new device per cell, no new leakage path, and a deterministic **all-zeros** landing state that R-15-182 and R-15-060 already fix as untagged NULL, so the ECC regeneration pass an OFF→ON transition would otherwise owe is obviated rather than scaled to capacity. The cost moves to wordline-driver sizing and instantaneous current, which the phase schedule turns into a scheduling quantity, and the discharge is taken at mode *exit* rather than on the next entry (R-15-247q).

  In every construction, tag invalidation is the first security boundary and full sanitization is the later confidentiality boundary. A failed or incomplete tag clear, data clear, or discharge is a fail-stop reset failure; it never releases a partially sanitized bank.
...
- **n-type only: it does not unlock the branch where the SRAM realization loses the vertical lever, it dissolves the grading that had the branch.** The 6T tier stack is hostage to array-grade *complementary* back-end devices, the latch's two pFETs being irreducible (the bitcell entry above; R-15-163's binary gate), which is why the ungraded case once collapsed capacity to the planar tier at 1-2 GB. A 2T0C cell contains no pFET at all, so taking it as the bulk class does not add a case to that grading: it removes the case, leaving one roster to demonstrate and a planar tier measured against hundreds of megabytes of scalar working set rather than against everything (R-15-173a). That, rather than the capacity arithmetic, is the ground the adoption stands on, and it is a structural argument about specification size booked as a judgment call rather than as a derivation. At ~4F² per deck against a 6T footprint tens of times larger, one oxide deck outweighs several hypothetical SRAM tiers before multi-deck stacking is even counted, directly against the R-15-170 budget and the R-15-171 floor.
...
**That ranking is what rejects the 2T-SOT-MRAM entry above**: it stands second within a class this cell occupies, so admitting it would mean displacing an adopted class rather than filling an empty slot, which is a different act with a different burden.

**Disposition: adopted.** Bespoke volatile SRAM stays normative as the first class, and oxide-semiconductor 2T0C decks are normative as the second (R-15-247), with composition-time placement, one bit per cell, native tags in each class's own array, and no cache, migration, tiering, or wake-on-access between them.
The seven measured triggers this entry sets are **qualification gates rather than re-open conditions**, and are collected at R-15-247m: usable macro density on a repaired megabit-class macro carrying complete tag, ECC, refresh, discharge, and routing overhead; fixed read and write latency at the macro across the retention corner; disturb characterization on the actual stacked array; Vth drift bounded under the composed bias profile; separate scrub-coverage and refresh-deadline postconditions for the shared sweep; a statically staggered current envelope closing droop, thermal-coupling, and power-signature bounds; and the reset sequence across a short power interruption inside the measured remanence window. The difference is not cosmetic: a macro that misses one is a part that does not qualify, not an architecture that reverts.

**Honest residual, booked in §17 rather than here:** the hours-scale retention that makes refresh cheap is also an hours-scale power-off remanence window, and it covers hundreds of gigabytes. Authority remanence is discharged by construction; **confidentiality remanence is not**, and R-15-247q's mode-exit discharge narrows it to the domains live at power loss without closing it (R-17-058f). Beside the single-ended read's Hamming-weight term stands a worse one: refresh is a read-and-rewrite of every cell running unconditionally over the whole array, so the signature is continuous and needs no access induced, and it reaches the user-derived fraction of bulk memory while the model weights that dominate that capacity are public (R-17-058e).
...
R-15-173a books the first class's exposure: where complementary back-end devices do not reach array-grade quality, its vertical lever is *absent rather than reduced* and its capacity is the single planar tier at order 1–2 GB. The steelman is stated at full strength against that exposure, and what bounds it is that the roster's bulk sits on the oxide second class (R-15-247), so the grading decides the headroom of the scalar working set rather than whether the machine has a phone envelope; the first count below prices exactly that.
...
- **It is a worse version of the lever it substitutes for, contingent on the same materials result.** A tier buys 2×; a ternary cell buys 1.585×. Where the p-type result lands, tiers are the cheaper multiple and R-15-173 runs as written; where it does not, 1.585× on 1–2 GB is 1.6–3.2 GB of *first-class* capacity, at a cost that branch is least able to pay, and it buys headroom for a scalar working set already measured in hundreds of megabytes rather than reaching a roster floor the bulk class now carries (R-15-247s). The lever was aimed at a shortfall the second class removed.
...
The one lever with a live claim on this design is (3), and only where R-15-173a's grading leaves the first class on a single planar tier. Re-open it iff all three hold:
(a) the complementary back-end result fails, the first class is the single planar tier, **and** the scalar working set is measured against that tier and found short, since the roster's bulk is on the second class and no longer competes for it, so a 1.585× is worth its cost rather than a strictly worse substitute for a 2× tier;
...
The TRON project's **STRON** (*Silicon TRON*) is the same experiment run against a **standardized, independently specified** kernel API rather than a vendor's own: most of µITRON's basic system calls expressed in HDL, synthesized in 0.8 µm CMOS and evaluated on FPGA, with the hardware portion executing in about **250 ns** and task scheduling in about **750 ns**, roughly **6–50× the software implementation** ([Nakano et al., *Systems and Computers in Japan* 27(6), 1996](https://doi.org/10.1002/scj.4690270601)).
...
- **The benefit/safety scissor: decisive.**
  ...
  **STRON is the sharper form of the same observation**, because the freeze objection cannot be raised against it: µITRON was a published, standardized, multiply-implemented specification, exactly the "genuinely frozen forever" premise the steelman asks for, and STRON was nevertheless built as a **peripheral chip beside a general-purpose microprocessor** rather than as the machine's kernel.
  A frozen specification is thus demonstrably not sufficient for whole fusion; the deployed shape of a hardware kernel is an accelerator with a software kernel still behind it, which is the scissor stated as an empirical result rather than as an argument.
...
The **RealFast RTU / Sierra** line and the TRON project's **STRON** are functional references that RTOS-in-gates is real and deterministic, **not** admissible artifacts: unverified VHDL/HDL, a dynamic-priority/semaphore shape the §15 admission test deletes, and accelerators that keep the software kernel rather than replacing it.
STRON carries the additional weight, being a *standardized* frozen kernel API realized in silicon and still shipped as a coprocessor: the freeze premise the steelman needs is available in the historical record and did not produce whole fusion there either.
Non-normative; no spec-body change.
...
## The deleted assembly-equivalence checker, read against the current record: the deletion stands, and its falsifier is named
...
## The push-button road: Hyperkernel, Serval, Komodo, Ironclad and IronFleet; declined for the encoding as much as for the solver, and for a theorem the fragment cannot state

The proposal is the one a reviewer arrives holding: an order of magnitude of the proof cost in this plan is buyable by designing the interface until verification is decidable and letting an SMT solver discharge it, as the University of Washington and Microsoft Research lines have repeatedly done. The design half of that proposal is adopted in full and is credited in [inspirations.md](inspirations.md) under Nickel; the tooling half is declined here, on three grounds that should be given in this order, because only the third is decisive.

**First, the trusted base is not the solver, it is the encoding, and the encoding is bigger and less tested.**
Taken from the papers' own statements rather than inferred: Hyperkernel trusts its specifications, its verifier, Z3, Python and LLVM, plus the kernel initialization and glue code; Nickel adds its two checkers and inherits the same Python and LLVM IR encoding; Komodo trusts Dafny, hence Boogie and Z3, plus an assembly pretty-printer whose emitted jumps at interrupt-handler exits are a stated residual; Ironclad trusts 15,302 lines of tooling plus a trusted assembler; IronFleet trusts Dafny, Z3, the .NET compiler and runtime, and Windows.
**Serval's case is the sharpest and it lands closest to home**: its verifier *is* an ISA specification, hand-written, trusted, and validated only by unit tests and `riscv-tests`, which is a testing-grade artifact in a proving-grade position.
The gaps are systematically at the edges, at initialization, glue, printers, assemblers and calling conventions, which is exactly where this design's attestation seam and its verified-initialisation premise live (§7, §9), so the road is weakest precisely where this design most needs strength.
Solver soundness is a real but secondary point, worth stating with its number rather than as a suspicion: OpFuzz reported 1,092 bugs against Z3 and CVC4 in a single year, 819 confirmed and **184 of them soundness bugs**, against a prior baseline of fewer than 150 reported Z3 soundness bugs in three years, and the authors observe that nearly all such bugs had historically been found by client applications rather than by testing. A soundness bug means an unsatisfiable answer to a satisfiable formula, which for every system above means verification succeeding on a false theorem. Honesty requires the symmetric admission: bugs have been found in Coq and in its extraction too, and the single-prover argument has never been that Coq is infallible, only that there is one checker, that it emits a proof term a second implementation can recheck, and that a second prover with a different logic roughly doubles the audit rather than halving it (§5, §6).

**Second, the cost advantage is real, is smaller than the headline, and is measured per unit of verified interface rather than per system.**
The honest figures: NiStar six weeks and NiKOS two weeks for one author each, against seL4's roughly 25 person-years for functional correctness and 51 person-months for information flow. The caveats any fair citation carries are equally clear: the six-week figure excludes about six months of implementation by several researchers and two months spent choosing a noninterference formulation, Hyperkernel's headline conceals three complete architectures across a year, and the kernels in question are uniprocessor and sequential with no concurrency story, with initialization and glue unverified. Push-button verification is genuinely about an order of magnitude cheaper for an interface designed to be finite, on a property inside a decidable fragment, with the boundaries left out.

**Third, and decisively, the fragment cannot state the theorem this design exists to prove.**
Serval could not express CertiKOS's or Komodo's noninterference **at all**, because both quantify over unbounded big-step traces, and its authors say so. The apex statement here is a whole-system noninfluence property read at an adversary set, which is categorically outside a decidable first-order fragment; and Hyperkernel proves functional correctness and memory isolation rather than noninterference, which is precisely why Nickel had to exist. A road that buys a six-week proof of a statement the design does not make is not cheaper, it is aimed elsewhere.
One contrast makes the independence of these axes concrete and is worth recording rather than glossing: Serval found a **real PID covert channel in the Coq-verified CertiKOS**, by applying Nickel's specification to it. Theorem strength and specification quality are separate advantages, a strong prover does not supply a good specification, and this design intends to take both.

**What the road did establish, and this design should cite rather than rediscover.**
Every serious effort in the line either removed address translation from the code under proof or paid heavily for keeping it: Hyperkernel identity-maps its kernel and runs processes under hardware virtualization *only* to stop every kernel pointer dereference from becoming a virtual-to-physical obligation, having tried the conventional upper-half kernel and abandoned it; CertiKOS's RISC-V port replaced paging with PMP ranges and pushed page tables out to untrusted supervisor mode; and, the strongest single datum, Serval's analysis of **Keystone concluded that its check on operating-system-supplied page tables was unnecessary because PMP alone suffices**, and Keystone removed it. The no-MMU, capabilities-only design (§7, §15) is the limit point of a trend this literature documents and never reaches, and CHERI supplies what PMP cannot, fine-grained delegatable unforgeable domains with no translation structure to prove anything about.
The caution travels with it and is this project's own risk rather than a borrowed one: nothing in that literature proves anything about a capability *architecture* as the isolation mechanism, seL4's capabilities being software objects, so the Sail model moves from a convenience to a load-bearing artifact, and **Serval found a bug in the Sail RISC-V specification** along with two in QEMU and two in a shipping SiFive U54 core, one of the latter creating covert channels through readable performance counters. That is the argument for [differential-corpus.md](differential-corpus.md) stated by somebody else's finding.

**Disposition:** the design thesis is adopted and credited to Nickel in [inspirations.md](inspirations.md), including its six interface rules and its unwinding architecture with untrusted invariant and equivalence; Z3, Rosette, Dafny, Boogie, and their LLVM IR and ISA-interpreter encodings are declined as trust-base members under §5's single-prover rule; the finiteness discipline (a constant number of objects touched per operation, independent of state size), metadata in arrays rather than linked structures, and linked structures demoted to untrusted hints validated on use are banked as design atoms carrying no prover with them.
Non-normative; no spec-body change.
...
**One member of the genus is Coq-native and has to be dispositioned separately, because on the anchor test it nearly wins.**
**Cava** ("Coq plus Lava"), from Google Research's **Silver Oak** project, builds circuits bottom-up from gates, registers and wires with higher-order combinators in the Lava style, so the description is a Coq term with a semantics Coq proofs can quantify over, and it emits SystemVerilog. Its target was not a toy: high-assurance variants of **OpenTitan** peripherals, the AES accelerator above all, with a documented path to rebuild OpenTitan substituting the Cava versions, and it reports hitting the efficiency of the hand-written SystemVerilog blocks, which is the standing objection to the whole approach answered by the only project that tried it against a taped-out root of trust. Two facts decide it anyway. Its own documentation states that **the verification covers the circuit designs and not the Cava-to-SystemVerilog compiler**, so the verified object is the Coq circuit term and not the netlist that ships, which reintroduces exactly the gap a mechanized-semantics HDL is chosen to close and which the Kami and Kôika line exists to hold. And the repository was **archived read-only in December 2022**, which disqualifies it as a trust-base member independently of merit, a base being a thing that must be maintained rather than a thing that was once good. It is recorded here as considered, as the closest Coq-native competitor, as the OpenTitan tie worth knowing about given the root of trust already chosen, and as a negative result arguing for the existing selection rather than against it.
...
Its language-level instantiation is Ada's **Ravenscar profile** (IRTAW 8, 1997; Burns, Dobbing and Vardanega), standard since Ada 2005 and sharing clause D.13 with the looser Jorvik profile since Ada 2022: a fixed task set with no allocators, no task hierarchy and no termination, no dynamic priorities, no abort, select or requeue, one entry per protected object with a queue length of one and static barriers, and ceiling locking with blocking detection, all of it mandating preemptive FIFO-within-priorities dispatching. The *method* is imported and named in [inspirations.md](inspirations.md): define the subset by removing exactly what defeats static analysis, and note that the restrictions exist to shrink the run-time library rather than the application, which is the same reason static composition here exists to shrink the kernel. The dispatching is declined with the rest of the family, and the title of the paper that carried the field across (Audsley, Tindell and Burns, *The end of the line for static cyclic scheduling?*, 1993) is the honest statement of the utilisation this design is choosing to pay.
