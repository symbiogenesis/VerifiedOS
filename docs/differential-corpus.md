...
Twenty-two purecap programs, hand-written in the frozen dialect, each running to a defined end on the curated Sail emulator and reporting through HTIF. Together they exercise the base ISA, the adopted extensions, the capability surface the M0.6e transplant and the M0.6f re-parameterization put in the model, the profile rows M0.6g and M0.6h add that no upstream model carries, the memory plan M0.14 and M0.16 put beneath all of it, the memory model M0.7 states over it, the core-class roster M0.8a indexes `mhartid` into, and the frozen Keccak unit M0.8d attaches to the classes that roster names.
...
**Two absences in the encoder are scope and not oversight.** The dictionary bundle format of [isa-profile.md](isa-profile.md) §1.1 is a fetch container the model does not implement yet, so an image here is a stream of canonical 32-bit instructions, which is what the curated model fetches. And the vector, matrix and FEC surface is absent because **M0.8a** lands the class table and the roster while the datapaths those classes name arrive with **M0.8b** to **M0.8d**; the corpus version that follows each of those adds its rows. What M0.8a does add is `platform-coreclass`, which needs none of that surface: the class is not architectural state, so what a program can reach is the identity and the geometry rather than the datapath.
...
| The vector and matrix datapaths | One core class until the model is parameterized. `keccak-perm` reaches the vector register file and the unit-stride vector load and store because the frozen permutation's element group is carried in them, so what is left absent here is the arithmetic surface rather than the file. | M0.8b, M0.8c |
| The FEC decoders | Not in the model, and not this item's to put there: [isa-profile.md](isa-profile.md) §3 books the unit and names no mnemonic, operand form, or encoding for it, and nothing enters the profile by inheritance (R-15-007j). | a profile row that names the surface |
