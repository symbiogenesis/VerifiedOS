...
Twenty-three purecap programs, hand-written in the frozen dialect, each running to a defined end on the curated Sail emulator and reporting through HTIF. Together they exercise the base ISA, the adopted extensions, the capability surface the M0.6e transplant and the M0.6f re-parameterization put in the model, the profile rows M0.6g and M0.6h add that no upstream model carries, the memory plan M0.14 and M0.16 put beneath all of it, the memory model M0.7 states over it, the core-class roster M0.8a indexes `mhartid` into, and the vector memory surface M0.8b puts the per-element capability check on.
...
| The matrix and FEC datapaths | The classes are declared and the datapaths those declarations name are not modeled yet. The vector one is: `vector-memory` and `vector-geometry` run it. | M0.8c, M0.8d |
