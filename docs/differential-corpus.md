...
One exclusion is the format's and not the machine's, and it is stated rather than left to be discovered. The **block operations** are out because `rvfi_write` stops the emulator on them, which is §9.3's last row seen from the side that has to generate around it.

**The second exclusion stands and its ground has changed under it.** A word whose low two bits are not `11` used to make the two emitters disagree about what was injected, the commit trace reporting a sixteen-bit halfword where the packet reported the whole word, because the fetch path still read a length out of those two bits. That is gone: the profile excludes `C` and fixes ILEN at 32, so there is no second length to read, such a word is an unallocated 32-bit encoding, both emitters report it whole, and it traps with the whole word in `mtval` (M0.20, [`cap-trap`](../corpus/cap-trap.s)'s ninth check). What keeps the generator forcing those two bits is arithmetic rather than a disagreement: three quarters of a uniform 32-bit draw would be words this machine allocates no encoding to, so a stream that did not force them would spend most of itself on one trap.
