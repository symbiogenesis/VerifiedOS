...
The remaining nine `CJ-` targets name **theorems, not specifications**, things proven rather than authored. They are not crown jewels in R-17-016's sense (a specification whose *correctness is unverifiable*, so that a proof matching it may still miss intent); a theorem's correctness is exactly what its proof establishes. They are listed here so that all 23 targets are accounted for, and because each one's premise includes a specification above, which is the seam structure R-18-031(a)'s machine-checked statement has to align with.
...
The split is one reason the inventory and the trace legend cannot be the same artifact (the other, granularity, is next). **A theorem with no specification to be proven against is not a deliverable**, so every row in this table depends on a row in the one above.

Two premises above also name specifications the inventory does not carry: CompCert's C semantics and the kernel's abstract specification. Neither is conferred, and their absence is the membership rule holding rather than an oversight. Both are proof-interior interfaces of the R-05-019b shape, one Coq term two proof developments quantify over from opposite sides, and neither appears in the statement of T or of any property a requirement names, so a wrong one fails to link at R-18-031(b) instead of yielding a correct proof of a wrong property, which is the failure R-17-016 defines the status by; R-07-049 says it of the kernel directly, booking the residual as the capability-widened memory model and the refinement over it *"rather than the CHERI-C semantics itself"*. The contrast with row 6 is what conferral is for: the Sail model sits between two proofs in exactly the same shape, but it also models the machine, and R-15-005 confers on precisely the fidelity no proof on either side of it checks.

## What the `CJ-` table is, and is not

The register's `CJ-` table is **healthy and should not be conflated with this inventory**: 23 targets, every one used by at least one `Trace:` line, none orphaned. But it is a *trace legend* (a controlled vocabulary letting each requirement name the specification it constrains) and its granularity is deliberately coarser than the inventory's. `CJ-FORMAT` is one target covering every Narcissus descriptor; `CJ-CRYPTO-SPEC` is one target covering every primitive's functional specification. R-05-046 and R-05-076 require the *members* to be enumerated, which a trace legend cannot do without one target per member.

So the two artifacts answer different questions and both are needed:

| Artifact | Question it answers | Granularity |
| --- | --- | --- |
| `CJ-` table (in the register) | *which crown jewel does this requirement constrain?* | 23 coarse targets |
