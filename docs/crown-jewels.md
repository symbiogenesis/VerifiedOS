...
1. the register's **`CJ-` trace-target table**, 24 entries, all used, internally consistent, but a *legend of trace targets* rather than an enumeration of specifications (below);
...
- **the review gate's subject.** R-05-150 gates release on independent specification review, and the crown jewels are what that review reads. R-18-032 calls the composition statement *"the review artifact the crown-jewel gate most needs"*: that gate needs a defined scope, and this inventory is that scope.
...
| 6 | **The frozen profile's Sail semantics**: the Ztso and static-prediction fetch statements, the frozen matrix-extension semantics, the **re-parameterized capability encoding** with its representation-correctness proof and enumerated permission lattice, and the native tag-bit layout, one model rather than three | `CJ-SAIL` | **R-15-005**, R-15-004, R-15-016, R-15-019, R-15-086, R-15-095, R-15-009, R-15-116, R-15-117, R-15-118, R-15-175, R-15-035, R-15-007, R-15-007a, R-15-007b, R-15-007e, R-15-007f, R-15-067a, R-15-067c | partial: each is stated in prose by the profile (§1, §2, §3, §4.1, §6, §7); no Sail-model statement of any of them is authored, and the capability encoding, the indexed load/store, and the bitfield extract/insert are original authorship rather than curation of the upstream model |
| 7 | **The `fence.t` flush-set statement**, over one structure rather than two | `CJ-ISOL` | R-15-213, R-15-215, R-15-217, **R-15-221** | partial: the four-class map is carried in [absence-contract.md](absence-contract.md) §6; the Sail statement is not authored |
...
| 19 | **The NR RRC reference state machine**, a curation of the Tamarin RRC-layer analyses (the 5G handover lineage) as its tracked upstream, procedures no analysis covers transcribed under the R-05-050 posture and named as such | `CJ-VELUS` | R-12-043b, **R-12-043c**, R-12-043e | not authored: the curation source exists in the literature |
| 20 | **The 5G NAS reference state machine**, including 5G-AKA, a curation of the Tamarin 5G authentication lineage as its tracked upstream | `CJ-VELUS` | R-12-043b, **R-12-043c**, R-12-043e | not authored: the curation source exists in the literature |
| 21 | **The 802.11 MLME reference state machine**, including the WPA3/SAE handshake, a curation of the Tamarin 802.11 handshake model and the machine-checked SAE analyses as its tracked upstreams | `CJ-VELUS` | R-12-043b, **R-12-043c**, R-12-043e | not authored: the curation sources exist in the literature |
| 22 | **The BT L2CAP/GATT pairing reference state machine**, a curation of the Tamarin Bluetooth key-agreement lineage as its tracked upstream | `CJ-VELUS` | R-12-043b, **R-12-043c**, R-12-043e | not authored: the curation source exists in the literature |
| 23 | **The probing-model statement**, the glitch- and transition-extended *d*-probing and composition statement the crypto core's masking theorems are verified against, an axiom about the silicon rather than a theorem over the Sail model | `CJ-LEAK` | **R-15-053a**, R-05-004a, R-17-058a | not authored |
...
| 25 | **The pinned Wasm guest semantics**, the version-frozen language definition the platform interpreter's soundness and confinement theorems are stated against, a curation of the mechanized WasmCert lineage with the SpecTec-mechanized official specification as its tracked upstream | `CJ-WASM` | **R-14-013b**, R-14-013a, R-14-013c, R-14-013d | not authored: the curation source exists in the literature |

Two rules decide how finely conferral is cut, and they cut in opposite directions. **A row is one specification, never one clause of one** (R-17-016a: *one row per crown-jewel specification*), so where a single artifact carries several statements it is one row: row 6 is the frozen profile's Sail model, not three rows for the three statements it will make, and row 8 is the isolation model, not a row per mechanism inside it, because R-15-211 makes non-interference *"decided against the isolation model's non-interference statement rather than against the list of mechanisms above it"* and a row per mechanism would reinstate that list. Row 16 is the same rule applied to a conferral phrased in the plural: R-17-044 names the Lustre program and the control/data boundary together, as one residual, and the boundary is the program's scope rather than a second artifact, since R-12-002 states the split by plane, R-05-052 mandates the node set that realizes it, and writing the program is what draws the boundary, so one act of authorship carries both statements. **But where a requirement confers per member, the members are rows**, which is why rows 19–22 are four and not one: R-12-043c confers *per protocol rather than per stack* and its acceptance criterion requires the inventory to carry **one row per reference model**, since it is a particular standard's state machine a particular sequencer is proved to refine. Row 10 sits between the two: R-05-046 confers on each descriptor individually, but the descriptor set is not knowable until R-05-042's inventory exists, so the row carries the enumeration obligation and names the member classes, and the per-descriptor enumeration lands in the row as that artifact is authored. Row 14 shares that shape: R-05-076 confers on each primitive's functional specification, and the per-primitive enumeration lands with the crypto inventory R-05-059 requires.

## The theorem targets

The remaining ten `CJ-` targets name **theorems, not specifications**, things proven rather than authored. They are not crown jewels in R-17-016's sense (a specification whose *correctness is unverifiable*, so that a proof matching it may still miss intent); a theorem's correctness is exactly what its proof establishes. They are listed here so that all 24 targets are accounted for, and because each one's premise includes a specification above or a judgment fixed outside this corpus, which is the seam structure R-18-031(a)'s machine-checked statement has to align with.
...
| `CJ-ADMIT-IMPL` | Admission-checker implementation refinement: the shipped TAL evaluator accepts exactly where the pinned rule table does, the shipped CIC kernel exactly where MetaCoq's Gallina checker does, each within the resource contract R-06-015a declares | the judgment each checker is refined against: the pinned CHERI-TAL rule table and MetaCoq's Gallina checker, neither an inventory row (below) |
...
| `CJ-WASM-SOUND` | Platform-interpreter soundness and robust guest confinement: the executable interpreter refines the pinned semantics, and an adversarial module influences its host and peers only through its declared imports and exports | the pinned Wasm guest semantics (row 25) |

The split is one reason the inventory and the trace legend cannot be the same artifact (the other, granularity, is next). **A theorem with no specification to be proven against is not a deliverable**, so every row in this table depends on a row in the one above, or on a judgment fixed outside this corpus that the paragraph below accounts for.

Three premises above also name specifications the inventory does not carry: CompCert's C semantics, the kernel's abstract specification, and the judgment each admission checker is refined against. None is conferred, and their absence is the membership rule holding rather than an oversight. The first two are proof-interior interfaces of the R-05-019b shape, one Coq term two proof developments quantify over from opposite sides, and neither appears in the statement of T or of any property a requirement names, so a wrong one fails to link at R-18-031(b) instead of yielding a correct proof of a wrong property, which is the failure R-17-016 defines the status by; R-07-049 says it of the kernel directly, booking the residual as the capability-widened memory model and the refinement over it *"rather than the CHERI-C semantics itself"*. The third splits along its two halves and neither half reaches the status: MetaCoq's Gallina checker is that same proof-interior shape, and the pinned CHERI-TAL rule table is the one premise here whose correctness a theorem already covers, `CJ-TAL-SOUND` being the statement that those rules are sound, so a wrong rule fails the metatheorem rather than yielding a correct proof of a wrong property. The contrast with row 6 is what conferral is for: the Sail model sits between two proofs in exactly the same shape, but it also models the machine, and R-15-005 confers on precisely the fidelity no proof on either side of it checks.

## What the `CJ-` table is, and is not

The register's `CJ-` table is **healthy and should not be conflated with this inventory**: 24 targets, every one used by at least one `Trace:` line, none orphaned. But it is a *trace legend* (a controlled vocabulary letting each requirement name the specification it constrains) and its granularity is deliberately coarser than the inventory's. `CJ-FORMAT` is one target covering every Narcissus descriptor; `CJ-CRYPTO-SPEC` is one target covering every primitive's functional specification. R-05-046 and R-05-076 require the *members* to be enumerated, which a trace legend cannot do without one target per member.

So the two artifacts answer different questions and both are needed:

| Artifact | Question it answers | Granularity |
| --- | --- | --- |
| `CJ-` table (in the register) | *which crown jewel does this requirement constrain?* | 24 coarse targets |
| This inventory | *what are the crown jewels, and does each one exist yet?* | 25 specifications, per-member where a requirement demands it |

## Standing obligations

- **The list is closed by amendment** (R-17-016). A requirement asserting crown-jewel status for a specification absent from this table is a review-gate finding, not a silent addition; that is what closure can mean where membership is conferred entry by entry, and `tools/check.py` reports the violation in both directions rather than leaving it to a reader's roll-call.
