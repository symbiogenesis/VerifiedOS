...
- [ ] **Reduce `fence` from 256 encodings to the two behaviors the machine can actually exhibit.**
  ...
  **Two** behaviors exist — drain, nop. Not 256. The `PI`/`PO` axis separates nothing either: under R-15-015b a device store leaves the core only after the buffer has drained and completes at the endpoint before it retires, and R-15-015a's destination set covers device endpoints, so device stores travel no independently-ordered path for the fence to name.
  ...
  *Touches:* R-15-017 (now retaining `fence` on the cross-island ring ground alone), R-12-008's acceptance criterion, and the §17 litmus set.
  *Already landed, and not this item's to claim:* R-15-015b and R-15-015a's extension to device endpoints. R-15-015b was on this document's rejected list — as the route to deleting `fence`, where it failed gate 2 — and was adopted on an unrelated ground: `fence.t`'s padded constant would otherwise be set by the slowest endpoint on the die (R-15-218). The device consumer's retirement is a *consequence* of that decision, not a payment this item makes.
  ...
  **Two adjacent claims to re-derive rather than inherit.** Both are stated in the register as though `fence` were load-bearing, and under TSO on single-copy memory neither obviously is:
  ...
  - **R-15-017's one surviving ground, "cross-island ring ordering over shared SRAM."** (Earlier revisions of this item cited R-15-061 for this; that is `Zicbom`'s exclusion and has nothing to do with ring ordering. The clause lives in R-15-017 itself, which since the device retirement rests on it alone.) Cross-island rings live in shared SRAM (R-15-223), and shared SRAM is the same single-copy memory under the same TSO. The fence there is either unnecessary for the same reason, or it is standing in for the fabric-ordering property R-15-015a makes an explicit obligation — in which case it is a fence papering over an interconnect guarantee, which is the wrong place to pay for it.

  **The device consumer is already retired, in the spec.** R-15-015b and R-15-015a's device-endpoint destination set make every edge into and out of device space structural, and R-15-017 has been amended to strike the I/O and device-ordering ground accordingly. That argument is normative now and is not restated here; what remains open below is the rest of it.

  **What the device retirement does *not* reach is the one relaxation the machine actually has.** Every device edge, and both re-derivations above, concerns ordering TSO already gives. The machine's sole relaxation is store→later-load bypass, and the drain is the only instruction that fixes it — so whether `fence` collapses to two behaviors or leaves the profile entirely turns on whether anything needs that fix.
  For system software the answer is already in the register, and it is *no consumer* in R-15-026's own words: the multikernel is share-nothing with no kernel locks, rings are single-writer SPSC, refcounts and status flags are single-instruction `Zaamo`, and no capability resides in shared mutable memory. Nothing in that enumeration performs a store to one location followed by a load of another.
  **What the register does not settle is userspace.** [userspace-porting.md](userspace-porting.md) invokes Ztso for ported code without saying whether a ported mutex, seqlock, or Dekker-shaped barrier exists on this platform — and each of those is a store→load pattern that needs the drain. So:
  - *Adopt now:* the **collapse to two behaviors**. It is unblocked, it is the whole of the Sail and litmus win above, and it holds whether or not the drain has a consumer.
  - *Blocked on the userspace question:* deleting `fence` outright, which additionally requires that no ported compartment needs store→load ordering. If that clears, the cut is the same *no consumer* deletion that took `Zacas` (R-15-026) and `Zifencei` (R-15-047), cited to R-15-026's enumeration rather than to the device argument. If it does not, the drain stays and this item ends at the collapse — which is the larger half of the win regardless.
