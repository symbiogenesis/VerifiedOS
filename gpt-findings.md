...
**1. Capability-integrity semantics remain undefined.** · **Open. Four items, all held in `cheri-todo.md`, plus a fifth this finding does not name.**

The four stand as written: tag-clearing versus trapping on non-monotonic modification, capability exception reporting through `mcause`/`mtval` (the `xtval` closure), the merged register file, and the clause that a vector store clears the capability tag of every granule it overwrites. The first three are `cheri-todo.md` §1 defects, the fourth a §3 statement. The assessment that these affect fault handling, context sanitization, WCET, and forgery rather than documentation is correct.

The fifth is `cheri-todo.md` §3's *the load instruction must permit revocation-driven tag clearing*. It is load-bearing twice over: the address-keyed load filter is the sole mechanism carrying both the object case and the delegation case (R-08-004a, R-08-005), so the architectural clause permitting the tag clear on the load is what both rest on.

**2. The ISA freeze has a circular prerequisite.** · **Open. The recommendation is sound and no decision is recorded.**

R-18-003a makes the profile freeze the root of the schedule with no build prerequisite. Yet R-15-067d makes the bitfield pair's carriage conditional on a measurement against generated output, and the dictionary's realized size and entry selection are measured against the emitted mix (`isa-profile.md` §2, *Density is a model, not a quotation*). Those inputs do not exist before the compiler and a composed image do.
...
**3. The proof target is behind the architecture.** · **Open, and structural.**
...
**4. "Mobile/laptop class" is conditional and unfalsifiable.** · **Open, and the repository already says so.**
...
**5. The cache deletion depends on an unauthored physical premise.** · **Open.**
...
**6. Package installation conflicts with build-time composition.** · **Open.**
...
**7. Two README closures are stronger than the normative design.** · **Open, both halves, and both are conceded in the specification's own residuals.**
...
**8. Mutable user-data rollback is not merely availability for every app.** · **Open, and the proposal is well-shaped.**
...
**Decide the product fork explicitly.** · *Open, and it is finding 4 restated as a decision rather than as a gap.* R-15-162 declines chiplets and bonded die-stacking outright, and R-15-163 makes the monolithic vertical lever conditional on an unsolved low-temperature p-type device result. Accepting authenticated external or bonded memory reopens R-15-162, which is a deliberate refusal carrying a stated ground, so this is not a free choice between two equally open options.

---

**What remains concretely closeable:** stale capability tags after non-capability writes, capability-fault misclassification, wrong responses to failed verdicts, runtime-dependent Tier-2 overflow, rollback of security-sensitive app state, and non-atomic package recomposition.
