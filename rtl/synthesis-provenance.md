...
**The boot ROM, the serial port and the block device are route-(c) functional references from `lowRISC/mocha`, on the disposition every imported core here already carries**, and the reading is written down for §4a's own reason: both readings are otherwise equally available from the artifacts. They are elaborated and differentially tested against the golden model; no FEV, no Kôika or Kami refinement, and no RTL ⊑ Sail claim covers any of them, and the thing to check against this record is that the co-simulation evidence never reads as refinement evidence.
...
**Two of the three exist at the pin and the third does not.** `rom_ctrl` and `uart` are in the bring-up SoC's own vendored OpenTitan tree, and a search of that tree for a block device returns nothing: its nearest is `spi_host`, which is a controller a card hangs off rather than a device, and the transfer interface arrives with the storage stack rather than here.

**What the ROM controller carries that this design does not take is named here rather than discovered at integration.** The reference's `rom_ctrl` is OpenTitan's measured-boot ROM whole: address and data scrambling under a nonce and a key the bring-up SoC states as literals in its own top package and marks as needing replacement before any real deployment, a KMAC-backed digest of the ROM contents, an alert path, and a report to the power and key managers. None of that is this platform's measured boot. R-09-002 puts every stage's measurement in the RoT and R-09-003 puts the first instruction in the RoT's on-die metal-mask ROM, which is a different ROM from the one `platform.boot_rom` places; the KMAC the reference instantiates beside it is configured with masking off and for the ROM check alone, where R-05-004a's masking obligations stand over this design's own crypto core. So what transfers is a ROM behind a bus port, and the integrity construction around it is the reference's own.

**What the platform boot ROM is fetched by is nothing, on the register's own chain.** R-09-006 has the RoT verify the boot payload, place the verified M-mode image in main SRAM, and release the boot core into the measured chain, so the application core's first fetch is into main memory. That is why the region holding this ROM is readable and not executable and why that is a statement about the die rather than a gap: an SoC boot ROM that a bring-up flow would put a reset vector in is an affordance this chain replaces.
...
**The netlist it is taken over is the scalar core's, and two rows above are stated against that boundary.** A-11 grounds its `n/a` on the tag cache sitting in the imported tag controller rather than in the core, and A-12a grounds its own on activity-driven power gating sitting in a memory controller that is neither this core nor this package. Both are true of the enumeration as it is taken today and both are claims about *where the boundary is*, so the act that elaborates an SoC top moves the boundary and owes those two grounds a re-reading in the same run. Neither is re-stated ahead of that elaboration: a ground rewritten against a netlist nobody has enumerated would be a claim about a build that does not exist, which is the state R-15-103 exists to prevent.
